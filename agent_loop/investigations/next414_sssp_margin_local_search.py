#!/usr/bin/env python3
"""Frozen one-term SSSP margin-local search on the NEXT224 frontier."""
...
n227 = n413.n268.n227
n223 = n261.n223
PROTOCOL = "2026-08-13-next414-sssp-margin-local-search-v1"
MANIFEST_NAME = "MANIFEST.json"
CATALOGUE_NAME = "NEXT414_SSSP_MARGIN_LOCAL_CATALOGUE.json"
EVALUATION_NAME = "NEXT414_DISCOVERY_EVALUATION.json"
FORMULA_NAME = "NEXT414_FROZEN_CANDIDATE.json"
SEARCH_NAME = "next414_sssp_margin_local_search.parquet"
SCORE_COMPOSITION = (
    "nonnegative_next224_plus_triangular_margin_local_signed_sssp_term"
)
...
EXPECTED_ELIGIBLE_HYPOTHESIS_SHA256 = (
    "8e9c3245dec5fe147f149d51d4c28358342871dda24f3f691f4d2e065512b729"
)
...
EXPECTED_DESIGN_SHA256 = n413.EXPECTED_INPUT_SHA256["design"]
...
EXPECTED_NEXT413_SOURCE_SHA256 = (
    "d94beea81a3696d145752538640110af3d36160a24cab43fb66917a3bdd935ef"
)
EXPECTED_INPUT_SHA256 = {
    **{
    ...
    "next413_manifest": (
        "bca2d137576aa67b7a583c290d30106f5f894f385fb97ea152da5f53be6f2b82"
    ),
    "next413_catalogue": (
        "a17da377a7f62f6c2e815d9a866468e925d7990cafc0a4c34cb1532f6504aabb"
    ),
    "next413_audit": (
        "8cb5eb95ad4c03965108120dd2b44759c31e3ac4689b58f8d5bc27d501fd3ab3"
    ),
    "next413_table": (
        "1904e44bfdb6b4b70a1a1ed6b9875667d0cdc6f8770069a185a45bffa9911080"
    ),
}


def sssp_margin_local_score(
    *,
    base_score: object,
    base_support: object,
    protection: object,
    ...
    """Apply the inherited triangular correction to frozen SSSP protection."""
    ...
    try:
        ...
        raise ValueError("NEXT414 margin-local score inputs differ") from exc


def build_sssp_candidate_specs(
    *,
    ...
    """Build one no-op plus the inherited, fixed 7 by 3 SSSP grammar."""
    ...
    if len({str(spec["candidate_key"]) for spec in result}) != len(result):
        raise RuntimeError("NEXT414 candidate keys are not unique")
    return result


def materialize_sssp_candidates(
    *,
    ...
    """Encode exact SSSP-augmented scores as virtual evaluator terms."""
    ...
    for term in terms:
        ...
        new_id = "next414_virtual_candidate__" + hashlib.sha256(
            key.encode()
        ).hexdigest()[:24]
        ...
        term.update(
            {
                "term_id": new_id,
                "feature": new_column,
                "group": "next414_sssp_margin_local",
            }
        )
...
def _paths(
    *,
    ...
    paths.update(
        {
            "design": design_path,
            "next413_manifest": roots["next413"] / n413.MANIFEST_NAME,
            "next413_catalogue": roots["next413"] / n413.CATALOGUE_NAME,
            "next413_audit": roots["next413"] / n413.AUDIT_NAME,
            "next413_table": roots["next413"] / n413.TABLE_NAME,
        }
    )
    return paths
...
def _verify_next413(
    ...
    base_paths = {key: paths[key] for key in n413.EXPECTED_INPUT_SHA256}
    base_paths["design"] = paths["next413_design"]
    base_hashes = {key: input_hashes[key] for key in n413.EXPECTED_INPUT_SHA256}
    base_hashes["design"] = input_hashes["next413_design"]
    prior = n413._verify(base_paths, base_hashes)
    manifest = json.loads(paths["next413_manifest"].read_text())
    catalogue = json.loads(paths["next413_catalogue"].read_text())
    audit = json.loads(paths["next413_audit"].read_text())
    table = pd.read_parquet(paths["next413_table"])
    ...
    expected_outputs = {
        n413.CATALOGUE_NAME: input_hashes["next413_catalogue"],
        n413.AUDIT_NAME: input_hashes["next413_audit"],
        n413.TABLE_NAME: input_hashes["next413_table"],
    }
    if (
        manifest.get("protocol") != n413.PROTOCOL
        or manifest.get("eligible_hypothesis_count")
        != EXPECTED_ELIGIBLE_HYPOTHESIS_COUNT
        or manifest.get("eligible_hypothesis_sha256")
        != EXPECTED_ELIGIBLE_HYPOTHESIS_SHA256
        or manifest.get("next414_search_authorized") is not True
        ...
        or manifest.get("executed_source_sha256", {}).get(
            "src/next413_sssp_feature_audit.py"
        )
        != EXPECTED_NEXT413_SOURCE_SHA256
        or _sha256_file(Path(n413.__file__).resolve())
        != EXPECTED_NEXT413_SOURCE_SHA256
        ...
        != EXPECTED_ELIGIBLE_HYPOTHESIS_SHA256
        or audit.get("next414_search_authorized") is not True
        ...
        raise ValueError("NEXT414 NEXT413 provenance differs")
...
def _formula_from_spec(spec: Mapping[str, object] | None) -> dict[str, object]:
    ...
    return {
        "protocol": PROTOCOL,
        "kind": "sssp_triangular_margin_local_x0_no_dft_score",
        "selected": True,
...
def select_best_new_record(records: pd.DataFrame) -> pd.Series | None:
    try:
        ...
        raise ValueError("NEXT414 reporting selection schema differs") from exc


def _attach_sssp_features(
    *,
    combined: pd.DataFrame,
    feature_tables: Mapping[str, pd.DataFrame],
    sssp_tables: Mapping[str, pd.DataFrame],
) -> None:
    source = combined["source_dataset"].astype(str).to_numpy()
    feature = n413.n411.FEATURE_NAMES[0]
    for source_name in ("scigen", "wyformer"):
        indexed = n413._index_by_id(
            table=sssp_tables[source_name],
            source=source_name,
...
def run_sssp_margin_local_search(
    *,
    ...
    """Run the complete frozen discovery-only NEXT414 search."""

    if not set(REQUIRED_STAGES).issubset(stage_dirs) or not set(
        REQUIRED_DESIGN_STAGES
    ).issubset(design_paths):
        raise ValueError("NEXT414 input universe differs")
    roots = {
    ...
    if type(search_workers) is not int or search_workers <= 0:
        raise ValueError("NEXT414 search_workers must be a positive exact integer")
    if any(not path.is_file() for path in paths.values()):
        raise FileNotFoundError("NEXT414 input is missing")
    ...
    if require_formal_inputs and input_hashes != EXPECTED_INPUT_SHA256:
        ...
        raise ValueError(f"NEXT414 formal input identity differs: {differing}")
    (
        eligible_prior,
        eligible214,
        primary_key,
        ...
        sssp_tables,
        eligible413,
    ) = _verify_next413(paths, input_hashes)
    ...
    if (
        ...
        raise ValueError("NEXT414 NEXT224 base identity differs")
    specs = build_sssp_candidate_specs(
    ...
    if len(specs) != EXPECTED_CANDIDATE_COUNT or eligible_count != EXPECTED_ELIGIBLE_COUNT:
        raise RuntimeError("NEXT414 frozen candidate universe differs")
    virtual, terms, runtime, activity = materialize_sssp_candidates(
        features=combined,
        base_score=base_score,
    ...
    if (
        ...
        raise RuntimeError("NEXT414 evaluator accounting differs")
    ...
    if len(no_op) != 1 or len(reference_no_op) != 1:
        raise RuntimeError("NEXT414 no-op reproduction identity differs")
    ...
    if selected_row is not None:
        ...
        if str(selected_result["selected"]["record"]["candidate_key"]) != selected_key:
            raise RuntimeError("NEXT414 selected candidate reproduction differs")
        ...
        if selected_spec.get("eligible_new_candidate") is not True:
            raise RuntimeError("NEXT414 reproduction control was selected")
    ...
    if selected is not None and passes != bool(
        ...
        raise RuntimeError("NEXT414 all-gate selection differs")
    if passes and selected is None:
        raise RuntimeError("NEXT414 all-gate candidate was not selected")
    ...
    catalogue = {
        ...
        "candidate_grid_selected_after_sssp_outcomes": False,
        "base_candidate_key_sha256": EXPECTED_BASE_CANDIDATE_KEY_SHA256,
        "base_threshold": EXPECTED_BASE_THRESHOLD,
        "repair_width": REPAIR_WIDTH,
        "eligible_hypotheses": list(eligible413["hypothesis"].astype(str)),
        "eligible_hypothesis_count": len(eligible413),
    ...
    evaluation = {
        "protocol": PROTOCOL,
        "evaluation_mode": "frozen_sssp_margin_local_discovery_search",
        ...
        "next415_diagnostic_authorized": bool(not passes and diagnostic_keys),
        "next415_candidate_count": len(diagnostic_keys),
        "next415_candidate_key_sha256": diagnostic_sha,
    ...
    source_paths = {
        ...
        "src/next414_sssp_margin_local_search.py": Path(__file__).resolve(),
    }
    ...
    try:
        ...
        if any(_sha256_file(path) != input_hashes[name] for name, path in paths.items()):
            raise RuntimeError("NEXT414 input changed before publication")
        if any(
            _sha256_file(path) != source_hashes[name]
            for name, path in source_paths.items()
        ):
            raise RuntimeError("NEXT414 source changed before publication")
...
def main() -> int:
    ...
    manifest = run_sssp_margin_local_search(
