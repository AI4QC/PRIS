#!/usr/bin/env python3
"""Discovery-only cross-source materialization of NEXT112 morphology."""
...
PROTOCOL = "2026-08-08-next113-cross-source-discovery-cmvom-v1"
GRAPH_MODE = "voronoi"
DISCOVERY_ROLE = "discovery"
MANIFEST_NAME = "MANIFEST.json"
CATALOGUE_NAME = "NEXT113_CMVOM_FEATURE_CATALOGUE.json"
FEATURE_FILES = {
    "scigen": "scigen_discovery_cmvom_features.parquet",
    "wyformer": "wyformer_discovery_cmvom_features.parquet",
}
CATALOGUE_MODES = ("core", "expanded")
NUMERIC_FEATURE_NAMES = tuple(
    name.replace("cmvom_", f"cmvom_{mode}_", 1)
...
STATUS_COLUMNS = tuple(
    ...
    for mode in CATALOGUE_MODES
    for name in (
        f"cmvom_{mode}_supported",
        f"cmvom_{mode}_failure",
        f"cmvom_{mode}_catalogue_sha256",
        f"cmvom_{mode}_pymatgen_version",
        f"cmvom_{mode}_scipy_version",
    )
)
...
EXPECTED_INPUT_SHA256 = {
    ...
    "design": "fe35baf70b6b88a9dfb92f3afe396445ed91ea75462d67bd6d8ded94330cf08d",
}


def compute_cmvom_feature_row(structure, *, graph_mode: str) -> dict[str, object]:
    """Compute core and expanded morphology with independent abstention."""
    ...
    for mode in CATALOGUE_MODES:
        try:
            ...
            statuses.update(
                {
                    f"cmvom_{mode}_supported": False,
                    f"cmvom_{mode}_failure": f"{type(exc).__name__}: {exc}",
                    f"cmvom_{mode}_catalogue_sha256": None,
                    f"cmvom_{mode}_pymatgen_version": None,
                    f"cmvom_{mode}_scipy_version": None,
                }
            )
            continue
        for name, value in result.features.items():
            row[name.replace("cmvom_", f"cmvom_{mode}_", 1)] = float(value)
        statuses.update(
            {
                f"cmvom_{mode}_supported": bool(result.supported),
                f"cmvom_{mode}_failure": result.failure_reason,
                f"cmvom_{mode}_catalogue_sha256": result.catalogue_sha256,
                f"cmvom_{mode}_pymatgen_version": result.pymatgen_version,
                f"cmvom_{mode}_scipy_version": result.scipy_version,
            }
        )
    row.update(statuses)
    if tuple(row) != FEATURE_COLUMNS:
        raise RuntimeError("NEXT113 feature row schema differs")
    return row
...
def _parse_error_row(message: str) -> dict[str, object]:
    ...
    for mode in CATALOGUE_MODES:
        row.update(
            {
                f"cmvom_{mode}_supported": False,
                f"cmvom_{mode}_failure": message,
                f"cmvom_{mode}_catalogue_sha256": None,
                f"cmvom_{mode}_pymatgen_version": None,
                f"cmvom_{mode}_scipy_version": None,
            }
...
def _compute_scigen_payload(item: tuple[str, bytes]) -> tuple[str, dict[str, object]]:
    ...
    try:
        ...
        row = compute_cmvom_feature_row(structure, graph_mode=GRAPH_MODE)
    except Exception as exc:
...
def _source_counts(table: pd.DataFrame) -> dict[str, object]:
    ...
    for mode in CATALOGUE_MODES:
        supported = f"cmvom_{mode}_supported"
        failure = f"cmvom_{mode}_failure"
...
def _solver_thread_environment(
    *, require_formal_inputs: bool
    ...
    """Capture and, for formal runs, enforce one solver thread per worker."""
    ...
    if require_formal_inputs and any(value != "1" for value in values.values()):
        raise ValueError("NEXT113 formal run requires a single-thread solver environment")
    return values


def build_cross_source_discovery_cmvom_features(
    *,
    ...
    """Materialize morphology only from physically split discovery geometries."""
    ...
    if any(not path.is_file() for path in paths.values()):
        raise FileNotFoundError("NEXT113 discovery input is missing")
    input_hashes = {name: _sha256_file(path) for name, path in paths.items()}
    if require_formal_inputs and input_hashes != EXPECTED_INPUT_SHA256:
        raise ValueError("NEXT113 formal input identity differs")
    ...
    source_paths = {
        ...
        "src/next112_obstruction_morphology.py": repository_root / "src/next112_obstruction_morphology.py",
        "src/next113_cross_source_cmvom_features.py": Path(__file__).resolve(),
    }
    ...
    try:
        for source in ("scigen", "wyformer"):
            ...
            if len(table) != len(metadata[source]) or any(
                ...
                raise RuntimeError(f"NEXT113 {source} row accounting differs")
        ...
        manifest: dict[str, object] = {
            "protocol": PROTOCOL,
            "mode": "cross_source_discovery_only_raw_x0_cmvom_feature_freeze",
            "graph_mode": GRAPH_MODE,
            "workers": workers,
        ...
        if any(_sha256_file(path) != input_hashes[name] for name, path in paths.items()):
            raise RuntimeError("NEXT113 input changed before publication")
        if any(_sha256_file(path) != source_hashes[name] for name, path in source_paths.items()):
            raise RuntimeError("NEXT113 source changed before publication")
...
def main() -> int:
    ...
    manifest = build_cross_source_discovery_cmvom_features(
