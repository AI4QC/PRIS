#!/usr/bin/env python3
"""Audit frozen MVBO certificates inside the exact NEXT224 rejected cohort."""
...
import src.next239_morphometric_voronoi_bond_order as n239
from src.next102_cross_source_dobvr_features import _sha256_file, _write_json


PROTOCOL = "2026-08-09-next240-mvbo-feature-audit-v1"
MANIFEST_NAME = "MANIFEST.json"
CATALOGUE_NAME = "NEXT240_MVBO_FEATURE_CATALOGUE.json"
AUDIT_NAME = "NEXT240_MVBO_FEATURE_AUDIT.json"
TABLE_NAME = "next240_mvbo_feature_audit.parquet"
EXPECTED_DESIGN_SHA256 = n239.EXPECTED_DESIGN_SHA256
HYPOTHESES = (
...
EXPECTED_NEXT239_SOURCE_SHA256 = (
    "8dab4a01551423edc4007b946d4eddf06c236c5604edb3b9dd02bf7a4e6914d3"
)
REQUIRED_STAGES = (*n227.REQUIRED_STAGES, 239)
REQUIRED_DESIGN_STAGES = (*n227.REQUIRED_DESIGN_STAGES, 227, 239)
...
EXPECTED_INPUT_SHA256 = {
    **{
    ...
    "next239_design": EXPECTED_DESIGN_SHA256,
    "design": EXPECTED_DESIGN_SHA256,
    "next239_manifest": (
        "4c0d9e5fc7019ce0b15d68abea88a8b102e050e480b212ddba2d622c4ab82833"
    ),
    "next239_catalogue": (
        "ff823e0b2fdd04ccfbbe7db7d7aca4cb769c8ce17bdffcbb847cf956274ba9e4"
    ),
    "next239_scigen": (
        "948e5f8734ae76d99fa8a48aeab2b59c5f32565af2968c2e3a634e601e1e79c0"
    ),
    "next239_wyformer": (
        "d28d6dd031aa4d16cf6b3eddcb959db34be099dea6eeb589f88261d36d687bd7"
    ),
}
...
def select_eligible_hypotheses(
    ...
    if not isinstance(frame, pd.DataFrame) or frame.empty or required - set(frame.columns):
        raise ValueError("NEXT240 audit ranking table differs")
    result = frame.copy()
...
def bounded_protection(
    *, values: object, direction: str, q_lo: float, q_hi: float
    ...
    if raw.ndim != 1 or direction not in n227.PROTECTION_DIRECTIONS or not (
        ...
        raise ValueError("NEXT240 bounded protection inputs differ")
...
def _index_mvbo_by_prefixed_material_id(
    *, table: pd.DataFrame, source: str, expected_material_ids: object
) -> pd.DataFrame:
    """Return a source-prefixed MVBO table after exact identity validation."""
    ...
    if set(extra["material_id"]) != expected:
        raise ValueError(f"NEXT240 {source} material identity differs")
...
def _paths(
    *,
    ...
    paths.update(
        {
            "next239_design": design_paths[239],
            "design": design_path,
            "next239_manifest": roots["next239"] / n239.MANIFEST_NAME,
            "next239_catalogue": roots["next239"] / n239.CATALOGUE_NAME,
            "next239_scigen": roots["next239"] / n239.FEATURE_FILES["scigen"],
            "next239_wyformer": roots["next239"] / n239.FEATURE_FILES["wyformer"],
        }
    )
    return paths
...
def _verify_next239(
    ...
    manifest = json.loads(paths["next239_manifest"].read_text())
    catalogue = json.loads(paths["next239_catalogue"].read_text())
    tables = {
        "scigen": pd.read_parquet(paths["next239_scigen"]),
        "wyformer": pd.read_parquet(paths["next239_wyformer"]),
    }
    expected_outputs = {
        n239.CATALOGUE_NAME: input_hashes["next239_catalogue"],
        n239.FEATURE_FILES["scigen"]: input_hashes["next239_scigen"],
        n239.FEATURE_FILES["wyformer"]: input_hashes["next239_wyformer"],
    }
    if (
        _sha256_file(Path(n227.__file__).resolve()) != EXPECTED_NEXT227_SOURCE_SHA256
        or manifest.get("protocol") != n239.PROTOCOL
        ...
        or manifest.get("executed_source_sha256", {}).get(
            "src/next239_morphometric_voronoi_bond_order.py"
        )
        != EXPECTED_NEXT239_SOURCE_SHA256
        or _sha256_file(Path(n239.__file__).resolve()) != EXPECTED_NEXT239_SOURCE_SHA256
        or any(manifest.get(key) is not value for key, value in n239.BOUNDARY_FLAGS.items())
        or catalogue.get("feature_names") != list(n239.FEATURE_NAMES)
        or catalogue.get("feature_count") != len(n239.FEATURE_NAMES)
        or catalogue.get("labels_opened") is not False
    ):
        raise ValueError("NEXT240 NEXT239 provenance differs")
    for source, expected_rows in (("scigen", 13470), ("wyformer", 5232)):
        ...
        if (
            ...
            or not table["mvbo_supported"].fillna(False).astype(bool).all()
            or any(
                not np.isfinite(pd.to_numeric(table[name], errors="coerce")).all()
                for name in n239.FEATURE_NAMES
            )
        ):
            raise ValueError(f"NEXT240 NEXT239 {source} table differs")
    return (*prior, tables)


def run_mvbo_feature_audit(
    *,
    ...
    """Audit frozen MVBO certificates without searching a new formula."""

    if not set(REQUIRED_STAGES).issubset(stage_dirs):
        raise ValueError("NEXT240 stage directory universe differs")
    if not set(REQUIRED_DESIGN_STAGES).issubset(design_paths):
        raise ValueError("NEXT240 design path universe differs")
    roots = {
    ...
    if any(not path.is_file() for path in paths.values()):
        raise FileNotFoundError("NEXT240 input is missing")
    ...
    if require_formal_inputs and input_hashes != EXPECTED_INPUT_SHA256:
        ...
        raise ValueError(f"NEXT240 formal input identity differs: {differing}")
    (
        eligible,
        eligible214,
        primary_key,
        ...
        formula222,
        mvbo_tables,
    ) = _verify_next239(paths, input_hashes)
    ...
    if cohort_counts != n227.EXPECTED_COHORT_COUNTS:
        raise ValueError("NEXT240 rejected cohort counts differ")
    ...
    audit = {
        "protocol": PROTOCOL,
        "audit_mode": "fixed_next224_rejected_extreme_mvbo_feature_audit",
        "base_candidate_key_sha256": n227.EXPECTED_BASE_CANDIDATE_KEY_SHA256,
        "base_threshold": n227.EXPECTED_BASE_THRESHOLD,
        "cohort_counts": {
        ...
        "next241_search_authorized": bool(selected is not None),
    ...
    source_paths = {
        ...
        "src/next239_morphometric_voronoi_bond_order.py": Path(n239.__file__).resolve(),
        "src/next240_mvbo_feature_audit.py": Path(__file__).resolve(),
    }
    ...
    try:
        ...
        manifest = {
            ...
            "next241_search_authorized": bool(selected is not None),
            "mvbo_branch_terminated": selected is None,
            "new_formula_searched": False,
            "new_formula_selected": False,
        ...
        if any(
            _sha256_file(path) != input_hashes[name] for name, path in paths.items()
        ):
            raise RuntimeError("NEXT240 input changed before publication")
        if any(
            _sha256_file(path) != source_hashes[name]
            for name, path in source_paths.items()
        ):
            raise RuntimeError("NEXT240 source changed before publication")
...
def main() -> int:
    ...
    manifest = run_mvbo_feature_audit(
