#!/usr/bin/env python3
"""Apply unchanged Pauling 2--5 controls to the NEXT24 generated x0 cohort."""
...
from src.next24_ssagen_holdout import PROTOCOL as HOLDOUT_PROTOCOL


PROTOCOL = "2026-08-03-next24-generated-pauling-controls-v1"
OUTPUT_NAME = "next24_pauling_controls.parquet"
...
def _load_holdout(
    *, metadata_path: Path, frames_zip_path: Path, manifest_path: Path
    ...
    manifest = _read_json(manifest_path, role="NEXT24 holdout manifest")
    _validate_manifest_output_hash(
        manifest, metadata_path, role="NEXT24 holdout metadata"
    )
    _validate_manifest_output_hash(
        manifest, frames_zip_path, role="NEXT24 holdout geometry"
    )
    if (
        ...
        raise ValueError("NEXT24 holdout crossed the label-free boundary")
    ...
    if loaded_ids != list(expected_ids):
        raise ValueError("NEXT24 holdout geometry order differs")
    if any(
        len(atoms) != int(natoms)
        for atoms, natoms in zip(structures, metadata["natoms"], strict=True)
    ):
        raise ValueError("NEXT24 holdout atom counts differ")
    return metadata, structures, manifest


def run_next24_pauling_controls(
    *,
    metadata_path: Path,
    ...
    if len(table) != len(metadata) or table["material_id"].duplicated().any():
        raise RuntimeError("NEXT24 Pauling row accounting differs")

    repository_root = Path(__file__).resolve().parents[1]
    source_paths = {
        "src/next24_pauling_controls.py": Path(__file__).resolve(),
    ...
    manifest: dict[str, object] = {
        ...
        "evidence_role": "label-free comparator before any DFT endpoint opening",
        "labels_opened": False,
...
__all__ = ["OUTPUT_NAME", "run_next24_pauling_controls", "main"]
