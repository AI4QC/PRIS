#!/usr/bin/env python3
"""Physically isolated formal feature build for frozen NEXT367 PBVEU."""
...
PROTOCOL = "2026-08-13-next367-pbveu-formal-build-v1"
MANIFEST_NAME = "MANIFEST.json"
CATALOGUE_NAME = "NEXT367_PBVEU_FEATURE_CATALOGUE.json"
FEATURE_FILES = {
    "scigen": "next367_scigen_periodic_bond_valence_equal_uniformity.parquet",
    "wyformer": "next367_wyformer_periodic_bond_valence_equal_uniformity.parquet",
}
EXPECTED_ROWS = {"scigen": 13_470, "wyformer": 5_232}
MINIMUM_FORMAL_COVERAGE = 0.90
EXPECTED_PROBE_SHA256 = "d20ee0f170592cae423b0f3e8aa5d830ab48c280b1d61dd6a9ad35496f6d6b1b"
EXPECTED_INPUT_SHA256 = {
    ...
    "next307_source": "49373c0246c1a997a56320bf1416419a6c91d77a01763bf8bbce7bfbec317a2f",
    "next367_source": "fe3247b4c2043261d7a4945d7f8415f8cbe1927014b8c58dc1a2fbd55c59dd5d",
...
def _error_row(exc: Exception) -> dict[str, object]:
    return {
        n367.FEATURE_NAMES[0]: math.nan,
        "pbveu_supported": False,
        "pbveu_failure": f"{type(exc).__name__}: {exc}",
        "pbveu_site_count": 0,
        "pbveu_edge_count": 0,
        "pbveu_minimum_degree": 0,
        "pbveu_maximum_degree": 0,
        "pbveu_valence_policy": None,
        "pbveu_parameter_exact_fraction": math.nan,
        "pbveu_parameter_generic_fraction": math.nan,
    }
...
def _compute_scigen_payload(item: tuple[str, bytes]) -> tuple[str, dict[str, object]]:
    ...
    try:
        ...
        return material_id, n367.compute_pbveu_row(atoms)
...
def _label_free_statistics(table) -> dict[str, object]:
    ...
    values = pd.to_numeric(table[n367.FEATURE_NAMES[0]], errors="coerce").dropna().to_numpy(float)
    if len(values) < 1:
        raise RuntimeError("NEXT367 label-free feature population is empty")
    return {
        n367.FEATURE_NAMES[0]: {
...
def build_cross_source_pbveu_features(
    *,
    ...
    """Build PBVEU from physically isolated discovery geometry only."""
    ...
    paths = {
        ...
        "next307_source": Path(n307.__file__).resolve(),
        "next367_source": Path(n367.__file__).resolve(),
    }
    if os.path.lexists(target):
        raise FileExistsError(str(target))
    if type(workers) is not int or workers <= 0:
        raise ValueError("NEXT367 workers must be a positive exact integer")
    if any(not path.is_file() for path in paths.values()):
        raise FileNotFoundError("NEXT367 input is missing")
    ...
    if require_formal_inputs and input_hashes != EXPECTED_INPUT_SHA256:
        ...
        raise ValueError(f"NEXT367 formal input identity differs: {differing}")
    ...
    if (
        probe.get("protocol") != "2026-08-13-next367-pbveu-label-blind-probe-v1"
        or probe.get("design_sha256") != n367.DESIGN_SHA256
        ...
        raise ValueError("NEXT367 label-blind probe authorization differs")
    ...
    if (
        ...
        raise ValueError("NEXT367 discovery geometry provenance differs")
    ...
    for source, frame in metadata.items():
        if required - set(frame.columns) or frame["material_id"].astype(str).duplicated().any():
            raise ValueError(f"NEXT367 {source} metadata differs")
        ...
        if len(selected) != EXPECTED_ROWS[source]:
            raise ValueError(f"NEXT367 {source} discovery identity differs")
    ...
    executed_paths = {
        "src/next367_pbveu_formal_build.py": Path(__file__).resolve(),
        "src/next367_periodic_bond_valence_equal_uniformity.py": Path(n367.__file__).resolve(),
        "experiments/next367_pbveu_label_blind_probe.py": root / "experiments/next367_pbveu_label_blind_probe.py",
        "tests/test_next367_periodic_bond_valence_equal_uniformity.py": root / "tests/test_next367_periodic_bond_valence_equal_uniformity.py",
        "tests/test_next367_pbveu_label_blind_probe.py": root / "tests/test_next367_pbveu_label_blind_probe.py",
        "tests/test_next367_pbveu_formal_build.py": root / "tests/test_next367_pbveu_formal_build.py",
    }
    executed_hashes = {
    ...
    try:
        ...
        for source in ("scigen", "wyformer"):
            ...
            if (
                ...
                raise RuntimeError(f"NEXT367 {source} material identity differs")
            ...
            finite = np.isfinite(
                pd.to_numeric(table[n367.FEATURE_NAMES[0]], errors="coerce").to_numpy(float)
            )
            if not finite[supported].all() or finite[~supported].any():
                raise RuntimeError(f"NEXT367 {source} finite support semantics differ")
            coverage = float(supported.mean())
            if coverage < MINIMUM_FORMAL_COVERAGE:
                raise RuntimeError(f"NEXT367 {source} coverage below frozen minimum")
            statistics[source] = _label_free_statistics(table)
            if statistics[source][n367.FEATURE_NAMES[0]]["unique_rounded_10"] < 20:
                raise RuntimeError(f"NEXT367 {source} feature is degenerate")
            failures = Counter(table.loc[~supported, "pbveu_failure"].fillna("unknown"))
            counts[source] = {
                "rows": int(len(table)),
                "supported": int(supported.sum()),
                "failures": int((~supported).sum()),
                "coverage": coverage,
                "site_count": int(pd.to_numeric(table.loc[supported, "pbveu_site_count"]).sum()),
                "edge_count": int(pd.to_numeric(table.loc[supported, "pbveu_edge_count"]).sum()),
                "minimum_finite_degree": int(
                    pd.to_numeric(table.loc[supported, "pbveu_minimum_degree"]).min()
                ),
                "maximum_finite_degree": int(
                    pd.to_numeric(table.loc[supported, "pbveu_maximum_degree"]).max()
                ),
        ...
        catalogue = {
            "protocol": PROTOCOL,
            "feature_count": 1,
            "feature_names": list(n367.FEATURE_NAMES),
            ...
            "formula": "q10_i exp(-KL(bond-valence shares at i || uniform degree at i))",
            "output_grid": 1.0 / n367.OUTPUT_GRID,
            "minimum_formal_coverage": MINIMUM_FORMAL_COVERAGE,
            "label_free_statistics": statistics,
            "next368_audit_authorized": True,
            **n367.BOUNDARY_FLAGS,
        }
        ...
        manifest = {
            "protocol": PROTOCOL,
            "mode": "physically_isolated_discovery_x0_periodic_bond_valence_equal_uniformity_freeze",
            "workers": workers,
        ...
        if any(
            ...
            raise RuntimeError("NEXT367 executed artifact changed before publication")
...
__all__ = [
    ...
    "_label_free_statistics",
    "build_cross_source_pbveu_features",
    "n267",
    "n367",
]
