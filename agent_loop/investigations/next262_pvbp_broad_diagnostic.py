#!/usr/bin/env python3
"""Diagnose the frozen NEXT261 PVBP BROAD residual."""
...
PROTOCOL = "2026-08-09-next262-pvbp-broad-diagnostic-v1"
MANIFEST_NAME = "MANIFEST.json"
DIAGNOSTIC_NAME = "NEXT262_PVBP_BROAD_DIAGNOSTIC.json"
TABLE_NAME = "next262_pvbp_broad_diagnostic.parquet"
EXPECTED_DESIGN_SHA256 = n261.EXPECTED_DESIGN_SHA256
...
EXPECTED_CANDIDATE_KEY_SHA256 = (
    "17c94d85ab6498c01726077265eb5403f2b3c42121c0d9920dc1f38db7b2699a"
)
EXPECTED_NEXT261_SEARCH_SHA256 = (
    "4af9665713cbf654a33ce9e4b4160253ec636a23ee3acced1a6314b8e8f27175"
)
NEXT235_REFERENCE_FAILED_COUNT = 5
NEXT235_REFERENCE_SHORTFALL = 0.12339543654931197
SEARCH_WORKERS = n261.SEARCH_WORKERS
BOUNDARY_FLAGS = n261.BOUNDARY_FLAGS
REQUIRED_STAGES = (*n261.REQUIRED_STAGES, 261)
REQUIRED_DESIGN_STAGES = (*n261.REQUIRED_DESIGN_STAGES, 261)
EXPECTED_INPUT_SHA256 = {
    **{
        key: value
        for key, value in n261.EXPECTED_INPUT_SHA256.items()
        if key != "design"
    },
    "next261_design": n261.EXPECTED_INPUT_SHA256["design"],
    "design": EXPECTED_DESIGN_SHA256,
    "next261_manifest": (
        "56bc36d8dab8ebfce707c629bba9978e4ef1e61b1f595a3e43e4a0b5facfa5e4"
    ),
    "next261_catalogue": (
        "145941899de98d564b52a17dd6c83af111b5b00f6f55ba806594d4e257916719"
    ),
    "next261_evaluation": (
        "904a354177dfee3c70a3020834d551f910e5a2663fb99c930e0eb1b5eb79f945"
    ),
    "next261_formula": (
        "ce35cdc1619436c4a78271bb4457e1df8bb3eb4113333cc78140fb5da47de506"
    ),
    "next261_search": EXPECTED_NEXT261_SEARCH_SHA256,
}
...
def select_diagnostic_candidates(published: pd.DataFrame) -> pd.DataFrame:
    ...
    if not isinstance(published, pd.DataFrame) or required - set(published.columns):
        raise ValueError("NEXT262 published candidate columns differ")
    mask = (
...
def candidate_key_sha256(frame: pd.DataFrame) -> str:
    if (
        ...
        raise ValueError("NEXT262 candidate identity table differs")
...
def select_closest_residual(frame: pd.DataFrame) -> pd.Series:
    ...
    if not isinstance(frame, pd.DataFrame) or frame.empty or required - set(frame.columns):
        raise ValueError("NEXT262 residual population differs")
...
def _paths(
    *,
    ...
    paths = n261._paths(
        ...
        design_paths={
            stage: design_paths[stage] for stage in n261.REQUIRED_DESIGN_STAGES
        },
        design_path=design_paths[261],
    )
    paths["next261_design"] = paths.pop("design")
    paths.update(
        {
            "design": design_path,
            "next261_manifest": roots["next261"] / n261.MANIFEST_NAME,
            "next261_catalogue": roots["next261"] / n261.CATALOGUE_NAME,
            "next261_evaluation": roots["next261"] / n261.EVALUATION_NAME,
            "next261_formula": roots["next261"] / n261.FORMULA_NAME,
            "next261_search": roots["next261"] / n261.SEARCH_NAME,
        }
    )
    return paths
...
def _verify_next261(
    ...
    prior_paths = {key: paths[key] for key in n261.EXPECTED_INPUT_SHA256}
    prior_paths["design"] = paths["next261_design"]
    prior_hashes = {key: input_hashes[key] for key in n261.EXPECTED_INPUT_SHA256}
    prior_hashes["design"] = input_hashes["next261_design"]
    prior = n261._verify_next260(prior_paths, prior_hashes)
    manifest = json.loads(paths["next261_manifest"].read_text())
    catalogue = json.loads(paths["next261_catalogue"].read_text())
    evaluation = json.loads(paths["next261_evaluation"].read_text())
    formula = json.loads(paths["next261_formula"].read_text())
    table = pd.read_parquet(paths["next261_search"])
    selected = select_diagnostic_candidates(table)
    expected_outputs = {
        n261.CATALOGUE_NAME: input_hashes["next261_catalogue"],
        n261.EVALUATION_NAME: input_hashes["next261_evaluation"],
        n261.FORMULA_NAME: input_hashes["next261_formula"],
        n261.SEARCH_NAME: input_hashes["next261_search"],
    }
    if (
        manifest.get("protocol") != n261.PROTOCOL
        or manifest.get("candidate_count") != n261.EXPECTED_CANDIDATE_COUNT
        or manifest.get("eligible_new_candidate_count")
        != n261.EXPECTED_ELIGIBLE_COUNT
        or manifest.get("passes_all_cross_source_discovery_gates") is not False
        or manifest.get("next262_diagnostic_authorized") is not True
        or manifest.get("next262_candidate_count") != EXPECTED_CANDIDATE_COUNT
        or manifest.get("next262_candidate_key_sha256")
        != EXPECTED_CANDIDATE_KEY_SHA256
        or manifest.get("outputs_sha256") != expected_outputs
        or manifest.get("executed_source_sha256", {}).get(
            "src/next261_pvbp_margin_local_search.py"
        )
        != EXPECTED_NEXT261_SOURCE_SHA256
        or _sha256_file(Path(n261.__file__).resolve())
        != EXPECTED_NEXT261_SOURCE_SHA256
        or any(manifest.get(key) is not value for key, value in BOUNDARY_FLAGS.items())
        or catalogue.get("candidate_count") != n261.EXPECTED_CANDIDATE_COUNT
        or evaluation.get("passes_all_cross_source_discovery_gates") is not False
        or evaluation.get("next262_candidate_count") != EXPECTED_CANDIDATE_COUNT
        or evaluation.get("next262_candidate_key_sha256")
        != EXPECTED_CANDIDATE_KEY_SHA256
        or formula.get("kind") != "pvbp_triangular_margin_local_x0_no_dft_score"
...
def run_pvbp_broad_diagnostic(
    *,
    ...
    """Reproduce NEXT261 and compute its unchanged discovery BROAD residual."""

    if not set(REQUIRED_STAGES).issubset(stage_dirs):
        raise ValueError("NEXT262 stage directory universe differs")
    if not set(REQUIRED_DESIGN_STAGES).issubset(design_paths):
        raise ValueError("NEXT262 design path universe differs")
    roots = {
    ...
    if type(search_workers) is not int or search_workers <= 0:
        raise ValueError("NEXT262 search_workers must be a positive exact integer")
    if any(not path.is_file() for path in paths.values()):
        raise FileNotFoundError("NEXT262 input is missing")
    ...
    if require_formal_inputs and input_hashes != EXPECTED_INPUT_SHA256:
        ...
        raise ValueError(f"NEXT262 formal input identity differs: {differing}")
    (
        eligible_prior,
        eligible214,
        primary_key,
        ...
        eligible260,
        published261,
    ) = _verify_next261(paths, input_hashes)
    combined, feature_tables, base_score, base_support, endpoint, _ = (
        n261.n227._reconstruct_next224_frontier(
            paths=paths,
    ...
    all_specs = n261.build_pvbp_candidate_specs(
        base_candidate_key=base_key,
        eligible_table=eligible260,
    )
    keys = set(published261["candidate_key"].astype(str))
    specs = [spec for spec in all_specs if str(spec["candidate_key"]) in keys]
    if len(specs) != EXPECTED_CANDIDATE_COUNT:
        raise ValueError("NEXT262 selected specification universe differs")
    virtual, terms, runtime, activity = n261.materialize_pvbp_candidates(
        features=combined,
    ...
    evaluator = (
        n261.n223.n222.n215.n214.n212.n210.n208.n205.n203.n202.n200.n194.n130.n125
    ...
    n164 = n261.n223.n222.n215.n214.n164
    n164._verify_reproduction(
        rerun=rerun["candidate_records"], published=published261
    )
    ...
    published_by_key = published261.set_index("candidate_key", drop=False)
    ...
    for term in terms:
        ...
        if not np.array_equal(support, base_support):
            raise RuntimeError("NEXT262 candidate support differs from NEXT214")
        ...
        if tables is None:
            raise RuntimeError("NEXT262 candidate has no threshold table")
        ...
        if residual["passes_broad"]:
            raise RuntimeError("NEXT262 contradicts NEXT261 BROAD result")
    ...
    if (
        ...
        raise RuntimeError("NEXT262 residual population differs")
    closest = select_closest_residual(per_candidate)
    improves = n261.n223.n222.strictly_improves(
        closest,
        {
    ...
    summary = {
        "protocol": PROTOCOL,
        "diagnostic_mode": "offline_discovery_label_next261_broad_residual",
        "candidate_count": len(per_candidate),
        "candidate_key_sha256": EXPECTED_CANDIDATE_KEY_SHA256,
        "next261_record_population_reproduced": True,
        "next261_candidate_universe_reproduced": True,
        ...
        "pvbp_certificate_branch_closed": bool(not improves),
    ...
    source_paths = {
        ...
        "src/next262_pvbp_broad_diagnostic.py": Path(__file__).resolve(),
    }
    ...
    try:
        ...
        manifest = {
            ...
            "next261_record_population_reproduced": True,
            "next261_candidate_universe_reproduced": True,
            "next261_all_gate_candidate_count": 0,
        ...
        if any(_sha256_file(path) != input_hashes[name] for name, path in paths.items()):
            raise RuntimeError("NEXT262 input changed before publication")
        if any(_sha256_file(path) != source_hashes[name] for name, path in source_paths.items()):
            raise RuntimeError("NEXT262 source changed before publication")
...
def main() -> int:
    ...
    manifest = run_pvbp_broad_diagnostic(
