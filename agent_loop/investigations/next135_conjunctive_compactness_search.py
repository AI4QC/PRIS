#!/usr/bin/env python3
"""Frozen coordination-by-compactness conjunctive protection search."""
...
PROTOCOL = "2026-08-08-next135-conjunctive-compactness-search-v1"
MANIFEST_NAME = "MANIFEST.json"
CATALOGUE_NAME = "NEXT135_CONJUNCTIVE_COMPACTNESS_CATALOGUE.json"
EVALUATION_NAME = "NEXT135_DISCOVERY_EVALUATION.json"
SEARCH_NAME = "next135_conjunctive_compactness_candidate_search.parquet"
EXPECTED_FREEZE_SHA256 = "3da510a1c7e2f95ea275b1cd293b10de965b34c9714a9dd40826358b474d2ede"
EXPECTED_BASE_COUNT = 11
EXPECTED_CONFIGURATION_COUNT = 49
EXPECTED_CANDIDATE_COUNT = 539
EXPECTED_CONFIGURATION_SHA256 = "0f785f513ae9272f7b75f2f085ba3fbaab4f6b02e2fe77efde9bd861480b284b"
EXPECTED_CANDIDATE_KEY_SHA256 = "9736eb28f6c270d0d33391d3236c4c54257552263f5584d1afeac3ad51add2e0"
PACKING_PRODUCT_TERM_ID = "coordination_covalent_packing_product__high"
VOLUME_PRODUCT_TERM_ID = "coordination_low_volume_product__high"
PACKING_PRODUCT_FEATURE = "coordination_covalent_packing_product"
VOLUME_PRODUCT_FEATURE = "coordination_low_volume_product"
PACKING_PRODUCT_SUPPORT = "coordination_covalent_packing_product_supported"
VOLUME_PRODUCT_SUPPORT = "coordination_low_volume_product_supported"
TERM_IDS = (PACKING_PRODUCT_TERM_ID, VOLUME_PRODUCT_TERM_ID)
WEIGHTS = (0.10, 0.25, 0.50, 1.00, 2.00, 4.00)
SEARCH_WORKERS = 4
EXPECTED_INPUT_SHA256 = {
    **{key: value for key, value in n134.EXPECTED_INPUT_SHA256.items() if key != "freeze"},
    "next134_manifest": "b699a5f471a5733bb90a59c2b4814819b086d9278596283552ab314542e495bf",
    "next134_catalogue": "0de2dacd6044ee3bd659026c781423fbe2d4b89368bc889c7f44a80a916ae006",
    "next134_evaluation": "47f163f5fd8af67c1e0bb2211a8a1fa6db1ee4d025ed26c9aad1419b5905162c",
    "next134_search_records": "0088ab82ef3e43dc29b89d247e28febffc8e1d23dd13e9e42f2134a424f6429b",
    "freeze": EXPECTED_FREEZE_SHA256,
}


def materialize_conjunctive_features(table: pd.DataFrame) -> pd.DataFrame:
    required = {
    ...
    if required - set(table.columns):
        raise ValueError("NEXT135 conjunctive feature schema differs")
    coordination = pd.to_numeric(table[n130.n129.FEATURE_NAME], errors="coerce").to_numpy(float)
    ...
    for feature, support, clip, out_feature, out_support in (
        (n133.PACKING_FEATURE, n133.PACKING_SUPPORT, n133.PACKING_CLIP, PACKING_PRODUCT_FEATURE, PACKING_PRODUCT_SUPPORT),
        (n133.VOLUME_FEATURE, n133.VOLUME_SUPPORT, n133.VOLUME_CLIP, VOLUME_PRODUCT_FEATURE, VOLUME_PRODUCT_SUPPORT),
    ):
        ...
        if np.any(active & (~np.isfinite(coordination) | ~np.isfinite(values))):
            raise ValueError("NEXT135 supported conjunctive operand differs")
        product = np.full(len(table), np.nan, dtype=float)
        product[active] = np.clip(
            coordination[active] / n130.n129.CLIP_NORMALIZED,
            0.0,
            1.0,
        ) * np.clip(values[active] / clip, 0.0, 1.0)
        result[out_feature] = product
        result[out_support] = active
    return pd.DataFrame(result, index=table.index)


def build_conjunctive_configurations() -> list[dict[str, object]]:
    ...
    *, bases: pd.DataFrame, physical_term_ids: set[str]
    ...
    for _, row in bases.iterrows():
        ...
        if not base_ids or len(base_ids) != len(base_weights) or any(term_id not in physical_term_ids for term_id in base_ids):
            raise ValueError("NEXT135 base formula differs")
        for configuration in build_conjunctive_configurations():
            payload = {
                "base_term_ids": base_ids,
                "base_weights": base_weights,
                "coordination_protection_term_id": n130.PROTECTION_TERM_ID,
                "coordination_protection_weight": 2.0,
                "conjunctive_term_ids": list(configuration["term_ids"]),
                "conjunctive_weights": list(configuration["weights"]),
            }
...
def materialize_candidates(
    *,
    ...
    feature_spec = {
        PACKING_PRODUCT_TERM_ID: (PACKING_PRODUCT_FEATURE, PACKING_PRODUCT_SUPPORT),
        VOLUME_PRODUCT_TERM_ID: (VOLUME_PRODUCT_FEATURE, VOLUME_PRODUCT_SUPPORT),
    }
    values = {term_id: features[name].to_numpy(float) for term_id, (name, _) in feature_spec.items()}
    active = {term_id: features[support].eq(True).to_numpy() for term_id, (_, support) in feature_spec.items()}
    ...
    for spec_raw in specs:
        ...
        term_ids = [str(value) for value in spec["conjunctive_term_ids"]]
        weights = [float(value) for value in spec["conjunctive_weights"]]
        if base_id is None or len(term_ids) != len(weights) or any(term_id not in values for term_id in term_ids):
            raise ValueError("NEXT135 candidate configuration differs")
        ...
        virtual_id = f"next135_virtual_candidate__{hashlib.sha256(key.encode()).hexdigest()[:24]}"
        feature_name = f"_{virtual_id}_value"
        if not np.isfinite(encoded[supported]).all():
            raise ValueError("NEXT135 virtual candidate encoding differs")
        ...
        terms.append(
            {
                ...
                "center": 0.0,
                "scale": 1.0 / divisor,
                "group": "next135_conjunctive_protected_candidate",
...
def verify_base_reproduction(
    *, result_records: Sequence[Mapping[str, object]], prior: pd.DataFrame
    ...
    for record in result_records:
        ...
        if payload["conjunctive_term_ids"]:
            continue
    ...
    if set(observed) != set(expected):
        raise RuntimeError("NEXT135 base reproduction identities differ")
    for identity, source in expected.items():
        ...
        if any(
            ...
            raise RuntimeError("NEXT135 base diagnostics do not reproduce NEXT130")


def _paths(roots: Mapping[str, Path], freeze_path: Path) -> dict[str, Path]:
    paths = n134._paths(roots, freeze_path)
    paths.update(
        {
            "next134_manifest": roots["next134"] / n134.MANIFEST_NAME,
            "next134_catalogue": roots["next134"] / n134.CATALOGUE_NAME,
            "next134_evaluation": roots["next134"] / n134.EVALUATION_NAME,
            "next134_search_records": roots["next134"] / n134.SEARCH_NAME,
        }
    )
    return paths


def run_conjunctive_compactness_search(
    *,
    ...
    if any(not path.is_file() for path in paths.values()):
        raise FileNotFoundError("NEXT135 input is missing")
    ...
    if require_formal_inputs and input_hashes != EXPECTED_INPUT_SHA256:
        ...
        raise ValueError(f"NEXT135 formal input identity differs: {differing}")
    manifest134 = json.loads(paths["next134_manifest"].read_text())
    if (
        manifest134.get("protocol") != n134.PROTOCOL
        or manifest134.get("passes_all_cross_source_discovery_gates") is not False
        or manifest134.get("opened_validation_outputs_used") is not False
        or manifest134.get("dft_values_used_by_executable_formula") is not False
        or manifest134.get("scigen_replication_endpoint_opened") is not False
        or manifest134.get("wyformer_replication_endpoint_opened") is not False
    ):
        raise ValueError("NEXT135 prior provenance differs")
    ...
    conjunctive = materialize_conjunctive_features(extended)
    ...
    if require_formal_inputs and (
        ...
        or len(specs) != EXPECTED_CANDIDATE_COUNT or base_key_sha != n132.EXPECTED_BASE_KEY_SHA256
        or base_formula_sha != n132.EXPECTED_BASE_FORMULA_SHA256 or config_sha != EXPECTED_CONFIGURATION_SHA256
        or candidate_sha != EXPECTED_CANDIDATE_KEY_SHA256
    ):
        raise ValueError("NEXT135 frozen candidate universe differs")
    ...
    def decorate(record: dict[str, object]) -> None:
        ...
        record["conjunctive_term_ids_json"] = json.dumps(payload["conjunctive_term_ids"], separators=(",", ":"))
        record["conjunctive_weights_json"] = json.dumps(payload["conjunctive_weights"], separators=(",", ":"))
        record["conjunctive_term_count"] = len(payload["conjunctive_term_ids"])
        record["score_composition"] = "max(0,coordination_weight2_base-sum(weights*conjunctive_products))"
    ...
    term_meta = {
        PACKING_PRODUCT_TERM_ID:{"feature":PACKING_PRODUCT_FEATURE,"definition":"normalized_coordination*normalized_covalent_packing"},
        VOLUME_PRODUCT_TERM_ID:{"feature":VOLUME_PRODUCT_FEATURE,"definition":"normalized_coordination*normalized_low_volume"},
    }
    formula["conjunctive_protections"] = [{"term_id":term_id,**term_meta[term_id],"weight":float(weight)} for term_id,weight in zip(payload["conjunctive_term_ids"],payload["conjunctive_weights"],strict=True)]
    formula["score_composition"] = "max(0,coordination_weight2_base-sum(weights*conjunctive_products))"
    formula["kind"] = "next130_coordination_base_with_optional_conjunctive_compactness_protection"
    ...
    counts = {
        ...
        for count, frame in records_frame.groupby("conjunctive_term_count", sort=True)
    }
    catalogue = {
        "protocol":PROTOCOL,"freeze_sha256":input_hashes["freeze"],"base_count":len(bases),
        "configuration_count":len(configurations),"candidate_count":len(specs),"weight_grid":list(WEIGHTS),
        "term_ids":list(TERM_IDS),"base_key_sha256":base_key_sha,"base_formula_sha256":base_formula_sha,
        "configuration_sha256":config_sha,"candidate_key_sha256":candidate_sha,
        "active_score":"max(0,coordination_weight2_base-sum(weights*conjunctive_products))",
        "base_support_unchanged":True,"validation_outputs_opened":False,"dft_values_used_by_executable_formula":False,
    }
    ...
    source_paths = {
        ...
        "src/next135_conjunctive_compactness_search.py":Path(__file__).resolve(),
    }
    ...
    try:
        ...
        _write_json(evaluation_path,{
            "protocol":PROTOCOL,"evaluation_mode":"fixed_conjunctive_compactness_protection",
            ...
            "counts_by_conjunctive_term_count":counts,"safe_gates":dict(n130.n125.n121.prior.DEFAULT_GATES),
            ...
            "passes_all_cross_source_discovery_gates":passes,"freeze_authorized":passes,"requires_unopened_internal_validation_before_claim":True,
        })
        records_frame.to_parquet(search_path,index=False); output_paths.extend([catalogue_path,evaluation_path,search_path])
        manifest={
            "protocol":PROTOCOL,"label_free_catalogue_sha256":catalogue_sha,"base_count":len(bases),"configuration_count":len(configurations),
            "candidate_count":int(result["candidate_count"]),"search_workers":search_workers,"base_only_reproduced_next130":True,
            "passes_all_cross_source_discovery_gates":passes,"freeze_authorized":passes,"requires_unopened_internal_validation_before_claim":True,
            "scigen_discovery_endpoint_opened":True,"wyformer_discovery_endpoint_opened":True,"discovery_outcomes_used_as_offline_labels":True,
            "opened_validation_outputs_used":False,"scigen_replication_endpoint_opened":False,"wyformer_replication_endpoint_opened":False,
            "formula_or_threshold_changed_after_search":False,"dft_calculation_executed":False,"dft_values_used_by_executable_formula":False,
            "learned_energy_force_stress_proxy_used":False,"physical_relaxation_executed":False,"scientific_improvement_claim":False,
            "inputs_sha256":input_hashes,"executed_source_sha256":source_hashes,"outputs_sha256":{path.name:_sha256_file(path) for path in output_paths},
        }
        _write_json(staging/MANIFEST_NAME,manifest)
        if any(_sha256_file(path)!=input_hashes[name] for name,path in paths.items()): raise RuntimeError("NEXT135 input changed before publication")
        if any(_sha256_file(path)!=source_hashes[name] for name,path in source_paths.items()): raise RuntimeError("NEXT135 source changed before publication")
...
def main() -> int:
    ...
    for stage in (98,110,111,113,114,116,117,120,121,122,124,125,129,130,133,134): parser.add_argument(f"--next{stage}-dir",type=Path,required=True)
    ...
    manifest=run_conjunctive_compactness_search(
        ...
        **{f"next{stage}_dir":getattr(args,f"next{stage}_dir") for stage in (98,110,111,113,114,116,117,120,121,122,124,125,129,130,133,134)},
        freeze_path=args.freeze_path,output_dir=args.output_dir,search_workers=args.search_workers,require_formal_inputs=not args.allow_nonformal_inputs,
    )
