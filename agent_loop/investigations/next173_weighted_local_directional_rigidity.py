"""Label-free graph-weighted local directional-rigidity features."""
...
PROTOCOL = "2026-08-08-next173-weighted-local-directional-rigidity-v1"
CATALOGUE_NAME = "NEXT173_WEIGHTED_LOCAL_DIRECTIONAL_RIGIDITY_CATALOGUE.json"
SCIGEN_NAME = "next173_scigen_weighted_local_directional_rigidity.parquet"
WYFORMER_NAME = "next173_wyformer_weighted_local_directional_rigidity.parquet"
MANIFEST_NAME = "MANIFEST.json"
GRAPH_MODES = n168.GRAPH_MODES
FEATURE_SUFFIXES = n168.FEATURE_SUFFIXES
FEATURE_NAMES = tuple(
    f"pwldr_{mode}_{suffix}" for mode in GRAPH_MODES for suffix in FEATURE_SUFFIXES
)
ROUND_OFF_TOLERANCE = n168.ROUND_OFF_TOLERANCE
EXPECTED_ROWS = n168.EXPECTED_ROWS
EXPECTED_DESIGN_SHA256 = "00c14e331d49bb9a8f4eafa16aaab099ca0b10f088b638b905feeb692bee7286"
EXPECTED_INPUT_SHA256 = {
    **n168.EXPECTED_INPUT_SHA256,
    "design": EXPECTED_DESIGN_SHA256,
}


def weighted_local_directional_rigidity_features(
    *,
    n_sites: int,
    ...
    """Summarize frozen-edge-weighted local direction Gram certificates."""
    ...
    weight_sum = np.zeros(n_sites, dtype=float)
    for index, (left, right) in enumerate(pair):
        contribution = weight[index] * np.outer(direction[index], direction[index])
        tensors[left] += contribution
        tensors[right] += contribution
        weight_sum[left] += weight[index]
        weight_sum[right] += weight[index]

    tightness = np.zeros(n_sites, dtype=float)
    volume = np.zeros(n_sites, dtype=float)
    for index in np.flatnonzero(weight_sum):
        gram = tensors[index] / weight_sum[index]
        ...
        if (
            ...
            raise RuntimeError("weighted local directional Gram spectrum is invalid")
    ...
    result = {
        ...
        f"{prefix}_tightness_q10": n168._inverted_cdf(tightness, 0.10),
    ...
    if (
        ...
        raise RuntimeError("NEXT173 feature schema or bounds differ")
    return result
...
def _weighted_geometry(
    ...
    endpoints, vectors = n168._vectors_from_edges(structure, edges)
    weights = np.asarray([float(edge.neighbor_weight) for edge in edges], dtype=float)
    return endpoints, vectors, weights


def compute_weighted_local_directional_rigidity(structure) -> dict[str, object]:
    ...
    for mode in GRAPH_MODES:
        row[f"pwldr_{mode}_supported"] = False
        row[f"pwldr_{mode}_failure"] = None
    ...
    if not assignment.supported or assignment.values is None:
        ...
        for mode in GRAPH_MODES:
            row[f"pwldr_{mode}_failure"] = reason
        return row
    for mode in GRAPH_MODES:
        ...
        if not geometry.supported:
            row[f"pwldr_{mode}_failure"] = geometry.failure_reason
            continue
        try:
            ...
            row.update(
                weighted_local_directional_rigidity_features(
                    ...
                    vectors=vectors,
                    weights=weights,
                    prefix=f"pwldr_{mode}",
                )
            )
        except Exception as exc:
            row[f"pwldr_{mode}_failure"] = f"{type(exc).__name__}: {exc}"
            continue
        row[f"pwldr_{mode}_supported"] = True
    expected = {
        *FEATURE_NAMES,
        *(f"pwldr_{mode}_supported" for mode in GRAPH_MODES),
        *(f"pwldr_{mode}_failure" for mode in GRAPH_MODES),
    }
    if set(row) != expected:
        raise RuntimeError("NEXT173 row schema differs")
    return row
...
def _compute_scigen_payload(item: tuple[str, bytes]) -> tuple[str, dict[str, object]]:
    ...
    try:
        ...
        row = compute_weighted_local_directional_rigidity(structure)
    except Exception as exc:
...
def _label_free_statistics(frame: pd.DataFrame) -> dict[str, object]:
    ...
    for mode in GRAPH_MODES:
        mask = frame[f"pwldr_{mode}_supported"].fillna(False).astype(bool)
        ...
        for name in FEATURE_NAMES:
            if not name.startswith(f"pwldr_{mode}_"):
                continue
            values = frame.loc[mask, name].to_numpy(dtype=float)
            if len(values) and not np.isfinite(values).all():
                raise RuntimeError(f"NEXT173 non-finite supported feature: {name}")
...
def build_weighted_local_directional_rigidity_features(
    *,
    ...
    if any(not path.is_file() for path in paths.values()):
        raise FileNotFoundError("NEXT173 input is missing")
    ...
    if require_formal_inputs and input_hashes != EXPECTED_INPUT_SHA256:
        ...
        raise ValueError(f"NEXT173 formal input identity differs: {differing}")
    ...
    if (
        ...
        raise ValueError("NEXT173 geometry provenance differs")
    ...
    with zipfile.ZipFile(paths["scigen_geometry_discovery"]) as archive:
        ...
        if names != sorted(names) or any(not name.endswith(".extxyz") for name in names):
            raise ValueError("NEXT173 SCIGEN geometry inventory differs")
    ...
    if (
        ...
        raise ValueError("NEXT173 WyFormer geometry inventory differs")
    ...
    if (
        ...
        raise ValueError("NEXT173 discovery row identity differs")
    ...
    for source, results in (("scigen", scigen_results), ("wyformer", wyformer_results)):
        ...
        if (
            ...
            raise RuntimeError(f"NEXT173 {source} output schema differs")
        frames[source] = frame
    statistics = {
    ...
    catalogue = {
        ...
        "definition": "neighbor_weighted_site_direction_gram_T_equals_3_lambda_min_V_equals_27_determinant",
    ...
    source_paths = {
        ...
        "src/next173_weighted_local_directional_rigidity.py": Path(__file__).resolve(),
    }
    ...
    try:
        ...
        if any(
            ...
            raise RuntimeError("NEXT173 input changed before publication")
        if any(
            _sha256_file(path) != source_hashes[name]
            for name, path in source_paths.items()
        ):
            raise RuntimeError("NEXT173 source changed before publication")
...
def main() -> None:
    ...
    manifest = build_weighted_local_directional_rigidity_features(
