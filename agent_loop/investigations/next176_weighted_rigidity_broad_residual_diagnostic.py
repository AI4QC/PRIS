#!/usr/bin/env python3
"""Diagnose BROAD residuals for frozen NEXT175 discovery candidates."""
...
PROTOCOL = "2026-08-08-next176-weighted-rigidity-broad-residual-diagnostic-v1"
MANIFEST_NAME = "MANIFEST.json"
DIAGNOSTIC_NAME = "NEXT176_WEIGHTED_RIGIDITY_BROAD_RESIDUAL_DIAGNOSTIC.json"
PER_CANDIDATE_NAME = "next176_weighted_rigidity_broad_residual_per_candidate.parquet"
EXPECTED_DESIGN_SHA256 = "5edd2f93a4c0c89e9dc24736af6436b7961ca65165112bd78647e945b85622e4"
...
EXPECTED_FEATURE_COUNTS = {
    "BASE": 1,
    "pwldr_crystalnn_tightness_mean": 8,
    "pwldr_crystalnn_tightness_min": 8,
    "pwldr_crystalnn_tightness_q10": 8,
    "pwldr_crystalnn_volume_mean": 7,
    "pwldr_crystalnn_volume_q10": 8,
}
SEARCH_WORKERS = 4
EXPECTED_INPUT_SHA256 = {
    **n175.EXPECTED_INPUT_SHA256,
    "design": EXPECTED_DESIGN_SHA256,
    "next175_manifest": "ffd9a4501f6adfda0b13fc3c4eb9ee981d812e412135ab565793c3915e0df982",
    "next175_catalogue": "ce74eeb0aee1aede28afce67c7abbba7ba69df57d569a734656e4e501489c80d",
    "next175_evaluation": "c7e7756be189b829b31441dabd72c6104ecd5d4dc7cac1345fa57cead3c0311f",
    "next175_formula": "43a6f947ec7593dabfb2cda735cfbde2b9be8f29ce1224302ea791b9c3014002",
    "next175_search": "b17e0be427dfa48bb3375a64663ce9f24ce1d09c698c2f6940aa4e94878abe80",
}
...
def select_diagnostic_candidates(published: pd.DataFrame) -> pd.DataFrame:
    ...
    if (
        ...
        raise ValueError("NEXT176 published candidate schema differs")
    mask = (
...
def _paths(
    ...
    paths = n175._paths(roots, freeze_path, design_path)
    paths.update(
        {
            "next175_manifest": roots["next175"] / n175.MANIFEST_NAME,
            "next175_catalogue": roots["next175"] / n175.CATALOGUE_NAME,
            "next175_evaluation": roots["next175"] / n175.EVALUATION_NAME,
            "next175_formula": roots["next175"] / n175.FORMULA_NAME,
            "next175_search": roots["next175"] / n175.SEARCH_NAME,
        }
    )
    return paths
...
def _closest(frame: pd.DataFrame) -> pd.Series:
    if frame.empty:
        raise ValueError("NEXT176 closest population is empty")
...
def run_weighted_rigidity_broad_residual_diagnostic(
    *,
    ...
    """Run the frozen discovery-only NEXT176 BROAD residual diagnostic."""
    ...
    if any(not path.is_file() for path in paths.values()):
        raise FileNotFoundError("NEXT176 input is missing")
    ...
    if require_formal_inputs and input_hashes != EXPECTED_INPUT_SHA256:
        ...
        raise ValueError(f"NEXT176 formal input identity differs: {differing}")

    manifest175 = json.loads(paths["next175_manifest"].read_text())
    evaluation175 = json.loads(paths["next175_evaluation"].read_text())
    expected_outputs175 = {
        n175.CATALOGUE_NAME: input_hashes["next175_catalogue"],
        n175.EVALUATION_NAME: input_hashes["next175_evaluation"],
        n175.FORMULA_NAME: input_hashes["next175_formula"],
        n175.SEARCH_NAME: input_hashes["next175_search"],
    }
    if (
        manifest175.get("protocol") != n175.PROTOCOL
        or manifest175.get("candidate_count") != n175.EXPECTED_CANDIDATE_COUNT
        or manifest175.get("base_endpoint_reproduced") is not True
        or manifest175.get("passes_all_cross_source_discovery_gates") is not False
        or manifest175.get("freeze_authorized") is not False
        or manifest175.get("weighted_rigidity_repair_width_branch_terminated")
        is not True
        or manifest175.get("opened_validation_outputs_used") is not False
        or manifest175.get("scigen_replication_endpoint_opened") is not False
        or manifest175.get("wyformer_replication_endpoint_opened") is not False
        or manifest175.get("dft_calculation_executed") is not False
        or manifest175.get("dft_values_used_by_executable_formula") is not False
        or manifest175.get("learned_energy_force_stress_proxy_used") is not False
        or manifest175.get("physical_relaxation_executed") is not False
        or manifest175.get("outputs_sha256") != expected_outputs175
        or manifest175.get("executed_source_sha256", {}).get(
            "src/next175_weighted_rigidity_repair_width_search.py"
        )
        != _sha256_file(Path(n175.__file__).resolve())
        or evaluation175.get("protocol") != n175.PROTOCOL
        or evaluation175.get("candidate_count") != n175.EXPECTED_CANDIDATE_COUNT
        or evaluation175.get("passes_all_cross_source_discovery_gates") is not False
        or evaluation175.get("freeze_authorized") is not False
    ):
        raise ValueError("NEXT176 NEXT175 provenance differs")
    ...
    if (
        ...
        != n175.EXPECTED_BASE_CANDIDATE_KEY_SHA256
    ):
        raise ValueError("NEXT176 base candidate identity differs")
    ...
    if len(selected_base_specs) != 1:
        raise ValueError("NEXT176 base reconstruction differs")
    ...
    if len(base_terms) != 1 or len(base_runtime) != 1:
        raise RuntimeError("NEXT176 base materialization differs")
    ...
    if not np.isfinite(endpoint).all() or len(combined) != len(base_score):
        raise ValueError("NEXT176 endpoint row accounting differs")

    specs = n175.build_candidate_specs(base_candidate_key=base_candidate_key)
    combined, virtual_terms, runtime = n175.materialize_interval_width_candidates(
        features=combined,
    ...
    published_all = pd.read_parquet(paths["next175_search"])
    if set(published_all["candidate_key"].astype(str)) != set(virtual_by_candidate):
        raise ValueError("NEXT176 published universe differs")
    ...
    if require_formal_inputs and (
        ...
        raise ValueError("NEXT176 frozen diagnostic population differs")
    ...
    for _, row in published.iterrows():
        ...
        if tables is None:
            raise RuntimeError("NEXT176 candidate has no threshold table")
        ...
        if diagnostic["passes_broad"] or bool(row["passes_broad_all_cells"]):
            raise RuntimeError("NEXT176 contradicts NEXT175 BROAD result")
    ...
    if len(base_rows) != 1:
        raise RuntimeError("NEXT176 base residual count differs")
    ...
    summary = {
        "protocol": PROTOCOL,
        "diagnostic_mode": "offline_discovery_label_next175_broad_constraint_residual",
    ...
    source_paths = {
        ...
        "src/next175_weighted_rigidity_repair_width_search.py": Path(n175.__file__).resolve(),
        "src/next176_weighted_rigidity_broad_residual_diagnostic.py": Path(__file__).resolve(),
    }
    ...
    try:
        ...
        manifest = {
            ...
            "candidate_count": int(len(per_candidate)),
            "candidate_key_sha256": candidate_sha,
            "next175_records_reproduced": True,
            "new_formula_searched": False,
            "new_formula_selected": False,
            "weighted_rigidity_broad_residual_diagnosed": True,
        ...
        if any(_sha256_file(path) != input_hashes[name] for name, path in paths.items()):
            raise RuntimeError("NEXT176 input changed before publication")
        if any(
            _sha256_file(path) != source_hashes[name]
            for name, path in source_paths.items()
        ):
            raise RuntimeError("NEXT176 source changed before publication")
...
def main() -> int:
    ...
    manifest = run_weighted_rigidity_broad_residual_diagnostic(
...
__all__ = [
    ...
    "run_weighted_rigidity_broad_residual_diagnostic",
]
