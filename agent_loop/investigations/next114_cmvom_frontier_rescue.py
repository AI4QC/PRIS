#!/usr/bin/env python3
"""Frozen second-round rescue of the NEXT111 AUC--SAFE frontier."""
...
from src.next111_cmvo_optional_search import (
    ...
    SEARCH_NAME as NEXT111_SEARCH_NAME,
...
PROTOCOL = "2026-08-08-next114-cmvom-frontier-rescue-v1"
MANIFEST_NAME = "MANIFEST.json"
CATALOGUE_NAME = "NEXT114_CMVOM_FRONTIER_CATALOGUE.json"
EVALUATION_NAME = "NEXT114_DISCOVERY_EVALUATION.json"
SEARCH_NAME = "next114_cmvom_frontier_candidate_search.parquet"
EXPECTED_FREEZE_SHA256 = "83414bc8c38cff32406f5f5f8b761993aa8e43d08b8c528fe04bbe726b76827d"
SINGLE_WEIGHT_GRID = (0.25, 0.5, 1.0, 2.0, 4.0)
PAIR_WEIGHT_GRID = (0.25, 0.5, 1.0, 2.0)
FRONTIER_AUC_SAFE_CELLS = 10
BASE_REPRODUCTION_AUC_TOLERANCE = 2.0e-5
EXPECTED_FRONTIER_BASES = 42
EXPECTED_AUC_FRONTIER_BASES = 38
EXPECTED_SAFE_FRONTIER_BASES = 4
EXPECTED_CONFIGURATION_COUNT = 63
EXPECTED_CANDIDATE_COUNT = 2_688
EXPECTED_BASE_CANDIDATE_KEY_SHA256 = (
    "f9636ddde7e685bbf3f90048a06bf2440eaae1b8e0da40d894fa9239ed434ec8"
)
EXPECTED_BASE_FORMULA_SHA256 = (
    "c413f8facec77b1ca395f9a5712806a81b113cd6b1952d9f48aa8ec9ab075244"
)

FROZEN_TERM_SPECS = (
    {
        "term_id": "cmvom_core_component_gap_site_mean__high",
        "raw_feature": "cmvom_core_component_gap_site_mean",
        "group": "cmvom_core",
        "support_column": "cmvom_core_supported",
        "center": 0.14285714285714285,
        "scale": 0.4666666666666667,
        "raw_p995": 0.8971158730158726,
        "clip_normalized": 1.6162687074829925,
    },
    {
        "term_id": "cmvom_core_localized_slack_severity__high",
        "raw_feature": "cmvom_core_localized_slack_severity",
        "group": "cmvom_core",
        "support_column": "cmvom_core_supported",
        "center": 0.041666666666666616,
        "scale": 0.15299036281179137,
        "raw_p995": 0.4966749999999987,
        "clip_normalized": 2.974098008337185,
    },
    {
        "term_id": "cmvom_core_side_slack_asymmetry__high",
        "raw_feature": "cmvom_core_side_slack_asymmetry",
        "group": "cmvom_core",
        "support_column": "cmvom_core_supported",
        "center": 0.07142857142857138,
        "scale": 0.2222222222222222,
        "raw_p995": 0.4444444444444444,
        "clip_normalized": 1.6785714285714286,
    },
)
EXCLUDED_RAW_FEATURES = (
    "cmvom_core_component_gap_site_rms",
    "cmvom_core_obstructed_site_fraction",
    "cmvom_core_side_slack_flexibility",
    "cmvom_expanded_component_gap_site_mean",
    "cmvom_expanded_component_gap_site_rms",
    "cmvom_expanded_obstructed_site_fraction",
    "cmvom_expanded_localized_slack_severity",
    "cmvom_expanded_side_slack_asymmetry",
    "cmvom_expanded_side_slack_flexibility",
)
...
EXPECTED_INPUT_SHA256 = {
    ...
    "next111_search_records": "b7329271b05cdb91e2881ca62d7fbb1ded0805a86f0355b44b7b089abb3cadaa",
...
def select_frontier_bases(records: pd.DataFrame) -> pd.DataFrame:
    """Freeze the AUC-best-SAFE and all-SAFE conflict fronts from NEXT111."""
    ...
    if required - set(records.columns):
        raise ValueError("NEXT114 frontier base schema differs")
    ...
    for _, row in records.loc[keep].iterrows():
        ...
        if (
            ...
            raise ValueError("NEXT114 flattened frontier formula differs")
...
def materialize_cmvom_tail_terms(
    features: pd.DataFrame,
) -> tuple[pd.DataFrame, list[dict[str, object]]]:
    """Encode the three frozen morphology high tails reversibly."""
    ...
    for raw_spec in FROZEN_TERM_SPECS:
        ...
        if raw_feature not in features or raw_support not in features:
            raise ValueError("NEXT114 frozen morphology feature is missing")
        ...
        if not all(math.isfinite(value) for value in (center, scale, cap)) or scale <= 0.0 or cap <= 0.0:
            raise ValueError("NEXT114 frozen morphology calibration differs")
        ...
        if not np.isfinite(encoded).all():
            raise ValueError("NEXT114 reversible morphology encoding overflowed")
...
def build_cmvom_guard_configurations(
    term_specs: Sequence[Mapping[str, object]],
) -> list[dict[str, object]]:
    """Enumerate three singles and every pair on the frozen grids."""
    ...
    if (
        len(terms) != 3
        or len(set(term_ids)) != 3
        or any(not isinstance(term_id, str) for term_id in term_ids)
        or any(term.get("group") != "cmvom_core" for term in terms)
    ):
        raise ValueError("NEXT114 frozen morphology term identity differs")
...
def build_frontier_candidate_specs(
    *,
    ...
    """Attach zero or one frozen morphology configuration to every frontier base."""

    required = {"prior_candidate_key", "term_ids_json", "weights_json"}
    if required - set(base_records.columns):
        raise ValueError("NEXT114 frontier candidate columns differ")
    ...
    for _, row in base_records.iterrows():
        ...
        if (
            ...
            raise ValueError("NEXT114 frontier base formula differs")
...
def _verify_base_reproduction(
    *, result_records: Sequence[Mapping[str, object]], prior: pd.DataFrame
    ...
    """Prove that flattening NEXT111 optional terms did not alter its bases."""
    ...
    if len(observed) != len(prior):
        raise RuntimeError("NEXT114 base-only reproduction count differs")
    for _, row in prior.iterrows():
        ...
        if record is None:
            raise RuntimeError("NEXT114 base-only formula identity differs")
        ...
        if any(
            ...
            raise RuntimeError("NEXT114 base-only diagnostics do not reproduce NEXT111")


def run_cmvom_frontier_rescue(
    *,
    ...
    """Run the frozen morphology rescue on the two NEXT111 conflict fronts."""
    ...
    paths = {
        ...
        "next111_search_records": roots["next111"] / NEXT111_SEARCH_NAME,
    ...
    if any(not path.is_file() for path in paths.values()):
        raise FileNotFoundError("NEXT114 discovery input is missing")
    input_hashes = {name: _sha256_file(path) for name, path in paths.items()}
    if require_formal_inputs and input_hashes != EXPECTED_INPUT_SHA256:
        raise ValueError("NEXT114 formal input identity differs")

    next98_manifest = _read_json(paths["next98_manifest"])
    next110_manifest = _read_json(paths["next110_manifest"])
    next111_manifest = _read_json(paths["next111_manifest"])
    next113_manifest = _read_json(paths["next113_manifest"])
    if (
        ...
        or next111_manifest.get("protocol") != NEXT111_PROTOCOL
        or next111_manifest.get("passes_all_cross_source_discovery_gates") is not False
        or next111_manifest.get("opened_validation_outputs_used") is not False
        or next111_manifest.get("scigen_replication_endpoint_opened") is not False
        or next111_manifest.get("wyformer_replication_endpoint_opened") is not False
        or next113_manifest.get("protocol") != NEXT113_PROTOCOL
        or next113_manifest.get("labels_opened") is not False
        or next113_manifest.get("endpoint_payloads_opened") is not False
        or next113_manifest.get("validation_geometry_opened") is not False
        or next113_manifest.get("replication_geometry_opened") is not False
        or next113_manifest.get("dft_values_used_by_features") is not False
    ):
        raise ValueError("NEXT114 prior provenance differs")
    for manifest, expected in (
        ...
        (
            next111_manifest,
            {
                ...
                NEXT111_SEARCH_NAME: "next111_search_records",
            },
        ),
        (
            next113_manifest,
        ...
        if not isinstance(outputs, Mapping) or any(
            ...
            raise ValueError("NEXT114 prior output provenance differs")
    ...
    for source in ("scigen", "wyformer"):
        old, cmvo, morph = old_tables[source], cmvo_tables[source], morph_tables[source]
        if any(
            table["material_id"].astype(str).duplicated().any()
            for table in (old, cmvo, morph)
        ) or set(NEXT110_FEATURE_COLUMNS) - set(cmvo.columns) or set(NEXT113_FEATURE_COLUMNS) - set(morph.columns):
            raise ValueError(f"NEXT114 {source} feature identity differs")
        merged = old.merge(
            cmvo.loc[:, ["material_id", *NEXT110_FEATURE_COLUMNS]],
            on="material_id",
            how="inner",
            validate="one_to_one",
        ).merge(
            morph.loc[:, ["material_id", *NEXT113_FEATURE_COLUMNS]],
            on="material_id",
            how="inner",
            validate="one_to_one",
        )
        if len(merged) != len(old) or len(merged) != len(cmvo) or len(merged) != len(morph):
            raise ValueError(f"NEXT114 {source} feature row accounting differs")
        merged = merged.copy()
    ...
    morphology_features, eligible_terms = materialize_cmvom_tail_terms(cmvo_features)
    ...
    if not isinstance(old_terms_raw, list) or next111_catalogue.get("eligible_optional_terms") != cmvo_terms:
        raise ValueError("NEXT114 prior term catalogue differs")
    old_terms = [*old_terms_raw, *cmvo_terms]
    old_term_ids = {str(term["term_id"]) for term in old_terms}

    prior_records = pd.read_parquet(paths["next111_search_records"])
    ...
    base_formula_sha = hashlib.sha256("\n".join(base_formulas).encode("utf-8")).hexdigest()
    ...
    label_free_catalogue = {
        "protocol": PROTOCOL,
        "calibration_stage": "new_morphology_label_free_before_endpoint_join",
        "adaptive_second_round_discovery": True,
        "freeze_sha256": input_hashes["freeze"],
        "eligible_optional_terms": eligible_terms,
        "excluded_raw_features": list(EXCLUDED_RAW_FEATURES),
        ...
        "discovery_endpoints_previously_opened_by_next111": True,
        "new_morphology_features_joined_to_endpoint_before_freeze": False,
    }
    ...
    if require_formal_inputs and (
        ...
        raise ValueError("NEXT114 frozen candidate universe differs")

    # Discovery endpoints are opened only after the new morphology catalogue is hashed.
    ...
    if (
        ...
        raise ValueError("NEXT114 discovery endpoint identities are duplicated")
    ...
    result["selected"]["formula"]["kind"] = (
        "next111_frontier_base_plus_up_to_two_canonical_obstruction_morphology_terms"
    )
    ...
    source_paths = {
        ...
        "src/next114_cmvom_frontier_rescue.py": Path(__file__).resolve(),
    }
    ...
    try:
        ...
        evaluation = {
            "protocol": PROTOCOL,
            "evaluation_mode": "adaptive_second_round_cross_source_discovery_cmvom_frontier_rescue",
            "rows": {
            ...
            "base_only_reproduced_next111": True,
        ...
        if any(_sha256_file(path) != input_hashes[name] for name, path in paths.items()):
            raise RuntimeError("NEXT114 input changed before publication")
        if any(_sha256_file(path) != source_hashes[name] for name, path in source_paths.items()):
            raise RuntimeError("NEXT114 source changed before publication")
...
def main() -> int:
    ...
    manifest = run_cmvom_frontier_rescue(
