#!/usr/bin/env python3
"""Audit frozen DVCI hypotheses inside the exact NEXT224 rejected cohort."""
...
import src.next255_delaunay_void_cage_isotropy as n255
from src.next102_cross_source_dobvr_features import _sha256_file, _write_json


PROTOCOL = "2026-08-09-next256-dvci-feature-audit-v1"
MANIFEST_NAME = "MANIFEST.json"
CATALOGUE_NAME = "NEXT256_DVCI_FEATURE_CATALOGUE.json"
AUDIT_NAME = "NEXT256_DVCI_FEATURE_AUDIT.json"
TABLE_NAME = "next256_dvci_feature_audit.parquet"
EXPECTED_DESIGN_SHA256 = n255.EXPECTED_DESIGN_SHA256
HYPOTHESES = tuple((name, "protected_high") for name in n255.FEATURE_NAMES)
QUANTILES = (1 / 16, 15 / 16)
EXPECTED_NEXT227_SOURCE_SHA256 = (
    "e092b45d8e3d01cb8c5754e7de0bd6908ea00aa49b4286234074db23758d4ef1"
)
EXPECTED_NEXT255_SOURCE_SHA256 = (
    "8ea76859dc40af08b7bcca67c540ae1dfbcd1988324d562890540aecc771b75d"
)
NEXT255_SOURCE_PATH = "src/next255_delaunay_void_cage_isotropy.py"
REQUIRED_STAGES = (*n227.REQUIRED_STAGES, 255)
REQUIRED_DESIGN_STAGES = (*n227.REQUIRED_DESIGN_STAGES, 227, 255)
...
EXPECTED_INPUT_SHA256 = {
    **{
    ...
    "next255_design": EXPECTED_DESIGN_SHA256,
    "design": EXPECTED_DESIGN_SHA256,
    "next255_manifest": (
        "a659228f8198b0d69784e9c47f0f31f6a69c304e26baad3f04b633a494f9f9b6"
    ),
    "next255_catalogue": (
        "545520c5071c16beb796277e1761dd9c5452829802af9b7786ffeeb26a487c81"
    ),
    "next255_scigen": (
        "0d5097e5ed511cc6d9f843be3fcf9db1b91144cd48c5515be03dfc29edd94ff5"
    ),
    "next255_wyformer": (
        "1885b233151e0c5416095dbf8d45d54cdeaa61da80b7f7446addc5f33a346c0a"
    ),
}
...
def select_eligible_hypotheses(
    ...
    if not isinstance(frame, pd.DataFrame) or frame.empty or required - set(frame.columns):
        raise ValueError("NEXT256 audit ranking table differs")
    result = frame.copy()
...
def bounded_protection(
    *, values: object, direction: str, q_lo: float, q_hi: float
    ...
    if raw.ndim != 1 or direction not in n227.PROTECTION_DIRECTIONS or not (
        ...
        raise ValueError("NEXT256 bounded protection inputs differ")
...
def _index_dvci_by_prefixed_material_id(
    *, table: pd.DataFrame, source: str, expected_material_ids: object
) -> pd.DataFrame:
    """Return a source-prefixed DVCI table after exact identity validation."""
    ...
    if set(extra["material_id"]) != expected:
        raise ValueError(f"NEXT256 {source} material identity differs")
...
def _paths(
    *,
    ...
    paths.update(
        {
            "next255_design": design_paths[255],
            "design": design_path,
            "next255_manifest": roots["next255"] / n255.MANIFEST_NAME,
            "next255_catalogue": roots["next255"] / n255.CATALOGUE_NAME,
            "next255_scigen": roots["next255"] / n255.FEATURE_FILES["scigen"],
            "next255_wyformer": roots["next255"] / n255.FEATURE_FILES["wyformer"],
        }
    )
    return paths
...
def _verify_next255(
    ...
    manifest = json.loads(paths["next255_manifest"].read_text())
    catalogue = json.loads(paths["next255_catalogue"].read_text())
    tables = {
        "scigen": pd.read_parquet(paths["next255_scigen"]),
        "wyformer": pd.read_parquet(paths["next255_wyformer"]),
    }
    expected_outputs = {
        n255.CATALOGUE_NAME: input_hashes["next255_catalogue"],
        n255.FEATURE_FILES["scigen"]: input_hashes["next255_scigen"],
        n255.FEATURE_FILES["wyformer"]: input_hashes["next255_wyformer"],
    }
    if (
        _sha256_file(Path(n227.__file__).resolve()) != EXPECTED_NEXT227_SOURCE_SHA256
        or manifest.get("protocol") != n255.PROTOCOL
        ...
        or manifest.get("executed_source_sha256", {}).get(NEXT255_SOURCE_PATH)
        != EXPECTED_NEXT255_SOURCE_SHA256
        or _sha256_file(Path(n255.__file__).resolve()) != EXPECTED_NEXT255_SOURCE_SHA256
        or any(manifest.get(key) is not value for key, value in n255.BOUNDARY_FLAGS.items())
        or catalogue.get("feature_names") != list(n255.FEATURE_NAMES)
        or catalogue.get("feature_count") != len(n255.FEATURE_NAMES)
        or catalogue.get("forced_tetrahedralization") is not False
        or catalogue.get("quantile_method") != "inverted_cdf"
        or catalogue.get("labels_opened") is not False
    ):
        raise ValueError("NEXT256 NEXT255 provenance differs")
    for source, expected_rows in (("scigen", 13_470), ("wyformer", 5_232)):
        ...
        if (
            ...
            or not table["dvci_supported"].fillna(False).astype(bool).all()
            or (pd.to_numeric(table["dvci_incidence_count"], errors="coerce") <= 0).any()
            or any(
                not np.isfinite(pd.to_numeric(table[name], errors="coerce")).all()
                for name in n255.FEATURE_NAMES
            )
        ):
            raise ValueError(f"NEXT256 NEXT255 {source} table differs")
    return (*prior, tables)


def run_dvci_feature_audit(
    *,
    ...
    """Audit frozen DVCI hypotheses without searching a new formula."""

    if not set(REQUIRED_STAGES).issubset(stage_dirs):
        raise ValueError("NEXT256 stage directory universe differs")
    if not set(REQUIRED_DESIGN_STAGES).issubset(design_paths):
        raise ValueError("NEXT256 design path universe differs")
    roots = {
    ...
    if any(not path.is_file() for path in paths.values()):
        raise FileNotFoundError("NEXT256 input is missing")
    ...
    if require_formal_inputs and input_hashes != EXPECTED_INPUT_SHA256:
        ...
        raise ValueError(f"NEXT256 formal input identity differs: {differing}")
    (
        eligible,
        eligible214,
        primary_key,
        ...
        formula222,
        dvci_tables,
    ) = _verify_next255(paths, input_hashes)
    ...
    if cohort_counts != n227.EXPECTED_COHORT_COUNTS:
        raise ValueError("NEXT256 rejected cohort counts differ")
    ...
    audit = {
        "protocol": PROTOCOL,
        "audit_mode": "fixed_next224_rejected_extreme_dvci_feature_audit",
        "base_candidate_key_sha256": n227.EXPECTED_BASE_CANDIDATE_KEY_SHA256,
        "base_threshold": n227.EXPECTED_BASE_THRESHOLD,
        "cohort_counts": {
        ...
        "next257_search_authorized": bool(selected is not None),
    ...
    source_paths = {
        ...
        NEXT255_SOURCE_PATH: Path(n255.__file__).resolve(),
        "src/next256_dvci_feature_audit.py": Path(__file__).resolve(),
    }
    ...
    try:
        ...
        manifest = {
            ...
            "next257_search_authorized": bool(selected is not None),
            "dvci_branch_terminated": selected is None,
            "new_formula_searched": False,
            "new_formula_selected": False,
        ...
        if any(_sha256_file(path) != input_hashes[name] for name, path in paths.items()):
            raise RuntimeError("NEXT256 input changed before publication")
        if any(_sha256_file(path) != source_hashes[name] for name, path in source_paths.items()):
            raise RuntimeError("NEXT256 source changed before publication")
...
def main() -> int:
    ...
    manifest = run_dvci_feature_audit(
