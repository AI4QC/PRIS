"""Bounded NEXT38 development scan and immutable label-free rule replay."""
...
PROTOCOL = "2026-08-03-next38-bond-valence-transport-compatibility-rule-v1"
SCAN_NAME = "next38_development_scan.parquet"
FROZEN_RULE_NAME = "NEXT38_FROZEN_BOND_VALENCE_TRANSPORT_COMPATIBILITY_RULE.json"
PREDICTIONS_NAME = "next38_predictions.parquet"
...
def _term(feature: str) -> dict[str, object]:
    return {"feature": feature, "direction": 1.0, "transform": "identity"}


TERM_CATALOGUE: Mapping[str, Mapping[str, object]] = {
    "bvtc_correction_rms_high": _term("bvtc_correction_rms"),
    "bvtc_compatible_rms_high": _term("bvtc_compatible_rms"),
    "bvtc_compatible_q95_high": _term("bvtc_compatible_q95"),
    "bvtc_incompatible_rms_high": _term("bvtc_incompatible_rms"),
    "bvtc_incompatible_fraction_high": _term("bvtc_incompatible_fraction"),
    "bvtc_compatible_localization_high": _term("bvtc_compatible_localization"),
    "scbv_mismatch_q95_high": _term("scbv_mismatch_q95"),
    "steric_rep12_vector_rms_high": _term("steric_rep12_vector_rms"),
    "steric_rep12_vector_max_high": _term("steric_rep12_vector_max"),
    "sivr_site_imbalance_high": _term("sivr_site_imbalance_rms"),
}

MECHANISM_PAIRS = (
    ("bvtc_correction_rms_high", "scbv_mismatch_q95_high"),
    ("bvtc_compatible_rms_high", "steric_rep12_vector_rms_high"),
    ("bvtc_compatible_q95_high", "steric_rep12_vector_max_high"),
    ("bvtc_incompatible_rms_high", "scbv_mismatch_q95_high"),
    ("bvtc_incompatible_fraction_high", "sivr_site_imbalance_high"),
    ("bvtc_compatible_localization_high", "steric_rep12_vector_max_high"),
...
def scan_development_candidates(
    ...
    """Evaluate the exact frozen NEXT38 catalogue on exposed development rows."""
    ...
    rule: dict[str, object] = {
        ...
        "bond_valence_transport_compatibility_used": any(
            term.startswith("bvtc_") for term in terms
        ),
...
def compute_bond_valence_transport_compatibility_risk(
    features: pd.DataFrame, rule: Mapping[str, object]
) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """Apply one frozen NEXT38 rule without endpoint access or refitting."""

    if rule.get("protocol") != PROTOCOL or rule.get("eligible") is not True:
        raise ValueError("frozen NEXT38 rule is not eligible")
    ...
    if not isinstance(raw_terms, list) or not raw_terms or len(raw_terms) > 2:
        raise ValueError("frozen NEXT38 terms are invalid")
    if not isinstance(raw_parameters, Mapping):
        raise ValueError("frozen NEXT38 term parameters are invalid")
    terms = tuple(str(term) for term in raw_terms)
    if terms not in CANDIDATE_TERM_SETS or set(raw_parameters) != set(terms):
        raise ValueError("frozen NEXT38 formula is not in the catalogue")
    ...
    for term in terms:
        ...
        if any(
            ...
            raise ValueError("frozen NEXT38 term specification changed")
    ...
    if not np.isfinite(threshold):
        raise ValueError("frozen NEXT38 threshold is invalid")
    reject = support & (score >= threshold)
...
def _validate_feature_artifact(
    ...
    if feature_path.name != FEATURE_NAME or not feature_path.is_file():
        raise ValueError("NEXT38 feature path/name is invalid")
    manifest = _strict_json(manifest_path, role="NEXT38 feature manifest")
    ...
    if (
        ...
        or manifest.get("bond_valence_transport_compatibility_used") is not True
        or manifest.get("coordinate_displacement_solved_or_applied") is not False
        ...
        raise ValueError("NEXT38 feature artifact crossed the label-free boundary")
    ...
    if forbidden:
        raise ValueError(f"NEXT38 feature columns crossed label-free boundary: {forbidden}")
    if (
        "material_id" not in frame
        or frame.material_id.isna().any()
        or frame.material_id.astype(str).duplicated().any()
    ):
        raise ValueError("NEXT38 feature material IDs are invalid")
    frame = frame.copy()
...
def freeze_development_rule(
    *,
    ...
    if paths["endpoints"].name != ENDPOINT_NAME or not paths["endpoints"].is_file():
        raise ValueError("NEXT38 development endpoint path/name is invalid")
    endpoint_manifest = _strict_json(
        paths["endpoints_manifest"], role="NEXT38 development endpoint manifest"
    )
    ...
    if (
        ...
        raise ValueError("NEXT38 development endpoint artifact is invalid")
    endpoints = pd.read_parquet(
        paths["endpoints"], columns=["material_id", "force_max", "force_rms", "stress_norm"]
    )
    ...
    manifest: dict[str, object] = {
        "protocol": PROTOCOL,
        "mode": "exposed_rattled_relax_next38_development_scan",
        ...
        "executed_source_sha256": {
            "src/next38_bond_valence_transport_compatibility_rule.py": source_hash
        },
    ...
    try:
        ...
        if any(_sha256(path) != input_hashes[role] for role, path in paths.items()):
            raise RuntimeError("NEXT38 development input changed before publication")
        if _sha256(source_path) != source_hash:
            raise RuntimeError("NEXT38 rule source changed before publication")
...
def apply_frozen_rule(
    *,
    ...
    """Publish label-free NEXT38 predictions for sealed feature cohorts."""
    ...
    if (
        ...
        raise ValueError("NEXT38 rule application inputs are invalid")
    rule_manifest = _strict_json(rule_manifest_path, role="NEXT38 frozen-rule manifest")
    ...
    if (
        ...
        raise ValueError("NEXT38 frozen rule is not promoted and hash-locked")
    try:
        rule = json.loads(rule_path.read_text("utf-8"))
    except (OSError, UnicodeError, json.JSONDecodeError) as exc:
        raise ValueError("NEXT38 frozen rule JSON is invalid") from exc
    if not isinstance(rule, dict):
        raise ValueError("NEXT38 frozen rule JSON is invalid")
    ...
    for feature, feature_manifest in zip(features, feature_manifests, strict=True):
        ...
        score, supported, reject = compute_bond_valence_transport_compatibility_risk(
            frame, rule
        )
        source = (
    ...
    if predictions.material_id.duplicated().any():
        raise ValueError("NEXT38 prediction material IDs are duplicated")
    source_path = Path(__file__).resolve()
    source_hash = _sha256(source_path)
    ...
    manifest: dict[str, object] = {
        "protocol": PROTOCOL,
        "mode": "prospective_label_free_bond_valence_compatibility_prediction",
        "labels_opened": False,
    ...
    try:
        ...
        if _sha256(rule_path) != input_hashes["frozen_rule"] or _sha256(
            ...
            raise RuntimeError("NEXT38 frozen rule changed before publication")
        for item in feature_inputs:
            if _sha256(Path(item["feature_path"])) != item["feature_sha256"] or _sha256(
                Path(item["manifest_path"])
            ) != item["manifest_sha256"]:
                raise RuntimeError("NEXT38 feature changed before publication")
        if _sha256(source_path) != source_hash:
            raise RuntimeError("NEXT38 rule source changed before publication")
        _publish_directory_no_replace(staging, target)
    finally:
...
__all__ = [
    ...
    "TERM_CATALOGUE",
    "apply_frozen_rule",
    "compute_bond_valence_transport_compatibility_risk",
