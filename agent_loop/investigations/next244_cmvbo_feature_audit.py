#!/usr/bin/env python3
"""Audit frozen CMVBO hypotheses inside the exact NEXT224 rejected cohort."""
...
import src.next243_coarse_grained_voronoi_bond_order as n243
from src.next102_cross_source_dobvr_features import _sha256_file, _write_json


PROTOCOL = "2026-08-09-next244-cmvbo-feature-audit-v1"
MANIFEST_NAME = "MANIFEST.json"
CATALOGUE_NAME = "NEXT244_CMVBO_FEATURE_CATALOGUE.json"
AUDIT_NAME = "NEXT244_CMVBO_FEATURE_AUDIT.json"
TABLE_NAME = "next244_cmvbo_feature_audit.parquet"
EXPECTED_DESIGN_SHA256 = n243.EXPECTED_DESIGN_SHA256
HYPOTHESES = (
...
EXPECTED_NEXT243_SOURCE_SHA256 = (
    "76dd09db39ee5d81624c5a654f1e52e71793ef5846c3a4a914252bf3ae494a13"
)
REQUIRED_STAGES = (*n227.REQUIRED_STAGES, 243)
REQUIRED_DESIGN_STAGES = (*n227.REQUIRED_DESIGN_STAGES, 227, 243)
...
EXPECTED_INPUT_SHA256 = {
    **{
    ...
    "next243_design": EXPECTED_DESIGN_SHA256,
    "design": EXPECTED_DESIGN_SHA256,
    "next243_manifest": (
        "5e9fc83abac17aa3e3fce4e720dcd46fb04315ec58e481f01cb7224a316f9cc7"
    ),
    "next243_catalogue": (
        "263b7aa06df86ecbcd9c5644e79753e6e6f572dc303b8e80b1d43b1f27eb8b0f"
    ),
    "next243_scigen": (
        "731ba75630990d4250aac2df676737f001dde3e0e7c675aa86af5a0cd690bc81"
    ),
    "next243_wyformer": (
        "44682dcc06a99b9c5edda55f970e1d360c4d35e2030bfe47f288a367a57c62d0"
    ),
}
...
def select_eligible_hypotheses(
    ...
    if not isinstance(frame, pd.DataFrame) or frame.empty or required - set(frame.columns):
        raise ValueError("NEXT244 audit ranking table differs")
    result = frame.copy()
...
def bounded_protection(
    *, values: object, direction: str, q_lo: float, q_hi: float
    ...
    if raw.ndim != 1 or direction not in n227.PROTECTION_DIRECTIONS or not (
        ...
        raise ValueError("NEXT244 bounded protection inputs differ")
...
def _index_cmvbo_by_prefixed_material_id(
    *, table: pd.DataFrame, source: str, expected_material_ids: object
) -> pd.DataFrame:
    """Return a source-prefixed CMVBO table after exact identity validation."""
    ...
    if set(extra["material_id"]) != expected:
        raise ValueError(f"NEXT244 {source} material identity differs")
...
def _paths(
    *,
    ...
    paths.update(
        {
            "next243_design": design_paths[243],
            "design": design_path,
            "next243_manifest": roots["next243"] / n243.MANIFEST_NAME,
            "next243_catalogue": roots["next243"] / n243.CATALOGUE_NAME,
            "next243_scigen": roots["next243"] / n243.FEATURE_FILES["scigen"],
            "next243_wyformer": roots["next243"] / n243.FEATURE_FILES["wyformer"],
        }
    )
    return paths
...
def _verify_next243(
    ...
    manifest = json.loads(paths["next243_manifest"].read_text())
    catalogue = json.loads(paths["next243_catalogue"].read_text())
    tables = {
        "scigen": pd.read_parquet(paths["next243_scigen"]),
        "wyformer": pd.read_parquet(paths["next243_wyformer"]),
    }
    expected_outputs = {
        n243.CATALOGUE_NAME: input_hashes["next243_catalogue"],
        n243.FEATURE_FILES["scigen"]: input_hashes["next243_scigen"],
        n243.FEATURE_FILES["wyformer"]: input_hashes["next243_wyformer"],
    }
    if (
        _sha256_file(Path(n227.__file__).resolve()) != EXPECTED_NEXT227_SOURCE_SHA256
        or manifest.get("protocol") != n243.PROTOCOL
        ...
        or manifest.get("executed_source_sha256", {}).get(
            "src/next243_coarse_grained_voronoi_bond_order.py"
        )
        != EXPECTED_NEXT243_SOURCE_SHA256
        or _sha256_file(Path(n243.__file__).resolve()) != EXPECTED_NEXT243_SOURCE_SHA256
        or any(manifest.get(key) is not value for key, value in n243.BOUNDARY_FLAGS.items())
        or catalogue.get("feature_names") != list(n243.FEATURE_NAMES)
        or catalogue.get("feature_count") != len(n243.FEATURE_NAMES)
        or catalogue.get("labels_opened") is not False
    ):
        raise ValueError("NEXT244 NEXT243 provenance differs")
    for source, expected_rows in (("scigen", 13470), ("wyformer", 5232)):
        ...
        if (
            ...
            or not table["cmvbo_supported"].fillna(False).astype(bool).all()
            or any(
                not np.isfinite(pd.to_numeric(table[name], errors="coerce")).all()
                for name in n243.FEATURE_NAMES
            )
        ):
            raise ValueError(f"NEXT244 NEXT243 {source} table differs")
    return (*prior, tables)


def run_cmvbo_feature_audit(
    *,
    ...
    """Audit frozen CMVBO hypotheses without searching a new formula."""

    if not set(REQUIRED_STAGES).issubset(stage_dirs):
        raise ValueError("NEXT244 stage directory universe differs")
    if not set(REQUIRED_DESIGN_STAGES).issubset(design_paths):
        raise ValueError("NEXT244 design path universe differs")
    roots = {
    ...
    if any(not path.is_file() for path in paths.values()):
        raise FileNotFoundError("NEXT244 input is missing")
    ...
    if require_formal_inputs and input_hashes != EXPECTED_INPUT_SHA256:
        ...
        raise ValueError(f"NEXT244 formal input identity differs: {differing}")
    (
        eligible,
        eligible214,
        primary_key,
        ...
        formula222,
        cmvbo_tables,
    ) = _verify_next243(paths, input_hashes)
    ...
    if cohort_counts != n227.EXPECTED_COHORT_COUNTS:
        raise ValueError("NEXT244 rejected cohort counts differ")
    ...
    audit = {
        "protocol": PROTOCOL,
        "audit_mode": "fixed_next224_rejected_extreme_cmvbo_feature_audit",
        "base_candidate_key_sha256": n227.EXPECTED_BASE_CANDIDATE_KEY_SHA256,
        "base_threshold": n227.EXPECTED_BASE_THRESHOLD,
        "cohort_counts": {
        ...
        "next245_search_authorized": bool(selected is not None),
    ...
    source_paths = {
        ...
        "src/next243_coarse_grained_voronoi_bond_order.py": Path(n243.__file__).resolve(),
        "src/next244_cmvbo_feature_audit.py": Path(__file__).resolve(),
    }
    ...
    try:
        ...
        manifest = {
            ...
            "next245_search_authorized": bool(selected is not None),
            "cmvbo_branch_terminated": selected is None,
            "new_formula_searched": False,
            "new_formula_selected": False,
        ...
        if any(_sha256_file(path) != input_hashes[name] for name, path in paths.items()):
            raise RuntimeError("NEXT244 input changed before publication")
        if any(_sha256_file(path) != source_hashes[name] for name, path in source_paths.items()):
            raise RuntimeError("NEXT244 source changed before publication")
...
def main() -> int:
    ...
    manifest = run_cmvbo_feature_audit(
