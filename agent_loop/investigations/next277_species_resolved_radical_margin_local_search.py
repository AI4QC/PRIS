#!/usr/bin/env python3
"""Run the frozen species-resolved radical margin-local discovery search."""
...
import src.next276_species_resolved_radical_feature_audit as n276
from src.next102_cross_source_dobvr_features import _sha256_file, _write_json


n227 = n276.n268.n227
n223 = n269.n223
PROTOCOL = "2026-08-09-next277-species-resolved-radical-margin-local-search-v1"
MANIFEST_NAME = "MANIFEST.json"
CATALOGUE_NAME = "NEXT277_PRVS_MARGIN_LOCAL_CATALOGUE.json"
EVALUATION_NAME = "NEXT277_DISCOVERY_EVALUATION.json"
FORMULA_NAME = "NEXT277_FROZEN_CANDIDATE.json"
SEARCH_NAME = "next277_species_resolved_radical_margin_local_search.parquet"
SCORE_COMPOSITION = (
    "nonnegative_next224_plus_triangular_margin_local_signed_species_resolved_radical_term"
)
...
EXPECTED_ELIGIBLE_HYPOTHESIS_SHA256 = (
    "2362824ff45901c7471b9d8309d1a9a8d3a37f5502abf3aff0bc85f93138d7c1"
)
...
EXPECTED_DESIGN_SHA256 = n276.EXPECTED_DESIGN_SHA256
...
BOUNDARY_FLAGS = n276.BOUNDARY_FLAGS
REQUIRED_STAGES = (*n276.REQUIRED_STAGES, 276)
REQUIRED_DESIGN_STAGES = (*n276.REQUIRED_DESIGN_STAGES, 276)
EXPECTED_NEXT269_SOURCE_SHA256 = (
    "5d87161be5a54182642b1df1012abe2771c1ec8ea0165f8c724ae90e4e8e3f95"
)
EXPECTED_NEXT276_SOURCE_SHA256 = (
    "babf4240509cf23bea9be25cb8849406b326b8f62acd9ced1d1052ba5855a338"
)
EXPECTED_INPUT_SHA256 = {
    **{
        key: value
        for key, value in n276.EXPECTED_INPUT_SHA256.items()
        if key != "design"
    },
    "next276_design": n276.EXPECTED_INPUT_SHA256["design"],
    "design": EXPECTED_DESIGN_SHA256,
    "next276_manifest": (
        "5e973e314da6052fe267f518bb52a3ff0b9e96c0acfe08c4bb4a94deca63955c"
    ),
    "next276_catalogue": (
        "c55d005e623fb4f66b57714ae14cc7b501a00b3f270423c71ea2b394822c871b"
    ),
    "next276_audit": (
        "48d381c08a6bce66450e57586ca277745dec6e9da1eb628363fb1a91b926f4b0"
    ),
    "next276_table": (
        "c418167cdc9c0b7cd431dc3f381544123ba054c999525c26a63ddd086f7a0549"
    ),
}


def prvs_margin_local_score(**kwargs: object):
    try:
        return n269.prv_margin_local_score(**kwargs)
    except ValueError as exc:
        raise ValueError("NEXT277 margin-local score inputs differ") from exc


def build_prvs_candidate_specs(
    *,
    ...
    if len({str(spec["candidate_key"]) for spec in result}) != len(result):
        raise RuntimeError("NEXT277 candidate keys are not unique")
    return result


def materialize_prvs_candidates(
    *,
    ...
    for term in terms:
        ...
        new_id = "next277_virtual_candidate__" + hashlib.sha256(
            key.encode()
        ).hexdigest()[:24]
        ...
        term.update(
            {
                "term_id": new_id,
                "feature": new_column,
                "group": "next277_species_resolved_radical_margin_local",
            }
        )
...
def _paths(
    *,
    ...
    paths = n276._paths(
        ...
        design_paths={
            stage: design_paths[stage] for stage in n276.REQUIRED_DESIGN_STAGES
        },
        design_path=design_paths[276],
    )
    paths["next276_design"] = paths.pop("design")
    paths.update(
        {
            "design": design_path,
            "next276_manifest": roots["next276"] / n276.MANIFEST_NAME,
            "next276_catalogue": roots["next276"] / n276.CATALOGUE_NAME,
            "next276_audit": roots["next276"] / n276.AUDIT_NAME,
            "next276_table": roots["next276"] / n276.TABLE_NAME,
        }
    )
    return paths


def _verify_next276(paths: Mapping[str, Path], input_hashes: Mapping[str, str]):
    prior_paths = {key: paths[key] for key in n276.EXPECTED_INPUT_SHA256}
    prior_paths["design"] = paths["next276_design"]
    prior_hashes = {key: input_hashes[key] for key in n276.EXPECTED_INPUT_SHA256}
    prior_hashes["design"] = input_hashes["next276_design"]
    prior = n276._verify_next275(prior_paths, prior_hashes)
    manifest = json.loads(paths["next276_manifest"].read_text())
    catalogue = json.loads(paths["next276_catalogue"].read_text())
    audit = json.loads(paths["next276_audit"].read_text())
    table = pd.read_parquet(paths["next276_table"])
    ...
    expected_outputs = {
        n276.CATALOGUE_NAME: input_hashes["next276_catalogue"],
        n276.AUDIT_NAME: input_hashes["next276_audit"],
        n276.TABLE_NAME: input_hashes["next276_table"],
    }
    if (
        _sha256_file(Path(n269.__file__).resolve()) != EXPECTED_NEXT269_SOURCE_SHA256
        or _sha256_file(Path(n276.__file__).resolve()) != EXPECTED_NEXT276_SOURCE_SHA256
        or manifest.get("protocol") != n276.PROTOCOL
        or manifest.get("eligible_hypothesis_count")
        != EXPECTED_ELIGIBLE_HYPOTHESIS_COUNT
        or manifest.get("eligible_hypothesis_sha256")
        != EXPECTED_ELIGIBLE_HYPOTHESIS_SHA256
        or manifest.get("next277_search_authorized") is not True
        ...
        or manifest.get("executed_source_sha256", {}).get(
            "src/next276_species_resolved_radical_feature_audit.py"
        )
        != EXPECTED_NEXT276_SOURCE_SHA256
        ...
        != EXPECTED_ELIGIBLE_HYPOTHESIS_SHA256
        or audit.get("next277_search_authorized") is not True
        ...
        raise ValueError("NEXT277 NEXT276 provenance differs")
...
def _formula_from_spec(spec: Mapping[str, object] | None) -> dict[str, object]:
    ...
    return {
        "protocol": PROTOCOL,
        "kind": "species_resolved_radical_triangular_margin_local_x0_no_dft_score",
        "selected": True,
...
def select_best_new_record(records: pd.DataFrame) -> pd.Series | None:
    try:
        ...
        raise ValueError("NEXT277 reporting selection schema differs") from exc


def _attach_prvs_features(
    *,
    combined: pd.DataFrame,
    feature_tables: Mapping[str, pd.DataFrame],
    prvs_tables: Mapping[str, pd.DataFrame],
) -> None:
    source = combined["source_dataset"].astype(str).to_numpy()
    for source_name in ("scigen", "wyformer"):
        indexed = n276._index_prvs_by_prefixed_material_id(
            table=prvs_tables[source_name],
            source=source_name,
        ...
        for name in n276.n275.FEATURE_NAMES:
            combined.loc[mask, name] = ordered_ids.map(indexed[name]).to_numpy()


def run_prvs_margin_local_search(
    *,
    ...
    """Run the complete frozen discovery-only NEXT277 search."""

    if not set(REQUIRED_STAGES).issubset(stage_dirs):
        raise ValueError("NEXT277 stage directory universe differs")
    if not set(REQUIRED_DESIGN_STAGES).issubset(design_paths):
        raise ValueError("NEXT277 design path universe differs")
    roots = {
    ...
    if type(search_workers) is not int or search_workers <= 0:
        raise ValueError("NEXT277 search_workers must be a positive exact integer")
    if any(not path.is_file() for path in paths.values()):
        raise FileNotFoundError("NEXT277 input is missing")
    ...
    if require_formal_inputs and input_hashes != EXPECTED_INPUT_SHA256:
        ...
        raise ValueError(f"NEXT277 formal input identity differs: {differing}")
    (
        eligible_prior,
        eligible214,
        primary_key,
        ...
        prvs_tables,
        eligible276,
    ) = _verify_next276(paths, input_hashes)
    ...
    _attach_prvs_features(
        combined=combined, feature_tables=feature_tables, prvs_tables=prvs_tables
    )
    ...
    if (
        ...
        raise ValueError("NEXT277 NEXT224 base identity differs")
    specs = build_prvs_candidate_specs(
        base_candidate_key=base_key, eligible_table=eligible276
    )
    ...
    if len(specs) != EXPECTED_CANDIDATE_COUNT or eligible_count != EXPECTED_ELIGIBLE_COUNT:
        raise RuntimeError("NEXT277 frozen candidate universe differs")
    virtual, terms, runtime, activity = materialize_prvs_candidates(
        features=combined,
        base_score=base_score,
    ...
    if (
        ...
        raise RuntimeError("NEXT277 evaluator accounting differs")
    ...
    if len(no_op) != 1 or len(reference_no_op) != 1:
        raise RuntimeError("NEXT277 no-op reproduction identity differs")
    ...
    if selected_row is not None:
        ...
        if str(selected_result["selected"]["record"]["candidate_key"]) != selected_key:
            raise RuntimeError("NEXT277 selected candidate reproduction differs")
        ...
        if selected_spec.get("eligible_new_candidate") is not True:
            raise RuntimeError("NEXT277 reproduction control was selected")
    ...
    if selected is not None and passes != bool(selected["record"]["passes_all_discovery_gates"]):
        raise RuntimeError("NEXT277 all-gate selection differs")
    if passes and selected is None:
        raise RuntimeError("NEXT277 all-gate candidate was not selected")
    ...
    catalogue = {
        ...
        "base_threshold": EXPECTED_BASE_THRESHOLD,
        "repair_width": REPAIR_WIDTH,
        "eligible_hypotheses": list(eligible276["hypothesis"].astype(str)),
        "eligible_hypothesis_count": len(eligible276),
    ...
    evaluation = {
        "protocol": PROTOCOL,
        "evaluation_mode": "frozen_species_resolved_radical_margin_local_discovery_search",
        ...
        "next278_diagnostic_authorized": bool(not passes and diagnostic_keys),
        "next278_candidate_count": len(diagnostic_keys),
        "next278_candidate_key_sha256": diagnostic_sha,
    ...
    source_paths = {
        ...
        "src/next276_species_resolved_radical_feature_audit.py": Path(n276.__file__).resolve(),
        "src/next277_species_resolved_radical_margin_local_search.py": Path(__file__).resolve(),
    }
    ...
    try:
        ...
        if any(_sha256_file(path) != input_hashes[name] for name, path in paths.items()):
            raise RuntimeError("NEXT277 input changed before publication")
        if any(_sha256_file(path) != source_hashes[name] for name, path in source_paths.items()):
            raise RuntimeError("NEXT277 source changed before publication")
...
def main() -> int:
    ...
    manifest = run_prvs_margin_local_search(
