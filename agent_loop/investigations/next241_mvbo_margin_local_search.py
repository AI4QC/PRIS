#!/usr/bin/env python3
"""Frozen one-term MVBO margin-local search on the NEXT224 frontier."""
...
import src.next240_mvbo_feature_audit as n240
from src.next102_cross_source_dobvr_features import _sha256_file, _write_json


n227 = n240.n227
n223 = n227.n226.n225.n223
PROTOCOL = "2026-08-09-next241-mvbo-margin-local-search-v1"
MANIFEST_NAME = "MANIFEST.json"
CATALOGUE_NAME = "NEXT241_MVBO_MARGIN_LOCAL_CATALOGUE.json"
EVALUATION_NAME = "NEXT241_DISCOVERY_EVALUATION.json"
FORMULA_NAME = "NEXT241_FROZEN_CANDIDATE.json"
SEARCH_NAME = "next241_mvbo_margin_local_search.parquet"
SCORE_COMPOSITION = "nonnegative_next224_plus_triangular_margin_local_signed_mvbo_term"
...
EXPECTED_ELIGIBLE_HYPOTHESIS_SHA256 = (
    "d112795af35615cf2814715150f1a22bb101d83952a98b407319144062a587e4"
)
...
EXPECTED_DESIGN_SHA256 = n240.EXPECTED_DESIGN_SHA256
...
BOUNDARY_FLAGS = n240.BOUNDARY_FLAGS
REQUIRED_STAGES = (*n240.REQUIRED_STAGES, 240)
REQUIRED_DESIGN_STAGES = (*n240.REQUIRED_DESIGN_STAGES, 240)
EXPECTED_NEXT240_SOURCE_SHA256 = (
    "e124c4f5d1c543691875227e4562c0e63f558365749879e65176be4996b0036b"
)
EXPECTED_INPUT_SHA256 = {
    **{
        key: value
        for key, value in n240.EXPECTED_INPUT_SHA256.items()
        if key != "design"
    },
    "next240_design": n240.EXPECTED_INPUT_SHA256["design"],
    "design": EXPECTED_DESIGN_SHA256,
    "next240_manifest": (
        "dc00e80d8c86718ed07ae48ab078e6ae5f1090a34fb3495c36fbca83c23081d1"
    ),
    "next240_catalogue": (
        "246f40b59117107fc9677d9aa5f139fe97197e5732149bfb4d1fbe9c959fe1d2"
    ),
    "next240_audit": (
        "5fe82b282a4afb725e7b12bebdb054d5292d375ec19f028f470291ded71ba748"
    ),
    "next240_table": (
        "63a36374f42be66f7ab70d7893914370809435792e47b127cda0b3055998799c"
    ),
}


def mvbo_margin_local_score(
    *,
    base_score: object,
    base_support: object,
    protection: object,
    ...
    if (
        ...
        raise ValueError("NEXT241 margin-local score inputs differ")
    h = fraction * width
...
def _grid_numerator(value: float, denominator: int, label: str) -> int:
    ...
    if (
        ...
        raise ValueError(f"NEXT241 {label} grid differs")
    return numerator


def build_mvbo_candidate_specs(
    *,
    ...
    if not math.isfinite(cutoff) or not math.isfinite(width) or width <= 0.0:
        raise ValueError("NEXT241 base population differs")
    ...
    if (
        ...
        raise ValueError("NEXT241 frozen grid differs")
    common = {
    ...
    for _, row in eligible.iterrows():
        ...
        if (
            ...
            raise ValueError("NEXT241 eligible hypothesis identity differs")
    ...
    if len({str(spec["candidate_key"]) for spec in specs}) != len(specs):
        raise RuntimeError("NEXT241 candidate keys are not unique")
    return specs


def materialize_mvbo_candidates(
    *,
    ...
    if (
        ...
        raise ValueError("NEXT241 materializer inputs differ")
    ...
    for spec in raw_specs:
        ...
        if feature is None:
            ...
            if feature not in features.columns:
                raise ValueError("NEXT241 materializer feature differs")
            hypothesis = str(spec["hypothesis"])
            if hypothesis not in protection_cache:
                values = pd.to_numeric(
                    features[feature], errors="coerce"
                ).to_numpy(float)
                protection_cache[hypothesis] = n240.bounded_protection(
                    values=values,
                    direction=str(spec["direction"]),
                    q_lo=float(spec["q_lo"]),
                    q_hi=float(spec["q_hi"]),
                )
            score, got_support, active, _ = mvbo_margin_local_score(
                base_score=base,
        ...
        if not np.array_equal(got_support, support):
            raise RuntimeError("NEXT241 support changed")
        ...
        terms.append(
            {
                ...
                "center": 0.0,
                "scale": 1.0 / divisor,
                "group": "next241_mvbo_margin_local",
...
def _paths(
    *,
    ...
    paths = n240._paths(
        ...
        design_paths={
            stage: design_paths[stage] for stage in n240.REQUIRED_DESIGN_STAGES
        },
        design_path=design_paths[240],
    )
    paths["next240_design"] = paths.pop("design")
    paths.update(
        {
            "design": design_path,
            "next240_manifest": roots["next240"] / n240.MANIFEST_NAME,
            "next240_catalogue": roots["next240"] / n240.CATALOGUE_NAME,
            "next240_audit": roots["next240"] / n240.AUDIT_NAME,
            "next240_table": roots["next240"] / n240.TABLE_NAME,
        }
    )
    return paths
...
def _verify_next240(
    ...
    base_paths = {key: paths[key] for key in n240.EXPECTED_INPUT_SHA256}
    base_paths["design"] = paths["next240_design"]
    base_hashes = {key: input_hashes[key] for key in n240.EXPECTED_INPUT_SHA256}
    base_hashes["design"] = input_hashes["next240_design"]
    prior = n240._verify_next239(base_paths, base_hashes)
    manifest = json.loads(paths["next240_manifest"].read_text())
    catalogue = json.loads(paths["next240_catalogue"].read_text())
    audit = json.loads(paths["next240_audit"].read_text())
    table = pd.read_parquet(paths["next240_table"])
    ...
    expected_outputs = {
        n240.CATALOGUE_NAME: input_hashes["next240_catalogue"],
        n240.AUDIT_NAME: input_hashes["next240_audit"],
        n240.TABLE_NAME: input_hashes["next240_table"],
    }
    if (
        manifest.get("protocol") != n240.PROTOCOL
        or manifest.get("eligible_hypothesis_count")
        != EXPECTED_ELIGIBLE_HYPOTHESIS_COUNT
        or manifest.get("eligible_hypothesis_sha256")
        != EXPECTED_ELIGIBLE_HYPOTHESIS_SHA256
        or manifest.get("next241_search_authorized") is not True
        ...
        or manifest.get("executed_source_sha256", {}).get(
            "src/next240_mvbo_feature_audit.py"
        )
        != EXPECTED_NEXT240_SOURCE_SHA256
        or _sha256_file(Path(n240.__file__).resolve())
        != EXPECTED_NEXT240_SOURCE_SHA256
        ...
        != EXPECTED_ELIGIBLE_HYPOTHESIS_SHA256
        or audit.get("next241_search_authorized") is not True
        ...
        raise ValueError("NEXT241 NEXT240 provenance differs")
...
def _formula_from_spec(spec: Mapping[str, object] | None) -> dict[str, object]:
    ...
    return {
        "protocol": PROTOCOL,
        "kind": "mvbo_triangular_margin_local_x0_no_dft_score",
        "selected": True,
...
def select_best_new_record(records: pd.DataFrame) -> pd.Series | None:
    ...
    if not isinstance(records, pd.DataFrame) or required - set(records.columns):
        raise ValueError("NEXT241 reporting selection schema differs")
...
def _attach_mvbo_features(
    *,
    combined: pd.DataFrame,
    feature_tables: Mapping[str, pd.DataFrame],
    mvbo_tables: Mapping[str, pd.DataFrame],
) -> None:
    source = combined["source_dataset"].astype(str).to_numpy()
    for source_name in ("scigen", "wyformer"):
        indexed = n240._index_mvbo_by_prefixed_material_id(
            table=mvbo_tables[source_name],
            source=source_name,
        ...
        for name in n240.n239.FEATURE_NAMES:
            combined.loc[mask, name] = ordered_ids.map(indexed[name]).to_numpy()


def run_mvbo_margin_local_search(
    *,
    ...
    """Run the complete frozen discovery-only NEXT241 search."""

    if not set(REQUIRED_STAGES).issubset(stage_dirs):
        raise ValueError("NEXT241 stage directory universe differs")
    if not set(REQUIRED_DESIGN_STAGES).issubset(design_paths):
        raise ValueError("NEXT241 design path universe differs")
    roots = {
    ...
    if type(search_workers) is not int or search_workers <= 0:
        raise ValueError("NEXT241 search_workers must be a positive exact integer")
    if any(not path.is_file() for path in paths.values()):
        raise FileNotFoundError("NEXT241 input is missing")
    ...
    if require_formal_inputs and input_hashes != EXPECTED_INPUT_SHA256:
        ...
        raise ValueError(f"NEXT241 formal input identity differs: {differing}")
    (
        eligible_prior,
        eligible214,
        primary_key,
        ...
        mvbo_tables,
        eligible240,
    ) = _verify_next240(paths, input_hashes)
    ...
    if (
        ...
        raise ValueError("NEXT241 NEXT224 base identity differs")
    specs = build_mvbo_candidate_specs(
    ...
    if len(specs) != EXPECTED_CANDIDATE_COUNT or eligible_count != EXPECTED_ELIGIBLE_COUNT:
        raise RuntimeError("NEXT241 frozen candidate universe differs")
    virtual, terms, runtime, activity = materialize_mvbo_candidates(
        features=combined,
    ...
    if (
        ...
        raise RuntimeError("NEXT241 evaluator accounting differs")
    ...
    if len(no_op) != 1 or len(reference_no_op) != 1:
        raise RuntimeError("NEXT241 no-op reproduction identity differs")
    ...
    if selected_row is not None:
        ...
        if str(selected_result["selected"]["record"]["candidate_key"]) != selected_key:
            raise RuntimeError("NEXT241 selected candidate reproduction differs")
        ...
        if selected_spec.get("eligible_new_candidate") is not True:
            raise RuntimeError("NEXT241 reproduction control was selected")
    ...
    if selected is not None and passes != bool(
        ...
        raise RuntimeError("NEXT241 all-gate selection differs")
    if passes and selected is None:
        raise RuntimeError("NEXT241 all-gate candidate was not selected")
    ...
    catalogue = {
        ...
        "base_threshold": EXPECTED_BASE_THRESHOLD,
        "repair_width": REPAIR_WIDTH,
        "eligible_hypotheses": list(eligible240["hypothesis"].astype(str)),
        "eligible_hypothesis_count": len(eligible240),
    ...
    evaluation = {
        "protocol": PROTOCOL,
        "evaluation_mode": "frozen_mvbo_margin_local_discovery_search",
        ...
        "next242_diagnostic_authorized": bool(not passes and diagnostic_keys),
        "next242_candidate_count": len(diagnostic_keys),
        "next242_candidate_key_sha256": diagnostic_sha,
    ...
    source_paths = {
        "src/next240_mvbo_feature_audit.py": Path(n240.__file__).resolve(),
        "src/next241_mvbo_margin_local_search.py": Path(__file__).resolve(),
    }
    ...
    try:
        ...
        if any(_sha256_file(path) != input_hashes[name] for name, path in paths.items()):
            raise RuntimeError("NEXT241 input changed before publication")
        if any(
            _sha256_file(path) != source_hashes[name]
            for name, path in source_paths.items()
        ):
            raise RuntimeError("NEXT241 source changed before publication")
...
def main() -> int:
    ...
    manifest = run_mvbo_margin_local_search(
...
__all__ = [
    ...
    "build_mvbo_candidate_specs",
    "materialize_mvbo_candidates",
    "mvbo_margin_local_score",
    "run_mvbo_margin_local_search",
