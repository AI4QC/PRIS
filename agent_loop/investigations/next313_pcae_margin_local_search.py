#!/usr/bin/env python3
"""Frozen one-term PCAE margin-local search on the NEXT224 frontier."""
...
import src.next312_pcae_feature_audit as n312
from src.next102_cross_source_dobvr_features import _sha256_file, _write_json


n227 = n312.n268.n227
n223 = n261.n223
PROTOCOL = "2026-08-09-next313-pcae-margin-local-search-v1"
MANIFEST_NAME = "MANIFEST.json"
CATALOGUE_NAME = "NEXT313_PCAE_MARGIN_LOCAL_CATALOGUE.json"
EVALUATION_NAME = "NEXT313_DISCOVERY_EVALUATION.json"
FORMULA_NAME = "NEXT313_FROZEN_CANDIDATE.json"
SEARCH_NAME = "next313_pcae_margin_local_search.parquet"
SCORE_COMPOSITION = "nonnegative_next224_plus_triangular_margin_local_signed_pcae_term"
...
EXPECTED_ELIGIBLE_HYPOTHESIS_SHA256 = (
    "86e51a41d4e668ad6f77e4ee3fc54083b07b46484b3a8c06a5e6db78897a7306"
)
...
EXPECTED_DESIGN_SHA256 = n312.EXPECTED_INPUT_SHA256["design"]
...
BOUNDARY_FLAGS = n312.BOUNDARY_FLAGS
REQUIRED_STAGES = (*n312.REQUIRED_STAGES, 312)
REQUIRED_DESIGN_STAGES = (*n312.REQUIRED_DESIGN_STAGES, 312)
EXPECTED_NEXT261_SOURCE_SHA256 = (
    "5283f4b6a58e313a735cd4f3245028a173caa77f0338a6d3e04e5a7244256599"
)
EXPECTED_NEXT312_SOURCE_SHA256 = (
    "a14eee6a6209c60fe62a0b06f7a0b373e5de95ebb2240ce9c6f6679a20cdda3f"
)
EXPECTED_INPUT_SHA256 = {
    **{
        key: value
        for key, value in n312.EXPECTED_INPUT_SHA256.items()
        if key != "design"
    },
    "next312_design": n312.EXPECTED_INPUT_SHA256["design"],
    "design": EXPECTED_DESIGN_SHA256,
    "next312_manifest": "3d0e14153a3dc2c77ca6d2cd4de40f29f3ee89691361ab0b54c44b1c4330bf47",
    "next312_catalogue": "ecdf9aedfde874595a45580c14654d5bc6597e98788785200d9c9e48d52c3bf3",
    "next312_audit": "4d729b469087576b2846fd3d0df91686f7f4601f19a4a7470cc76c2366430329",
    "next312_table": "78bface7ed84e6628bb51df90b036992017ac45a7930f014e86ea0f4e0be9bd7",
}


def pcae_margin_local_score(
    *,
    base_score: object,
    base_support: object,
    protection: object,
    ...
    try:
        ...
        raise ValueError("NEXT313 margin-local score inputs differ") from exc


def build_pcae_candidate_specs(
    *,
    ...
    if len({str(spec["candidate_key"]) for spec in result}) != len(result):
        raise RuntimeError("NEXT313 candidate keys are not unique")
    return result


def materialize_pcae_candidates(
    *,
    ...
    for term in terms:
        ...
        new_id = "next313_virtual_candidate__" + hashlib.sha256(key.encode()).hexdigest()[:24]
        ...
        term.update(
            {
                "term_id": new_id,
                "feature": new_column,
                "group": "next313_pcae_margin_local",
            }
        )
...
def _paths(
    *,
    ...
    paths = n312._paths(
        ...
        design_paths={stage: design_paths[stage] for stage in n312.REQUIRED_DESIGN_STAGES},
        design_path=design_paths[312],
    )
    paths["next312_design"] = paths.pop("design")
    paths.update(
        {
            "design": design_path,
            "next312_manifest": roots["next312"] / n312.MANIFEST_NAME,
            "next312_catalogue": roots["next312"] / n312.CATALOGUE_NAME,
            "next312_audit": roots["next312"] / n312.AUDIT_NAME,
            "next312_table": roots["next312"] / n312.TABLE_NAME,
        }
    )
    return paths


def _verify_next312(paths: Mapping[str, Path], input_hashes: Mapping[str, str]):
    base_paths = {key: paths[key] for key in n312.EXPECTED_INPUT_SHA256}
    base_paths["design"] = paths["next312_design"]
    base_hashes = {key: input_hashes[key] for key in n312.EXPECTED_INPUT_SHA256}
    base_hashes["design"] = input_hashes["next312_design"]
    prior = n312._verify_next311(base_paths, base_hashes)
    manifest = json.loads(paths["next312_manifest"].read_text())
    catalogue = json.loads(paths["next312_catalogue"].read_text())
    audit = json.loads(paths["next312_audit"].read_text())
    table = pd.read_parquet(paths["next312_table"])
    ...
    expected_outputs = {
        n312.CATALOGUE_NAME: input_hashes["next312_catalogue"],
        n312.AUDIT_NAME: input_hashes["next312_audit"],
        n312.TABLE_NAME: input_hashes["next312_table"],
    }
    if (
        manifest.get("protocol") != n312.PROTOCOL
        ...
        or manifest.get("next313_search_authorized") is not True
        ...
        or manifest.get("executed_source_sha256", {}).get(
            "src/next312_pcae_feature_audit.py"
        )
        != EXPECTED_NEXT312_SOURCE_SHA256
        or _sha256_file(Path(n312.__file__).resolve()) != EXPECTED_NEXT312_SOURCE_SHA256
        ...
        or audit.get("next313_search_authorized") is not True
        ...
        raise ValueError("NEXT313 NEXT312 provenance differs")
...
def _formula_from_spec(spec: Mapping[str, object] | None) -> dict[str, object]:
    ...
    return {
        "protocol": PROTOCOL,
        "kind": "pcae_triangular_margin_local_x0_no_dft_score",
        "selected": True,
...
def select_best_new_record(records: pd.DataFrame) -> pd.Series | None:
    try:
        ...
        raise ValueError("NEXT313 reporting selection schema differs") from exc


def _attach_pcae_features(
    *,
    combined: pd.DataFrame,
    feature_tables: Mapping[str, pd.DataFrame],
    pcae_tables: Mapping[str, pd.DataFrame],
) -> None:
    source = combined["source_dataset"].astype(str).to_numpy()
    for source_name in ("scigen", "wyformer"):
        indexed = n312._index_pcae_by_prefixed_material_id(
            table=pcae_tables[source_name],
            source=source_name,
        ...
        for name in n312.n311.FEATURE_NAMES:
            combined.loc[mask, name] = ordered_ids.map(indexed[name]).to_numpy()


def run_pcae_margin_local_search(
    *,
    ...
    """Run the complete frozen discovery-only NEXT313 search."""

    if not set(REQUIRED_STAGES).issubset(stage_dirs):
        raise ValueError("NEXT313 stage directory universe differs")
    if not set(REQUIRED_DESIGN_STAGES).issubset(design_paths):
        raise ValueError("NEXT313 design path universe differs")
    roots = {
    ...
    if type(search_workers) is not int or search_workers <= 0:
        raise ValueError("NEXT313 search_workers must be a positive exact integer")
    if any(not path.is_file() for path in paths.values()):
        raise FileNotFoundError("NEXT313 input is missing")
    ...
    if require_formal_inputs and input_hashes != EXPECTED_INPUT_SHA256:
        ...
        raise ValueError(f"NEXT313 formal input identity differs: {differing}")
    (
        eligible_prior,
        eligible214,
        primary_key,
        ...
        pcae_tables,
        eligible312,
    ) = _verify_next312(paths, input_hashes)
    ...
    if (
        ...
        raise ValueError("NEXT313 NEXT224 base identity differs")
    specs = build_pcae_candidate_specs(
    ...
    if len(specs) != EXPECTED_CANDIDATE_COUNT or eligible_count != EXPECTED_ELIGIBLE_COUNT:
        raise RuntimeError("NEXT313 frozen candidate universe differs")
    virtual, terms, runtime, activity = materialize_pcae_candidates(
        features=combined,
        base_score=base_score,
    ...
    if (
        ...
        raise RuntimeError("NEXT313 evaluator accounting differs")
    ...
    if len(no_op) != 1 or len(reference_no_op) != 1:
        raise RuntimeError("NEXT313 no-op reproduction identity differs")
    ...
    if selected_row is not None:
        ...
        if str(selected_result["selected"]["record"]["candidate_key"]) != selected_key:
            raise RuntimeError("NEXT313 selected candidate reproduction differs")
        ...
        if selected_spec.get("eligible_new_candidate") is not True:
            raise RuntimeError("NEXT313 reproduction control was selected")
    ...
    if selected is not None and passes != bool(selected["record"]["passes_all_discovery_gates"]):
        raise RuntimeError("NEXT313 all-gate selection differs")
    if passes and selected is None:
        raise RuntimeError("NEXT313 all-gate candidate was not selected")
    ...
    catalogue = {
        ...
        "base_threshold": EXPECTED_BASE_THRESHOLD,
        "repair_width": REPAIR_WIDTH,
        "eligible_hypotheses": list(eligible312["hypothesis"].astype(str)),
        "eligible_hypothesis_count": len(eligible312),
    ...
    evaluation = {
        "protocol": PROTOCOL,
        "evaluation_mode": "frozen_pcae_margin_local_discovery_search",
        ...
        "next314_diagnostic_authorized": bool(not passes and diagnostic_keys),
        "next314_candidate_count": len(diagnostic_keys),
        "next314_candidate_key_sha256": diagnostic_sha,
    ...
    source_paths = {
        ...
        "src/next312_pcae_feature_audit.py": Path(n312.__file__).resolve(),
        "src/next313_pcae_margin_local_search.py": Path(__file__).resolve(),
    }
    ...
    try:
        ...
        if any(_sha256_file(path) != input_hashes[name] for name, path in paths.items()):
            raise RuntimeError("NEXT313 input changed before publication")
        if any(_sha256_file(path) != source_hashes[name] for name, path in source_paths.items()):
            raise RuntimeError("NEXT313 source changed before publication")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    manifest = run_pcae_margin_local_search(
...
__all__ = [
    ...
    "REQUIRED_STAGES",
    "build_pcae_candidate_specs",
    "pcae_margin_local_score",
    "run_pcae_margin_local_search",
