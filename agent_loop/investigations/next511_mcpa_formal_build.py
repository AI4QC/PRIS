#!/usr/bin/env python3
"""Physically isolated full discovery build for frozen NEXT510 MCPA."""
...
PROTOCOL = "2026-08-13-next511-mcpa-formal-build-v1"
MANIFEST_NAME = "MANIFEST.json"
CATALOGUE_NAME = "NEXT511_MCPA_FEATURE_CATALOGUE.json"
FEATURE_FILES = {
    "scigen": "next511_scigen_madelung_charge_potential_antitonicity.parquet",
    "wyformer": "next511_wyformer_madelung_charge_potential_antitonicity.parquet",
}
EXPECTED_ROWS = {"scigen": 13_470, "wyformer": 5_232}
MINIMUM_FORMAL_COVERAGE = 0.95
EXPECTED_PROBE_SHA256 = "e145807b295dd30a687c1a97dae62249748ec7409743d8707afc51c3dfb1173d"
EXPECTED_INPUT_SHA256 = {
    "design": "97c3d6220106b31d6dede1ed41c8e83eb3515f51d329d8a23d0076db2d7f5c21",
    "engineering_probe_result": "66da0767250845163ee7f874807cd81e92f03c6ec34a7e274c81a0bd524bc982",
    ...
    "next510_source": "b66c17012aa6dc3c43b0706595b6da8aad79715dbac94859c0e10a78a97416df",
...
def validate_probe_authorization(probe: dict[str, object]) -> None:
    ...
    if (
        probe.get("protocol")
        != "2026-08-13-next510-mcpa-label-blind-novelty-probe-v1"
        or probe.get("design_sha256") != n510.DESIGN_SHA256
        or probe.get("minimum_novelty_joint_finite") != 40
        or probe.get("probe_passed") is not True
        or probe.get("next511_formal_build_authorized") is not True
        ...
        raise ValueError("NEXT511 label-blind probe authorization differs")
...
def _validate_engineering_authorization(probe: dict[str, object]) -> None:
    ...
    if (
        probe.get("protocol")
        != "2026-08-13-next510-mcpa-label-blind-engineering-probe-v1"
        or probe.get("design_sha256") != n510.DESIGN_SHA256
        ...
        raise ValueError("NEXT511 engineering authorization differs")


def _error_row(exc: Exception) -> dict[str, object]:
    return {
        n510.FEATURE_NAMES[0]: math.nan,
        "mcpa_supported": False,
        "mcpa_failure": f"{type(exc).__name__}: {exc}",
        "mcpa_site_count": 0,
        "mcpa_pair_count": 0,
        "mcpa_minimum_exchange_margin": math.nan,
        "mcpa_mean_exchange_margin": math.nan,
        "mcpa_maximum_exchange_margin": math.nan,
        "mcpa_ewald_identity_relative_error": math.nan,
        "mcpa_valence_policy": None,
    }
...
def _compute_scigen_payload(item):
    ...
    try:
        ...
        return material_id, n510.compute_mcpa_row(atoms)
...
def label_free_statistics(table: pd.DataFrame) -> dict[str, object]:
    values = pd.to_numeric(table[n510.FEATURE_NAMES[0]], errors="coerce").dropna().to_numpy(float)
    if len(values) < 1:
        raise RuntimeError("NEXT511 label-free feature population is empty")
    return {
        n510.FEATURE_NAMES[0]: {
...
def _formal_paths(
    *,
    ...
    return {
        ...
        "next510_source": Path(n510.__file__).resolve(),
    }
...
def _validate_geometry_provenance(paths: dict[str, Path]) -> None:
    ...
    if (
        ...
        raise ValueError("NEXT511 discovery geometry provenance differs")


def build_cross_source_mcpa_features(
    *,
    ...
    if type(workers) is not int or workers <= 0:
        raise ValueError("NEXT511 workers must be a positive exact integer")
    if any(not path.is_file() for path in paths.values()):
        raise FileNotFoundError("NEXT511 input is missing")
    ...
    if require_formal_inputs and input_hashes != EXPECTED_INPUT_SHA256:
        ...
        raise ValueError(f"NEXT511 formal input identity differs: {differing}")
    ...
    for source, frame in metadata.items():
        if required - set(frame) or frame["material_id"].astype(str).duplicated().any():
            raise ValueError(f"NEXT511 {source} metadata differs")
        selected = (
        ...
        if len(selected) != EXPECTED_ROWS[source]:
            raise ValueError(f"NEXT511 {source} discovery identity differs")
    ...
    executed_paths = {
        "src/next511_mcpa_formal_build.py": Path(__file__).resolve(),
        "src/next510_madelung_charge_potential_antitonicity.py": Path(n510.__file__).resolve(),
        "experiments/next510_mcpa_label_blind_engineering_probe.py": root
        / "experiments/next510_mcpa_label_blind_engineering_probe.py",
        "experiments/next510_mcpa_label_blind_novelty_probe.py": root
        / "experiments/next510_mcpa_label_blind_novelty_probe.py",
        "tests/test_next510_madelung_charge_potential_antitonicity.py": root
        / "tests/test_next510_madelung_charge_potential_antitonicity.py",
        "tests/test_next510_mcpa_label_blind_engineering_probe.py": root
        / "tests/test_next510_mcpa_label_blind_engineering_probe.py",
        "tests/test_next510_mcpa_label_blind_novelty_probe.py": root
        / "tests/test_next510_mcpa_label_blind_novelty_probe.py",
        "tests/test_next511_mcpa_formal_build.py": root
        / "tests/test_next511_mcpa_formal_build.py",
    }
    ...
    try:
        ...
        for source in ("scigen", "wyformer"):
            ...
            if (
                ...
                raise RuntimeError(f"NEXT511 {source} material identity differs")
            ...
            finite = np.isfinite(
                pd.to_numeric(table[n510.FEATURE_NAMES[0]], errors="coerce").to_numpy(float)
            )
            if not finite[supported].all() or finite[~supported].any():
                raise RuntimeError(f"NEXT511 {source} finite support semantics differ")
            coverage = float(supported.mean())
            statistics[source] = label_free_statistics(table)
            failures = Counter(table.loc[~supported, "mcpa_failure"].fillna("unknown"))
            counts[source] = {
                "rows": int(len(table)),
                ...
                "site_count": int(pd.to_numeric(table.loc[supported, "mcpa_site_count"]).sum()),
                "pair_count": int(pd.to_numeric(table.loc[supported, "mcpa_pair_count"]).sum()),
                "finite_feature_count": int(finite.sum()),
                "maximum_ewald_identity_relative_error": float(
                    pd.to_numeric(
                        table.loc[supported, "mcpa_ewald_identity_relative_error"]
                    ).max()
                ),
        ...
        catalogue = {
            "protocol": PROTOCOL,
            "feature_count": 1,
            "feature_names": list(n510.FEATURE_NAMES),
            "feature_directions": n510.FEATURE_DIRECTIONS,
            "directions_frozen_before_outcome": True,
            "pair_population": "all unordered unequal-formal-charge site pairs",
            "formula": "(1+mean(-sign(dq)*dphi/(abs(phi_i)+abs(phi_j))))/2",
            "output_grid": 1.0 / n510.OUTPUT_GRID,
            "minimum_formal_coverage": MINIMUM_FORMAL_COVERAGE,
            "label_free_statistics": statistics,
            "coverage_gate_passed": coverage_gate_passed,
            "next512_audit_authorized": coverage_gate_passed,
            **n510.BOUNDARY_FLAGS,
        }
        ...
        manifest = {
            "protocol": PROTOCOL,
            "mode": "physically_isolated_discovery_x0_mcpa_freeze",
            "workers": workers,
        ...
        if any(
            ...
            raise RuntimeError("NEXT511 executed artifact changed before publication")
...
def main() -> int:
    ...
    manifest = build_cross_source_mcpa_features(
...
__all__ = [
    ...
    "PROBE_BOUNDARY_NAMES",
    "PROTOCOL",
    "build_cross_source_mcpa_features",
