#!/usr/bin/env python3
"""Audit frozen PCGR hypotheses on physically isolated discovery outcomes."""
...
import src.next315_periodic_charge_green_resistance as n315
from src.next102_cross_source_dobvr_features import _sha256_file, _write_json


PROTOCOL = "2026-08-09-next316-pcgr-feature-audit-v1"
MANIFEST_NAME = "MANIFEST.json"
CATALOGUE_NAME = "NEXT316_PCGR_FEATURE_CATALOGUE.json"
AUDIT_NAME = "NEXT316_PCGR_FEATURE_AUDIT.json"
TABLE_NAME = "next316_pcgr_feature_audit.parquet"
HYPOTHESES = tuple((name, n315.FEATURE_DIRECTIONS[name]) for name in n315.FEATURE_NAMES)
QUANTILES = n268.QUANTILES
EXPECTED_NEXT268_SOURCE_SHA256 = (
    "36af0e632c8c3aae93f78271f837f693d49eff6ae88cdcad8edde42b3e0eaf64"
)
EXPECTED_NEXT315_SOURCE_SHA256 = (
    "3fb7ee58097e8480cb83740e898312749f3f9da240db1fae7bd3d5a795829ac8"
)
NEXT315_SOURCE_PATH = "src/next315_periodic_charge_green_resistance.py"
REQUIRED_STAGES = (*n268.REQUIRED_STAGES, 315)
...
EXPECTED_INPUT_SHA256 = {
    **{
    ...
    "design": n315.EXPECTED_DESIGN_SHA256,
    "next315_manifest": "e941927dacfe34703de77dff77636f9c33b58343182805ae8809dd93cfeba39f",
    "next315_catalogue": "c056426bbb5bec1a88a8a04a5a85aa0264e048b5da355b98b869d49251213633",
    "next315_scigen": "e805db3d4d55379b9b803c7491936331d39e14e861d11757cac238950f514a84",
    "next315_wyformer": "981a60b5fd7fa602d1decc21f8921a1fde173d783f5e6aa665505c3f37cd47ec",
}
...
def select_eligible_hypotheses(
    ...
    try:
        ...
        raise ValueError("NEXT316 audit ranking table differs") from exc
...
def bounded_protection(
    *, values: object, direction: str, q_lo: float, q_hi: float
    ...
    if direction != "protected_low":
        raise ValueError("NEXT316 bounded protection inputs differ")
    try:
        return n268.bounded_protection(
            values=values, direction=direction, q_lo=q_lo, q_hi=q_hi
        )
    except ValueError as exc:
        raise ValueError("NEXT316 bounded protection inputs differ") from exc


def _index_pcgr_by_prefixed_material_id(
    *, table: pd.DataFrame, source: str, expected_material_ids: object
    ...
    if set(extra["material_id"]) != expected:
        raise ValueError(f"NEXT316 {source} material identity differs")
...
def _paths(
    *,
    ...
    paths.update(
        {
            "design": design_path,
            "next315_manifest": roots["next315"] / n315.MANIFEST_NAME,
            "next315_catalogue": roots["next315"] / n315.CATALOGUE_NAME,
            "next315_scigen": roots["next315"] / n315.FEATURE_FILES["scigen"],
            "next315_wyformer": roots["next315"] / n315.FEATURE_FILES["wyformer"],
        }
    )
    return paths


def _verify_next315(paths: Mapping[str, Path], input_hashes: Mapping[str, str]):
    ...
    manifest = json.loads(paths["next315_manifest"].read_text())
    catalogue = json.loads(paths["next315_catalogue"].read_text())
    tables = {
        "scigen": pd.read_parquet(paths["next315_scigen"]),
        "wyformer": pd.read_parquet(paths["next315_wyformer"]),
    }
    expected_outputs = {
        n315.CATALOGUE_NAME: input_hashes["next315_catalogue"],
        n315.FEATURE_FILES["scigen"]: input_hashes["next315_scigen"],
        n315.FEATURE_FILES["wyformer"]: input_hashes["next315_wyformer"],
    }
    if (
        _sha256_file(Path(n268.__file__).resolve()) != EXPECTED_NEXT268_SOURCE_SHA256
        or _sha256_file(Path(n315.__file__).resolve()) != EXPECTED_NEXT315_SOURCE_SHA256
        or manifest.get("protocol") != n315.PROTOCOL
        or manifest.get("next316_audit_authorized") is not True
        ...
        or manifest.get("executed_source_sha256", {}).get(NEXT315_SOURCE_PATH)
        != EXPECTED_NEXT315_SOURCE_SHA256
        or any(manifest.get(key) is not value for key, value in BOUNDARY_FLAGS.items())
        or catalogue.get("feature_names") != list(n315.FEATURE_NAMES)
        or catalogue.get("feature_directions") != n315.FEATURE_DIRECTIONS
        or catalogue.get("feature_count") != len(n315.FEATURE_NAMES)
        or catalogue.get("graph")
        != "NEXT279 full reciprocal periodic radical active-facet quotient multigraph"
        or catalogue.get("laplacian") != "L = diag(W 1) - W"
        or catalogue.get("solver")
        != "symmetric eigensystem Moore-Penrose pseudoinverse"
        or catalogue.get("charge_resistance") != "(q^T L^+ q) / (q^T q)"
        or catalogue.get("voltage_normalization") != "RMS(q)"
        or catalogue.get("minimum_formal_coverage") != n315.MINIMUM_FORMAL_COVERAGE
        or catalogue.get("quantile_method") != "inverted_cdf"
        or catalogue.get("directions_frozen_before_outcome") is not True
    ):
        raise ValueError("NEXT316 NEXT315 provenance differs")
    for source, expected_rows in n315.EXPECTED_ROWS.items():
        table = tables[source]
        supported = table["pcgr_supported"].fillna(False).astype(bool)
        values = np.column_stack(
            [
                pd.to_numeric(table[name], errors="coerce").to_numpy(float)
                for name in n315.FEATURE_NAMES
            ]
        )
        finite = np.isfinite(values)
        sites = pd.to_numeric(table["pcgr_site_count"], errors="coerce")
        contacts = pd.to_numeric(
            table["pcgr_directed_contact_count"], errors="coerce"
        )
        intersite = pd.to_numeric(
            table["pcgr_intersite_contact_count"], errors="coerce"
        )
        positive_modes = pd.to_numeric(
            table["pcgr_positive_mode_count"], errors="coerce"
        )
        nullity = pd.to_numeric(table["pcgr_nullity"], errors="coerce")
        minimum_eigenvalue = pd.to_numeric(
            table["pcgr_minimum_positive_eigenvalue"], errors="coerce"
        )
        maximum_eigenvalue = pd.to_numeric(
            table["pcgr_maximum_eigenvalue"], errors="coerce"
        )
        policy = table["pcgr_valence_policy"]
        if (
            ...
            or float(supported.mean()) < n315.MINIMUM_FORMAL_COVERAGE
            ...
            or not (intersite[supported] > 0).all()
            or not (positive_modes[supported] > 0).all()
            or not (nullity[supported] >= 1).all()
            or not np.isfinite(minimum_eigenvalue[supported]).all()
            or not np.isfinite(maximum_eigenvalue[supported]).all()
            or not (minimum_eigenvalue[supported] > 0.0).all()
            or not (maximum_eigenvalue[supported] >= minimum_eigenvalue[supported]).all()
            or policy[supported].isna().any()
            or not (sites[~supported] == 0).all()
            or not (contacts[~supported] == 0).all()
            or not (intersite[~supported] == 0).all()
            or not (positive_modes[~supported] == 0).all()
            or not (nullity[~supported] == 0).all()
        ):
            raise ValueError(f"NEXT316 NEXT315 {source} table differs")
    return (*prior[:-1], tables)


def run_pcgr_feature_audit(
    *,
    ...
    """Audit exactly the three frozen PCGR hypotheses."""

    if not set(REQUIRED_STAGES).issubset(stage_dirs):
        raise ValueError("NEXT316 stage directory universe differs")
    if not set(REQUIRED_DESIGN_STAGES).issubset(design_paths):
        raise ValueError("NEXT316 design path universe differs")
    roots = {
    ...
    if any(not path.is_file() for path in paths.values()):
        raise FileNotFoundError("NEXT316 input is missing")
    ...
    if require_formal_inputs and input_hashes != EXPECTED_INPUT_SHA256:
        ...
        raise ValueError(f"NEXT316 formal input identity differs: {differing}")
    (
        eligible,
        eligible214,
        primary_key,
        ...
        formula222,
        pcgr_tables,
    ) = _verify_next315(paths, input_hashes)
    n227 = n268.n227
    ...
    for source_name in ("scigen", "wyformer"):
        indexed = _index_pcgr_by_prefixed_material_id(
    ...
    if cohort_counts != n227.EXPECTED_COHORT_COUNTS:
        raise ValueError("NEXT316 rejected cohort counts differ")
    ...
    for feature, direction in HYPOTHESES:
        ...
        if not len(finite):
            raise ValueError(f"NEXT316 {feature} has no finite discovery values")
    ...
    catalogue = {
        ...
        "features": list(n315.FEATURE_NAMES),
        "feature_directions": n315.FEATURE_DIRECTIONS,
    ...
    audit = {
        "protocol": PROTOCOL,
        "audit_mode": "fixed_next224_rejected_extreme_pcgr_audit",
        "base_candidate_key_sha256": n227.EXPECTED_BASE_CANDIDATE_KEY_SHA256,
        "base_threshold": n227.EXPECTED_BASE_THRESHOLD,
        "cohort_counts": {
        ...
        "next317_search_authorized": bool(selected is not None),
    ...
    source_paths = {
        ...
        NEXT315_SOURCE_PATH: Path(n315.__file__).resolve(),
        "src/next316_pcgr_feature_audit.py": Path(__file__).resolve(),
    }
    ...
    try:
        ...
        manifest = {
            ...
            "next317_search_authorized": bool(selected is not None),
            "pcgr_branch_terminated": selected is None,
            "new_formula_searched": False,
            "new_formula_selected": False,
        ...
        if any(_sha256_file(path) != input_hashes[name] for name, path in paths.items()):
            raise RuntimeError("NEXT316 input changed before publication")
        if any(_sha256_file(path) != source_hashes[name] for name, path in source_paths.items()):
            raise RuntimeError("NEXT316 source changed before publication")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    manifest = run_pcgr_feature_audit(
