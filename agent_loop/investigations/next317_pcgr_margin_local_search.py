#!/usr/bin/env python3
"""Frozen one-term PCGR margin-local search on the NEXT224 frontier."""
...
import src.next316_pcgr_feature_audit as n316
from src.next102_cross_source_dobvr_features import _sha256_file, _write_json


n227 = n316.n268.n227
n223 = n261.n223
PROTOCOL = "2026-08-09-next317-pcgr-margin-local-search-v1"
MANIFEST_NAME = "MANIFEST.json"
CATALOGUE_NAME = "NEXT317_PCGR_MARGIN_LOCAL_CATALOGUE.json"
EVALUATION_NAME = "NEXT317_DISCOVERY_EVALUATION.json"
FORMULA_NAME = "NEXT317_FROZEN_CANDIDATE.json"
SEARCH_NAME = "next317_pcgr_margin_local_search.parquet"
SCORE_COMPOSITION = "nonnegative_next224_plus_triangular_margin_local_signed_pcgr_term"
...
EXPECTED_ELIGIBLE_HYPOTHESIS_SHA256 = (
    "5ca6128696517b5dfa7feb29ab2c0ea49ddccff488e69e837260490098c151a7"
)
...
EXPECTED_DESIGN_SHA256 = n316.EXPECTED_INPUT_SHA256["design"]
...
BOUNDARY_FLAGS = n316.BOUNDARY_FLAGS
REQUIRED_STAGES = (*n316.REQUIRED_STAGES, 316)
REQUIRED_DESIGN_STAGES = (*n316.REQUIRED_DESIGN_STAGES, 316)
EXPECTED_NEXT261_SOURCE_SHA256 = (
    "5283f4b6a58e313a735cd4f3245028a173caa77f0338a6d3e04e5a7244256599"
)
EXPECTED_NEXT316_SOURCE_SHA256 = (
    "e324720ba744990275affaaf657206d84d909cdab141c08b0aaab35fb5683edb"
)
EXPECTED_INPUT_SHA256 = {
    **{
        key: value
        for key, value in n316.EXPECTED_INPUT_SHA256.items()
        if key != "design"
    },
    "next316_design": n316.EXPECTED_INPUT_SHA256["design"],
    "design": EXPECTED_DESIGN_SHA256,
    "next316_manifest": "ef9678589ec20bb0514702e2980c3482038620f1b6d35de22eb2d3904c6b3181",
    "next316_catalogue": "366ef9818636d25adc92100240b4f9118ca72a25cb16475658e696ed2e034176",
    "next316_audit": "da640e9c957eae53671f37d970807948fc848881cc978acd01f6b1fe7bd80342",
    "next316_table": "db419bd0a12c49176522c1a55e2b4f0fe6413b94456eaa96f6f6697f90dd0258",
}


def pcgr_margin_local_score(
    *,
    base_score: object,
    base_support: object,
    protection: object,
    ...
    try:
        ...
        raise ValueError("NEXT317 margin-local score inputs differ") from exc


def build_pcgr_candidate_specs(
    *,
    ...
    if len({str(spec["candidate_key"]) for spec in result}) != len(result):
        raise RuntimeError("NEXT317 candidate keys are not unique")
    return result


def materialize_pcgr_candidates(
    *,
    ...
    for term in terms:
        ...
        new_id = "next317_virtual_candidate__" + hashlib.sha256(key.encode()).hexdigest()[:24]
        ...
        term.update(
            {
                "term_id": new_id,
                "feature": new_column,
                "group": "next317_pcgr_margin_local",
            }
        )
...
def _paths(
    *,
    ...
    paths = n316._paths(
        ...
        design_paths={stage: design_paths[stage] for stage in n316.REQUIRED_DESIGN_STAGES},
        design_path=design_paths[316],
    )
    paths["next316_design"] = paths.pop("design")
    paths.update(
        {
            "design": design_path,
            "next316_manifest": roots["next316"] / n316.MANIFEST_NAME,
            "next316_catalogue": roots["next316"] / n316.CATALOGUE_NAME,
            "next316_audit": roots["next316"] / n316.AUDIT_NAME,
            "next316_table": roots["next316"] / n316.TABLE_NAME,
        }
    )
    return paths


def _verify_next316(paths: Mapping[str, Path], input_hashes: Mapping[str, str]):
    base_paths = {key: paths[key] for key in n316.EXPECTED_INPUT_SHA256}
    base_paths["design"] = paths["next316_design"]
    base_hashes = {key: input_hashes[key] for key in n316.EXPECTED_INPUT_SHA256}
    base_hashes["design"] = input_hashes["next316_design"]
    prior = n316._verify_next315(base_paths, base_hashes)
    manifest = json.loads(paths["next316_manifest"].read_text())
    catalogue = json.loads(paths["next316_catalogue"].read_text())
    audit = json.loads(paths["next316_audit"].read_text())
    table = pd.read_parquet(paths["next316_table"])
    ...
    expected_outputs = {
        n316.CATALOGUE_NAME: input_hashes["next316_catalogue"],
        n316.AUDIT_NAME: input_hashes["next316_audit"],
        n316.TABLE_NAME: input_hashes["next316_table"],
    }
    if (
        manifest.get("protocol") != n316.PROTOCOL
        ...
        or manifest.get("next317_search_authorized") is not True
        ...
        or manifest.get("executed_source_sha256", {}).get(
            "src/next316_pcgr_feature_audit.py"
        )
        != EXPECTED_NEXT316_SOURCE_SHA256
        or _sha256_file(Path(n316.__file__).resolve()) != EXPECTED_NEXT316_SOURCE_SHA256
        ...
        or audit.get("next317_search_authorized") is not True
        ...
        raise ValueError("NEXT317 NEXT316 provenance differs")
...
def _formula_from_spec(spec: Mapping[str, object] | None) -> dict[str, object]:
    ...
    return {
        "protocol": PROTOCOL,
        "kind": "pcgr_triangular_margin_local_x0_no_dft_score",
        "selected": True,
...
def select_best_new_record(records: pd.DataFrame) -> pd.Series | None:
    try:
        ...
        raise ValueError("NEXT317 reporting selection schema differs") from exc


def _attach_pcgr_features(
    *,
    combined: pd.DataFrame,
    feature_tables: Mapping[str, pd.DataFrame],
    pcgr_tables: Mapping[str, pd.DataFrame],
) -> None:
    source = combined["source_dataset"].astype(str).to_numpy()
    for source_name in ("scigen", "wyformer"):
        indexed = n316._index_pcgr_by_prefixed_material_id(
            table=pcgr_tables[source_name],
            source=source_name,
        ...
        for name in n316.n315.FEATURE_NAMES:
            combined.loc[mask, name] = ordered_ids.map(indexed[name]).to_numpy()


def run_pcgr_margin_local_search(
    *,
    ...
    """Run the complete frozen discovery-only NEXT317 search."""

    if not set(REQUIRED_STAGES).issubset(stage_dirs):
        raise ValueError("NEXT317 stage directory universe differs")
    if not set(REQUIRED_DESIGN_STAGES).issubset(design_paths):
        raise ValueError("NEXT317 design path universe differs")
    roots = {
    ...
    if type(search_workers) is not int or search_workers <= 0:
        raise ValueError("NEXT317 search_workers must be a positive exact integer")
    if any(not path.is_file() for path in paths.values()):
        raise FileNotFoundError("NEXT317 input is missing")
    ...
    if require_formal_inputs and input_hashes != EXPECTED_INPUT_SHA256:
        ...
        raise ValueError(f"NEXT317 formal input identity differs: {differing}")
    (
        eligible_prior,
        eligible214,
        primary_key,
        ...
        pcgr_tables,
        eligible316,
    ) = _verify_next316(paths, input_hashes)
    ...
    if (
        ...
        raise ValueError("NEXT317 NEXT224 base identity differs")
    specs = build_pcgr_candidate_specs(
    ...
    if len(specs) != EXPECTED_CANDIDATE_COUNT or eligible_count != EXPECTED_ELIGIBLE_COUNT:
        raise RuntimeError("NEXT317 frozen candidate universe differs")
    virtual, terms, runtime, activity = materialize_pcgr_candidates(
        features=combined,
        base_score=base_score,
    ...
    if (
        ...
        raise RuntimeError("NEXT317 evaluator accounting differs")
    ...
    if len(no_op) != 1 or len(reference_no_op) != 1:
        raise RuntimeError("NEXT317 no-op reproduction identity differs")
    ...
    if selected_row is not None:
        ...
        if str(selected_result["selected"]["record"]["candidate_key"]) != selected_key:
            raise RuntimeError("NEXT317 selected candidate reproduction differs")
        ...
        if selected_spec.get("eligible_new_candidate") is not True:
            raise RuntimeError("NEXT317 reproduction control was selected")
    ...
    if selected is not None and passes != bool(selected["record"]["passes_all_discovery_gates"]):
        raise RuntimeError("NEXT317 all-gate selection differs")
    if passes and selected is None:
        raise RuntimeError("NEXT317 all-gate candidate was not selected")
    ...
    catalogue = {
        ...
        "base_threshold": EXPECTED_BASE_THRESHOLD,
        "repair_width": REPAIR_WIDTH,
        "eligible_hypotheses": list(eligible316["hypothesis"].astype(str)),
        "eligible_hypothesis_count": len(eligible316),
    ...
    evaluation = {
        "protocol": PROTOCOL,
        "evaluation_mode": "frozen_pcgr_margin_local_discovery_search",
        ...
        "next318_diagnostic_authorized": bool(not passes and diagnostic_keys),
        "next318_candidate_count": len(diagnostic_keys),
        "next318_candidate_key_sha256": diagnostic_sha,
    ...
    source_paths = {
        ...
        "src/next316_pcgr_feature_audit.py": Path(n316.__file__).resolve(),
        "src/next317_pcgr_margin_local_search.py": Path(__file__).resolve(),
    }
    ...
    try:
        ...
        if any(_sha256_file(path) != input_hashes[name] for name, path in paths.items()):
            raise RuntimeError("NEXT317 input changed before publication")
        if any(_sha256_file(path) != source_hashes[name] for name, path in source_paths.items()):
            raise RuntimeError("NEXT317 source changed before publication")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    manifest = run_pcgr_margin_local_search(
...
__all__ = [
    ...
    "REQUIRED_STAGES",
    "build_pcgr_candidate_specs",
    "pcgr_margin_local_score",
    "run_pcgr_margin_local_search",
