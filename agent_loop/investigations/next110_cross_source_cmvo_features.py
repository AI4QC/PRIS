#!/usr/bin/env python3
"""Discovery-only cross-source materialization of NEXT109 CMVO certificates."""
...
PROTOCOL = "2026-08-08-next110-cross-source-discovery-cmvo-v1"
GRAPH_MODE = "voronoi"
DISCOVERY_ROLE = "discovery"
MANIFEST_NAME = "MANIFEST.json"
CATALOGUE_NAME = "NEXT110_CMVO_FEATURE_CATALOGUE.json"
FEATURE_FILES = {
    "scigen": "scigen_discovery_cmvo_features.parquet",
    "wyformer": "wyformer_discovery_cmvo_features.parquet",
}
CATALOGUE_MODES = ("core", "expanded")
NUMERIC_FEATURE_NAMES = tuple(
    name.replace("cmvo_", f"cmvo_{mode}_", 1)
...
STATUS_COLUMNS = tuple(
    ...
    for mode in CATALOGUE_MODES
    for name in (
        f"cmvo_{mode}_supported",
        f"cmvo_{mode}_failure",
        f"cmvo_{mode}_catalogue_sha256",
        f"cmvo_{mode}_pymatgen_version",
        f"cmvo_{mode}_scipy_version",
    )
)
...
EXPECTED_INPUT_SHA256 = {
    ...
    "design": "fb74b3ed4ac1aef153891e5f93ac025af0d745567d57d02ef04fd1133a204c3e",
}


def compute_cmvo_feature_row(structure, *, graph_mode: str) -> dict[str, object]:
    """Compute core and expanded obstructions with independent abstention."""
    ...
    for mode in CATALOGUE_MODES:
        try:
            ...
            statuses.update(
                {
                    f"cmvo_{mode}_supported": False,
                    f"cmvo_{mode}_failure": f"{type(exc).__name__}: {exc}",
                    f"cmvo_{mode}_catalogue_sha256": None,
                    f"cmvo_{mode}_pymatgen_version": None,
                    f"cmvo_{mode}_scipy_version": None,
                }
            )
            continue
        for name, value in result.features.items():
            row[name.replace("cmvo_", f"cmvo_{mode}_", 1)] = float(value)
        statuses.update(
            {
                f"cmvo_{mode}_supported": bool(result.supported),
                f"cmvo_{mode}_failure": result.failure_reason,
                f"cmvo_{mode}_catalogue_sha256": result.catalogue_sha256,
                f"cmvo_{mode}_pymatgen_version": result.pymatgen_version,
                f"cmvo_{mode}_scipy_version": result.scipy_version,
            }
        )
    row.update(statuses)
    if tuple(row) != FEATURE_COLUMNS:
        raise RuntimeError("NEXT110 feature row schema differs")
    return row
...
def _compute_scigen_payload(item: tuple[str, bytes]) -> tuple[str, dict[str, object]]:
    ...
    try:
        ...
        row = compute_cmvo_feature_row(structure, graph_mode=GRAPH_MODE)
    except Exception as exc:
...
def _source_counts(table: pd.DataFrame) -> dict[str, object]:
    ...
    for mode in CATALOGUE_MODES:
        supported = f"cmvo_{mode}_supported"
        failure = f"cmvo_{mode}_failure"
...
def _solver_thread_environment(
    *, require_formal_inputs: bool
    ...
    if require_formal_inputs and any(value != "1" for value in values.values()):
        raise ValueError("NEXT110 formal run requires a single-thread solver environment")
    return values


def build_cross_source_discovery_cmvo_features(
    *,
    ...
    """Materialize CMVO only from physically split discovery geometries."""
    ...
    if any(not path.is_file() for path in paths.values()):
        raise FileNotFoundError("NEXT110 discovery input is missing")
    input_hashes = {name: _sha256_file(path) for name, path in paths.items()}
    if require_formal_inputs and input_hashes != EXPECTED_INPUT_SHA256:
        raise ValueError("NEXT110 formal input identity differs")
    ...
    source_paths = {
        ...
        "src/next110_cross_source_cmvo_features.py": Path(__file__).resolve(),
    }
    ...
    try:
        for source in ("scigen", "wyformer"):
            ...
            if len(table) != len(metadata[source]) or any(
                ...
                raise RuntimeError(f"NEXT110 {source} row accounting differs")
        ...
        manifest: dict[str, object] = {
            "protocol": PROTOCOL,
            "mode": "cross_source_discovery_only_raw_x0_cmvo_feature_freeze",
            "graph_mode": GRAPH_MODE,
            "workers": workers,
        ...
        if any(_sha256_file(path) != input_hashes[name] for name, path in paths.items()):
            raise RuntimeError("NEXT110 input changed before publication")
        if any(_sha256_file(path) != source_hashes[name] for name, path in source_paths.items()):
            raise RuntimeError("NEXT110 source changed before publication")
