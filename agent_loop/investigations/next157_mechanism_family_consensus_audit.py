#!/usr/bin/env python3
"""Audit consensus statistics across four frozen physical-mechanism families."""
...
PROTOCOL = "2026-08-08-next157-mechanism-family-consensus-audit-v1"
MANIFEST_NAME = "MANIFEST.json"
AUDIT_NAME = "NEXT157_MECHANISM_FAMILY_CONSENSUS_AUDIT.json"
TABLE_NAME = "next157_mechanism_family_consensus_audit.parquet"
EXPECTED_DESIGN_SHA256 = "2ee1dd722fa26ae01abb310609f4f2eee9d722491a5af0b81f3ab0ea913e7be7"
...
FIXED_DIRECTIONS = {
    ...
    "family_max_second": -1,
    "family_max_third": -1,
    "family_max_sum_minus_largest": -1,
    "family_max_geomean1p": -1,
    "family_active_count_0p25": -1,
    "family_active_count_0p5": -1,
    "family_capped_mean_sum": -1,
    "family_rational_mean_sum": -1,
}
...
def mechanism_family_statistics(
    ...
    if (
        ...
        raise ValueError("NEXT157 contribution matrix differs")
    ...
    for index, term_id in enumerate(term_ids):
        ...
        if len(matches) != 1:
            raise ValueError("NEXT157 term-to-family assignment differs")
        members[matches[0]].append(index)
    if any(not indices for indices in members.values()):
        raise ValueError("NEXT157 family coverage differs")
    means = np.column_stack(
        [values[:, indices].mean(axis=1) for indices in members.values()]
    )
    ...
    rational_means = np.column_stack(
        [
            (values[:, indices] / (1.0 + values[:, indices])).mean(axis=1)
            for indices in members.values()
        ]
    )
    ordered = np.sort(maxima, axis=1)[:, ::-1]
    result = {
        "family_mean_sum": means.sum(axis=1),
        "family_max_sum": maxima.sum(axis=1),
        "family_max_second": ordered[:, 1],
        "family_max_third": ordered[:, 2],
        "family_max_sum_minus_largest": maxima.sum(axis=1) - ordered[:, 0],
        "family_max_geomean1p": np.expm1(np.log1p(maxima).mean(axis=1)),
        "family_active_count_0p25": (maxima > 0.25).sum(axis=1).astype(float),
        "family_active_count_0p5": (maxima > 0.5).sum(axis=1).astype(float),
        "family_capped_mean_sum": capped_means.sum(axis=1),
        "family_rational_mean_sum": rational_means.sum(axis=1),
    }
    if set(result) != set(FIXED_DIRECTIONS):
        raise RuntimeError("NEXT157 statistic schema differs")
    return result
...
def run_mechanism_family_consensus_audit(
    *,
    ...
    if any(not path.is_file() for path in paths.values()):
        raise FileNotFoundError("NEXT157 input is missing")
    ...
    if require_formal_inputs and input_hashes != EXPECTED_INPUT_SHA256:
        ...
        raise ValueError(f"NEXT157 formal input identity differs: {differing}")
    ...
    if (
        ...
        raise ValueError("NEXT157 prior provenance differs")
    ...
    if (
        ...
        raise ValueError("NEXT157 frozen candidate differs")
    ...
    if len(selected_specs) != 1:
        raise ValueError("NEXT157 candidate reconstruction differs")
    ...
    if not np.array_equal(contribution_support, published_support):
        raise ValueError("NEXT157 contribution support differs")
    contributions = np.column_stack(contribution_columns)
    statistics = mechanism_family_statistics(
    ...
    if min(
        ...
        raise ValueError("NEXT157 audit population differs")
    ...
    for name in sorted(statistics):
        ...
        for population, mask in populations.items():
            ...
            if result is None:
                raise ValueError("NEXT157 AUC population differs")
    ...
    source_paths = {
        ...
        "src/next157_mechanism_family_consensus_audit.py": Path(__file__).resolve(),
    }
    source_hashes = {
    ...
    try:
        ...
        if any(_sha256_file(path) != input_hashes[name] for name, path in paths.items()):
            raise RuntimeError("NEXT157 input changed before publication")
        if any(
            _sha256_file(path) != source_hashes[name]
            for name, path in source_paths.items()
        ):
            raise RuntimeError("NEXT157 source changed before publication")
...
def main() -> int:
    ...
    manifest = run_mechanism_family_consensus_audit(
...
__all__ = [
    ...
    "run_mechanism_family_consensus_audit",
]
