#!/usr/bin/env python3
"""Diagnose the frozen NEXT228 eligible BROAD residual population."""
...
import src.next228_margin_local_redistribution_search as n228
from src.next102_cross_source_dobvr_features import _sha256_file, _write_json


PROTOCOL = "2026-08-09-next229-margin-local-broad-diagnostic-v1"
MANIFEST_NAME = "MANIFEST.json"
DIAGNOSTIC_NAME = "NEXT229_MARGIN_LOCAL_BROAD_DIAGNOSTIC.json"
TABLE_NAME = "next229_margin_local_broad_diagnostic.parquet"
EXPECTED_DESIGN_SHA256 = n228.EXPECTED_DESIGN_SHA256
EXPECTED_NEXT228_SOURCE_SHA256 = (
    "711b1076b2329957d5cb404f7c131b4c62fde4bd1c28fe49dbbea3c75b907b4a"
)
EXPECTED_CANDIDATE_COUNT = 388
EXPECTED_CANDIDATE_KEY_SHA256 = (
    "6f7e170470dc5f4d1a3b62022ed9e98245007749ff50df4405c969dadbc4aa39"
)
EXPECTED_BASE_FAILED_COUNT = n228.EXPECTED_BASE_FAILED_COUNT
EXPECTED_BASE_SHORTFALL = n228.EXPECTED_BASE_SHORTFALL
SEARCH_WORKERS = n228.SEARCH_WORKERS
BOUNDARY_FLAGS = n228.BOUNDARY_FLAGS
REQUIRED_STAGES = (*n228.REQUIRED_STAGES, 228)
REQUIRED_DESIGN_STAGES = (*n228.REQUIRED_DESIGN_STAGES, 228)
EXPECTED_INPUT_SHA256 = {
    **{
        key: value
        for key, value in n228.EXPECTED_INPUT_SHA256.items()
        if key != "design"
    },
    "next228_design": n228.EXPECTED_INPUT_SHA256["design"],
    "design": EXPECTED_DESIGN_SHA256,
    "next228_manifest": (
        "b1745ed2b583e3e5078e0b5cee0173dbb19e04b170fd8873a3b223f3aebcaf81"
    ),
    "next228_catalogue": (
        "c1292f0c24127a72ce65fed09b5bcee05fd2a6bdf964c2aae802c8efa8bfffcb"
    ),
    "next228_evaluation": (
        "eeed9dde22124639ba37599ecc347812521c988c0af7f89cc44f35359ac2ccdd"
    ),
    "next228_formula": (
        "00ba5dc6d710812da06351f7ec5c64ddbb65d714bebb68749113213b70967e6a"
    ),
    "next228_search": (
        "e367e983fce522b0d3b944b952a93b4623f4bbaed459e19545a6c11b9734952e"
    ),
}


def select_diagnostic_candidates(published: pd.DataFrame) -> pd.DataFrame:
    """Select the preregistered eligible AUC+SAFE/non-BROAD population."""
    ...
    if required - set(published.columns):
        raise ValueError("NEXT229 published candidate columns differ")
    mask = (
...
def select_closest_residual(frame: pd.DataFrame) -> pd.Series:
    ...
    if frame.empty or required - set(frame.columns):
        raise ValueError("NEXT229 residual population differs")
...
def _paths(
    *,
    ...
    paths = n228._paths(
        ...
        design_paths={
            stage: design_paths[stage] for stage in n228.REQUIRED_DESIGN_STAGES
        },
        design_path=design_paths[228],
    )
    paths["next228_design"] = paths.pop("design")
    paths.update(
        {
            "design": design_path,
            "next228_manifest": roots["next228"] / n228.MANIFEST_NAME,
            "next228_catalogue": roots["next228"] / n228.CATALOGUE_NAME,
            "next228_evaluation": roots["next228"] / n228.EVALUATION_NAME,
            "next228_formula": roots["next228"] / n228.FORMULA_NAME,
            "next228_search": roots["next228"] / n228.SEARCH_NAME,
        }
    )
    return paths
...
def _verify_next228(
    ...
    prior_paths["design"] = paths["next228_design"]
    prior_hashes = dict(input_hashes)
    prior_hashes["design"] = input_hashes["next228_design"]
    prior = n228._verify_next227(prior_paths, prior_hashes)
    manifest = json.loads(paths["next228_manifest"].read_text())
    catalogue = json.loads(paths["next228_catalogue"].read_text())
    evaluation = json.loads(paths["next228_evaluation"].read_text())
    formula = json.loads(paths["next228_formula"].read_text())
    table = pd.read_parquet(paths["next228_search"])
    selected = select_diagnostic_candidates(table)
    expected_outputs = {
        n228.CATALOGUE_NAME: input_hashes["next228_catalogue"],
        n228.EVALUATION_NAME: input_hashes["next228_evaluation"],
        n228.FORMULA_NAME: input_hashes["next228_formula"],
        n228.SEARCH_NAME: input_hashes["next228_search"],
    }
    if (
        manifest.get("protocol") != n228.PROTOCOL
        or manifest.get("candidate_count") != n228.EXPECTED_CANDIDATE_COUNT
        or manifest.get("eligible_new_candidate_count")
        != n228.EXPECTED_ELIGIBLE_COUNT
        or manifest.get("passes_all_cross_source_discovery_gates") is not False
        or manifest.get("next229_diagnostic_authorized") is not True
        or manifest.get("next229_candidate_count") != EXPECTED_CANDIDATE_COUNT
        or manifest.get("next229_candidate_key_sha256")
        != EXPECTED_CANDIDATE_KEY_SHA256
        ...
        != EXPECTED_NEXT228_SOURCE_SHA256
        or _sha256_file(Path(n228.__file__).resolve())
        != EXPECTED_NEXT228_SOURCE_SHA256
        or catalogue.get("candidate_count") != n228.EXPECTED_CANDIDATE_COUNT
        or catalogue.get("eligible_hypothesis_sha256")
        != n228.EXPECTED_ELIGIBLE_HYPOTHESIS_SHA256
        or evaluation.get("passes_all_cross_source_discovery_gates") is not False
        or evaluation.get("next229_candidate_count") != EXPECTED_CANDIDATE_COUNT
        or evaluation.get("next229_candidate_key_sha256")
        != EXPECTED_CANDIDATE_KEY_SHA256
        or formula.get("kind")
        != "triangular_margin_local_signed_x0_no_dft_score"
        or any(
        ...
        or len(table) != n228.EXPECTED_CANDIDATE_COUNT
        or len(selected) != EXPECTED_CANDIDATE_COUNT
        or candidate_key_sha256(selected) != EXPECTED_CANDIDATE_KEY_SHA256
    ):
        raise ValueError("NEXT229 NEXT228 provenance differs")
    return (*prior, selected)


def run_margin_local_broad_diagnostic(
    *,
    ...
    """Reproduce NEXT228 and compute its unchanged discovery BROAD residual."""

    if not set(REQUIRED_STAGES).issubset(stage_dirs):
        raise ValueError("NEXT229 stage directory universe differs")
    if not set(REQUIRED_DESIGN_STAGES).issubset(design_paths):
        raise ValueError("NEXT229 design path universe differs")
    roots = {
    ...
    if type(search_workers) is not int or search_workers <= 0:
        raise ValueError("NEXT229 search_workers must be a positive exact integer")
    if any(not path.is_file() for path in paths.values()):
        raise FileNotFoundError("NEXT229 input is missing")
    ...
    if require_formal_inputs and input_hashes != EXPECTED_INPUT_SHA256:
        ...
        raise ValueError(f"NEXT229 formal input identity differs: {differing}")
    (
        eligible_prior,
        eligible214,
        primary_key,
        ...
        eligible227,
        published,
    ) = _verify_next228(paths, input_hashes)
    combined, _, base_score, base_support, endpoint, _ = (
        n228.n227._reconstruct_next224_frontier(
            paths=paths,
    ...
    all_specs = n228.build_margin_local_candidate_specs(
    ...
    if len(specs) != EXPECTED_CANDIDATE_COUNT:
        raise ValueError("NEXT229 selected specification universe differs")
    virtual, terms, runtime, activity = n228.materialize_margin_local_candidates(
        features=combined,
        base_score=base_score,
        base_support=base_support,
        specs=specs,
    ...
    n164 = n228.n223.n222.n215.n214.n164
    ...
    for term in terms:
        ...
        if not np.array_equal(support, base_support):
            raise RuntimeError("NEXT229 candidate support differs from NEXT214")
        tables = n164._threshold_tables(
            score=score, supported=support, endpoint=endpoint, cells=cells
        )
        if tables is None:
            raise RuntimeError("NEXT229 candidate has no threshold table")
        ...
        if residual["passes_broad"]:
            raise RuntimeError("NEXT229 contradicts NEXT228 BROAD result")
    ...
    if (
        ...
        raise RuntimeError("NEXT229 residual population differs")
    closest = select_closest_residual(per_candidate)
    improves = n228.n223.n222.strictly_improves(
        closest,
        {
    ...
    summary = {
        "protocol": PROTOCOL,
        "diagnostic_mode": "offline_discovery_label_next228_broad_residual",
        "candidate_count": len(per_candidate),
        "candidate_key_sha256": EXPECTED_CANDIDATE_KEY_SHA256,
        "next228_record_population_reproduced": True,
        "next228_candidate_universe_reproduced": True,
        "global_closest": residual_record(closest),
        "next224_reference_failed_constraint_count": EXPECTED_BASE_FAILED_COUNT,
        "next224_reference_normalized_shortfall_sum": EXPECTED_BASE_SHORTFALL,
        "normalized_shortfall_reduction_from_next224": (
            EXPECTED_BASE_SHORTFALL
            - float(closest["normalized_shortfall_sum"])
        ),
        "strict_improvement_over_next224_diagnostic": improves,
        "failure_frequency": dict(sorted(frequency.items())),
        "margin_local_branch_closed": not improves,
    ...
    source_paths = {
        ...
        "src/next228_margin_local_redistribution_search.py": Path(
            n228.__file__
        ).resolve(),
        "src/next229_margin_local_broad_diagnostic.py": Path(__file__).resolve(),
    }
    source_hashes = {
    ...
    try:
        ...
        manifest = {
            ...
            "next228_record_population_reproduced": True,
            "next228_candidate_universe_reproduced": True,
            "next228_all_gate_candidate_count": 0,
        ...
        if any(
            ...
            raise RuntimeError("NEXT229 input changed before publication")
        if any(
            _sha256_file(path) != source_hashes[name]
            for name, path in source_paths.items()
        ):
            raise RuntimeError("NEXT229 source changed before publication")
...
def main() -> int:
    ...
    manifest = run_margin_local_broad_diagnostic(
