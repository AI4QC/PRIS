#!/usr/bin/env python3
"""Audit frozen PCSN hypotheses on physically isolated discovery outcomes."""
...
PROTOCOL = "2026-08-09-next320-pcsn-feature-audit-v1"
MANIFEST_NAME = "MANIFEST.json"
CATALOGUE_NAME = "NEXT320_PCSN_FEATURE_CATALOGUE.json"
AUDIT_NAME = "NEXT320_PCSN_FEATURE_AUDIT.json"
TABLE_NAME = "next320_pcsn_feature_audit.parquet"
HYPOTHESES = tuple((name, n319.FEATURE_DIRECTIONS[name]) for name in n319.FEATURE_NAMES)
QUANTILES = n268.QUANTILES
EXPECTED_NEXT268_SOURCE_SHA256 = (
    "36af0e632c8c3aae93f78271f837f693d49eff6ae88cdcad8edde42b3e0eaf64"
)
EXPECTED_NEXT319_SOURCE_SHA256 = (
    "cac92a55bef6991cc3a7e6fe650b3df79a7cebffedef7a9499bc77fe01745ed5"
)
NEXT319_SOURCE_PATH = "src/next319_periodic_contact_shell_neutralization.py"
REQUIRED_STAGES = (*n268.REQUIRED_STAGES, 319)
...
EXPECTED_INPUT_SHA256 = {
    **{
    ...
    "design": n319.EXPECTED_DESIGN_SHA256,
    "next319_manifest": "3ff64bbc460ff3a6c601f3a96558029d5a20219ce2745aba5f5f7fa129cae1b5",
    "next319_catalogue": "420100924397c67616390f98f5fada360a76ead7eb7effb28daed58a5bec8e3f",
    "next319_scigen": "f08c32e84ffaf08734ccc5bf1af1f527c344bbfa9de5b927f0494554ac3f5488",
    "next319_wyformer": "5416e90cc730ef1f84999a10df77b27750e0119ea6085ef857d9eadc2f72bb94",
}
...
def select_eligible_hypotheses(
    ...
    try:
        ...
        raise ValueError("NEXT320 audit ranking table differs") from exc
...
def bounded_protection(
    *, values: object, direction: str, q_lo: float, q_hi: float
    ...
    if direction != "protected_low":
        raise ValueError("NEXT320 bounded protection inputs differ")
    try:
        return n268.bounded_protection(
            values=values, direction=direction, q_lo=q_lo, q_hi=q_hi
        )
    except ValueError as exc:
        raise ValueError("NEXT320 bounded protection inputs differ") from exc


def _index_pcsn_by_prefixed_material_id(
    *, table: pd.DataFrame, source: str, expected_material_ids: object
    ...
    if set(extra["material_id"]) != expected:
        raise ValueError(f"NEXT320 {source} material identity differs")
...
def _paths(
    *,
    ...
    paths.update(
        {
            "design": design_path,
            "next319_manifest": roots["next319"] / n319.MANIFEST_NAME,
            "next319_catalogue": roots["next319"] / n319.CATALOGUE_NAME,
            "next319_scigen": roots["next319"] / n319.FEATURE_FILES["scigen"],
            "next319_wyformer": roots["next319"] / n319.FEATURE_FILES["wyformer"],
        }
    )
    return paths


def _verify_next319(paths: Mapping[str, Path], input_hashes: Mapping[str, str]):
    ...
    manifest = json.loads(paths["next319_manifest"].read_text())
    catalogue = json.loads(paths["next319_catalogue"].read_text())
    tables = {
        "scigen": pd.read_parquet(paths["next319_scigen"]),
        "wyformer": pd.read_parquet(paths["next319_wyformer"]),
    }
    expected_outputs = {
        n319.CATALOGUE_NAME: input_hashes["next319_catalogue"],
        n319.FEATURE_FILES["scigen"]: input_hashes["next319_scigen"],
        n319.FEATURE_FILES["wyformer"]: input_hashes["next319_wyformer"],
    }
    if (
        _sha256_file(Path(n268.__file__).resolve()) != EXPECTED_NEXT268_SOURCE_SHA256
        or _sha256_file(Path(n319.__file__).resolve()) != EXPECTED_NEXT319_SOURCE_SHA256
        or manifest.get("protocol") != n319.PROTOCOL
        or manifest.get("next320_audit_authorized") is not True
        ...
        or manifest.get("executed_source_sha256", {}).get(NEXT319_SOURCE_PATH)
        != EXPECTED_NEXT319_SOURCE_SHA256
        or any(manifest.get(key) is not value for key, value in BOUNDARY_FLAGS.items())
        or catalogue.get("feature_names") != list(n319.FEATURE_NAMES)
        or catalogue.get("feature_directions") != n319.FEATURE_DIRECTIONS
        or catalogue.get("feature_count") != len(n319.FEATURE_NAMES)
        or catalogue.get("graph")
        != "NEXT279 reciprocal translated active-facet infinite periodic cover"
        or catalogue.get("shell_depths") != [1, 2]
        or catalogue.get("maximum_cover_vertices_per_root")
        != n319.MAX_COVER_VERTICES_PER_ROOT
        or catalogue.get("residual")
        != "abs(sum(q over B_h(i))) / sum(abs(q over B_h(i)))"
        or catalogue.get("minimum_formal_coverage") != n319.MINIMUM_FORMAL_COVERAGE
        or catalogue.get("quantile_method") != "inverted_cdf"
        or catalogue.get("directions_frozen_before_outcome") is not True
    ):
        raise ValueError("NEXT320 NEXT319 provenance differs")
    for source, expected_rows in n319.EXPECTED_ROWS.items():
        table = tables[source]
        supported = table["pcsn_supported"].fillna(False).astype(bool)
        values = np.column_stack(
            [
                pd.to_numeric(table[name], errors="coerce").to_numpy(float)
                for name in n319.FEATURE_NAMES
            ]
        )
        finite = np.isfinite(values)
        sites = pd.to_numeric(table["pcsn_site_count"], errors="coerce")
        contacts = pd.to_numeric(
            table["pcsn_directed_contact_count"], errors="coerce"
        )
        unique_contacts = pd.to_numeric(
            table["pcsn_unique_contact_count"], errors="coerce"
        )
        shell1_min = pd.to_numeric(
            table["pcsn_minimum_shell1_population"], errors="coerce"
        )
        shell1_max = pd.to_numeric(
            table["pcsn_maximum_shell1_population"], errors="coerce"
        )
        shell2_min = pd.to_numeric(
            table["pcsn_minimum_shell2_population"], errors="coerce"
        )
        shell2_max = pd.to_numeric(
            table["pcsn_maximum_shell2_population"], errors="coerce"
        )
        policy = table["pcsn_valence_policy"]
        if (
            ...
            or float(supported.mean()) < n319.MINIMUM_FORMAL_COVERAGE
            ...
            or not (unique_contacts[supported] > 0).all()
            or not (unique_contacts[supported] <= contacts[supported]).all()
            or not (shell1_min[supported] >= 2).all()
            or not (shell1_max[supported] >= shell1_min[supported]).all()
            or not (shell2_min[supported] > shell1_min[supported]).all()
            or not (shell2_max[supported] >= shell2_min[supported]).all()
            or not (shell2_max[supported] <= n319.MAX_COVER_VERTICES_PER_ROOT).all()
            or policy[supported].isna().any()
            or not (sites[~supported] == 0).all()
            or not (contacts[~supported] == 0).all()
            or not (unique_contacts[~supported] == 0).all()
            or not (shell1_min[~supported] == 0).all()
            or not (shell1_max[~supported] == 0).all()
            or not (shell2_min[~supported] == 0).all()
            or not (shell2_max[~supported] == 0).all()
        ):
            raise ValueError(f"NEXT320 NEXT319 {source} table differs")
    return (*prior[:-1], tables)


def run_pcsn_feature_audit(
    *,
    ...
    """Audit exactly the two frozen PCSN hypotheses."""

    if not set(REQUIRED_STAGES).issubset(stage_dirs):
        raise ValueError("NEXT320 stage directory universe differs")
    if not set(REQUIRED_DESIGN_STAGES).issubset(design_paths):
        raise ValueError("NEXT320 design path universe differs")
    roots = {
    ...
    if any(not path.is_file() for path in paths.values()):
        raise FileNotFoundError("NEXT320 input is missing")
    ...
    if require_formal_inputs and input_hashes != EXPECTED_INPUT_SHA256:
        ...
        raise ValueError(f"NEXT320 formal input identity differs: {differing}")
    (
        eligible,
        eligible214,
        primary_key,
        ...
        formula222,
        pcsn_tables,
    ) = _verify_next319(paths, input_hashes)
    n227 = n268.n227
    ...
    if cohort_counts != n227.EXPECTED_COHORT_COUNTS:
        raise ValueError("NEXT320 rejected cohort counts differ")
    ...
    for feature, direction in HYPOTHESES:
        ...
        if not len(finite):
            raise ValueError(f"NEXT320 {feature} has no finite discovery values")
    ...
    catalogue = {
        ...
        "features": list(n319.FEATURE_NAMES),
        "feature_directions": n319.FEATURE_DIRECTIONS,
    ...
    audit = {
        "protocol": PROTOCOL,
        "audit_mode": "fixed_next224_rejected_extreme_pcsn_audit",
        "base_candidate_key_sha256": n227.EXPECTED_BASE_CANDIDATE_KEY_SHA256,
        "base_threshold": n227.EXPECTED_BASE_THRESHOLD,
        "cohort_counts": {
        ...
        "next321_search_authorized": bool(selected is not None),
    ...
    source_paths = {
        ...
        NEXT319_SOURCE_PATH: Path(n319.__file__).resolve(),
        "src/next320_pcsn_feature_audit.py": Path(__file__).resolve(),
    }
    ...
    try:
        ...
        manifest = {
            ...
            "next321_search_authorized": bool(selected is not None),
            "pcsn_branch_terminated": selected is None,
            "new_formula_searched": False,
            "new_formula_selected": False,
        ...
        if any(_sha256_file(path) != input_hashes[name] for name, path in paths.items()):
            raise RuntimeError("NEXT320 input changed before publication")
        if any(_sha256_file(path) != source_hashes[name] for name, path in source_paths.items()):
            raise RuntimeError("NEXT320 source changed before publication")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    manifest = run_pcsn_feature_audit(
