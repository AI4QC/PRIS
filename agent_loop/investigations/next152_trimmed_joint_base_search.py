#!/usr/bin/env python3
"""Frozen search over bases trimmed by their two largest risk contributions."""
...
PROTOCOL = "2026-08-08-next152-trimmed-joint-base-search-v1"
MANIFEST_NAME = "MANIFEST.json"
CATALOGUE_NAME = "NEXT152_TRIMMED_JOINT_BASE_CATALOGUE.json"
EVALUATION_NAME = "NEXT152_DISCOVERY_EVALUATION.json"
SEARCH_NAME = "next152_trimmed_joint_base_candidate_search.parquet"
EXPECTED_DESIGN_SHA256 = "514794d6c2e65a2f12c5348fe30b79461b8db48375bab765c2ade659fa982160"
...
EXPECTED_CANDIDATE_KEY_SHA256 = "6433e8f2b1fab1235a37ecbe53011a466eb26d6d8bd458578fa36729b98fc058"
AGGREGATION = "sum_minus_top2"
...
EXPECTED_INPUT_SHA256 = {
    **n135.EXPECTED_INPUT_SHA256,
    ...
    "next151_manifest": "3f38157b8a20416c7ae1513c0dfbefd498f4f3cc20ebfdc67eae9e417294ab64",
    "next151_audit": "b1d272774c61d3b6a46a7096c79d800267552b2eca81ffe1dada497feebe696d",
    "next151_table": "03a38157176d1aabe3c2cdd55f07e4f3c8fcfb1a95287dddc2d8883f383969fc",
}


def sum_minus_top2(contributions: object) -> np.ndarray:
    """Return the nonnegative row sum after removing its two largest terms."""
    ...
    if (
        ...
        raise ValueError("NEXT152 contribution matrix differs")
    values = np.maximum(values, 0.0)
    largest = np.partition(values, kth=values.shape[1] - 2, axis=1)[:, -2:]
    return np.maximum(0.0, values.sum(axis=1) - largest.sum(axis=1))


def compose_trimmed_joint_score(
    *,
    ...
    """Trim a physical base, then subtract active bounded protections."""
    ...
    if (
        ...
        raise ValueError("NEXT152 score arrays differ")
    ...
    if (
        ...
        raise ValueError("NEXT152 supported score values differ")
    score = np.full(len(values), np.nan, dtype=float)
    if supported.any():
        score[supported] = sum_minus_top2(values[supported])
...
def build_candidate_specs(
    *, bases: pd.DataFrame, physical_term_ids: set[str]
    ...
    """Enumerate the frozen 11 x 4 x 4 trimmed-score universe."""

    if {"term_ids_json", "weights_json"} - set(bases.columns):
        raise ValueError("NEXT152 base schema differs")
    ...
    for _, row in bases.iterrows():
        ...
        if (
            ...
            raise ValueError("NEXT152 physical base formula differs")
...
def materialize_candidates(
    *,
    ...
    """Encode each exact trimmed physical score as one evaluator-compatible term."""

    physical_by_id = {str(term["term_id"]): dict(term) for term in physical_terms}
    if len(physical_by_id) != len(physical_terms):
        raise ValueError("NEXT152 physical term identities are duplicated")
    if {
        ...
        raise ValueError("NEXT152 protection feature schema differs")
    ...
    for raw_spec in specs:
        ...
        if (
            ...
            raise ValueError("NEXT152 candidate materialization differs")
        seen.add(key)
        ...
        if not np.isfinite(encoded[supported]).all():
            raise ValueError("NEXT152 virtual score encoding overflowed")
        digest = hashlib.sha256(key.encode()).hexdigest()[:24]
        virtual_id = f"next152_virtual_candidate__{digest}"
        ...
        virtual_terms.append(
            {
                ...
                "center": 0.0,
                "scale": 1.0 / divisor,
                "group": "next152_trimmed_joint_base_candidate",
                "encoding": "asinh_sinh_exact_trimmed_joint_score",
...
def _paths(
    ...
    paths.update(
        {
            "design": design_path,
            "next151_manifest": roots["next151"] / n151.MANIFEST_NAME,
            "next151_audit": roots["next151"] / n151.AUDIT_NAME,
            "next151_table": roots["next151"] / n151.TABLE_NAME,
        }
    )
    return paths


def run_trimmed_joint_base_search(
    *,
    ...
    if any(not path.is_file() for path in paths.values()):
        raise FileNotFoundError("NEXT152 input is missing")
    ...
    if require_formal_inputs and input_hashes != EXPECTED_INPUT_SHA256:
        ...
        raise ValueError(f"NEXT152 formal input identity differs: {differing}")
    manifest151 = json.loads(paths["next151_manifest"].read_text())
    audit151 = json.loads(paths["next151_audit"].read_text())
    if (
        manifest151.get("protocol") != n151.PROTOCOL
        or manifest151.get("eligible_aggregation_count") != 1
        or manifest151.get("opened_validation_outputs_used") is not False
        or manifest151.get("dft_values_used_by_executable_formula") is not False
        or manifest151.get("outputs_sha256", {}).get(n151.AUDIT_NAME)
        != input_hashes["next151_audit"]
        or audit151.get("eligible_aggregations") != [AGGREGATION]
        or audit151.get("selected_aggregation", {}).get("aggregation") != AGGREGATION
        or audit151.get("validation_or_replication_opened") is not False
        or audit151.get("dft_values_used_by_executable_formula") is not False
    ):
        raise ValueError("NEXT152 audit provenance differs")
    ...
    if require_formal_inputs and (
        ...
        raise ValueError("NEXT152 frozen candidate universe differs")
    ...
    def decorate(record: dict[str, object]) -> None:
        ...
        record["score_composition"] = (
            "max(0,sum_minus_top2(weighted_base_contributions)"
    ...
    formula["kind"] = "trimmed_joint_physical_base_with_bounded_protections"
    ...
    catalogue = {
        ...
        "next151_selected_aggregation": AGGREGATION,
    ...
    source_paths = {
        ...
        "src/next152_trimmed_joint_base_search.py": Path(__file__).resolve(),
    }
    source_hashes = {
    ...
    try:
        ...
        _write_json(
            ...
            {
                "protocol": PROTOCOL,
                "evaluation_mode": "fixed_trimmed_joint_base_search",
                "rows": {
        ...
        manifest = {
            ...
            "freeze_authorized": passes,
            "requires_unopened_internal_validation_before_claim": True,
            "trimmed_joint_base_branch_terminated": not passes,
        ...
        if any(_sha256_file(path) != input_hashes[name] for name, path in paths.items()):
            raise RuntimeError("NEXT152 input changed before publication")
        if any(
            _sha256_file(path) != source_hashes[name]
            for name, path in source_paths.items()
        ):
            raise RuntimeError("NEXT152 source changed before publication")
...
def main() -> int:
    ...
    manifest = run_trimmed_joint_base_search(
