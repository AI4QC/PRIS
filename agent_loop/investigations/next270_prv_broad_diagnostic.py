#!/usr/bin/env python3
"""Diagnose the frozen NEXT269 PRV BROAD residual."""
...
PROTOCOL = "2026-08-09-next270-prv-broad-diagnostic-v1"
MANIFEST_NAME = "MANIFEST.json"
DIAGNOSTIC_NAME = "NEXT270_PRV_BROAD_DIAGNOSTIC.json"
TABLE_NAME = "next270_prv_broad_diagnostic.parquet"
EXPECTED_DESIGN_SHA256 = n269.EXPECTED_DESIGN_SHA256
...
EXPECTED_NEXT269_SEARCH_SHA256 = (
    "a1c5cefb76f9a44261502ed845bf6e9f86997592a3d52128e9af4d6ca0e4259b"
)
...
REQUIRED_STAGES = (*n269.REQUIRED_STAGES, 269)
REQUIRED_DESIGN_STAGES = (*n269.REQUIRED_DESIGN_STAGES, 269)
EXPECTED_INPUT_SHA256 = {
    **{
        key: value
        for key, value in n269.EXPECTED_INPUT_SHA256.items()
        if key != "design"
    },
    "next269_design": n269.EXPECTED_INPUT_SHA256["design"],
    "design": EXPECTED_DESIGN_SHA256,
    "next269_manifest": (
        "5b6658c52d6693da51bb60d1c174368a5a98a563a16ea6bb625a3f72cf747a41"
    ),
    "next269_catalogue": (
        "e41ef78bbd727e3eb478c1c42e75f5bbd8ed62066017c1e373f361c007c169bb"
    ),
    "next269_evaluation": (
        "bd0719aba919fa2a355764caa187ec67033d85bba9e6e6e88554d576f216c6f4"
    ),
    "next269_formula": (
        "a375ba34ff3a268c0c7d7b4a134cd97bcb93d7402638603841e59d76b6d50f49"
    ),
    "next269_search": EXPECTED_NEXT269_SEARCH_SHA256,
}
...
def select_diagnostic_candidates(published: pd.DataFrame) -> pd.DataFrame:
    try:
        ...
        raise ValueError("NEXT270 published candidate columns differ") from exc
...
def candidate_key_sha256(frame: pd.DataFrame) -> str:
    try:
        ...
        raise ValueError("NEXT270 candidate identity table differs") from exc
...
def select_closest_residual(frame: pd.DataFrame) -> pd.Series:
    try:
        ...
        raise ValueError("NEXT270 residual population differs") from exc
...
def _paths(
    *,
    ...
    paths = n269._paths(
        ...
        design_paths={
            stage: design_paths[stage] for stage in n269.REQUIRED_DESIGN_STAGES
        },
        design_path=design_paths[267],
    )
    paths["next269_design"] = paths.pop("design")
    paths.update(
        {
            "design": design_path,
            "next269_manifest": roots["next269"] / n269.MANIFEST_NAME,
            "next269_catalogue": roots["next269"] / n269.CATALOGUE_NAME,
            "next269_evaluation": roots["next269"] / n269.EVALUATION_NAME,
            "next269_formula": roots["next269"] / n269.FORMULA_NAME,
            "next269_search": roots["next269"] / n269.SEARCH_NAME,
        }
    )
    return paths
...
def _verify_next269(
    ...
    prior_paths = {key: paths[key] for key in n269.EXPECTED_INPUT_SHA256}
    prior_paths["design"] = paths["next269_design"]
    prior_hashes = {key: input_hashes[key] for key in n269.EXPECTED_INPUT_SHA256}
    prior_hashes["design"] = input_hashes["next269_design"]
    prior = n269._verify_next268(prior_paths, prior_hashes)
    manifest = json.loads(paths["next269_manifest"].read_text())
    catalogue = json.loads(paths["next269_catalogue"].read_text())
    evaluation = json.loads(paths["next269_evaluation"].read_text())
    formula = json.loads(paths["next269_formula"].read_text())
    table = pd.read_parquet(paths["next269_search"])
    selected = select_diagnostic_candidates(table)
    expected_outputs = {
        n269.CATALOGUE_NAME: input_hashes["next269_catalogue"],
        n269.EVALUATION_NAME: input_hashes["next269_evaluation"],
        n269.FORMULA_NAME: input_hashes["next269_formula"],
        n269.SEARCH_NAME: input_hashes["next269_search"],
    }
    if (
        manifest.get("protocol") != n269.PROTOCOL
        or manifest.get("candidate_count") != n269.EXPECTED_CANDIDATE_COUNT
        or manifest.get("eligible_new_candidate_count")
        != n269.EXPECTED_ELIGIBLE_COUNT
        or manifest.get("passes_all_cross_source_discovery_gates") is not False
        or manifest.get("next270_diagnostic_authorized") is not True
        or manifest.get("next270_candidate_count") != EXPECTED_CANDIDATE_COUNT
        or manifest.get("next270_candidate_key_sha256")
        != EXPECTED_CANDIDATE_KEY_SHA256
        or manifest.get("outputs_sha256") != expected_outputs
        or manifest.get("executed_source_sha256", {}).get(
            "src/next269_prv_margin_local_search.py"
        )
        != EXPECTED_NEXT269_SOURCE_SHA256
        or _sha256_file(Path(n269.__file__).resolve())
        != EXPECTED_NEXT269_SOURCE_SHA256
        or _sha256_file(Path(n262.__file__).resolve())
        != EXPECTED_NEXT262_SOURCE_SHA256
        or any(manifest.get(key) is not value for key, value in BOUNDARY_FLAGS.items())
        or catalogue.get("candidate_count") != n269.EXPECTED_CANDIDATE_COUNT
        or evaluation.get("passes_all_cross_source_discovery_gates") is not False
        or evaluation.get("next270_candidate_count") != EXPECTED_CANDIDATE_COUNT
        or evaluation.get("next270_candidate_key_sha256")
        != EXPECTED_CANDIDATE_KEY_SHA256
        or formula.get("kind") != "prv_triangular_margin_local_x0_no_dft_score"
...
def run_prv_broad_diagnostic(
    *,
    ...
    """Reproduce NEXT269 and compute its unchanged discovery BROAD residual."""

    if not set(REQUIRED_STAGES).issubset(stage_dirs):
        raise ValueError("NEXT270 stage directory universe differs")
    if not set(REQUIRED_DESIGN_STAGES).issubset(design_paths):
        raise ValueError("NEXT270 design path universe differs")
    roots = {
    ...
    if type(search_workers) is not int or search_workers <= 0:
        raise ValueError("NEXT270 search_workers must be a positive exact integer")
    if any(not path.is_file() for path in paths.values()):
        raise FileNotFoundError("NEXT270 input is missing")
    ...
    if require_formal_inputs and input_hashes != EXPECTED_INPUT_SHA256:
        ...
        raise ValueError(f"NEXT270 formal input identity differs: {differing}")
    (
        eligible_prior,
        eligible214,
        primary_key,
        ...
        eligible268,
        published269,
    ) = _verify_next269(paths, input_hashes)
    combined, feature_tables, base_score, base_support, endpoint, _ = (
        n269.n227._reconstruct_next224_frontier(
            paths=paths,
    ...
    all_specs = n269.build_prv_candidate_specs(
        base_candidate_key=base_key,
        eligible_table=eligible268,
    )
    keys = set(published269["candidate_key"].astype(str))
    specs = [spec for spec in all_specs if str(spec["candidate_key"]) in keys]
    if len(specs) != EXPECTED_CANDIDATE_COUNT:
        raise ValueError("NEXT270 selected specification universe differs")
    virtual, terms, runtime, activity = n269.materialize_prv_candidates(
        features=combined,
    ...
    evaluator = (
        n269.n223.n222.n215.n214.n212.n210.n208.n205.n203.n202.n200.n194.n130.n125
    ...
    n164 = n269.n223.n222.n215.n214.n164
    n164._verify_reproduction(
        rerun=rerun["candidate_records"], published=published269
    )
    ...
    published_by_key = published269.set_index("candidate_key", drop=False)
    ...
    for term in terms:
        ...
        if not np.array_equal(support, base_support):
            raise RuntimeError("NEXT270 candidate support differs from NEXT214")
        ...
        if tables is None:
            raise RuntimeError("NEXT270 candidate has no threshold table")
        ...
        if residual["passes_broad"]:
            raise RuntimeError("NEXT270 contradicts NEXT269 BROAD result")
    ...
    if (
        ...
        raise RuntimeError("NEXT270 residual population differs")
    closest = select_closest_residual(per_candidate)
    improves = n269.n223.n222.strictly_improves(
        closest,
        {
    ...
    summary = {
        "protocol": PROTOCOL,
        "diagnostic_mode": "offline_discovery_label_next269_broad_residual",
        "candidate_count": len(per_candidate),
        "candidate_key_sha256": EXPECTED_CANDIDATE_KEY_SHA256,
        "next269_record_population_reproduced": True,
        "next269_candidate_universe_reproduced": True,
        ...
        "prv_certificate_branch_closed": bool(not improves),
    ...
    source_paths = {
        ...
        "src/next270_prv_broad_diagnostic.py": Path(__file__).resolve(),
    }
    ...
    try:
        ...
        manifest = {
            ...
            "next269_record_population_reproduced": True,
            "next269_candidate_universe_reproduced": True,
            "next269_all_gate_candidate_count": 0,
        ...
        if any(_sha256_file(path) != input_hashes[name] for name, path in paths.items()):
            raise RuntimeError("NEXT270 input changed before publication")
        if any(_sha256_file(path) != source_hashes[name] for name, path in source_paths.items()):
            raise RuntimeError("NEXT270 source changed before publication")
...
def main() -> int:
    ...
    manifest = run_prv_broad_diagnostic(
