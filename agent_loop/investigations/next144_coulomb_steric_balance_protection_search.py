#!/usr/bin/env python3
"""Frozen Coulomb--steric balance protection search on coordination-weight-2 bases."""
...
PROTOCOL = "2026-08-08-next144-coulomb-steric-balance-protection-search-v1"
MANIFEST_NAME = "MANIFEST.json"
CATALOGUE_NAME = "NEXT144_COULOMB_STERIC_BALANCE_PROTECTION_SEARCH_CATALOGUE.json"
EVALUATION_NAME = "NEXT144_DISCOVERY_EVALUATION.json"
SEARCH_NAME = "next144_coulomb_steric_balance_protection_candidate_search.parquet"
PROTECTION_TERM_ID = "coulomb_steric_balance_protection__high"
WEIGHTS = (0.0, 0.1, 0.25, 0.5, 1.0, 2.0, 4.0, 8.0)
SEARCH_WORKERS = 4
EXPECTED_FREEZE_SHA256 = "1dce88337445534c93a48a839acf04bea5b3ff9d5cbb7b697b6f8c784d61efe7"
...
EXPECTED_CANDIDATE_KEY_SHA256 = "e80196eb8d646fd86c97c3f5a075d6e8acff5c8e8918d2ecdd361029050236ec"
...
def apply_coulomb_steric_balance_protection(
    *,
    base_score: object,
    ...
    if supported.shape != score.shape or values.shape != score.shape or active.shape != score.shape:
        raise ValueError("NEXT144 protection arrays differ")
    if weight not in WEIGHTS or np.any(active & (~np.isfinite(values) | (values < 0.0))):
        raise ValueError("NEXT144 protection values differ")
    ...
    *, bases: pd.DataFrame, physical_term_ids: set[str]
    ...
    for _, row in bases.iterrows():
        ...
        if not base_ids or len(base_ids) != len(base_weights) or any(term_id not in physical_term_ids for term_id in base_ids):
            raise ValueError("NEXT144 base formula differs")
        for weight in WEIGHTS:
            payload = {
                ...
                "coulomb_steric_balance_protection_term_id": None if weight == 0.0 else PROTECTION_TERM_ID,
                "coulomb_steric_balance_protection_weight": float(weight),
            }
...
def materialize_candidates(
    *,
    ...
    if required - set(features.columns):
        raise ValueError("NEXT144 protection feature schema differs")
    protection = pd.to_numeric(features[n143.FEATURE_NAME], errors="coerce").to_numpy(float)
    active = features[n143.SUPPORT_COLUMN].eq(True).to_numpy()
    if np.any(active & (~np.isfinite(protection) | (protection < 0.0) | (protection > 1.0 + 1e-12))):
        raise ValueError("NEXT144 protection feature values differ")
    ...
    for spec_raw in specs:
        ...
        weight = float(spec["coulomb_steric_balance_protection_weight"])
        expected_term = None if weight == 0.0 else PROTECTION_TERM_ID
        if base_id is None or spec["coulomb_steric_balance_protection_term_id"] != expected_term:
            raise ValueError("NEXT144 candidate configuration differs")
        score, supported = apply_coulomb_steric_balance_protection(
        ...
        virtual_id = f"next144_virtual_candidate__{hashlib.sha256(key.encode()).hexdigest()[:24]}"
        ...
        terms.append(
            {
                ...
                "center": 0.0,
                "scale": 1.0 / divisor,
                "group": "next144_coulomb_steric_balance_protected_candidate",
...
def verify_base_reproduction(
    *, result_records: Sequence[Mapping[str, object]], prior: pd.DataFrame
    ...
    for record in result_records:
        ...
        if float(payload["coulomb_steric_balance_protection_weight"]) != 0.0:
            continue
    ...
    if set(observed) != set(expected):
        raise RuntimeError("NEXT144 base reproduction identities differ")
    for identity, source in expected.items():
        ...
        if any(not math.isclose(float(record[name]), float(source[name]), rel_tol=0.0, abs_tol=n130.BASE_REPRODUCTION_AUC_TOLERANCE) for name in metrics) or any(bool(record[name]) != bool(source[name]) for name in ("passes_source_auc_gates", "passes_safe_all_cells")) or int(record["safe_passing_cells"]) != int(source["safe_passing_cells"]):
            raise RuntimeError("NEXT144 base diagnostics do not reproduce NEXT130")
...
def run_coulomb_steric_balance_protection_search(
    *,
    ...
    next142_dir: Path, next143_dir: Path,
    freeze_path: Path,
    output_dir: Path,
    ...
    roots = {
        ...
        **{f"next{stage}": Path(value).resolve() for stage, value in (
            ...
            (142,next142_dir),(143,next143_dir),
        )},
    }
    ...
    if any(not path.is_file() for path in paths.values()):
        raise FileNotFoundError("NEXT144 input is missing")
    ...
    if require_formal_inputs and input_hashes != EXPECTED_INPUT_SHA256:
        ...
        raise ValueError(f"NEXT144 formal input identity differs: {differing}")
    ...
    if (
        ...
        raise ValueError("NEXT144 prior provenance differs")
    ...
    if len(extended) != len(protection):
        raise ValueError("NEXT144 protection row accounting differs")
    ...
    if require_formal_inputs and (
        ...
        raise ValueError("NEXT144 frozen candidate universe differs")
    ...
    def decorate(record: dict[str, object]) -> None:
        ...
        record["coulomb_steric_balance_protection_term_id"] = payload["coulomb_steric_balance_protection_term_id"]
        record["coulomb_steric_balance_protection_weight"] = float(payload["coulomb_steric_balance_protection_weight"])
        record["score_composition"] = "max(0,coordination_weight2_base-coulomb_steric_balance_weight*coulomb_steric_balance_protection)"
    ...
    formula["coulomb_steric_balance_protection"] = None if float(payload["coulomb_steric_balance_protection_weight"]) == 0.0 else {"term_id":PROTECTION_TERM_ID,"feature":n143.FEATURE_NAME,"weight":float(payload["coulomb_steric_balance_protection_weight"]),"missing_policy":"TERM_OFF_KEEP_BASE"}
    formula["score_composition"] = "max(0,coordination_weight2_base-coulomb_steric_balance_weight*coulomb_steric_balance_protection)"
    formula["kind"] = "next130_coordination_base_with_optional_coulomb_steric_balance_protection"
    passes = bool(selected["record"]["passes_all_discovery_gates"])
    records_frame = pd.DataFrame(result["candidate_records"])
    counts = {f"{float(weight):g}":{"candidates":int(len(frame)),"passes_source_auc_gates":int(frame["passes_source_auc_gates"].sum()),"passes_safe_all_cells":int(frame["passes_safe_all_cells"].sum()),"passes_broad_all_cells":int(frame["passes_broad_all_cells"].sum()),"passes_all_discovery_gates":int(frame["passes_all_discovery_gates"].sum())} for weight,frame in records_frame.groupby("coulomb_steric_balance_protection_weight",sort=True)}
    catalogue = {"protocol":PROTOCOL,"freeze_sha256":input_hashes["freeze"],"base_count":len(bases),"candidate_count":len(specs),"weight_grid":list(WEIGHTS),"weight_grid_sha256":weight_sha,"candidate_key_sha256":candidate_sha,"protection_term_id":PROTECTION_TERM_ID,"protection_feature":n143.FEATURE_NAME,"active_score":"max(0,coordination_weight2_base-coulomb_steric_balance_weight*coulomb_steric_balance_protection)","validation_outputs_opened":False,"dft_values_used_by_executable_formula":False,"analytic_coulomb_steric_vectors_used":True}
    catalogue_sha = hashlib.sha256(json.dumps(catalogue,indent=2,sort_keys=True).encode()+b"\n").hexdigest()
    target.parent.mkdir(parents=True, exist_ok=True)
    staging = Path(tempfile.mkdtemp(prefix=f".{target.name}.staging-", dir=target.parent))
    source_paths = {"src/next130_coordination_protection_search.py":Path(n130.__file__).resolve(),"src/next134_compactness_protection_search.py":Path(n134.__file__).resolve(),"src/next144_coulomb_steric_balance_protection_search.py":Path(__file__).resolve()}
    ...
    try:
        ...
        if any(_sha256_file(path)!=input_hashes[name] for name,path in paths.items()): raise RuntimeError("NEXT144 input changed before publication")
        if any(_sha256_file(path)!=source_hashes[name] for name,path in source_paths.items()): raise RuntimeError("NEXT144 source changed before publication")
...
def main() -> int:
    ...
    for stage in (98,110,111,113,114,116,117,120,121,122,124,125,129,130,133,134,142,143): parser.add_argument(f"--next{stage}-dir",type=Path,required=True)
...
__all__=["apply_coulomb_steric_balance_protection","build_candidate_specs","materialize_candidates","run_coulomb_steric_balance_protection_search"]
