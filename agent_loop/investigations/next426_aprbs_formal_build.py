#!/usr/bin/env python3
"""Physically isolated full discovery build for frozen NEXT425 APRBS."""
...
import src.next425_apriori_bond_strength_length_order as n425


PROTOCOL = "2026-08-13-next426-aprbs-formal-build-v1"
MANIFEST_NAME = "MANIFEST.json"
CATALOGUE_NAME = "NEXT426_APRBS_FEATURE_CATALOGUE.json"
FEATURE_FILES = {
    "scigen": "next426_scigen_apriori_bond_strength_length_order.parquet",
    "wyformer": "next426_wyformer_apriori_bond_strength_length_order.parquet",
}
EXPECTED_ROWS = {"scigen": 13_470, "wyformer": 5_232}
MINIMUM_FORMAL_COVERAGE = 0.95
EXPECTED_PROBE_SHA256 = "41ec9da48c1f2b0385999b406b4d93c8e7ad3e2948b3d39812cd20d2fa4725d5"
EXPECTED_INPUT_SHA256 = {
    "design": "384327f7cdcbb9d05a38834a20d89f025040297fc8eba986fdc13e43cd058944",
    "probe_result": EXPECTED_PROBE_SHA256,
    "next267_source": "8f1e7ed9eb73a81a5755d455ffc05aab6f539cbd66afbbbfc384ca88391adca1",
    "next425_source": "a03090f869434cf8191c733012cfea576714d4a8bc64f536cbd28dbc57eeecd3",
...
def validate_probe_authorization(probe: dict[str, object]) -> None:
    ...
    if (
        probe.get("protocol") != "2026-08-13-next425-aprbs-label-blind-probe-v1"
        or probe.get("design_sha256") != n425.DESIGN_SHA256
        or probe.get("minimum_novelty_joint_finite") != 40
        or probe.get("probe_passed") is not True
        or probe.get("next426_formal_build_authorized") is not True
        or not isinstance(gates, dict)
        or set(gates.values()) != {True}
        or any(probe.get(name) is not False for name in PROBE_BOUNDARY_NAMES)
    ):
        raise ValueError("NEXT426 label-blind probe authorization differs")


def _error_row(exc: Exception) -> dict[str, object]:
    return {
        n425.FEATURE_NAMES[0]: math.nan,
        "aprbs_supported": False,
        "aprbs_failure": f"{type(exc).__name__}: {exc}",
        "aprbs_site_count": 0,
        "aprbs_edge_count": 0,
        "aprbs_pair_count": 0,
        "aprbs_informative_weight": math.nan,
        "aprbs_violation_weight": math.nan,
        "aprbs_maximum_marginal_residual": math.nan,
        "aprbs_iterations": 0,
        "aprbs_valence_policy": None,
    }
...
def _compute_scigen_payload(item):
    ...
    try:
        ...
        return material_id, n425.compute_aprbs_row(atoms)
...
def label_free_statistics(table: pd.DataFrame) -> dict[str, object]:
    values = pd.to_numeric(
        table[n425.FEATURE_NAMES[0]], errors="coerce"
    ).dropna().to_numpy(float)
    if len(values) < 1:
        raise RuntimeError("NEXT426 label-free feature population is empty")
    return {
        n425.FEATURE_NAMES[0]: {
...
def build_cross_source_aprbs_features(
    *, scigen_cohort_dir: Path, wyformer_cohort_dir: Path, design_path: Path,
    ...
    paths = {
        ...
        "next425_source": Path(n425.__file__).resolve(),
    }
    if os.path.lexists(target):
        raise FileExistsError(str(target))
    if type(workers) is not int or workers <= 0:
        raise ValueError("NEXT426 workers must be a positive exact integer")
    if any(not path.is_file() for path in paths.values()):
        raise FileNotFoundError("NEXT426 input is missing")
    ...
    if require_formal_inputs and input_hashes != EXPECTED_INPUT_SHA256:
        ...
        raise ValueError(f"NEXT426 formal input identity differs: {differing}")
    ...
    if (
        ...
        raise ValueError("NEXT426 discovery geometry provenance differs")
    metadata = {
    ...
    for source, frame in metadata.items():
        if required - set(frame) or frame["material_id"].astype(str).duplicated().any():
            raise ValueError(f"NEXT426 {source} metadata differs")
        ...
        if len(selected) != EXPECTED_ROWS[source]:
            raise ValueError(f"NEXT426 {source} discovery identity differs")
    ...
    executed_paths = {
        "src/next426_aprbs_formal_build.py": Path(__file__).resolve(),
        "src/next425_apriori_bond_strength_length_order.py": Path(n425.__file__).resolve(),
        "experiments/next425_aprbs_label_blind_probe.py": root / "experiments/next425_aprbs_label_blind_probe.py",
        "tests/test_next425_apriori_bond_strength_length_order.py": root / "tests/test_next425_apriori_bond_strength_length_order.py",
        "tests/test_next425_aprbs_label_blind_probe.py": root / "tests/test_next425_aprbs_label_blind_probe.py",
        "tests/test_next426_aprbs_formal_build.py": root / "tests/test_next426_aprbs_formal_build.py",
    }
    executed_hashes = {
    ...
    try:
        ...
        for source in ("scigen", "wyformer"):
            ...
            if (
                ...
                raise RuntimeError(f"NEXT426 {source} material identity differs")
            table = discovery[source].merge(
                computed_frame, on="material_id", how="left", validate="one_to_one"
            )
            supported = table["aprbs_supported"].fillna(False).astype(bool)
            finite = np.isfinite(
                pd.to_numeric(table[n425.FEATURE_NAMES[0]], errors="coerce").to_numpy(float)
            )
            if not finite[supported].all() or finite[~supported].any():
                raise RuntimeError(f"NEXT426 {source} finite support semantics differ")
            coverage = float(supported.mean())
            if coverage < MINIMUM_FORMAL_COVERAGE:
                raise RuntimeError(f"NEXT426 {source} coverage below frozen minimum")
            statistics[source] = label_free_statistics(table)
            if statistics[source][n425.FEATURE_NAMES[0]]["unique_rounded_10"] < 20:
                raise RuntimeError(f"NEXT426 {source} feature is degenerate")
            failures = Counter(table.loc[~supported, "aprbs_failure"].fillna("unknown"))
            counts[source] = {
                "rows": int(len(table)), "supported": int(supported.sum()),
                "failures": int((~supported).sum()), "coverage": coverage,
                "site_count": int(pd.to_numeric(table.loc[supported, "aprbs_site_count"]).sum()),
                "edge_count": int(pd.to_numeric(table.loc[supported, "aprbs_edge_count"]).sum()),
                "pair_count": int(pd.to_numeric(table.loc[supported, "aprbs_pair_count"]).sum()),
        ...
        catalogue = {
            "protocol": PROTOCOL,
            "feature_count": 1,
            "feature_names": list(n425.FEATURE_NAMES),
            "feature_directions": n425.FEATURE_DIRECTIONS,
            "directions_frozen_before_outcome": True,
            "graph": "opposite-sign periodic Voronoi",
            "topology_field": "maximum entropy under exact site-charge marginals",
            "formula": "1 - sum(max(dx*dd,0))/sum(abs(dx*dd)); neutral 1/2",
            "output_grid": 1.0 / n425.OUTPUT_GRID,
            "minimum_formal_coverage": MINIMUM_FORMAL_COVERAGE,
            "label_free_statistics": statistics,
            "next427_audit_authorized": True,
            **n425.BOUNDARY_FLAGS,
        }
        ...
        manifest = {
            "protocol": PROTOCOL,
            "mode": "physically_isolated_discovery_x0_aprbs_freeze",
            "workers": workers,
        ...
        if any(
            ...
            raise RuntimeError("NEXT426 executed artifact changed before publication")
...
def main() -> int:
    ...
    manifest = build_cross_source_aprbs_features(
...
__all__ = [
    ...
    "build_cross_source_aprbs_features", "label_free_statistics", "n425",
