#!/usr/bin/env python3
"""Physically isolated full discovery build for frozen NEXT505 MVCH."""
...
PROTOCOL = "2026-08-13-next506-mvch-formal-build-v1"
MANIFEST_NAME = "MANIFEST.json"
CATALOGUE_NAME = "NEXT506_MVCH_FEATURE_CATALOGUE.json"
FEATURE_FILES = {
    "scigen": "next506_scigen_madelung_valence_class_homogeneity.parquet",
    "wyformer": "next506_wyformer_madelung_valence_class_homogeneity.parquet",
}
EXPECTED_ROWS = {"scigen": 13_470, "wyformer": 5_232}
MINIMUM_FORMAL_COVERAGE = 0.95
EXPECTED_PROBE_SHA256 = "4254c7062ea2fd443e2644564fe464e267babce8c1f0fae4c8da0abfa8a363c7"
EXPECTED_INPUT_SHA256 = {
    "design": "a440a4e4d345348b1e946b486d0b8a2f840db5bb44201a52f20ad1d341c832da",
    "engineering_probe_result": "0f5a0be78373f838178f7cf2bc66248e2217f30ecfcd6319018f750fae08cba7",
    ...
    "next505_source": "aa5db30c7685e0e47f82cc988050dcdc1f436e16c105c5050f691d3deb953c69",
...
def validate_probe_authorization(probe: dict[str, object]) -> None:
    ...
    if (
        probe.get("protocol")
        != "2026-08-13-next505-mvch-label-blind-novelty-probe-v1"
        or probe.get("design_sha256") != n505.DESIGN_SHA256
        or probe.get("minimum_novelty_joint_finite") != 40
        or probe.get("probe_passed") is not True
        or probe.get("next506_formal_build_authorized") is not True
        ...
        raise ValueError("NEXT506 label-blind probe authorization differs")
...
def _validate_engineering_authorization(probe: dict[str, object]) -> None:
    ...
    if (
        probe.get("protocol")
        != "2026-08-13-next505-mvch-label-blind-engineering-probe-v1"
        or probe.get("design_sha256") != n505.DESIGN_SHA256
        ...
        raise ValueError("NEXT506 engineering authorization differs")


def _error_row(exc: Exception) -> dict[str, object]:
    return {
        n505.FEATURE_NAMES[0]: math.nan,
        "mvch_supported": False,
        "mvch_failure": f"{type(exc).__name__}: {exc}",
        "mvch_site_count": 0,
        "mvch_class_count": 0,
        "mvch_repeated_class_site_fraction": math.nan,
        "mvch_within_sum_squares": math.nan,
        "mvch_total_sum_squares": math.nan,
        "mvch_valence_policy": None,
    }
...
def _compute_scigen_payload(item):
    ...
    try:
        ...
        return material_id, n505.compute_mvch_row(atoms)
...
def label_free_statistics(table: pd.DataFrame) -> dict[str, object]:
    ...
    if len(values) < 1:
        raise RuntimeError("NEXT506 label-free feature population is empty")
    return {
        n505.FEATURE_NAMES[0]: {
...
def _formal_paths(
    *,
    ...
    return {
        ...
        "next505_source": Path(n505.__file__).resolve(),
    }
...
def _validate_geometry_provenance(paths: dict[str, Path]) -> None:
    ...
    if (
        ...
        raise ValueError("NEXT506 discovery geometry provenance differs")


def build_cross_source_mvch_features(
    *,
    ...
    if type(workers) is not int or workers <= 0:
        raise ValueError("NEXT506 workers must be a positive exact integer")
    if any(not path.is_file() for path in paths.values()):
        raise FileNotFoundError("NEXT506 input is missing")
    ...
    if require_formal_inputs and input_hashes != EXPECTED_INPUT_SHA256:
        ...
        raise ValueError(f"NEXT506 formal input identity differs: {differing}")
    ...
    for source, frame in metadata.items():
        if required - set(frame) or frame["material_id"].astype(str).duplicated().any():
            raise ValueError(f"NEXT506 {source} metadata differs")
        selected = (
        ...
        if len(selected) != EXPECTED_ROWS[source]:
            raise ValueError(f"NEXT506 {source} discovery identity differs")
    ...
    executed_paths = {
        "src/next506_mvch_formal_build.py": Path(__file__).resolve(),
        "src/next505_madelung_valence_class_homogeneity.py": Path(
            n505.__file__
        ).resolve(),
        "experiments/next505_mvch_label_blind_engineering_probe.py": root
        / "experiments/next505_mvch_label_blind_engineering_probe.py",
        "experiments/next505_mvch_label_blind_novelty_probe.py": root
        / "experiments/next505_mvch_label_blind_novelty_probe.py",
        "tests/test_next505_madelung_valence_class_homogeneity.py": root
        / "tests/test_next505_madelung_valence_class_homogeneity.py",
        "tests/test_next505_mvch_label_blind_engineering_probe.py": root
        / "tests/test_next505_mvch_label_blind_engineering_probe.py",
        "tests/test_next505_mvch_label_blind_novelty_probe.py": root
        / "tests/test_next505_mvch_label_blind_novelty_probe.py",
        "tests/test_next506_mvch_formal_build.py": root
        / "tests/test_next506_mvch_formal_build.py",
    }
    executed_hashes = {
    ...
    try:
        ...
        for source in ("scigen", "wyformer"):
            ...
            if (
                ...
                raise RuntimeError(f"NEXT506 {source} material identity differs")
            ...
            if not finite[supported].all() or finite[~supported].any():
                raise RuntimeError(f"NEXT506 {source} finite support semantics differ")
            coverage = float(supported.mean())
            statistics[source] = label_free_statistics(table)
            failures = Counter(table.loc[~supported, "mvch_failure"].fillna("unknown"))
            counts[source] = {
                "rows": int(len(table)),
                ...
                "site_count": int(
                    pd.to_numeric(table.loc[supported, "mvch_site_count"]).sum()
                ),
                "class_count": int(
                    pd.to_numeric(table.loc[supported, "mvch_class_count"]).sum()
                ),
        ...
        catalogue = {
            "protocol": PROTOCOL,
            "feature_count": 1,
            "feature_names": list(n505.FEATURE_NAMES),
            "feature_directions": n505.FEATURE_DIRECTIONS,
            "directions_frozen_before_outcome": True,
            "site_partition": "(atomic_number, exact_formal_valence)",
            "formula": "1-within_class_site_energy_SSE/total_site_energy_SSE",
            "output_grid": 1.0 / n505.OUTPUT_GRID,
            "minimum_formal_coverage": MINIMUM_FORMAL_COVERAGE,
            "label_free_statistics": statistics,
            "coverage_gate_passed": coverage_gate_passed,
            "next507_audit_authorized": coverage_gate_passed,
            **n505.BOUNDARY_FLAGS,
        }
        ...
        manifest = {
            "protocol": PROTOCOL,
            "mode": "physically_isolated_discovery_x0_mvch_freeze",
            "workers": workers,
        ...
        if any(
            ...
            raise RuntimeError("NEXT506 executed artifact changed before publication")
...
def main() -> int:
    ...
    manifest = build_cross_source_mvch_features(
...
__all__ = [
    ...
    "PROBE_BOUNDARY_NAMES",
    "PROTOCOL",
    "build_cross_source_mvch_features",
