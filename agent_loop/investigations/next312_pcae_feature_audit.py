#!/usr/bin/env python3
"""Audit frozen PCAE hypotheses on physically isolated discovery outcomes."""
...
import src.next311_periodic_charge_alternation_eigenmode as n311
from src.next102_cross_source_dobvr_features import _sha256_file, _write_json


PROTOCOL = "2026-08-09-next312-pcae-feature-audit-v1"
MANIFEST_NAME = "MANIFEST.json"
CATALOGUE_NAME = "NEXT312_PCAE_FEATURE_CATALOGUE.json"
AUDIT_NAME = "NEXT312_PCAE_FEATURE_AUDIT.json"
TABLE_NAME = "next312_pcae_feature_audit.parquet"
HYPOTHESES = tuple((name, n311.FEATURE_DIRECTIONS[name]) for name in n311.FEATURE_NAMES)
QUANTILES = n268.QUANTILES
EXPECTED_NEXT268_SOURCE_SHA256 = (
    "36af0e632c8c3aae93f78271f837f693d49eff6ae88cdcad8edde42b3e0eaf64"
)
EXPECTED_NEXT311_SOURCE_SHA256 = (
    "e4814255f68e04dbd05b7ed3dadc3984751d32673ac4f29fffb151b9faf510be"
)
NEXT311_SOURCE_PATH = "src/next311_periodic_charge_alternation_eigenmode.py"
REQUIRED_STAGES = (*n268.REQUIRED_STAGES, 311)
...
EXPECTED_INPUT_SHA256 = {
    **{
    ...
    "design": n311.EXPECTED_DESIGN_SHA256,
    "next311_manifest": "9fda4ef85daeac4fb28693c0af65ed9985408ccc0f007a4e0e7af7d4326944bf",
    "next311_catalogue": "59dc3671620007b799d942b8513692e193aee5e4fb693ca780db5c610bc9ef14",
    "next311_scigen": "2389d4d9669d0870fa03f325087dd68b81797d7abe7b37dbc8f266662b63840b",
    "next311_wyformer": "149ec4a530efe1991e5a4007c869dbb27c975f53b6156d8b2d36a54d7851205e",
}
...
def select_eligible_hypotheses(
    ...
    try:
        ...
        raise ValueError("NEXT312 audit ranking table differs") from exc
...
def bounded_protection(
    *, values: object, direction: str, q_lo: float, q_hi: float
    ...
    if direction != "protected_low":
        raise ValueError("NEXT312 bounded protection inputs differ")
    try:
        return n268.bounded_protection(
            values=values, direction=direction, q_lo=q_lo, q_hi=q_hi
        )
    except ValueError as exc:
        raise ValueError("NEXT312 bounded protection inputs differ") from exc


def _index_pcae_by_prefixed_material_id(
    *, table: pd.DataFrame, source: str, expected_material_ids: object
    ...
    if set(extra["material_id"]) != expected:
        raise ValueError(f"NEXT312 {source} material identity differs")
...
def _paths(
    *,
    ...
    paths.update(
        {
            "design": design_path,
            "next311_manifest": roots["next311"] / n311.MANIFEST_NAME,
            "next311_catalogue": roots["next311"] / n311.CATALOGUE_NAME,
            "next311_scigen": roots["next311"] / n311.FEATURE_FILES["scigen"],
            "next311_wyformer": roots["next311"] / n311.FEATURE_FILES["wyformer"],
        }
    )
    return paths


def _verify_next311(paths: Mapping[str, Path], input_hashes: Mapping[str, str]):
    ...
    manifest = json.loads(paths["next311_manifest"].read_text())
    catalogue = json.loads(paths["next311_catalogue"].read_text())
    tables = {
        "scigen": pd.read_parquet(paths["next311_scigen"]),
        "wyformer": pd.read_parquet(paths["next311_wyformer"]),
    }
    expected_outputs = {
        n311.CATALOGUE_NAME: input_hashes["next311_catalogue"],
        n311.FEATURE_FILES["scigen"]: input_hashes["next311_scigen"],
        n311.FEATURE_FILES["wyformer"]: input_hashes["next311_wyformer"],
    }
    if (
        _sha256_file(Path(n268.__file__).resolve()) != EXPECTED_NEXT268_SOURCE_SHA256
        or _sha256_file(Path(n311.__file__).resolve()) != EXPECTED_NEXT311_SOURCE_SHA256
        or manifest.get("protocol") != n311.PROTOCOL
        or manifest.get("next312_audit_authorized") is not True
        ...
        or manifest.get("executed_source_sha256", {}).get(NEXT311_SOURCE_PATH)
        != EXPECTED_NEXT311_SOURCE_SHA256
        or any(manifest.get(key) is not value for key, value in BOUNDARY_FLAGS.items())
        or catalogue.get("feature_names") != list(n311.FEATURE_NAMES)
        or catalogue.get("feature_directions") != n311.FEATURE_DIRECTIONS
        or catalogue.get("feature_count") != len(n311.FEATURE_NAMES)
        or catalogue.get("graph")
        != "NEXT279 full reciprocal periodic radical active-facet quotient multigraph"
        or catalogue.get("normalized_adjacency") != "D^(-1/2) W D^(-1/2)"
        or catalogue.get("normalized_signal") != "formal_charge / sqrt(contact_degree)"
        or catalogue.get("target_eigenvalue") != -1
        or catalogue.get("minimum_formal_coverage") != n311.MINIMUM_FORMAL_COVERAGE
        or catalogue.get("quantile_method") != "inverted_cdf"
        or catalogue.get("directions_frozen_before_outcome") is not True
    ):
        raise ValueError("NEXT312 NEXT311 provenance differs")
    for source, expected_rows in n311.EXPECTED_ROWS.items():
        table = tables[source]
        supported = table["pcae_supported"].fillna(False).astype(bool)
        values = np.column_stack(
            [
                pd.to_numeric(table[name], errors="coerce").to_numpy(float)
                for name in n311.FEATURE_NAMES
            ]
        )
        finite = np.isfinite(values)
        sites = pd.to_numeric(table["pcae_site_count"], errors="coerce")
        contacts = pd.to_numeric(
            table["pcae_directed_contact_count"], errors="coerce"
        )
        minimum_degree = pd.to_numeric(table["pcae_minimum_degree"], errors="coerce")
        maximum_degree = pd.to_numeric(table["pcae_maximum_degree"], errors="coerce")
        policy = table["pcae_valence_policy"]
        if (
            ...
            or float(supported.mean()) < n311.MINIMUM_FORMAL_COVERAGE
            ...
            or not np.isfinite(minimum_degree[supported]).all()
            or not np.isfinite(maximum_degree[supported]).all()
            or not (minimum_degree[supported] > 0.0).all()
            or not (maximum_degree[supported] >= minimum_degree[supported]).all()
            or policy[supported].isna().any()
            or not (sites[~supported] == 0).all()
            or not (contacts[~supported] == 0).all()
        ):
            raise ValueError(f"NEXT312 NEXT311 {source} table differs")
    return (*prior[:-1], tables)


def run_pcae_feature_audit(
    *,
    ...
    """Audit exactly the three frozen PCAE hypotheses."""

    if not set(REQUIRED_STAGES).issubset(stage_dirs):
        raise ValueError("NEXT312 stage directory universe differs")
    if not set(REQUIRED_DESIGN_STAGES).issubset(design_paths):
        raise ValueError("NEXT312 design path universe differs")
    roots = {
    ...
    if any(not path.is_file() for path in paths.values()):
        raise FileNotFoundError("NEXT312 input is missing")
    ...
    if require_formal_inputs and input_hashes != EXPECTED_INPUT_SHA256:
        ...
        raise ValueError(f"NEXT312 formal input identity differs: {differing}")
    (
        eligible,
        eligible214,
        primary_key,
        ...
        formula222,
        pcae_tables,
    ) = _verify_next311(paths, input_hashes)
    n227 = n268.n227
    ...
    for source_name in ("scigen", "wyformer"):
        indexed = _index_pcae_by_prefixed_material_id(
    ...
    if cohort_counts != n227.EXPECTED_COHORT_COUNTS:
        raise ValueError("NEXT312 rejected cohort counts differ")
    ...
    for feature, direction in HYPOTHESES:
        ...
        if not len(finite):
            raise ValueError(f"NEXT312 {feature} has no finite discovery values")
    ...
    catalogue = {
        ...
        "features": list(n311.FEATURE_NAMES),
        "feature_directions": n311.FEATURE_DIRECTIONS,
    ...
    audit = {
        "protocol": PROTOCOL,
        "audit_mode": "fixed_next224_rejected_extreme_pcae_audit",
        "base_candidate_key_sha256": n227.EXPECTED_BASE_CANDIDATE_KEY_SHA256,
        "base_threshold": n227.EXPECTED_BASE_THRESHOLD,
        "cohort_counts": {
        ...
        "next313_search_authorized": bool(selected is not None),
    ...
    source_paths = {
        ...
        NEXT311_SOURCE_PATH: Path(n311.__file__).resolve(),
        "src/next312_pcae_feature_audit.py": Path(__file__).resolve(),
    }
    ...
    try:
        ...
        manifest = {
            ...
            "next313_search_authorized": bool(selected is not None),
            "pcae_branch_terminated": selected is None,
            "new_formula_searched": False,
            "new_formula_selected": False,
        ...
        if any(_sha256_file(path) != input_hashes[name] for name, path in paths.items()):
            raise RuntimeError("NEXT312 input changed before publication")
        if any(_sha256_file(path) != source_hashes[name] for name, path in source_paths.items()):
            raise RuntimeError("NEXT312 source changed before publication")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    manifest = run_pcae_feature_audit(
