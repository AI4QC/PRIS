#!/usr/bin/env python3
"""Frozen final-stage threshold-local signed redistribution search."""
...
import src.next233_final_stagewise_margin_local_feature_audit as n233
...
n228 = n231.n228
n216 = n228.n216
PROTOCOL = "2026-08-09-next234-final-stagewise-margin-local-search-v1"
MANIFEST_NAME = "MANIFEST.json"
CATALOGUE_NAME = "NEXT234_FINAL_STAGEWISE_MARGIN_LOCAL_CATALOGUE.json"
EVALUATION_NAME = "NEXT234_DISCOVERY_EVALUATION.json"
FORMULA_NAME = "NEXT234_FROZEN_CANDIDATE.json"
SEARCH_NAME = "next234_final_stagewise_margin_local_search.parquet"
SCORE_COMPOSITION = "nonnegative_next232_plus_third_triangular_margin_local_term"
...
EXPECTED_ELIGIBLE_HYPOTHESIS_COUNT = 27
EXPECTED_ELIGIBLE_HYPOTHESIS_SHA256 = (
    "0d815b6c745fd7586c079568ecd817da5816b09e5ca931275fe6e7705cd80a29"
)
...
EXPECTED_DESIGN_SHA256 = n233.EXPECTED_DESIGN_SHA256
EXPECTED_BASE_CANDIDATE_KEY_SHA256 = n233.EXPECTED_BASE_CANDIDATE_KEY_SHA256
EXPECTED_BASE_THRESHOLD = n233.EXPECTED_BASE_THRESHOLD
EXPECTED_BASE_SUPPORT_COUNT = n233.EXPECTED_BASE_SUPPORT_COUNT
EXPECTED_BASE_FAILED_COUNT = n233.EXPECTED_BASE_FAILED_COUNT
EXPECTED_BASE_SHORTFALL = n233.EXPECTED_BASE_SHORTFALL
REPAIR_WIDTH = n228.REPAIR_WIDTH
SEARCH_WORKERS = n228.SEARCH_WORKERS
BOUNDARY_FLAGS = n233.BOUNDARY_FLAGS
REQUIRED_STAGES = (*n233.REQUIRED_STAGES, 233)
REQUIRED_DESIGN_STAGES = (*n233.REQUIRED_DESIGN_STAGES, 233)
EXPECTED_NEXT233_SOURCE_SHA256 = (
    "37e10eec69b799dc26aa5c440aab165d0b1943cdb8561639941fcfbfa644b0c4"
)
EXPECTED_INPUT_SHA256 = {
    **{
        key: value
        for key, value in n233.EXPECTED_INPUT_SHA256.items()
        if key != "design"
    },
    "next233_design": n233.EXPECTED_INPUT_SHA256["design"],
    "design": EXPECTED_DESIGN_SHA256,
    "next233_manifest": (
        "9454f17daf5338dc47348b7f197d118912218c03de0c5f1d695d39303f221da6"
    ),
    "next233_catalogue": (
        "662fcc9b72de07a2ae4b04d9366bde264d389d0ce726050356f49582e6dd5c61"
    ),
    "next233_audit": (
        "facb24b23855c3f900a58cdf6e99f06e08bca903fb8ff3fddcc0d7a236d2a65e"
    ),
    "next233_table": (
        "b5855bdef67e10bf6b36c94b03a85750f254bc66c915e3ee7f4c6910c90adcae"
    ),
}


def final_stagewise_margin_local_score(
    *,
    base_score: object,
    base_support: object,
    ...
    """Add the frozen final triangular signed term to the NEXT232 score."""
    ...
    if (
        ...
        or direction not in n233.PROTECTION_DIRECTIONS
        ...
        raise ValueError("NEXT234 stagewise score inputs differ")
...
def _grid_numerator(value: float, denominator: int, label: str) -> int:
    ...
    if (
        ...
        raise ValueError(f"NEXT234 {label} grid differs")
    return numerator


def build_final_stagewise_candidate_specs(
    *,
    ...
    """Build one exact base control plus the complete final-stage grammar."""

    if not isinstance(base_candidate_key, str) or not base_candidate_key:
        raise ValueError("NEXT234 base candidate key differs")
    if not isinstance(features, pd.DataFrame):
        raise ValueError("NEXT234 feature table differs")
    base = np.asarray(base_score, dtype=float)
    support = np.asarray(base_support, dtype=bool)
    cutoff = float(threshold)
    width = float(repair_width)
    ...
    if (
        ...
        raise ValueError("NEXT234 frozen grid differs")
    common = {
        ...
        "missing_policy": "THIRD_TERM_OFF_KEEP_NEXT232_SCORE",
    ...
    for hypothesis in hypotheses:
        try:
            ...
            raise ValueError("NEXT234 eligible hypothesis identity differs") from error
        if (
            not feature
            or direction not in n233.PROTECTION_DIRECTIONS
            or feature not in features.columns
        ):
            raise ValueError("NEXT234 eligible hypothesis identity differs")
    ...
    if len({str(spec["candidate_key"]) for spec in specs}) != len(specs):
        raise RuntimeError("NEXT234 candidate keys are not unique")
    return specs


def materialize_final_stagewise_candidates(
    *,
    features: pd.DataFrame,
    base_score: object,
    base_support: object,
    ...
    """Encode every exact three-stage score as one evaluator term."""
    ...
    if (
        ...
        raise ValueError("NEXT234 materializer inputs differ")
    ...
    for spec in raw_specs:
        ...
        if feature is None:
            ...
            if feature not in features.columns:
                raise ValueError("NEXT234 materializer feature differs")
            if feature not in raw_cache:
                raw_cache[feature] = pd.to_numeric(
                    features[feature], errors="coerce"
                ).to_numpy(float)
            score, got_support, active, _, _ = final_stagewise_margin_local_score(
                base_score=base,
        ...
        if not np.array_equal(got_support, support):
            raise RuntimeError("NEXT234 support changed")
        ...
        terms.append(
            {
                ...
                "center": 0.0,
                "scale": 1.0 / divisor,
                "group": "next234_stagewise_margin_local",
                "encoding": "asinh_sinh_exact_three_stage_margin_local_score",
...
def _paths(
    *,
    ...
    paths = n233._paths(
        ...
        design_paths={
            stage: design_paths[stage] for stage in n233.REQUIRED_DESIGN_STAGES
        },
        design_path=design_paths[233],
    )
    paths["next233_design"] = paths.pop("design")
    paths.update(
        {
            "design": design_path,
            "next233_manifest": roots["next233"] / n233.MANIFEST_NAME,
            "next233_catalogue": roots["next233"] / n233.CATALOGUE_NAME,
            "next233_audit": roots["next233"] / n233.AUDIT_NAME,
            "next233_table": roots["next233"] / n233.TABLE_NAME,
        }
    )
    return paths
...
def _verify_next233(
    ...
    manifest = json.loads(paths["next233_manifest"].read_text())
    catalogue = json.loads(paths["next233_catalogue"].read_text())
    audit = json.loads(paths["next233_audit"].read_text())
    table = pd.read_parquet(paths["next233_table"])
    ...
    expected_outputs = {
        n233.CATALOGUE_NAME: input_hashes["next233_catalogue"],
        n233.AUDIT_NAME: input_hashes["next233_audit"],
        n233.TABLE_NAME: input_hashes["next233_table"],
    }
    if (
        manifest.get("protocol") != n233.PROTOCOL
        or manifest.get("eligible_hypothesis_count")
        != EXPECTED_ELIGIBLE_HYPOTHESIS_COUNT
        or manifest.get("eligible_hypothesis_sha256")
        != EXPECTED_ELIGIBLE_HYPOTHESIS_SHA256
        or manifest.get("next234_search_authorized") is not True
        ...
        != EXPECTED_NEXT233_SOURCE_SHA256
        or _sha256_file(Path(n233.__file__).resolve())
        != EXPECTED_NEXT233_SOURCE_SHA256
        ...
        != EXPECTED_ELIGIBLE_HYPOTHESIS_SHA256
        or audit.get("next234_search_authorized") is not True
        or eligible != table_eligible
        or hashlib.sha256("\n".join(eligible).encode()).hexdigest()
        != EXPECTED_ELIGIBLE_HYPOTHESIS_SHA256
    ):
        raise ValueError("NEXT234 NEXT233 provenance differs")
    return eligible


def _formula_from_specs(
    base_spec: Mapping[str, object], third_spec: Mapping[str, object] | None
) -> dict[str, object]:
    common = {
        "protocol": PROTOCOL,
        "base_protocol": n231.PROTOCOL,
        "base_candidate_key_sha256": EXPECTED_BASE_CANDIDATE_KEY_SHA256,
        "second_stage_base_term": {
            name: base_spec[name]
            for name in (
    ...
    return {
        **common,
        "kind": "three_stage_triangular_margin_local_x0_no_dft_score",
        "selected": True,
        "third_stage_threshold": EXPECTED_BASE_THRESHOLD,
        "repair_width": REPAIR_WIDTH,
        "third_stage_term": {
            name: third_spec[name]
            for name in (
                "hypothesis", "feature", "direction", "q_lo", "q_hi",
                "local_width_fraction", "amplitude_fraction",
            )
...
def run_final_stagewise_margin_local_search(
    *,
    ...
    """Run the complete frozen discovery-only NEXT234 search."""

    if not set(REQUIRED_STAGES).issubset(stage_dirs):
        raise ValueError("NEXT234 stage directory universe differs")
    if not set(REQUIRED_DESIGN_STAGES).issubset(design_paths):
        raise ValueError("NEXT234 design path universe differs")
    roots = {
    ...
    if type(search_workers) is not int or search_workers <= 0:
        raise ValueError("NEXT234 search_workers must be a positive exact integer")
    if any(not path.is_file() for path in paths.values()):
        raise FileNotFoundError("NEXT234 input is missing")
    ...
    if require_formal_inputs and input_hashes != EXPECTED_INPUT_SHA256:
        ...
        raise ValueError(f"NEXT234 formal input identity differs: {differing}")
    eligible233 = _verify_next233(paths, input_hashes)
    combined, base_score, support, endpoint, _ = (
        n233._verify_and_reconstruct_next232(paths, input_hashes)
    )
    ...
    published231 = pd.read_parquet(paths["next231_search"])
    ...
    if (
        ...
        raise RuntimeError("NEXT234 frozen candidate universe differs")
    ...
    if (
        ...
        raise RuntimeError("NEXT234 evaluator accounting differs")
    ...
    reference = published231.loc[published231["candidate_key"].eq(base_key)]
    if len(no_op) != 1 or len(reference) != 1:
        raise RuntimeError("NEXT234 no-op reproduction identity differs")
    ...
    if selected_row is not None:
        ...
        if str(selected_result["selected"]["record"]["candidate_key"]) != selected_key:
            raise RuntimeError("NEXT234 selected candidate reproduction differs")
    ...
    if selected is not None and passes != bool(
        ...
        raise RuntimeError("NEXT234 all-gate selection differs")
    ...
    catalogue = {
        ...
        "repair_width": REPAIR_WIDTH,
        "eligible_hypotheses": list(eligible233),
        "eligible_hypothesis_count": len(eligible233),
    ...
    evaluation = {
        ...
        "next232_frontier_reproduced": True,
        "rows": {
            "scigen": int(
                combined["source_dataset"].astype(str).eq("scigen").sum()
            ),
            "wyformer": int(
                combined["source_dataset"].astype(str).eq("wyformer").sum()
            ),
        ...
        "next235_diagnostic_authorized": bool(not passes and diagnostic_keys),
        "next235_candidate_count": len(diagnostic_keys),
        "next235_candidate_key_sha256": diagnostic_sha,
    ...
    source_paths = {
        "src/next233_final_stagewise_margin_local_feature_audit.py": Path(
            n233.__file__
        ).resolve(),
        "src/next234_final_stagewise_margin_local_search.py": Path(__file__).resolve(),
    }
    source_hashes = {
    ...
    try:
        ...
        if any(
            ...
            raise RuntimeError("NEXT234 input changed before publication")
        if any(
            _sha256_file(path) != source_hashes[name]
            for name, path in source_paths.items()
        ):
            raise RuntimeError("NEXT234 source changed before publication")
...
def main() -> int:
    ...
    manifest = run_final_stagewise_margin_local_search(
