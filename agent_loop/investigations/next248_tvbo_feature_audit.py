#!/usr/bin/env python3
"""Audit frozen TVBO hypotheses inside the exact NEXT224 rejected cohort."""
...
import src.next247_third_order_voronoi_bond_order as n247
from src.next102_cross_source_dobvr_features import _sha256_file, _write_json


PROTOCOL = "2026-08-09-next248-tvbo-feature-audit-v1"
MANIFEST_NAME = "MANIFEST.json"
CATALOGUE_NAME = "NEXT248_TVBO_FEATURE_CATALOGUE.json"
AUDIT_NAME = "NEXT248_TVBO_FEATURE_AUDIT.json"
TABLE_NAME = "next248_tvbo_feature_audit.parquet"
EXPECTED_DESIGN_SHA256 = n247.EXPECTED_DESIGN_SHA256
HYPOTHESES = (
...
EXPECTED_NEXT247_SOURCE_SHA256 = (
    "c02953e9ab2b1f79e18ba2391dfcc0215d1f25dc9e3e930871ab90100cf29ae9"
)
NEXT247_SOURCE_PATH = "src/next247_third_order_voronoi_bond_order.py"
REQUIRED_STAGES = (*n227.REQUIRED_STAGES, 247)
REQUIRED_DESIGN_STAGES = (*n227.REQUIRED_DESIGN_STAGES, 227, 247)
...
EXPECTED_INPUT_SHA256 = {
    **{
    ...
    "next247_design": EXPECTED_DESIGN_SHA256,
    "design": EXPECTED_DESIGN_SHA256,
    "next247_manifest": (
        "04fd70c2a152d9135e5aa6cb5c4c534c8fb1cf635c1e753c76199b3bf5351e6e"
    ),
    "next247_catalogue": (
        "35b468ad648ec2446e5bdbea34007292c45566e7d0a12c47f6a7adda7b7ec919"
    ),
    "next247_scigen": (
        "d8aa8b4a2012c0b1f69680e51315dbe680062396385a6525d6f5b10ede0ade6d"
    ),
    "next247_wyformer": (
        "b6ed82d379208f1c2b52ceb81a0bd00e7a67f03c74187134ceef4c17b874f552"
    ),
}
...
def select_eligible_hypotheses(
    ...
    if not isinstance(frame, pd.DataFrame) or frame.empty or required - set(frame.columns):
        raise ValueError("NEXT248 audit ranking table differs")
    result = frame.copy()
...
def bounded_protection(
    *, values: object, direction: str, q_lo: float, q_hi: float
    ...
    if raw.ndim != 1 or direction not in n227.PROTECTION_DIRECTIONS or not (
        ...
        raise ValueError("NEXT248 bounded protection inputs differ")
...
def _index_tvbo_by_prefixed_material_id(
    *, table: pd.DataFrame, source: str, expected_material_ids: object
) -> pd.DataFrame:
    """Return a source-prefixed TVBO table after exact identity validation."""
    ...
    if set(extra["material_id"]) != expected:
        raise ValueError(f"NEXT248 {source} material identity differs")
...
def _paths(
    *,
    ...
    paths.update(
        {
            "next247_design": design_paths[247],
            "design": design_path,
            "next247_manifest": roots["next247"] / n247.MANIFEST_NAME,
            "next247_catalogue": roots["next247"] / n247.CATALOGUE_NAME,
            "next247_scigen": roots["next247"] / n247.FEATURE_FILES["scigen"],
            "next247_wyformer": roots["next247"] / n247.FEATURE_FILES["wyformer"],
        }
    )
    return paths
...
def _verify_next247(
    ...
    manifest = json.loads(paths["next247_manifest"].read_text())
    catalogue = json.loads(paths["next247_catalogue"].read_text())
    tables = {
        "scigen": pd.read_parquet(paths["next247_scigen"]),
        "wyformer": pd.read_parquet(paths["next247_wyformer"]),
    }
    expected_outputs = {
        n247.CATALOGUE_NAME: input_hashes["next247_catalogue"],
        n247.FEATURE_FILES["scigen"]: input_hashes["next247_scigen"],
        n247.FEATURE_FILES["wyformer"]: input_hashes["next247_wyformer"],
    }
    if (
        _sha256_file(Path(n227.__file__).resolve()) != EXPECTED_NEXT227_SOURCE_SHA256
        or manifest.get("protocol") != n247.PROTOCOL
        ...
        or manifest.get("executed_source_sha256", {}).get(NEXT247_SOURCE_PATH)
        != EXPECTED_NEXT247_SOURCE_SHA256
        or _sha256_file(Path(n247.__file__).resolve()) != EXPECTED_NEXT247_SOURCE_SHA256
        or any(manifest.get(key) is not value for key, value in n247.BOUNDARY_FLAGS.items())
        or catalogue.get("feature_names") != list(n247.FEATURE_NAMES)
        or catalogue.get("feature_count") != len(n247.FEATURE_NAMES)
        or catalogue.get("labels_opened") is not False
    ):
        raise ValueError("NEXT248 NEXT247 provenance differs")
    for source, expected_rows in (("scigen", 13470), ("wyformer", 5232)):
        ...
        if (
            ...
            or not table["tvbo_supported"].fillna(False).astype(bool).all()
            or any(
                not np.isfinite(pd.to_numeric(table[name], errors="coerce")).all()
                for name in n247.FEATURE_NAMES
            )
        ):
            raise ValueError(f"NEXT248 NEXT247 {source} table differs")
    return (*prior, tables)


def run_tvbo_feature_audit(
    *,
    ...
    """Audit frozen TVBO hypotheses without searching a new formula."""

    if not set(REQUIRED_STAGES).issubset(stage_dirs):
        raise ValueError("NEXT248 stage directory universe differs")
    if not set(REQUIRED_DESIGN_STAGES).issubset(design_paths):
        raise ValueError("NEXT248 design path universe differs")
    roots = {
    ...
    if any(not path.is_file() for path in paths.values()):
        raise FileNotFoundError("NEXT248 input is missing")
    ...
    if require_formal_inputs and input_hashes != EXPECTED_INPUT_SHA256:
        ...
        raise ValueError(f"NEXT248 formal input identity differs: {differing}")
    (
        eligible,
        eligible214,
        primary_key,
        ...
        formula222,
        tvbo_tables,
    ) = _verify_next247(paths, input_hashes)
    ...
    if cohort_counts != n227.EXPECTED_COHORT_COUNTS:
        raise ValueError("NEXT248 rejected cohort counts differ")
    ...
    audit = {
        "protocol": PROTOCOL,
        "audit_mode": "fixed_next224_rejected_extreme_tvbo_feature_audit",
        "base_candidate_key_sha256": n227.EXPECTED_BASE_CANDIDATE_KEY_SHA256,
        "base_threshold": n227.EXPECTED_BASE_THRESHOLD,
        "cohort_counts": {
        ...
        "next249_search_authorized": bool(selected is not None),
    ...
    source_paths = {
        ...
        NEXT247_SOURCE_PATH: Path(n247.__file__).resolve(),
        "src/next248_tvbo_feature_audit.py": Path(__file__).resolve(),
    }
    ...
    try:
        ...
        manifest = {
            ...
            "next249_search_authorized": bool(selected is not None),
            "tvbo_branch_terminated": selected is None,
            "new_formula_searched": False,
            "new_formula_selected": False,
        ...
        if any(_sha256_file(path) != input_hashes[name] for name, path in paths.items()):
            raise RuntimeError("NEXT248 input changed before publication")
        if any(_sha256_file(path) != source_hashes[name] for name, path in source_paths.items()):
            raise RuntimeError("NEXT248 source changed before publication")
...
def main() -> int:
    ...
    manifest = run_tvbo_feature_audit(
