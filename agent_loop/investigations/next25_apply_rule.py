#!/usr/bin/env python3
"""Apply the frozen NEXT23 law to NEXT25 OMatG x0 without DFT labels."""
...
from src.next24_apply_rule import _exact_source_paths
from src.next25_omatg_holdout import PROTOCOL as COHORT_PROTOCOL


PROTOCOL = "2026-08-03-next25-omatg-label-free-transport-application-v1"
PREDICTIONS_NAME = "next25_omatg_transport_predictions.parquet"
...
def apply_transport_rule(
    *,
    ...
    """Seal OMatG predictions using only frozen selected analytic sources."""
    ...
    cohort_manifest = _read_json(
        core_paths["cohort_manifest"], role="NEXT25 cohort manifest"
    )
    _validate_manifest_output_hash(
        cohort_manifest, core_paths["cohort_metadata"], role="NEXT25 cohort"
    )
    if (
        cohort_manifest.get("protocol") != COHORT_PROTOCOL
        or cohort_manifest.get("input_role") != "unrelaxed_x0_geometry_only"
        or cohort_manifest.get("all_generator_outputs_retained") is not True
        ...
        raise ValueError("NEXT25 cohort crossed the label-free boundary")
    ...
    score, support = score_candidate(combined, selected_terms, law["base_parameters"])
    ...
    source_paths = {
        "src/next25_apply_rule.py": Path(__file__).resolve(),
    ...
    try:
        ...
        if not pd.read_parquet(predictions_path).equals(predictions):
            raise ValueError("NEXT25 prediction prepublication validation failed")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    apply_transport_rule(
        ...
        feature_paths={"sivr": arguments.sivr_features, "scbve": arguments.scbve_features},
