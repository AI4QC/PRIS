#!/usr/bin/env python3
"""Audit frozen PRCB hypotheses on physically isolated discovery outcomes."""
...
import src.next303_periodic_reciprocal_cage_balance as n303
from src.next102_cross_source_dobvr_features import _sha256_file, _write_json


PROTOCOL = "2026-08-09-next304-prcb-feature-audit-v1"
MANIFEST_NAME = "MANIFEST.json"
CATALOGUE_NAME = "NEXT304_PRCB_FEATURE_CATALOGUE.json"
AUDIT_NAME = "NEXT304_PRCB_FEATURE_AUDIT.json"
TABLE_NAME = "next304_prcb_feature_audit.parquet"
HYPOTHESES = tuple((name, n303.FEATURE_DIRECTIONS[name]) for name in n303.FEATURE_NAMES)
QUANTILES = n268.QUANTILES
EXPECTED_NEXT268_SOURCE_SHA256 = (
    "36af0e632c8c3aae93f78271f837f693d49eff6ae88cdcad8edde42b3e0eaf64"
)
EXPECTED_NEXT303_SOURCE_SHA256 = (
    "abba56e29d693e1e879e0c69c29f2bfc386e4ed61cd1b841319327a75130050e"
)
NEXT303_SOURCE_PATH = "src/next303_periodic_reciprocal_cage_balance.py"
REQUIRED_STAGES = (*n268.REQUIRED_STAGES, 303)
...
EXPECTED_INPUT_SHA256 = {
    **{
    ...
    "design": n303.EXPECTED_DESIGN_SHA256,
    "next303_manifest": "89af207ac8c92bcdfaa2d978ec5d5bd874229dc0b0f8a71de1a1585343c2b12e",
    "next303_catalogue": "cdbff7cd5d40afd4966cbc564484e5aaf1dde7b879a2b51082844dd387599d2f",
    "next303_scigen": "bc3afef126526edc419e9e4060de074a0eae0cce80534439a4f1dabe2c3a6c14",
    "next303_wyformer": "4e84f7334aa93644383bdac76b289e3477f4f76f132261cbef29b52c53d32580",
}
...
def select_eligible_hypotheses(
    ...
    try:
        ...
        raise ValueError("NEXT304 audit ranking table differs") from exc
...
def bounded_protection(
    *, values: object, direction: str, q_lo: float, q_hi: float
    ...
    if direction != "protected_high":
        raise ValueError("NEXT304 bounded protection inputs differ")
    try:
        return n268.bounded_protection(
            values=values, direction=direction, q_lo=q_lo, q_hi=q_hi
        )
    except ValueError as exc:
        raise ValueError("NEXT304 bounded protection inputs differ") from exc


def _index_prcb_by_prefixed_material_id(
    *, table: pd.DataFrame, source: str, expected_material_ids: object
    ...
    if set(extra["material_id"]) != expected:
        raise ValueError(f"NEXT304 {source} material identity differs")
...
def _paths(
    *,
    ...
    paths.update(
        {
            "design": design_path,
            "next303_manifest": roots["next303"] / n303.MANIFEST_NAME,
            "next303_catalogue": roots["next303"] / n303.CATALOGUE_NAME,
            "next303_scigen": roots["next303"] / n303.FEATURE_FILES["scigen"],
            "next303_wyformer": roots["next303"] / n303.FEATURE_FILES["wyformer"],
        }
    )
    return paths


def _verify_next303(paths: Mapping[str, Path], input_hashes: Mapping[str, str]):
    ...
    manifest = json.loads(paths["next303_manifest"].read_text())
    catalogue = json.loads(paths["next303_catalogue"].read_text())
    tables = {
        "scigen": pd.read_parquet(paths["next303_scigen"]),
        "wyformer": pd.read_parquet(paths["next303_wyformer"]),
    }
    expected_outputs = {
        n303.CATALOGUE_NAME: input_hashes["next303_catalogue"],
        n303.FEATURE_FILES["scigen"]: input_hashes["next303_scigen"],
        n303.FEATURE_FILES["wyformer"]: input_hashes["next303_wyformer"],
    }
    if (
        _sha256_file(Path(n268.__file__).resolve()) != EXPECTED_NEXT268_SOURCE_SHA256
        or _sha256_file(Path(n303.__file__).resolve()) != EXPECTED_NEXT303_SOURCE_SHA256
        or manifest.get("protocol") != n303.PROTOCOL
        or manifest.get("next304_audit_authorized") is not True
        ...
        or manifest.get("executed_source_sha256", {}).get(NEXT303_SOURCE_PATH)
        != EXPECTED_NEXT303_SOURCE_SHA256
        or any(manifest.get(key) is not value for key, value in BOUNDARY_FLAGS.items())
        or catalogue.get("feature_names") != list(n303.FEATURE_NAMES)
        or catalogue.get("feature_directions") != n303.FEATURE_DIRECTIONS
        or catalogue.get("feature_count") != len(n303.FEATURE_NAMES)
        or catalogue.get("periodic_edge_orbit")
        != "lexicographic_min_i_j_T_or_j_i_minus_T"
        or catalogue.get("exact_positive_self_stress_floor_excluded_as_degenerate")
        is not True
        or catalogue.get("quantile_method") != "inverted_cdf"
        or catalogue.get("labels_opened") is not False
    ):
        raise ValueError("NEXT304 NEXT303 provenance differs")
    for source, expected_rows in n303.EXPECTED_ROWS.items():
        table = tables[source]
        supported = table["prcb_supported"].fillna(False).astype(bool)
        values = np.column_stack(
            [
                pd.to_numeric(table[name], errors="coerce").to_numpy(float)
                for name in n303.FEATURE_NAMES
            ]
        )
        finite = np.isfinite(values)
        sites = pd.to_numeric(table["prcb_site_count"], errors="coerce")
        edges = pd.to_numeric(table["prcb_edge_count"], errors="coerce")
        mutual = pd.to_numeric(table["prcb_mutual_edge_count"], errors="coerce")
        ...
        if (
            ...
            or float(supported.mean()) < n303.MINIMUM_FORMAL_COVERAGE
            ...
            or not (mutual[supported] >= 0).all()
            or not (mutual[supported] <= edges[supported]).all()
            or not (translation[supported] >= 1).all()
            or not (translation[supported] <= n303.n299.MAX_TRANSLATION_RANGE).all()
            or not (sites[~supported] == 0).all()
            or not (edges[~supported] == 0).all()
            or not (mutual[~supported] == 0).all()
            or not (translation[~supported] == 0).all()
        ):
            raise ValueError(f"NEXT304 NEXT303 {source} table differs")
    return (*prior[:-1], tables)


def run_prcb_feature_audit(
    *,
    ...
    """Audit exactly the thirteen frozen PRCB hypotheses."""

    if not set(REQUIRED_STAGES).issubset(stage_dirs):
        raise ValueError("NEXT304 stage directory universe differs")
    if not set(REQUIRED_DESIGN_STAGES).issubset(design_paths):
        raise ValueError("NEXT304 design path universe differs")
    roots = {
    ...
    if any(not path.is_file() for path in paths.values()):
        raise FileNotFoundError("NEXT304 input is missing")
    ...
    if require_formal_inputs and input_hashes != EXPECTED_INPUT_SHA256:
        ...
        raise ValueError(f"NEXT304 formal input identity differs: {differing}")
    (
        eligible,
        eligible214,
        primary_key,
        ...
        formula222,
        prcb_tables,
    ) = _verify_next303(paths, input_hashes)
    n227 = n268.n227
    ...
    for source_name in ("scigen", "wyformer"):
        indexed = _index_prcb_by_prefixed_material_id(
    ...
    if cohort_counts != n227.EXPECTED_COHORT_COUNTS:
        raise ValueError("NEXT304 rejected cohort counts differ")
    ...
    for feature, direction in HYPOTHESES:
        ...
        if not len(finite):
            raise ValueError(f"NEXT304 {feature} has no finite discovery values")
    ...
    catalogue = {
        ...
        "features": list(n303.FEATURE_NAMES),
        "feature_directions": n303.FEATURE_DIRECTIONS,
    ...
    audit = {
        "protocol": PROTOCOL,
        "audit_mode": "fixed_next224_rejected_extreme_prcb_audit",
        "base_candidate_key_sha256": n227.EXPECTED_BASE_CANDIDATE_KEY_SHA256,
        "base_threshold": n227.EXPECTED_BASE_THRESHOLD,
        "cohort_counts": {
        ...
        "next305_search_authorized": bool(selected is not None),
    ...
    source_paths = {
        ...
        NEXT303_SOURCE_PATH: Path(n303.__file__).resolve(),
        "src/next304_prcb_feature_audit.py": Path(__file__).resolve(),
    }
    ...
    try:
        ...
        manifest = {
            ...
            "next305_search_authorized": bool(selected is not None),
            "prcb_branch_terminated": selected is None,
            "new_formula_searched": False,
            "new_formula_selected": False,
        ...
        if any(_sha256_file(path) != input_hashes[name] for name, path in paths.items()):
            raise RuntimeError("NEXT304 input changed before publication")
        if any(_sha256_file(path) != source_hashes[name] for name, path in source_paths.items()):
            raise RuntimeError("NEXT304 source changed before publication")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    manifest = run_prcb_feature_audit(
