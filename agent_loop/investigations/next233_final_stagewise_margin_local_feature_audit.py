#!/usr/bin/env python3
"""Audit raw x0 features at the frozen final-stage NEXT232 frontier."""
...
import src.next232_stagewise_margin_local_broad_diagnostic as n232
from src.next102_cross_source_dobvr_features import _sha256_file, _write_json


n230 = n232.n231.n230
n207 = n230.n207
PROTOCOL = "2026-08-09-next233-final-stagewise-margin-local-feature-audit-v1"
MANIFEST_NAME = "MANIFEST.json"
CATALOGUE_NAME = "NEXT233_FINAL_STAGEWISE_MARGIN_LOCAL_FEATURE_CATALOGUE.json"
AUDIT_NAME = "NEXT233_FINAL_STAGEWISE_MARGIN_LOCAL_FEATURE_AUDIT.json"
TABLE_NAME = "next233_final_stagewise_margin_local_feature_audit.parquet"
EXPECTED_DESIGN_SHA256 = (
    "de85154f4c1566e1a812e6f01fd9d6deb0fb05a749cb9b963bc2ea89de01e8f9"
)
EXPECTED_BASE_CANDIDATE_KEY_SHA256 = (
    "6313dbca02ced34ec999f9c5b13616ad4e1d69d1ceef01bf3b3c57efe45ac913"
)
EXPECTED_BASE_THRESHOLD = 0.13768812689345988
EXPECTED_BASE_FAILED_COUNT = 5
EXPECTED_BASE_SHORTFALL = 0.12730274611313003
EXPECTED_BASE_SUPPORT_COUNT = n230.EXPECTED_BASE_SUPPORT_COUNT
EXPECTED_FEATURE_COUNT = n230.EXPECTED_FEATURE_COUNT
EXPECTED_FEATURE_NAME_SHA256 = n230.EXPECTED_FEATURE_NAME_SHA256
EXPECTED_HYPOTHESIS_COUNT = n230.EXPECTED_HYPOTHESIS_COUNT
PROTECTION_DIRECTIONS = n230.PROTECTION_DIRECTIONS
MINIMUM_COVERAGE = n230.MINIMUM_COVERAGE
MINIMUM_CLASS_COUNT = n230.MINIMUM_CLASS_COUNT
MINIMUM_AGGREGATE_AUC = n230.MINIMUM_AGGREGATE_AUC
MINIMUM_MACRO_AUC = n230.MINIMUM_MACRO_AUC
MINIMUM_WORST_AUC = n230.MINIMUM_WORST_AUC
EXPECTED_COHORT_COUNTS = {
    ...
    "wyformer:fold0": (74, 97),
    "wyformer:fold1": (58, 104),
    "wyformer:fold2": (66, 113),
    "wyformer:fold3": (71, 114),
    "wyformer:fold4": (60, 96),
}
REQUIRED_STAGES = (*n232.REQUIRED_STAGES, 232)
REQUIRED_DESIGN_STAGES = (*n232.REQUIRED_DESIGN_STAGES, 232)
BOUNDARY_FLAGS = n232.BOUNDARY_FLAGS
EXPECTED_NEXT232_SOURCE_SHA256 = (
    "aa1f9c4f77b1ab9f83f9c62bb0b3585d049fb05ffcb773034d12c590be05c8b9"
)
EXPECTED_NEXT232_CANDIDATE_COUNT = 450
EXPECTED_NEXT232_CANDIDATE_KEY_SHA256 = (
    "5a0f44bccbd7fcb57f7a79caf0c1dc6e25acd689cd5943cfd61f08c93254f199"
)
EXPECTED_INPUT_SHA256 = {
    **{
        key: value
        for key, value in n232.EXPECTED_INPUT_SHA256.items()
        if key != "design"
    },
    "next232_design": n232.EXPECTED_INPUT_SHA256["design"],
    "design": EXPECTED_DESIGN_SHA256,
    "next232_manifest": (
        "b6be058367c4e60a5106b4a1941c0b34e49e6980603d616d0b775a62085a1ef4"
    ),
    "next232_diagnostic": (
        "fd3664496d614969ae8d65646ba3e117f9428a23a55c27df9d57c7c1f28826b1"
    ),
    "next232_table": (
        "00202720a6ca79d261e467cd226443025d20675e0ffb531ee34bb33eb152cfa4"
    ),
}


select_auditable_features = n230.select_auditable_features
audit_one_source = n230.audit_one_source
select_residual_hypothesis = n230.select_residual_hypothesis
build_rejected_extreme_cohort = n230.build_rejected_extreme_cohort
...
def _paths(
    *,
    ...
    paths = n232._paths(
        ...
        design_paths={
            stage: design_paths[stage] for stage in n232.REQUIRED_DESIGN_STAGES
        },
        design_path=design_paths[232],
    )
    paths["next232_design"] = paths.pop("design")
    paths.update(
        {
            "design": design_path,
            "next232_manifest": roots["next232"] / n232.MANIFEST_NAME,
            "next232_diagnostic": roots["next232"] / n232.DIAGNOSTIC_NAME,
            "next232_table": roots["next232"] / n232.TABLE_NAME,
        }
    )
    return paths


def _verify_and_reconstruct_next232(
    paths: Mapping[str, Path], input_hashes: Mapping[str, str]
) -> tuple[pd.DataFrame, np.ndarray, np.ndarray, np.ndarray, dict[str, object]]:
    prior_paths = {key: paths[key] for key in n232.EXPECTED_INPUT_SHA256}
    prior_paths["design"] = paths["next232_design"]
    prior_hashes = {key: input_hashes[key] for key in n232.EXPECTED_INPUT_SHA256}
    prior_hashes["design"] = input_hashes["next232_design"]
    (
        eligible,
        eligible214,
        primary_key,
        ...
        eligible230,
        published231,
    ) = n232._verify_next231(prior_paths, prior_hashes)
    manifest = json.loads(paths["next232_manifest"].read_text())
    diagnostic = json.loads(paths["next232_diagnostic"].read_text())
    table = pd.read_parquet(paths["next232_table"])
    closest = dict(diagnostic["global_closest"])
    key = str(closest["candidate_key"])
    expected_outputs = {
        n232.DIAGNOSTIC_NAME: input_hashes["next232_diagnostic"],
        n232.TABLE_NAME: input_hashes["next232_table"],
    }
    population = n232.select_diagnostic_candidates(published231)
    if (
        manifest.get("protocol") != n232.PROTOCOL
        or manifest.get("candidate_count") != EXPECTED_NEXT232_CANDIDATE_COUNT
        or manifest.get("candidate_key_sha256")
        != EXPECTED_NEXT232_CANDIDATE_KEY_SHA256
        or manifest.get("strict_improvement_over_next229_diagnostic") is not True
        ...
        != EXPECTED_NEXT232_SOURCE_SHA256
        or _sha256_file(Path(n232.__file__).resolve())
        != EXPECTED_NEXT232_SOURCE_SHA256
        or any(manifest.get(k) is not value for k, value in BOUNDARY_FLAGS.items())
        or diagnostic.get("protocol") != n232.PROTOCOL
        or diagnostic.get("candidate_count") != EXPECTED_NEXT232_CANDIDATE_COUNT
        or n232.candidate_key_sha256(population)
        != EXPECTED_NEXT232_CANDIDATE_KEY_SHA256
        ...
        or len(table) != EXPECTED_NEXT232_CANDIDATE_COUNT
        or n232.candidate_key_sha256(table)
        != EXPECTED_NEXT232_CANDIDATE_KEY_SHA256
    ):
        raise ValueError("NEXT233 NEXT232 provenance differs")

    combined, _, score229, support, endpoint, _ = (
        n232.n231.n230._reconstruct_next229_frontier(
            paths=prior_paths,
            eligible=eligible,
    ...
    specs = n232.n231.build_stagewise_candidate_specs(
        base_candidate_key=str(closest229["candidate_key"]),
        eligible_hypotheses=eligible230,
        features=combined,
        base_score=score229,
        base_support=support,
    )
    selected_specs = [spec for spec in specs if str(spec["candidate_key"]) == key]
    if len(selected_specs) != 1:
        raise ValueError("NEXT233 NEXT232 specification differs")
    virtual, terms, runtime, activity = n232.n231.materialize_stagewise_candidates(
        features=combined,
        base_score=score229,
        base_support=support,
        specs=selected_specs,
    )
    n164 = n232.n231.n228.n223.n222.n215.n214.n164
    ...
    if (
        ...
        raise RuntimeError("NEXT233 NEXT232 support differs")
    expected = published231.loc[published231["candidate_key"].eq(key)]
    if len(expected) != 1:
        raise ValueError("NEXT233 NEXT231 published record differs")
    evaluator = (
        n232.n231.n228.n223.n222.n215.n214.n212.n210.n208.n205.n203.n202.n200.n194.n130.n125
    ...
    cells = n164.build_source_fold_cells(source=source, folds=folds)
    pauling_by_cell = {
    ...
    if tables is None:
        raise RuntimeError("NEXT233 NEXT232 threshold table differs")
    residual = n164.diagnose_broad_threshold_tables(
        tables=tables,
        cells=cells,
        pauling_by_cell=pauling_by_cell,
        safe_threshold=float(expected.iloc[0]["safe_threshold"]),
    )
    if (
        int(residual["failed_constraint_count"]) != EXPECTED_BASE_FAILED_COUNT
        or not math.isclose(
        ...
        raise ValueError("NEXT233 NEXT232 diagnostic reproduction differs")
    return combined, score, support, endpoint, activity[key]


def run_final_stagewise_margin_local_feature_audit(
    *,
    ...
    """Audit the exact NEXT232 rejected frontier without searching a law."""

    if not set(REQUIRED_STAGES).issubset(stage_dirs):
        raise ValueError("NEXT233 stage directory universe differs")
    if not set(REQUIRED_DESIGN_STAGES).issubset(design_paths):
        raise ValueError("NEXT233 design path universe differs")
    roots = {
    ...
    if any(not path.is_file() for path in paths.values()):
        raise FileNotFoundError("NEXT233 input is missing")
    ...
    if require_formal_inputs and input_hashes != EXPECTED_INPUT_SHA256:
        ...
        raise ValueError(f"NEXT233 formal input identity differs: {differing}")
    combined, score, support, endpoint, base_activity = (
        _verify_and_reconstruct_next232(paths, input_hashes)
    )
    ...
    if cohort_counts != EXPECTED_COHORT_COUNTS:
        raise ValueError("NEXT233 rejected cohort counts differ")
    ...
    if (
        ...
        raise ValueError("NEXT233 frozen feature universe differs")
    ...
    if len(raw_table) != EXPECTED_HYPOTHESIS_COUNT:
        raise RuntimeError("NEXT233 hypothesis count differs")
    ...
    audit = {
        ...
        "next234_search_authorized": bool(selected is not None),
    ...
    try:
        ...
        manifest = {
            ...
            "next232_diagnostic_population_reproduced": True,
            "next234_search_authorized": bool(selected is not None),
            "final_stagewise_margin_local_branch_terminated": selected is None,
            "new_formula_searched": False,
            "new_formula_selected": False,
        ...
        if any(_sha256_file(path) != input_hashes[name] for name, path in paths.items()):
            raise RuntimeError("NEXT233 input changed before publication")
        if any(_sha256_file(path) != source_hashes[name] for name, path in source_paths.items()):
            raise RuntimeError("NEXT233 source changed before publication")
...
def main() -> int:
    ...
    manifest = run_final_stagewise_margin_local_feature_audit(
