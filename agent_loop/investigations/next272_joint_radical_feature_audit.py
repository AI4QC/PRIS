#!/usr/bin/env python3
"""Audit the frozen joint radical-packing hypotheses on discovery labels."""
...
PROTOCOL = "2026-08-09-next272-joint-radical-feature-audit-v1"
MANIFEST_NAME = "MANIFEST.json"
CATALOGUE_NAME = "NEXT272_JOINT_RADICAL_FEATURE_CATALOGUE.json"
AUDIT_NAME = "NEXT272_JOINT_RADICAL_FEATURE_AUDIT.json"
TABLE_NAME = "next272_joint_radical_feature_audit.parquet"
EXPECTED_DESIGN_SHA256 = n271.EXPECTED_DESIGN_SHA256
HYPOTHESES = tuple(
    (name, direction)
    for name in n271.FEATURE_NAMES
...
EXPECTED_NEXT271_SOURCE_SHA256 = (
    "ad1197ab828558519c59bb68e7d69c2c8d0ffb6967f533c9113b8e77f0fc96ad"
)
NEXT271_SOURCE_PATH = "src/next271_joint_radical_packing_features.py"
REQUIRED_STAGES = (*n268.REQUIRED_STAGES, 271)
...
EXPECTED_INPUT_SHA256 = {
    **{
    ...
    "next271_manifest": (
        "9703aee963c065f57bbf59ba3cd9879394c35f8f168ae382c165794b918703fd"
    ),
    "next271_catalogue": (
        "be0b773d42ad2da69465595ad21dcd45a44e32a0588b7da9a6d13a5c3e8ddb26"
    ),
    "next271_scigen": (
        "73da0a6a1876490c53dd2df3a21e1cc14a0359deded8b8b4df9c12f7e458447a"
    ),
    "next271_wyformer": (
        "0d52704acb27dde38df0faf5b38a6f3093be69d653e77044fb7be66d53cbf2fa"
    ),
}
...
def select_eligible_hypotheses(
    ...
    try:
        ...
        raise ValueError("NEXT272 audit ranking table differs") from exc
...
def bounded_protection(
    *, values: object, direction: str, q_lo: float, q_hi: float
    ...
    try:
        ...
        raise ValueError("NEXT272 bounded protection inputs differ") from exc


def _index_joint_by_prefixed_material_id(
    *, table: pd.DataFrame, source: str, expected_material_ids: object
    ...
    if set(extra["material_id"]) != expected:
        raise ValueError(f"NEXT272 {source} material identity differs")
...
def _paths(
    *,
    ...
    paths.update(
        {
            "design": design_path,
            "next271_manifest": roots["next271"] / n271.MANIFEST_NAME,
            "next271_catalogue": roots["next271"] / n271.CATALOGUE_NAME,
            "next271_scigen": roots["next271"] / n271.FEATURE_FILES["scigen"],
            "next271_wyformer": roots["next271"] / n271.FEATURE_FILES["wyformer"],
        }
    )
    return paths
...
def _verify_next271(
    ...
    manifest = json.loads(paths["next271_manifest"].read_text())
    catalogue = json.loads(paths["next271_catalogue"].read_text())
    tables = {
        "scigen": pd.read_parquet(paths["next271_scigen"]),
        "wyformer": pd.read_parquet(paths["next271_wyformer"]),
    }
    expected_outputs = {
        n271.CATALOGUE_NAME: input_hashes["next271_catalogue"],
        n271.FEATURE_FILES["scigen"]: input_hashes["next271_scigen"],
        n271.FEATURE_FILES["wyformer"]: input_hashes["next271_wyformer"],
    }
    if (
        _sha256_file(Path(n268.__file__).resolve()) != EXPECTED_NEXT268_SOURCE_SHA256
        or _sha256_file(Path(n271.__file__).resolve()) != EXPECTED_NEXT271_SOURCE_SHA256
        or manifest.get("protocol") != n271.PROTOCOL
        or manifest.get("feature_names") != list(n271.FEATURE_NAMES)
        or manifest.get("feature_count") != len(n271.FEATURE_NAMES)
        or manifest.get("next267_support_preserved") is not True
        or manifest.get("next272_audit_authorized") is not True
        or manifest.get("validation_outputs_opened") is not False
        or manifest.get("replication_outputs_opened") is not False
        or manifest.get("outputs_sha256") != expected_outputs
        or manifest.get("executed_source_sha256", {}).get(NEXT271_SOURCE_PATH)
        != EXPECTED_NEXT271_SOURCE_SHA256
        or any(manifest.get(key) is not value for key, value in BOUNDARY_FLAGS.items())
        or catalogue.get("feature_names") != list(n271.FEATURE_NAMES)
        or catalogue.get("feature_count") != len(n271.FEATURE_NAMES)
        or catalogue.get("base_features")
        != ["prv_volume_ratio_cv", "prv_chebyshev_ratio_cv"]
        or catalogue.get("support_policy")
        != "EXACT_NEXT267_SUPPORT_NO_IMPUTATION"
        or catalogue.get("endpoint_or_label_input_accepted") is not False
    ):
        raise ValueError("NEXT272 NEXT271 provenance differs")
    for source, expected_rows in (("scigen", 13_470), ("wyformer", 5_232)):
        table = tables[source]
        supported = table["prvj_supported"].fillna(False).astype(bool)
        prv_supported = table["prv_supported"].fillna(False).astype(bool)
        ...
        if (
            ...
            or not np.array_equal(supported.to_numpy(), prv_supported.to_numpy())
            or int(supported.sum()) != expected_supported
            or not finite[supported].all()
            or finite[~supported].any()
        ):
            raise ValueError(f"NEXT272 NEXT271 {source} table differs")
    return (*prior[:-1], tables)


def run_joint_radical_feature_audit(
    *,
    ...
    """Audit the frozen 24 joint-radical hypotheses without formula search."""

    if not set(REQUIRED_STAGES).issubset(stage_dirs):
        raise ValueError("NEXT272 stage directory universe differs")
    if not set(REQUIRED_DESIGN_STAGES).issubset(design_paths):
        raise ValueError("NEXT272 design path universe differs")
    roots = {
    ...
    if any(not path.is_file() for path in paths.values()):
        raise FileNotFoundError("NEXT272 input is missing")
    ...
    if require_formal_inputs and input_hashes != EXPECTED_INPUT_SHA256:
        ...
        raise ValueError(f"NEXT272 formal input identity differs: {differing}")
    (
        eligible,
        eligible214,
        primary_key,
        ...
        formula222,
        joint_tables,
    ) = _verify_next271(paths, input_hashes)
    n227 = n268.n227
    ...
    for source_name in ("scigen", "wyformer"):
        indexed = _index_joint_by_prefixed_material_id(
    ...
    if cohort_counts != n227.EXPECTED_COHORT_COUNTS:
        raise ValueError("NEXT272 rejected cohort counts differ")
    rows: list[dict[str, object]] = []
    for feature, direction in HYPOTHESES:
        values = pd.to_numeric(combined[feature], errors="coerce").to_numpy(float)
        finite = values[np.isfinite(values)]
        if not len(finite):
            raise ValueError(f"NEXT272 {feature} has no finite discovery values")
        ...
        if not math.isfinite(q_lo) or not math.isfinite(q_hi) or q_hi <= q_lo:
            ...
            source_results = {
                source_name: n227.audit_one_source(
                    values=protection[source == source_name],
    ...
    catalogue = {
        ...
        "features": list(n271.FEATURE_NAMES),
    ...
    audit = {
        "protocol": PROTOCOL,
        "audit_mode": "fixed_next224_rejected_extreme_joint_radical_feature_audit",
        "base_candidate_key_sha256": n227.EXPECTED_BASE_CANDIDATE_KEY_SHA256,
        "base_threshold": n227.EXPECTED_BASE_THRESHOLD,
        "cohort_counts": {
        ...
        "next273_search_authorized": bool(selected is not None),
    ...
    source_paths = {
        ...
        NEXT271_SOURCE_PATH: Path(n271.__file__).resolve(),
        "src/next272_joint_radical_feature_audit.py": Path(__file__).resolve(),
    }
    ...
    try:
        ...
        manifest = {
            ...
            "next273_search_authorized": bool(selected is not None),
            "joint_radical_branch_terminated": selected is None,
            "new_formula_searched": False,
            "new_formula_selected": False,
        ...
        if any(_sha256_file(path) != input_hashes[name] for name, path in paths.items()):
            raise RuntimeError("NEXT272 input changed before publication")
        if any(_sha256_file(path) != source_hashes[name] for name, path in source_paths.items()):
            raise RuntimeError("NEXT272 source changed before publication")
...
def main() -> int:
    ...
    manifest = run_joint_radical_feature_audit(
