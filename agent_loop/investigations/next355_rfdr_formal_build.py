#!/usr/bin/env python3
"""Physically isolated formal feature build for frozen NEXT355 RFDR."""
...
PROTOCOL = "2026-08-13-next355-rfdr-formal-build-v1"
MANIFEST_NAME = "MANIFEST.json"
CATALOGUE_NAME = "NEXT355_RFDR_FEATURE_CATALOGUE.json"
FEATURE_FILES = {
    "scigen": "next355_scigen_radical_facet_deviatoric_rigidity.parquet",
    "wyformer": "next355_wyformer_radical_facet_deviatoric_rigidity.parquet",
}
EXPECTED_ROWS = {"scigen": 13_470, "wyformer": 5_232}
MINIMUM_FORMAL_COVERAGE = 0.90
EXPECTED_PROBE_SHA256 = "2f7accaa28e8e3995f3c80ac4205cc7998234791cd8ede1a62a6b70f9aae9021"
EXPECTED_INPUT_SHA256 = {
    ...
    "next351_source": "e86e105e611a2ab6b6773b8ee82ed20591d8944a2910db7fbdad34bc0a477af8",
    "next355_source": "431eb8323e12e61ad93c316ac599bd0d4bd122784151fe1354d954d11256d8a3",
...
def _error_row(exc: Exception) -> dict[str, object]:
    return {
        n355.FEATURE_NAMES[0]: math.nan,
        "rfdr_supported": False,
        "rfdr_failure": f"{type(exc).__name__}: {exc}",
        "rfdr_site_count": 0,
        "rfdr_edge_count": 0,
        "rfdr_minimum_facet_area": math.nan,
        "rfdr_maximum_reciprocal_area_relative_error": math.nan,
        "rfdr_maximum_orthogonality_residual": math.nan,
        "rfdr_volume_tiling_relative_error": math.nan,
        "rfdr_affine_minimum_eigenvalue": math.nan,
    }
...
def _compute_scigen_payload(item: tuple[str, bytes]) -> tuple[str, dict[str, object]]:
    ...
    try:
        ...
        return material_id, n355.compute_rfdr_row(atoms)
...
def _label_free_statistics(table) -> dict[str, object]:
    ...
    values = pd.to_numeric(table[n355.FEATURE_NAMES[0]], errors="coerce").dropna().to_numpy(float)
    if len(values) < 1:
        raise RuntimeError("NEXT355 label-free feature population is empty")
    return {
        n355.FEATURE_NAMES[0]: {
...
def build_cross_source_rfdr_features(
    *,
    ...
    """Build RFDR from physically isolated discovery geometry only."""
    ...
    paths = {
        ...
        "next351_source": Path(n351.__file__).resolve(),
        "next355_source": Path(n355.__file__).resolve(),
    }
    if os.path.lexists(target):
        raise FileExistsError(str(target))
    if type(workers) is not int or workers <= 0:
        raise ValueError("NEXT355 workers must be a positive exact integer")
    if any(not path.is_file() for path in paths.values()):
        raise FileNotFoundError("NEXT355 input is missing")
    ...
    if require_formal_inputs and input_hashes != EXPECTED_INPUT_SHA256:
        ...
        raise ValueError(f"NEXT355 formal input identity differs: {differing}")
    ...
    if (
        probe.get("protocol") != "2026-08-13-next355-rfdr-label-blind-probe-v1"
        or probe.get("design_sha256") != n355.DESIGN_SHA256
        ...
        raise ValueError("NEXT355 label-blind probe authorization differs")
    ...
    if (
        ...
        raise ValueError("NEXT355 discovery geometry provenance differs")
    ...
    for source, frame in metadata.items():
        ...
        if required - set(frame.columns) or frame["material_id"].astype(str).duplicated().any():
            raise ValueError(f"NEXT355 {source} metadata differs")
        ...
        if len(selected) != EXPECTED_ROWS[source]:
            raise ValueError(f"NEXT355 {source} discovery identity differs")
    ...
    executed_paths = {
        "src/next355_rfdr_formal_build.py": Path(__file__).resolve(),
        "src/next355_radical_facet_deviatoric_rigidity.py": Path(n355.__file__).resolve(),
        "experiments/next355_rfdr_label_blind_probe.py": root / "experiments/next355_rfdr_label_blind_probe.py",
        "tests/test_next355_radical_facet_deviatoric_rigidity.py": root / "tests/test_next355_radical_facet_deviatoric_rigidity.py",
        "tests/test_next355_rfdr_label_blind_probe.py": root / "tests/test_next355_rfdr_label_blind_probe.py",
        "tests/test_next355_rfdr_formal_build.py": root / "tests/test_next355_rfdr_formal_build.py",
    }
    ...
    try:
        ...
        for source in ("scigen", "wyformer"):
            ...
            if (
                ...
                raise RuntimeError(f"NEXT355 {source} material identity differs")
            ...
            finite = np.isfinite(pd.to_numeric(table[n355.FEATURE_NAMES[0]], errors="coerce").to_numpy(float))
            if not finite[supported].all() or finite[~supported].any():
                raise RuntimeError(f"NEXT355 {source} finite support semantics differ")
            coverage = float(supported.mean())
            if coverage < MINIMUM_FORMAL_COVERAGE:
                raise RuntimeError(f"NEXT355 {source} coverage below frozen minimum")
            statistics[source] = _label_free_statistics(table)
            if statistics[source][n355.FEATURE_NAMES[0]]["unique_rounded_10"] < 20:
                raise RuntimeError(f"NEXT355 {source} feature is degenerate")
            failures = Counter(table.loc[~supported, "rfdr_failure"].fillna("unknown"))
            counts[source] = {
                "rows": int(len(table)),
                "supported": int(supported.sum()),
                "failures": int((~supported).sum()),
                "coverage": coverage,
                "site_count": int(pd.to_numeric(table.loc[supported, "rfdr_site_count"]).sum()),
                "edge_count": int(pd.to_numeric(table.loc[supported, "rfdr_edge_count"]).sum()),
                "maximum_finite_orthogonality_residual": float(pd.to_numeric(table.loc[supported, "rfdr_maximum_orthogonality_residual"]).max()),
                "maximum_finite_reciprocal_area_relative_error": float(pd.to_numeric(table.loc[supported, "rfdr_maximum_reciprocal_area_relative_error"]).max()),
                "maximum_finite_volume_tiling_relative_error": float(pd.to_numeric(table.loc[supported, "rfdr_volume_tiling_relative_error"]).max()),
        ...
        catalogue = {
            "protocol": PROTOCOL,
            "feature_count": 1,
            "feature_names": list(n355.FEATURE_NAMES),
            "feature_directions": n355.FEATURE_DIRECTIONS,
            "directions_frozen_before_outcome": True,
            "graph": "NEXT339 all reciprocal radius-weighted radical facets with A/d weights",
            "formula": "lambda_min(H0^-1/2 D.T(I-UU+)D H0^-1/2)",
            "strain_space": "five-dimensional symmetric trace-free",
            "output_grid": 1.0 / n355.OUTPUT_GRID,
            "minimum_formal_coverage": MINIMUM_FORMAL_COVERAGE,
            "label_free_statistics": statistics,
            "next356_audit_authorized": True,
            **n355.BOUNDARY_FLAGS,
        }
        ...
        manifest = {
            "protocol": PROTOCOL,
            "mode": "physically_isolated_discovery_x0_radical_facet_deviatoric_rigidity_freeze",
            "workers": workers,
        ...
        if any(n347._sha256_file(path) != executed_hashes[name] for name, path in executed_paths.items()):
            raise RuntimeError("NEXT355 executed artifact changed before publication")
...
__all__ = [
    "CATALOGUE_NAME", "EXPECTED_ROWS", "FEATURE_FILES", "MANIFEST_NAME",
    "MINIMUM_FORMAL_COVERAGE", "PROTOCOL", "_compute_wyformer_payload",
    "_label_free_statistics", "build_cross_source_rfdr_features", "n267", "n355",
]
