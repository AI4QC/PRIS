"""Bounded NEXT36 development scan and immutable label-free rule replay."""
...
PROTOCOL = "2026-08-03-next36-weighted-charge-spectrum-rule-v1"
SCAN_NAME = "next36_development_scan.parquet"
FROZEN_RULE_NAME = "NEXT36_FROZEN_CHARGE_SPECTRUM_RULE.json"
PREDICTIONS_NAME = "next36_predictions.parquet"
...
TERM_CATALOGUE: Mapping[str, Mapping[str, object]] = {
    "csf_gaussian_t025_high": _term("csf_gaussian_t025"),
    "csf_gaussian_t040_high": _term("csf_gaussian_t040"),
    "csf_gaussian_t060_high": _term("csf_gaussian_t060"),
    "csf_long_fraction_high": _term("csf_long_fraction"),
    "csf_long_peak_fraction_high": _term("csf_long_peak_fraction"),
    "csf_long_anisotropy_high": _term("csf_long_anisotropy"),
...
MECHANISM_PAIRS = (
    ("csf_gaussian_t025_high", "steric_rep12_vector_max_high"),
    ("csf_gaussian_t040_high", "steric_rep12_vector_rms_high"),
    ("csf_gaussian_t060_high", "sivr_site_imbalance_high"),
    ("csf_long_fraction_high", "sivr_site_imbalance_high"),
    ("csf_long_anisotropy_high", "steric_rep12_vector_rms_high"),
    ("csf_long_peak_fraction_high", "aefi_residual_max_high"),
...
def scan_development_candidates(
    ...
    """Evaluate the exact frozen NEXT36 catalogue on exposed development rows."""
    ...
    rule: dict[str, object] = {
        ...
        "weighted_charge_spectrum_used": any(term.startswith("csf_") for term in terms),
...
def compute_charge_spectrum_risk(
    features: pd.DataFrame, rule: Mapping[str, object]
) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """Apply one frozen NEXT36 rule without endpoint access or refitting."""

    if rule.get("protocol") != PROTOCOL or rule.get("eligible") is not True:
        raise ValueError("frozen NEXT36 rule is not eligible")
    ...
    if not isinstance(raw_terms, list) or not raw_terms or len(raw_terms) > 2:
        raise ValueError("frozen NEXT36 terms are invalid")
    if not isinstance(raw_parameters, Mapping):
        raise ValueError("frozen NEXT36 term parameters are invalid")
    terms = tuple(str(term) for term in raw_terms)
    if terms not in CANDIDATE_TERM_SETS or set(raw_parameters) != set(terms):
        raise ValueError("frozen NEXT36 formula is not in the catalogue")
    ...
    for term in terms:
        ...
        if any(
            ...
            raise ValueError("frozen NEXT36 term specification changed")
    ...
    if not np.isfinite(threshold):
        raise ValueError("frozen NEXT36 threshold is invalid")
    reject = support & (score >= threshold)
...
def _validate_feature_artifact(
    ...
    if feature_path.name != FEATURE_NAME or not feature_path.is_file():
        raise ValueError("NEXT36 feature path/name is invalid")
    manifest = _strict_json(manifest_path, role="NEXT36 feature manifest")
    ...
    if (
        ...
        or manifest.get("weighted_charge_spectrum_used") is not True
        or manifest.get("thermodynamic_limit_hyperuniformity_claimed") is not False
        ...
        raise ValueError("NEXT36 feature artifact crossed the label-free boundary")
    ...
    if forbidden:
        raise ValueError(
            f"NEXT36 feature columns crossed the label-free boundary: {forbidden}"
        )
    if (
        ...
        raise ValueError("NEXT36 feature material IDs are invalid")
    frame = frame.copy()
...
def freeze_development_rule(
    *,
    ...
    if paths["endpoints"].name != ENDPOINT_NAME or not paths["endpoints"].is_file():
        raise ValueError("NEXT36 development endpoint path/name is invalid")
    endpoint_manifest = _strict_json(
        paths["endpoints_manifest"], role="NEXT36 development endpoint manifest"
    )
    ...
    if (
        ...
        raise ValueError("NEXT36 development endpoint artifact is invalid")
    ...
    manifest: dict[str, object] = {
        "protocol": PROTOCOL,
        "mode": "exposed_rattled_relax_next36_development_scan",
        ...
        "executed_source_sha256": {
            "src/next36_charge_spectrum_rule.py": source_hash
        },
    ...
    try:
        ...
        if any(_sha256(path) != input_hashes[role] for role, path in paths.items()):
            raise RuntimeError("NEXT36 development input changed before publication")
        if _sha256(source_path) != source_hash:
            raise RuntimeError("NEXT36 rule source changed before publication")
...
def apply_frozen_rule(
    *,
    ...
    """Publish label-free NEXT36 predictions for one or more sealed cohorts."""
    ...
    if (
        ...
        raise ValueError("NEXT36 rule application inputs are invalid")
    rule_manifest = _strict_json(rule_manifest_path, role="NEXT36 frozen-rule manifest")
    ...
    if (
        ...
        raise ValueError("NEXT36 frozen rule is not promoted and hash-locked")
    try:
        rule = json.loads(rule_path.read_text("utf-8"))
    except (OSError, UnicodeError, json.JSONDecodeError) as exc:
        raise ValueError("NEXT36 frozen rule JSON is invalid") from exc
    if not isinstance(rule, dict):
        raise ValueError("NEXT36 frozen rule JSON is invalid")
    ...
    for feature, feature_manifest in zip(features, feature_manifests, strict=True):
        ...
        score, supported, reject = compute_charge_spectrum_risk(frame, rule)
        source = (
    ...
    if predictions.material_id.duplicated().any():
        raise ValueError("NEXT36 prediction material IDs are duplicated")
    source_path = Path(__file__).resolve()
    source_hash = _sha256(source_path)
    ...
    manifest: dict[str, object] = {
        "protocol": PROTOCOL,
        "mode": "prospective_label_free_charge_spectrum_prediction",
        "labels_opened": False,
    ...
    try:
        ...
        if _sha256(rule_path) != input_hashes["frozen_rule"] or _sha256(
            ...
            raise RuntimeError("NEXT36 frozen rule changed before publication")
        for item in feature_inputs:
            if _sha256(Path(item["feature_path"])) != item["feature_sha256"] or _sha256(
                Path(item["manifest_path"])
            ) != item["manifest_sha256"]:
                raise RuntimeError("NEXT36 feature changed before publication")
        if _sha256(source_path) != source_hash:
            raise RuntimeError("NEXT36 rule source changed before publication")
        _publish_directory_no_replace(staging, target)
    finally:
