"""Bounded NEXT37 development scan and immutable label-free rule replay."""
...
PROTOCOL = "2026-08-03-next37-self-stress-compatibility-rule-v1"
SCAN_NAME = "next37_development_scan.parquet"
FROZEN_RULE_NAME = "NEXT37_FROZEN_SELF_STRESS_COMPATIBILITY_RULE.json"
PREDICTIONS_NAME = "next37_predictions.parquet"
...
TERM_CATALOGUE: Mapping[str, Mapping[str, object]] = {
    "sscp_load_fraction_high": _term("sscp_load_fraction"),
    "sscp_load_rms_high": _term("sscp_load_rms"),
    "sscp_load_q95_high": _term("sscp_load_q95"),
    "sscp_atomic_load_fraction_high": _term("sscp_atomic_load_fraction"),
    "sscp_cell_load_fraction_high": _term("sscp_cell_load_fraction"),
    "sscp_load_localization_high": _term("sscp_load_localization"),
...
MECHANISM_PAIRS = (
    ("sscp_load_fraction_high", "sivr_site_imbalance_high"),
    ("sscp_load_rms_high", "steric_rep12_vector_rms_high"),
    ("sscp_load_q95_high", "steric_rep12_vector_max_high"),
    ("sscp_atomic_load_fraction_high", "sivr_site_imbalance_high"),
    ("sscp_cell_load_fraction_high", "aefi_residual_max_high"),
    ("sscp_load_localization_high", "steric_rep12_vector_max_high"),
...
def scan_development_candidates(
    ...
    """Evaluate the exact frozen NEXT37 catalogue on exposed development rows."""
    ...
    rule: dict[str, object] = {
        ...
        "self_stress_compatibility_projection_used": any(
            term.startswith("sscp_") for term in terms
        ),
...
def compute_self_stress_compatibility_risk(
    features: pd.DataFrame, rule: Mapping[str, object]
) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """Apply one frozen NEXT37 rule without endpoint access or refitting."""

    if rule.get("protocol") != PROTOCOL or rule.get("eligible") is not True:
        raise ValueError("frozen NEXT37 rule is not eligible")
    ...
    if not isinstance(raw_terms, list) or not raw_terms or len(raw_terms) > 2:
        raise ValueError("frozen NEXT37 terms are invalid")
    if not isinstance(raw_parameters, Mapping):
        raise ValueError("frozen NEXT37 term parameters are invalid")
    terms = tuple(str(term) for term in raw_terms)
    if terms not in CANDIDATE_TERM_SETS or set(raw_parameters) != set(terms):
        raise ValueError("frozen NEXT37 formula is not in the catalogue")
    ...
    for term in terms:
        ...
        if any(
            ...
            raise ValueError("frozen NEXT37 term specification changed")
    ...
    if not np.isfinite(threshold):
        raise ValueError("frozen NEXT37 threshold is invalid")
    reject = support & (score >= threshold)
...
def _validate_feature_artifact(
    ...
    if feature_path.name != FEATURE_NAME or not feature_path.is_file():
        raise ValueError("NEXT37 feature path/name is invalid")
    manifest = _strict_json(manifest_path, role="NEXT37 feature manifest")
    ...
    if (
        ...
        or manifest.get("self_stress_compatibility_projection_used") is not True
        ...
        raise ValueError("NEXT37 feature artifact crossed the label-free boundary")
    ...
    if forbidden:
        raise ValueError(f"NEXT37 feature columns crossed the label-free boundary: {forbidden}")
    if (
        "material_id" not in frame
        or frame.material_id.isna().any()
        or frame.material_id.astype(str).duplicated().any()
    ):
        raise ValueError("NEXT37 feature material IDs are invalid")
    frame = frame.copy()
...
def freeze_development_rule(
    *,
    ...
    if paths["endpoints"].name != ENDPOINT_NAME or not paths["endpoints"].is_file():
        raise ValueError("NEXT37 development endpoint path/name is invalid")
    endpoint_manifest = _strict_json(
        paths["endpoints_manifest"], role="NEXT37 development endpoint manifest"
    )
    ...
    if (
        ...
        raise ValueError("NEXT37 development endpoint artifact is invalid")
    ...
    manifest: dict[str, object] = {
        "protocol": PROTOCOL,
        "mode": "exposed_rattled_relax_next37_development_scan",
        ...
        "executed_source_sha256": {
            "src/next37_self_stress_compatibility_rule.py": source_hash
        },
    ...
    try:
        ...
        if any(_sha256(path) != input_hashes[role] for role, path in paths.items()):
            raise RuntimeError("NEXT37 development input changed before publication")
        if _sha256(source_path) != source_hash:
            raise RuntimeError("NEXT37 rule source changed before publication")
...
def apply_frozen_rule(
    *,
    ...
    """Publish label-free NEXT37 predictions for one or more sealed cohorts."""
    ...
    if (
        ...
        raise ValueError("NEXT37 rule application inputs are invalid")
    rule_manifest = _strict_json(rule_manifest_path, role="NEXT37 frozen-rule manifest")
    ...
    if (
        ...
        raise ValueError("NEXT37 frozen rule is not promoted and hash-locked")
    try:
        rule = json.loads(rule_path.read_text("utf-8"))
    except (OSError, UnicodeError, json.JSONDecodeError) as exc:
        raise ValueError("NEXT37 frozen rule JSON is invalid") from exc
    if not isinstance(rule, dict):
        raise ValueError("NEXT37 frozen rule JSON is invalid")
    ...
    for feature, feature_manifest in zip(features, feature_manifests, strict=True):
        ...
        score, supported, reject = compute_self_stress_compatibility_risk(frame, rule)
        source = (
    ...
    if predictions.material_id.duplicated().any():
        raise ValueError("NEXT37 prediction material IDs are duplicated")
    source_path = Path(__file__).resolve()
    source_hash = _sha256(source_path)
    ...
    manifest: dict[str, object] = {
        "protocol": PROTOCOL,
        "mode": "prospective_label_free_compatibility_prediction",
        "labels_opened": False,
    ...
    try:
        ...
        if _sha256(rule_path) != input_hashes["frozen_rule"] or _sha256(
            ...
            raise RuntimeError("NEXT37 frozen rule changed before publication")
        for item in feature_inputs:
            if _sha256(Path(item["feature_path"])) != item["feature_sha256"] or _sha256(
                Path(item["manifest_path"])
            ) != item["manifest_sha256"]:
                raise RuntimeError("NEXT37 feature changed before publication")
        if _sha256(source_path) != source_hash:
            raise RuntimeError("NEXT37 rule source changed before publication")
        _publish_directory_no_replace(staging, target)
    finally:
...
__all__ = [
    ...
    "TERM_CATALOGUE",
    "apply_frozen_rule",
    "compute_self_stress_compatibility_risk",
