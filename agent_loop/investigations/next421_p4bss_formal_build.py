#!/usr/bin/env python3
"""Physically isolated full discovery build for frozen NEXT420 P4BSS."""
...
PROTOCOL = "2026-08-13-next421-p4bss-formal-build-v1"
MANIFEST_NAME = "MANIFEST.json"
CATALOGUE_NAME = "NEXT421_P4BSS_FEATURE_CATALOGUE.json"
FEATURE_FILES = {
    "scigen": "next421_scigen_pauling4_bond_strength_segregation.parquet",
    "wyformer": "next421_wyformer_pauling4_bond_strength_segregation.parquet",
}
EXPECTED_ROWS = {"scigen": 13_470, "wyformer": 5_232}
MINIMUM_FORMAL_COVERAGE = 0.95
EXPECTED_PROBE_SHA256 = "110aa0604dc63140617c5172241aa21f9c63e0835a4c0091d18af354d27f7662"
EXPECTED_INPUT_SHA256 = {
    "design": "10af2eea8312f0010fd7913edcc68d7ee723173fedd6642f35dc6695daaf7a96",
    "probe_result": EXPECTED_PROBE_SHA256,
    "next267_source": "8f1e7ed9eb73a81a5755d455ffc05aab6f539cbd66afbbbfc384ca88391adca1",
    "next420_source": "6f931f3292d5d3d45b036b71ab85f39fd35587eadfc1a501905547db4c99062f",
...
def validate_probe_authorization(probe: dict[str, object]) -> None:
    ...
    if (
        probe.get("protocol") != "2026-08-13-next420-p4bss-label-blind-probe-v1"
        or probe.get("design_sha256") != n420.DESIGN_SHA256
        or probe.get("minimum_novelty_joint_finite") != 40
        or probe.get("probe_passed") is not True
        or probe.get("next421_formal_build_authorized") is not True
        or not isinstance(gates, dict)
        or set(gates.values()) != {True}
        or any(probe.get(name) is not False for name in PROBE_BOUNDARY_NAMES)
    ):
        raise ValueError("NEXT421 label-blind probe authorization differs")


def _error_row(exc: Exception) -> dict[str, object]:
    return {
        n420.FEATURE_NAMES[0]: math.nan,
        "p4bss_supported": False,
        "p4bss_failure": f"{type(exc).__name__}: {exc}",
        "p4bss_site_count": 0,
        "p4bss_edge_count": 0,
        "p4bss_pair_count": 0,
        "p4bss_expected_product": math.nan,
        "p4bss_observed_product": math.nan,
        "p4bss_valence_policy": None,
    }
...
def _compute_scigen_payload(item):
    ...
    try:
        ...
        return material_id, n420.compute_p4bss_row(atoms)
...
def label_free_statistics(table: pd.DataFrame) -> dict[str, object]:
    values = pd.to_numeric(table[n420.FEATURE_NAMES[0]], errors="coerce").dropna().to_numpy(float)
    if len(values) < 1:
        raise RuntimeError("NEXT421 label-free feature population is empty")
    return {
        n420.FEATURE_NAMES[0]: {
...
def build_cross_source_p4bss_features(
    *, scigen_cohort_dir: Path, wyformer_cohort_dir: Path, design_path: Path,
    ...
    paths = {
        ...
        "next420_source": Path(n420.__file__).resolve(),
    }
    if os.path.lexists(target):
        raise FileExistsError(str(target))
    if type(workers) is not int or workers <= 0:
        raise ValueError("NEXT421 workers must be a positive exact integer")
    if any(not path.is_file() for path in paths.values()):
        raise FileNotFoundError("NEXT421 input is missing")
    ...
    if require_formal_inputs and input_hashes != EXPECTED_INPUT_SHA256:
        ...
        raise ValueError(f"NEXT421 formal input identity differs: {differing}")
    ...
    if (
        ...
        raise ValueError("NEXT421 discovery geometry provenance differs")
    metadata = {
    ...
    for source, frame in metadata.items():
        if required - set(frame) or frame["material_id"].astype(str).duplicated().any():
            raise ValueError(f"NEXT421 {source} metadata differs")
        ...
        if len(selected) != EXPECTED_ROWS[source]:
            raise ValueError(f"NEXT421 {source} discovery identity differs")
    ...
    executed_paths = {
        "src/next421_p4bss_formal_build.py": Path(__file__).resolve(),
        "src/next420_pauling4_bond_strength_segregation.py": Path(n420.__file__).resolve(),
        "experiments/next420_p4bss_label_blind_probe.py": root / "experiments/next420_p4bss_label_blind_probe.py",
        "tests/test_next420_pauling4_bond_strength_segregation.py": root / "tests/test_next420_pauling4_bond_strength_segregation.py",
        "tests/test_next420_p4bss_label_blind_probe.py": root / "tests/test_next420_p4bss_label_blind_probe.py",
        "tests/test_next421_p4bss_formal_build.py": root / "tests/test_next421_p4bss_formal_build.py",
    }
    ...
    try:
        ...
        for source in ("scigen", "wyformer"):
            ...
            if computed_frame["material_id"].astype(str).duplicated().any() or set(computed_frame["material_id"].astype(str)) != set(discovery[source]["material_id"].astype(str)):
                raise RuntimeError(f"NEXT421 {source} material identity differs")
            ...
            finite = np.isfinite(pd.to_numeric(table[n420.FEATURE_NAMES[0]], errors="coerce").to_numpy(float))
            if not finite[supported].all() or finite[~supported].any():
                raise RuntimeError(f"NEXT421 {source} finite support semantics differ")
            coverage = float(supported.mean())
            if coverage < MINIMUM_FORMAL_COVERAGE:
                raise RuntimeError(f"NEXT421 {source} coverage below frozen minimum")
            statistics[source] = label_free_statistics(table)
            if statistics[source][n420.FEATURE_NAMES[0]]["unique_rounded_10"] < 20:
                raise RuntimeError(f"NEXT421 {source} feature is degenerate")
            failures = Counter(table.loc[~supported, "p4bss_failure"].fillna("unknown"))
            counts[source] = {
                "rows": int(len(table)), "supported": int(supported.sum()),
                "failures": int((~supported).sum()), "coverage": coverage,
                "site_count": int(pd.to_numeric(table.loc[supported, "p4bss_site_count"]).sum()),
                "edge_count": int(pd.to_numeric(table.loc[supported, "p4bss_edge_count"]).sum()),
                "pair_count": int(pd.to_numeric(table.loc[supported, "p4bss_pair_count"]).sum()),
        ...
        catalogue = {
            ...
            "feature_names": list(n420.FEATURE_NAMES),
            "feature_directions": n420.FEATURE_DIRECTIONS,
            "directions_frozen_before_outcome": True,
            "graph": "opposite-sign periodic Voronoi",
            "formula": "mean(stub_strength)^2 / (mean(stub_strength)^2 + mean(co-anion strength product))",
            "output_grid": 1.0 / n420.OUTPUT_GRID,
            "minimum_formal_coverage": MINIMUM_FORMAL_COVERAGE,
            "label_free_statistics": statistics,
            "next422_audit_authorized": True,
            **n420.BOUNDARY_FLAGS,
        }
        ...
        manifest = {
            "protocol": PROTOCOL,
            "mode": "physically_isolated_discovery_x0_pauling4_bond_strength_segregation_freeze",
            ...
            "scientific_improvement_claim": False, "next422_audit_authorized": True,
            **n420.BOUNDARY_FLAGS, "inputs_sha256": input_hashes,
        ...
        if any(n347._sha256_file(path) != executed_hashes[name] for name, path in executed_paths.items()):
            raise RuntimeError("NEXT421 executed artifact changed before publication")
...
def main() -> int:
    ...
    manifest = build_cross_source_p4bss_features(
...
__all__ = [
    ...
    "build_cross_source_p4bss_features", "label_free_statistics", "n420",
