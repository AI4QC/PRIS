#!/usr/bin/env python3
"""Frozen one-term DVCI margin-local search on the NEXT224 frontier."""
...
PROTOCOL = "2026-08-09-next257-dvci-margin-local-search-v1"
MANIFEST_NAME = "MANIFEST.json"
CATALOGUE_NAME = "NEXT257_DVCI_MARGIN_LOCAL_CATALOGUE.json"
EVALUATION_NAME = "NEXT257_DISCOVERY_EVALUATION.json"
FORMULA_NAME = "NEXT257_FROZEN_CANDIDATE.json"
SEARCH_NAME = "next257_dvci_margin_local_search.parquet"
SCORE_COMPOSITION = "nonnegative_next224_plus_triangular_margin_local_signed_dvci_term"
...
EXPECTED_ELIGIBLE_HYPOTHESIS_COUNT = 2
EXPECTED_ELIGIBLE_HYPOTHESIS_SHA256 = (
    "e0ca04d38207ffa4d069cc8702eed559e532b218f5408000ee74bedb53491806"
)
...
EXPECTED_DESIGN_SHA256 = n256.EXPECTED_DESIGN_SHA256
...
BOUNDARY_FLAGS = n256.BOUNDARY_FLAGS
REQUIRED_STAGES = (*n256.REQUIRED_STAGES, 256)
REQUIRED_DESIGN_STAGES = (*n256.REQUIRED_DESIGN_STAGES, 256)
EXPECTED_NEXT256_SOURCE_SHA256 = (
    "1a18cd6fea41316cf096f9a478324a8033d371c010a948025cef62442020162d"
)
EXPECTED_INPUT_SHA256 = {
    **{
        key: value
        for key, value in n256.EXPECTED_INPUT_SHA256.items()
        if key != "design"
    },
    "next256_design": n256.EXPECTED_INPUT_SHA256["design"],
    "design": EXPECTED_DESIGN_SHA256,
    "next256_manifest": (
        "f0615e8fce1fa7daf430f4f80274715a33501c03fefc3dc7bb9c6526cf688ce8"
    ),
    "next256_catalogue": (
        "21d8d71994bb43c0df2a735e4a7bd6139c1845ed487fff7d7e7aa173d729cc28"
    ),
    "next256_audit": (
        "6ab2fd4edee5d212a8dbbebe96a5bfc742ab988b2ba32f8f370216a7fa15c891"
    ),
    "next256_table": (
        "d87fd0cee2378ccd59180f4291ef174aad1c1b6d0226e6725aa4c1ab209753c8"
    ),
}


def dvci_margin_local_score(
    *,
    base_score: object,
    base_support: object,
    protection: object,
    ...
    if (
        ...
        raise ValueError("NEXT257 margin-local score inputs differ")
    h = fraction * width
...
def _grid_numerator(value: float, denominator: int, label: str) -> int:
    ...
    if numerator <= 0 or not math.isclose(
        ...
        raise ValueError(f"NEXT257 {label} grid differs")
    return numerator


def build_dvci_candidate_specs(
    *,
    ...
    if not math.isfinite(cutoff) or not math.isfinite(width) or width <= 0.0:
        raise ValueError("NEXT257 base population differs")
    ...
    if (
        not fractions
        or fractions != tuple(value for value in LOCAL_WIDTH_FRACTIONS if value in fractions)
        or len(fractions) != len(set(fractions))
        or not amplitudes
        or amplitudes != tuple(value for value in AMPLITUDE_FRACTIONS if value in amplitudes)
        or len(amplitudes) != len(set(amplitudes))
    ):
        raise ValueError("NEXT257 frozen grid differs")
    common = {
        "base_candidate_key_sha256": hashlib.sha256(base_candidate_key.encode()).hexdigest(),
    ...
    for _, row in eligible.iterrows():
        ...
        if (
            ...
            raise ValueError("NEXT257 eligible hypothesis identity differs")
    ...
    if len({str(spec["candidate_key"]) for spec in specs}) != len(specs):
        raise RuntimeError("NEXT257 candidate keys are not unique")
    return specs


def materialize_dvci_candidates(
    *,
    ...
    if (
        ...
        or np.any(~np.isfinite(base[support]))
        or not raw_specs
        or len({str(spec.get("candidate_key", "")) for spec in raw_specs}) != len(raw_specs)
        or "source_dataset" not in features.columns
    ):
        raise ValueError("NEXT257 materializer inputs differ")
    ...
    for spec in raw_specs:
        ...
        if feature is None:
            ...
            if feature not in features.columns:
                raise ValueError("NEXT257 materializer feature differs")
            ...
            score, got_support, active, _ = dvci_margin_local_score(
                base_score=base,
        ...
        if not np.array_equal(got_support, support):
            raise RuntimeError("NEXT257 support changed")
        ...
        term_id = "next257_virtual_candidate__" + hashlib.sha256(key.encode()).hexdigest()[:24]
        ...
        terms.append(
            {
                ...
                "center": 0.0,
                "scale": 1.0 / divisor,
                "group": "next257_dvci_margin_local",
...
def _paths(
    *,
    ...
    paths = n256._paths(
        ...
        design_paths={
            stage: design_paths[stage] for stage in n256.REQUIRED_DESIGN_STAGES
        },
        design_path=design_paths[256],
    )
    paths["next256_design"] = paths.pop("design")
    paths.update(
        {
            "design": design_path,
            "next256_manifest": roots["next256"] / n256.MANIFEST_NAME,
            "next256_catalogue": roots["next256"] / n256.CATALOGUE_NAME,
            "next256_audit": roots["next256"] / n256.AUDIT_NAME,
            "next256_table": roots["next256"] / n256.TABLE_NAME,
        }
    )
    return paths
...
def _verify_next256(
    ...
    base_paths = {key: paths[key] for key in n256.EXPECTED_INPUT_SHA256}
    base_paths["design"] = paths["next256_design"]
    base_hashes = {key: input_hashes[key] for key in n256.EXPECTED_INPUT_SHA256}
    base_hashes["design"] = input_hashes["next256_design"]
    prior = n256._verify_next255(base_paths, base_hashes)
    manifest = json.loads(paths["next256_manifest"].read_text())
    catalogue = json.loads(paths["next256_catalogue"].read_text())
    audit = json.loads(paths["next256_audit"].read_text())
    table = pd.read_parquet(paths["next256_table"])
    ...
    expected_outputs = {
        n256.CATALOGUE_NAME: input_hashes["next256_catalogue"],
        n256.AUDIT_NAME: input_hashes["next256_audit"],
        n256.TABLE_NAME: input_hashes["next256_table"],
    }
    if (
        manifest.get("protocol") != n256.PROTOCOL
        ...
        or manifest.get("next257_search_authorized") is not True
        ...
        or manifest.get("executed_source_sha256", {}).get("src/next256_dvci_feature_audit.py")
        != EXPECTED_NEXT256_SOURCE_SHA256
        or _sha256_file(Path(n256.__file__).resolve()) != EXPECTED_NEXT256_SOURCE_SHA256
        ...
        or audit.get("next257_search_authorized") is not True
        ...
        raise ValueError("NEXT257 NEXT256 provenance differs")
...
def _formula_from_spec(spec: Mapping[str, object] | None) -> dict[str, object]:
    ...
    return {
        "protocol": PROTOCOL,
        "kind": "dvci_triangular_margin_local_x0_no_dft_score",
        "selected": True,
...
def select_best_new_record(records: pd.DataFrame) -> pd.Series | None:
    ...
    if not isinstance(records, pd.DataFrame) or required - set(records.columns):
        raise ValueError("NEXT257 reporting selection schema differs")
    eligible = records.loc[records["eligible_new_candidate"].fillna(False).astype(bool)]
    auc_safe = (
...
def _attach_dvci_features(
    *,
    combined: pd.DataFrame,
    feature_tables: Mapping[str, pd.DataFrame],
    dvci_tables: Mapping[str, pd.DataFrame],
) -> None:
    source = combined["source_dataset"].astype(str).to_numpy()
    for source_name in ("scigen", "wyformer"):
        indexed = n256._index_dvci_by_prefixed_material_id(
            table=dvci_tables[source_name],
            source=source_name,
        ...
        for name in n256.n255.FEATURE_NAMES:
            combined.loc[mask, name] = ordered_ids.map(indexed[name]).to_numpy()


def run_dvci_margin_local_search(
    *,
    ...
    """Run the complete frozen discovery-only NEXT257 search."""

    if not set(REQUIRED_STAGES).issubset(stage_dirs):
        raise ValueError("NEXT257 stage directory universe differs")
    if not set(REQUIRED_DESIGN_STAGES).issubset(design_paths):
        raise ValueError("NEXT257 design path universe differs")
    roots = {
    ...
    if type(search_workers) is not int or search_workers <= 0:
        raise ValueError("NEXT257 search_workers must be a positive exact integer")
    if any(not path.is_file() for path in paths.values()):
        raise FileNotFoundError("NEXT257 input is missing")
    ...
    if require_formal_inputs and input_hashes != EXPECTED_INPUT_SHA256:
        ...
        raise ValueError(f"NEXT257 formal input identity differs: {differing}")
    (
        eligible_prior,
        eligible214,
        primary_key,
        ...
        dvci_tables,
        eligible256,
    ) = _verify_next256(paths, input_hashes)
    ...
    if (
        ...
        raise ValueError("NEXT257 NEXT224 base identity differs")
    specs = build_dvci_candidate_specs(
    ...
    if len(specs) != EXPECTED_CANDIDATE_COUNT or eligible_count != EXPECTED_ELIGIBLE_COUNT:
        raise RuntimeError("NEXT257 frozen candidate universe differs")
    virtual, terms, runtime, activity = materialize_dvci_candidates(
        features=combined,
    ...
    if (
        ...
        raise RuntimeError("NEXT257 evaluator accounting differs")
    ...
    records = pd.DataFrame(raw_records).sort_values("candidate_key", kind="mergesort").reset_index(drop=True)
    ...
    if len(no_op) != 1 or len(reference_no_op) != 1:
        raise RuntimeError("NEXT257 no-op reproduction identity differs")
    ...
    if selected_row is not None:
        ...
        if str(selected_result["selected"]["record"]["candidate_key"]) != selected_key:
            raise RuntimeError("NEXT257 selected candidate reproduction differs")
        ...
        if selected_spec.get("eligible_new_candidate") is not True:
            raise RuntimeError("NEXT257 reproduction control was selected")
    ...
    if selected is not None and passes != bool(selected["record"]["passes_all_discovery_gates"]):
        raise RuntimeError("NEXT257 all-gate selection differs")
    if passes and selected is None:
        raise RuntimeError("NEXT257 all-gate candidate was not selected")
    ...
    catalogue = {
        ...
        "base_threshold": EXPECTED_BASE_THRESHOLD,
        "repair_width": REPAIR_WIDTH,
        "eligible_hypotheses": list(eligible256["hypothesis"].astype(str)),
        "eligible_hypothesis_count": len(eligible256),
    ...
    evaluation = {
        "protocol": PROTOCOL,
        "evaluation_mode": "frozen_dvci_margin_local_discovery_search",
        ...
        "next258_diagnostic_authorized": bool(not passes and diagnostic_keys),
        "next258_candidate_count": len(diagnostic_keys),
        "next258_candidate_key_sha256": diagnostic_sha,
    ...
    source_paths = {
        ...
        "src/next257_dvci_margin_local_search.py": Path(__file__).resolve(),
    }
    ...
    try:
        ...
        if any(_sha256_file(path) != input_hashes[name] for name, path in paths.items()):
            raise RuntimeError("NEXT257 input changed before publication")
        if any(_sha256_file(path) != source_hashes[name] for name, path in source_paths.items()):
            raise RuntimeError("NEXT257 source changed before publication")
...
def main() -> int:
    ...
    manifest = run_dvci_margin_local_search(
...
__all__ = [
    ...
    "build_dvci_candidate_specs",
    "dvci_margin_local_score",
    "materialize_dvci_candidates",
    "run_dvci_margin_local_search",
