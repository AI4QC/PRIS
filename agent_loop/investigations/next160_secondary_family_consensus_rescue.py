#!/usr/bin/env python3
"""Frozen rescue using secondary cross-mechanism concurrence statistics."""
...
import src.next157_mechanism_family_consensus_audit as n157
...
PROTOCOL = "2026-08-08-next160-secondary-family-consensus-rescue-v1"
MANIFEST_NAME = "MANIFEST.json"
CATALOGUE_NAME = "NEXT160_SECONDARY_FAMILY_CONSENSUS_CATALOGUE.json"
EVALUATION_NAME = "NEXT160_DISCOVERY_EVALUATION.json"
SEARCH_NAME = "next160_secondary_family_consensus_candidate_search.parquet"
EXPECTED_DESIGN_SHA256 = "27b24b33c4a68b47d4a24d17891b2a5b6e0edd63f8dba00e95541e8b02fa6759"
EXPECTED_BASE_COUNT = 11
EXPECTED_CANDIDATE_COUNT = 352
EXPECTED_BASE_FORMULA_SHA256 = "d1f8763331cbe36f54e898e4efc88d0f88d2ae5d6284883acc4850e58d9678b5"
EXPECTED_CANDIDATE_KEY_SHA256 = "d61b06bc2a117c208e2fdbd68dece32dd933b4a1f5201b93f03b20fc01d2e235"
AGGREGATIONS = ("family_max_second", "family_max_sum_minus_largest")
FAMILY_PREFIXES = n157.FAMILY_PREFIXES
...
EXPECTED_INPUT_SHA256 = {
    **n135.EXPECTED_INPUT_SHA256,
    ...
    "next157_manifest": "e9d02b9a67f6339dce115650ace658f8dd35d360ae753f53f6c3fd19a9155a7a",
    "next157_audit": "77174edb0b5c89fd31f0d12a0cea1a7d80c502379d4655e8e538e784fe38e212",
    "next157_table": "7debe3173f44c5272dc995c8af62ea0b6394f81509782021b202f5ef9cdbd5f8",
...
def secondary_family_statistics(
    contributions: object, term_ids: Sequence[str]
) -> dict[str, np.ndarray]:
    """Return the two frozen cross-family concurrence statistics."""
    ...
    if (
        ...
        raise ValueError("NEXT160 contribution matrix differs")
    ...
    for index, term_id in enumerate(term_ids):
        ...
        if len(matches) != 1:
            raise ValueError("NEXT160 term-to-family assignment differs")
        members[matches[0]].append(index)
    if any(not indices for indices in members.values()):
        raise ValueError("NEXT160 family coverage differs")
    maxima = np.column_stack(
        [values[:, indices].max(axis=1) for indices in members.values()]
    )
    ordered = np.sort(maxima, axis=1)[:, ::-1]
    return {
        "family_max_second": ordered[:, 1],
        "family_max_sum_minus_largest": maxima.sum(axis=1) - ordered[:, 0],
    }


def compose_secondary_family_score(
    *,
    ...
    if (
        ...
        raise ValueError("NEXT160 score arrays differ")
    ...
    if (
        ...
        raise ValueError("NEXT160 supported score values differ")
    score = np.full(len(values), np.nan, dtype=float)
    if supported.any():
        score[supported] = secondary_family_statistics(
...
def build_candidate_specs(
    *, bases: pd.DataFrame, physical_term_ids: set[str]
    ...
    """Enumerate the frozen 2 x 11 x 4 x 4 secondary-family universe."""

    if {"term_ids_json", "weights_json"} - set(bases.columns):
        raise ValueError("NEXT160 base schema differs")
    ...
    for _, row in bases.iterrows():
        ...
        if (
            ...
            raise ValueError("NEXT160 physical base formula differs")
        family_members = {
            family: [
                term_id
        ...
        if (
            ...
            raise ValueError("NEXT160 frozen family membership differs")
...
def materialize_candidates(
    *,
    ...
    """Encode each exact secondary-family score as an evaluator-compatible term."""

    physical_by_id = {str(term["term_id"]): dict(term) for term in physical_terms}
    if len(physical_by_id) != len(physical_terms):
        raise ValueError("NEXT160 physical term identities are duplicated")
    if {
        ...
        raise ValueError("NEXT160 protection feature schema differs")
    ...
    for raw_spec in specs:
        ...
        if (
            key in seen
            or spec.get("aggregation") not in AGGREGATIONS
            ...
            raise ValueError("NEXT160 candidate materialization differs")
        seen.add(key)
        ...
        if not np.isfinite(encoded[supported]).all():
            raise ValueError("NEXT160 virtual score encoding overflowed")
        digest = hashlib.sha256(key.encode()).hexdigest()[:24]
        virtual_id = f"next160_virtual_candidate__{digest}"
        ...
        virtual_terms.append(
            {
                ...
                "center": 0.0,
                "scale": 1.0 / divisor,
                "group": "next160_secondary_family_consensus_candidate",
                "encoding": "asinh_sinh_exact_secondary_family_consensus_score",
...
def _paths(
    ...
    paths.update(
        {
            "design": design_path,
            "next157_manifest": roots["next157"] / n157.MANIFEST_NAME,
            "next157_audit": roots["next157"] / n157.AUDIT_NAME,
            "next157_table": roots["next157"] / n157.TABLE_NAME,
...
def run_secondary_family_consensus_rescue(
    *,
    ...
    if any(not path.is_file() for path in paths.values()):
        raise FileNotFoundError("NEXT160 input is missing")
    ...
    if require_formal_inputs and input_hashes != EXPECTED_INPUT_SHA256:
        ...
        raise ValueError(f"NEXT160 formal input identity differs: {differing}")
    manifest157 = json.loads(paths["next157_manifest"].read_text())
    audit157 = json.loads(paths["next157_audit"].read_text())
    manifest159 = json.loads(paths["next159_manifest"].read_text())
    diagnostic159 = json.loads(paths["next159_diagnostic"].read_text())
    if (
        manifest157.get("protocol") != n157.PROTOCOL
        or manifest157.get("eligible_statistic_count") != 3
        or manifest157.get("opened_validation_outputs_used") is not False
        or manifest157.get("dft_values_used_by_executable_formula") is not False
        or manifest157.get("outputs_sha256", {}).get(n157.AUDIT_NAME)
        != input_hashes["next157_audit"]
        or audit157.get("eligible_statistics")
        != [
            "family_capped_mean_sum",
            "family_max_sum_minus_largest",
            "family_max_second",
        ]
        or audit157.get("selected_statistic", {}).get("statistic")
        != "family_capped_mean_sum"
        or audit157.get("validation_or_replication_opened") is not False
        or audit157.get("dft_values_used_by_executable_formula") is not False
        or manifest159.get("protocol") != n159.PROTOCOL
        or manifest159.get("candidate_count") != 3
        ...
        != input_hashes["next159_diagnostic"]
        or diagnostic159.get("global_closest", {}).get("failed_constraint_count")
        != 6
        or set(diagnostic159.get("failure_frequency_at_per_candidate_optima", {}))
        != {
            "scigen:all::protected_kept",
            "scigen:fold0::protected_kept",
            "scigen:fold1::protected_kept",
            "scigen:fold2::protected_kept",
            "scigen:fold3::protected_kept",
            "scigen:fold4::protected_kept",
        }
    ):
        raise ValueError("NEXT160 family-audit provenance differs")
    ...
    if require_formal_inputs and (
        ...
        raise ValueError("NEXT160 frozen candidate universe differs")
    ...
    def decorate(record: dict[str, object]) -> None:
        ...
        record["aggregation"] = str(payload["aggregation"])
        ...
        record["score_composition"] = str(payload["aggregation"]) + (
            "; max(0,aggregation-alpha*coordination"
            "-beta*coordination_covalent_packing)"
        )
    ...
    formula["aggregation"] = str(payload["aggregation"])
    formula["family_prefixes"] = payload["family_prefixes"]
    formula["within_family_reduction"] = "maximum"
    ...
    formula["kind"] = (
        "secondary_family_consensus_physical_base_with_bounded_protections"
    )
    passes = bool(selected["record"]["passes_all_discovery_gates"])
    records_frame = pd.DataFrame(result["candidate_records"])
    counts = {}
    for (aggregation, alpha, beta), frame in records_frame.groupby(
        [
            "aggregation",
        ...
        counts[f"aggregation={aggregation},alpha={alpha:g},beta={beta:g}"] = {
    ...
    catalogue = {
        ...
        "next157_admitted_secondary_statistics": list(AGGREGATIONS),
        "family_prefixes": {
            family: list(prefixes) for family, prefixes in FAMILY_PREFIXES.items()
        },
        "within_family_reduction": "maximum",
        ...
        "active_score": (
            "max(0,frozen_secondary_family_aggregation-alpha*coordination"
            "-beta*coordination_covalent_packing)"
        ),
    ...
    source_paths = {
        ...
        "src/next157_mechanism_family_consensus_audit.py": Path(n157.__file__).resolve(),
        "src/next159_mechanism_family_broad_residual_diagnostic.py": Path(n159.__file__).resolve(),
        "src/next160_secondary_family_consensus_rescue.py": Path(__file__).resolve(),
    }
    source_hashes = {
    ...
    try:
        ...
        _write_json(
            ...
            {
                "protocol": PROTOCOL,
                "evaluation_mode": "fixed_secondary_family_consensus_rescue",
                "rows": {
        ...
        manifest = {
            ...
            "freeze_authorized": passes,
            "requires_unopened_internal_validation_before_claim": True,
            "secondary_family_consensus_branch_terminated": not passes,
        ...
        if any(_sha256_file(path) != input_hashes[name] for name, path in paths.items()):
            raise RuntimeError("NEXT160 input changed before publication")
        if any(
            _sha256_file(path) != source_hashes[name]
            for name, path in source_paths.items()
        ):
            raise RuntimeError("NEXT160 source changed before publication")
...
def main() -> int:
    ...
    manifest = run_secondary_family_consensus_rescue(
...
__all__ = [
    ...
    "run_secondary_family_consensus_rescue",
]
