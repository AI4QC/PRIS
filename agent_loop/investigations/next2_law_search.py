#!/usr/bin/env python3
"""np-next-20260802 law loop: frozen valence-confidence guard extension.

Identical search mechanics to ``next_law_search`` on the same physically
isolated tables and the same 61 frozen descriptor columns; the only change
is the guarded-candidate guard vocabulary, extended by the three frozen
composition-only columns from ``next2_guards``.  Emits the same report
schema so ``law_falsification.py`` runs unmodified.
"""
...
NEW_GUARD_ONLY_COLUMNS = ("z_an_abs", "oxi_n_guesses", "oxi_unique")

ADDITIVE_VARIANT = "additive_next2_loop"
ADDITIVE_GUARDED_VARIANT = "additive_next2_anion_guarded_loop"


def load_next2_search_frames(
    isolated_dir: Path,
    ...
    """Isolated law tables + np-next-20260801 descriptors + guard columns."""
    ...
    if guards_real["source_id"].duplicated().any():
        raise ValueError("guard source_id is not unique")
    if guards_bad["parent"].duplicated().any():
        raise ValueError("guard parent is not unique")
    frames = []
    for name, frame, key, guards in (
        ("discovery real", dr, "source_id", guards_real),
        ("calibration real", cr, "source_id", guards_real),
        ("discovery bad", db, "parent", guards_bad),
        ("calibration bad", cb, "parent", guards_bad),
    ):
        before = len(frame)
        merged = frame.merge(guards, on=key, how="left", validate="many_to_one")
        if len(merged) != before:
            raise AssertionError(f"guard merge changed the {name} row set")
        frames.append(merged)
        merged.attrs["source_access_audit"] = frame.attrs["source_access_audit"]
    return tuple(frames)


def build_next2_candidate_sets(
    real: pd.DataFrame,
    bad: pd.DataFrame,
    *,
    min_coverage: float,
    ...
    """Same pools as np-next-20260801, with the extended guard vocabulary."""

    if p1_vocabulary != "frozen":
        raise ValueError(
            "np-next-20260802 froze exactly one vocabulary; no expanded branch"
        )
    alphas = (
        0.0003,
        0.0007,
    ...
    old_features = [
        ...
        and column not in NEW_GUARD_ONLY_COLUMNS
    ]
    new_features = [
        column
        for column in real
    ...
    guarded = build_guarded_candidates(
        ...
        guard_columns=NEXT2_GUARD_COLUMNS,
    ...
    counts = {
        ...
        "guard_columns": len(NEXT2_GUARD_COLUMNS),
    }
    return {
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    candidate_sets, candidate_counts = build_next2_candidate_sets(
        dr,
        db,
    ...
    report: dict[str, object] = {
        "protocol": {
            ...
            **source_access_audit,
            ...
            "new_vocabulary": "frozen P1+P2+P3+P5 descriptors; guard vocabulary +z_an_abs/oxi_n_guesses/oxi_unique",
        },
        "counts": {
