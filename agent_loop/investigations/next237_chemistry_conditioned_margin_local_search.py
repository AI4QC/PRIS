...
import src.next236_chemistry_conditioned_feature_audit as n236
...
PROTOCOL = "2026-08-09-next237-chemistry-conditioned-margin-local-search-v2"
MANIFEST_NAME = "MANIFEST.json"
CATALOGUE_NAME = "NEXT237_CHEMISTRY_CONDITIONED_CATALOGUE.json"
EVALUATION_NAME = "NEXT237_DISCOVERY_EVALUATION.json"
FORMULA_NAME = "NEXT237_FROZEN_CANDIDATE.json"
SEARCH_NAME = "next237_chemistry_conditioned_margin_local_search.parquet"
...
EXPECTED_ELIGIBLE_HYPOTHESIS_COUNT = 125
EXPECTED_ELIGIBLE_HYPOTHESIS_SHA256 = (
    "9e950fd99e860ff7f9af6caaaea60c784e9badbe363bde8fe6ef99d2aaf33611"
)
...
EXPECTED_DESIGN_SHA256 = n236.EXPECTED_DESIGN_SHA256
...
BOUNDARY_FLAGS = n236.BOUNDARY_FLAGS
REQUIRED_STAGES = (*n236.REQUIRED_STAGES, 236)
REQUIRED_DESIGN_STAGES = (*n236.REQUIRED_DESIGN_STAGES, 236)
EXPECTED_NEXT236_SOURCE_SHA256 = (
    "b2d8b2f12896a3c6deeffb415ea3579f0b3f6ef8bda0f528b6a3117b6e18c72a"
)
EXPECTED_INPUT_SHA256 = {
    **{
        key: value
        for key, value in n236.EXPECTED_INPUT_SHA256.items()
        if key != "design"
    },
    "next236_design": n236.EXPECTED_INPUT_SHA256["design"],
    "design": EXPECTED_DESIGN_SHA256,
    "next236_manifest": (
        "74246ffde988a6ffb500d00178a95c16d5e03b2d58fad547ce49a86ff859c1ca"
    ),
    "next236_catalogue": (
        "9872f1a9c84cb759ec57a5838a75ac430cea2949d880a82aeca66dee5291210c"
    ),
    "next236_audit": (
        "6370a3efa4e7e5e0cbe7abdc94eddca9ada6a43265df6451c5ed4e15321df73e"
    ),
    "next236_table": (
        "68ee4e5c48f41cbc561ed16d5708dd967cf35851b9368eef6b2136b08e4a68d3"
    ),
}


def conditioned_margin_local_score(
    *,
    base_score: object,
    base_support: object,
    protection: object,
    ...
    if (
        ...
        raise ValueError("NEXT237 margin-local score inputs differ")
    h = fraction * width
...
def _grid_numerator(value: float, denominator: int, label: str) -> int:
    ...
    if (
        ...
        raise ValueError(f"NEXT237 {label} grid differs")
    return numerator


def build_conditioned_candidate_specs(
    *,
    ...
    if not isinstance(base_candidate_key, str) or not base_candidate_key:
        raise ValueError("NEXT237 base candidate key differs")
    cutoff = float(threshold)
    width = float(repair_width)
    if (
        not isinstance(eligible_table, pd.DataFrame)
        or eligible_table.empty
        or not math.isfinite(cutoff)
        or not math.isfinite(width)
        or width <= 0.0
    ):
        raise ValueError("NEXT237 conditioned specification inputs differ")
    required = {
        "hypothesis", "feature", "conditioner", "direction",
        "stratum_edges_json", "q_lo_by_stratum_json", "q_hi_by_stratum_json",
    }
    if required - set(eligible_table.columns):
        raise ValueError("NEXT237 eligible hypothesis schema differs")
    eligible = eligible_table.sort_values("hypothesis", kind="mergesort")
    if eligible["hypothesis"].astype(str).duplicated().any():
        raise ValueError("NEXT237 eligible hypothesis universe differs")
    ...
    if (
        ...
        raise ValueError("NEXT237 frozen grid differs")
    common = {
        ...
        "normalization_population": "COMPOSITION_CONDITIONED_COMBINED_DISCOVERY",
    ...
    payloads: list[dict[str, object]] = [
        {
            **common,
            ...
            "direction": None,
            "stratum_edges": None,
            "q_lo_by_stratum": None,
            "q_hi_by_stratum": None,
    ...
    for _, row in eligible.iterrows():
        ...
        conditioner = str(row["conditioner"])
        direction = str(row["direction"])
        edges = json.loads(str(row["stratum_edges_json"]))
        q_lo = json.loads(str(row["q_lo_by_stratum_json"]))
        q_hi = json.loads(str(row["q_hi_by_stratum_json"]))
        if (
            not feature
            or not conditioner
            ...
            raise ValueError("NEXT237 eligible hypothesis identity differs")
    ...
    if len({str(spec["candidate_key"]) for spec in specs}) != len(specs):
        raise RuntimeError("NEXT237 candidate keys are not unique")
    return specs


def materialize_conditioned_candidates(
    *,
    ...
    if (
        ...
        raise ValueError("NEXT237 materializer inputs differ")
    ...
    for spec in raw_specs:
        ...
        if feature is None:
            ...
            feature = str(feature)
            conditioner = str(spec.get("conditioner"))
            if feature not in features.columns or conditioner not in features.columns:
                raise ValueError("NEXT237 materializer feature differs")
            for name in (feature, conditioner):
                if name not in raw_cache:
                    raw_cache[name] = pd.to_numeric(
                        features[name], errors="coerce"
                    ).to_numpy(float)
            model = {
                "stratum_edges": spec["stratum_edges"],
                "q_lo_by_stratum": spec["q_lo_by_stratum"],
                "q_hi_by_stratum": spec["q_hi_by_stratum"],
            }
            protection = n236.chemistry_conditioned_protection(
                values=raw_cache[feature],
                conditioner=raw_cache[conditioner],
                direction=str(spec["direction"]),
                model=model,
            )
            score, got_support, active, _ = conditioned_margin_local_score(
                base_score=base,
        ...
        if not np.array_equal(got_support, support):
            raise RuntimeError("NEXT237 support changed")
        ...
        terms.append(
            {
                ...
                "center": 0.0,
                "scale": 1.0 / divisor,
                "group": "next237_margin_local_redistribution",
...
def _paths(
    *,
    ...
    paths = n236._paths(
        ...
        design_paths={
            stage: design_paths[stage] for stage in n236.REQUIRED_DESIGN_STAGES
        },
        design_path=design_paths[236],
    )
    paths["next236_design"] = paths.pop("design")
    paths.update(
        {
            "design": design_path,
            "next236_manifest": roots["next236"] / n236.MANIFEST_NAME,
            "next236_catalogue": roots["next236"] / n236.CATALOGUE_NAME,
            "next236_audit": roots["next236"] / n236.AUDIT_NAME,
            "next236_table": roots["next236"] / n236.TABLE_NAME,
        }
    )
    return paths
...
def _verify_next236(
    ...
    n236._verify_next235_comparison(paths, input_hashes)
    manifest = json.loads(paths["next236_manifest"].read_text())
    catalogue = json.loads(paths["next236_catalogue"].read_text())
    audit = json.loads(paths["next236_audit"].read_text())
    table = pd.read_parquet(paths["next236_table"])
    ...
    expected_outputs = {
        n236.CATALOGUE_NAME: input_hashes["next236_catalogue"],
        n236.AUDIT_NAME: input_hashes["next236_audit"],
        n236.TABLE_NAME: input_hashes["next236_table"],
    }
    if (
        manifest.get("protocol") != n236.PROTOCOL
        or manifest.get("eligible_hypothesis_count")
        != EXPECTED_ELIGIBLE_HYPOTHESIS_COUNT
        or manifest.get("eligible_hypothesis_sha256")
        != EXPECTED_ELIGIBLE_HYPOTHESIS_SHA256
        or manifest.get("next237_search_authorized") is not True
        ...
        or manifest.get("executed_source_sha256", {}).get(
            "src/next236_chemistry_conditioned_feature_audit.py"
        )
        != EXPECTED_NEXT236_SOURCE_SHA256
        or _sha256_file(Path(n236.__file__).resolve())
        != EXPECTED_NEXT236_SOURCE_SHA256
        ...
        != EXPECTED_ELIGIBLE_HYPOTHESIS_SHA256
        or audit.get("next237_search_authorized") is not True
        ...
        raise ValueError("NEXT237 NEXT236 provenance differs")
...
def _formula_from_spec(spec: Mapping[str, object] | None) -> dict[str, object]:
    ...
    return {
        "protocol": PROTOCOL,
        "kind": "chemistry_conditioned_triangular_margin_local_x0_no_dft_score",
        "selected": True,
...
def select_best_new_record(records: pd.DataFrame) -> pd.Series | None:
    ...
    if not isinstance(records, pd.DataFrame) or required - set(records.columns):
        raise ValueError("NEXT237 reporting selection schema differs")
...
def run_chemistry_conditioned_margin_local_search(
    *,
    ...
    """Run the complete frozen discovery-only NEXT237 search."""

    if not set(REQUIRED_STAGES).issubset(stage_dirs):
        raise ValueError("NEXT237 stage directory universe differs")
    if not set(REQUIRED_DESIGN_STAGES).issubset(design_paths):
        raise ValueError("NEXT237 design path universe differs")
    roots = {
    ...
    if type(search_workers) is not int or search_workers <= 0:
        raise ValueError("NEXT237 search_workers must be a positive exact integer")
    if any(not path.is_file() for path in paths.values()):
        raise FileNotFoundError("NEXT237 input is missing")
    ...
    if require_formal_inputs and input_hashes != EXPECTED_INPUT_SHA256:
        ...
        raise ValueError(f"NEXT237 formal input identity differs: {differing}")
    (
        eligible_prior,
        eligible214,
        primary_key,
        ...
        formula222,
        eligible236,
    ) = _verify_next236(paths, input_hashes)
    ...
    if (
        ...
        raise ValueError("NEXT237 NEXT224 base identity differs")
    specs = build_conditioned_candidate_specs(
        base_candidate_key=base_key,
        eligible_table=eligible236,
    )
    ...
    if len(specs) != EXPECTED_CANDIDATE_COUNT or eligible_count != EXPECTED_ELIGIBLE_COUNT:
        raise RuntimeError("NEXT237 frozen candidate universe differs")
    ...
    if (
        ...
        raise RuntimeError("NEXT237 evaluator accounting differs")
        ...
        record.update(
            {
                ...
                "stratum_edges_json": json.dumps(
                    spec["stratum_edges"], separators=(",", ":")
                ),
                "q_lo_by_stratum_json": json.dumps(
                    spec["q_lo_by_stratum"], separators=(",", ":")
                ),
                "q_hi_by_stratum_json": json.dumps(
                    spec["q_hi_by_stratum"], separators=(",", ":")
                ),
                "local_width_fraction": spec["local_width_fraction"],
    ...
    if len(no_op) != 1 or len(reference_no_op) != 1:
        raise RuntimeError("NEXT237 no-op reproduction identity differs")
    ...
    if selected_row is not None:
        ...
        if str(selected_result["selected"]["record"]["candidate_key"]) != selected_key:
            raise RuntimeError("NEXT237 selected candidate reproduction differs")
        ...
        if selected_spec.get("eligible_new_candidate") is not True:
            raise RuntimeError("NEXT237 reproduction control was selected")
    ...
    if selected is not None and passes != bool(
        ...
        raise RuntimeError("NEXT237 all-gate selection differs")
    if passes and selected is None:
        raise RuntimeError("NEXT237 all-gate candidate was not selected")
    ...
    catalogue = {
        ...
        "base_threshold": EXPECTED_BASE_THRESHOLD,
        "repair_width": REPAIR_WIDTH,
        "eligible_hypotheses": list(eligible236["hypothesis"].astype(str)),
        "eligible_hypothesis_count": len(eligible236),
    ...
    evaluation = {
        "protocol": PROTOCOL,
        "evaluation_mode": "frozen_chemistry_conditioned_margin_local_discovery_search",
        ...
        "next238_diagnostic_authorized": bool(not passes and diagnostic_keys),
        "next238_candidate_count": len(diagnostic_keys),
        "next238_candidate_key_sha256": diagnostic_sha,
    ...
    source_paths = {
        "src/next236_chemistry_conditioned_feature_audit.py": Path(n236.__file__).resolve(),
        "src/next237_chemistry_conditioned_margin_local_search.py": Path(__file__).resolve(),
    }
    source_hashes = {
    ...
    try:
        ...
        if any(
            ...
            raise RuntimeError("NEXT237 input changed before publication")
        if any(
            _sha256_file(path) != source_hashes[name]
            for name, path in source_paths.items()
        ):
            raise RuntimeError("NEXT237 source changed before publication")
...
def main() -> int:
    ...
    manifest = run_chemistry_conditioned_margin_local_search(
