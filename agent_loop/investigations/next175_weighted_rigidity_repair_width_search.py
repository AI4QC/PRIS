#!/usr/bin/env python3
"""Finite no-DFT search over weighted-rigidity repair-width protection."""
...
PROTOCOL = "2026-08-08-next175-weighted-rigidity-repair-width-search-v1"
MANIFEST_NAME = "MANIFEST.json"
CATALOGUE_NAME = "NEXT175_WEIGHTED_RIGIDITY_REPAIR_WIDTH_CATALOGUE.json"
EVALUATION_NAME = "NEXT175_DISCOVERY_EVALUATION.json"
FORMULA_NAME = "NEXT175_FROZEN_CANDIDATE.json"
SEARCH_NAME = "next175_weighted_rigidity_repair_width_search.parquet"
EXPECTED_DESIGN_SHA256 = "ab24ebb8ba52a3f2cc4acf0ab27853748e5eec40f4e4803c3fef2ab3c84d5762"
...
EXPECTED_INPUT_SHA256 = {
    **n174.EXPECTED_INPUT_SHA256,
    ...
    "next174_manifest": "515398fe6fcf76142c3897d8849c80b8e2389e8d4c13e8da8dc555b629b24d66",
    "next174_audit": "be473332c534338f6df6a758c6f0448d1eda1c2b7ce4d7a52778c43b97da6fd1",
    "next174_table": "8b667a157a31392dd775f00520acbd1fa8e3080271ae93f4dd8145f329ead72f",
}


def interval_width_protect_score(
    *,
    base_score: object,
    base_support: object,
    feature: object,
    ...
    """Subtract a weighted fraction of the frozen repair-shell width."""
    ...
    if (
        ...
        raise ValueError("NEXT175 protection input differs")
    finite = np.isfinite(rigidity)
    if np.any((rigidity[finite] < -1.0e-12) | (rigidity[finite] > 1.0 + 1.0e-12)):
        raise ValueError("NEXT175 weighted rigidity is outside [0,1]")
    active = (
        support
        & finite
...
def build_candidate_specs(*, base_candidate_key: str) -> list[dict[str, object]]:
    """Build the exact frozen 41-candidate universe."""

    if not isinstance(base_candidate_key, str) or not base_candidate_key:
        raise ValueError("NEXT175 base candidate key must be nonempty")
    ...
    if len(specs) != EXPECTED_CANDIDATE_COUNT or len(
        ...
        raise RuntimeError("NEXT175 candidate universe differs")
    return specs


def materialize_interval_width_candidates(
    *,
    ...
    if score.shape != (len(features),) or support.shape != score.shape:
        raise ValueError("NEXT175 base score shape differs")
    if set(ELIGIBLE_FEATURES) - set(features.columns):
        raise ValueError("NEXT175 eligible feature schema differs")
    ...
    for raw_spec in specs:
        ...
        if (
            ...
            raise ValueError("NEXT175 candidate spec differs")
        seen.add(key)
        feature = (
        ...
        corrected, corrected_support, _active = interval_width_protect_score(
            base_score=score,
        ...
        if not np.isfinite(encoded[corrected_support]).all():
            raise ValueError("NEXT175 virtual score encoding overflowed")
        digest = hashlib.sha256(key.encode()).hexdigest()[:24]
        virtual_id = f"next175_virtual_candidate__{digest}"
        ...
        terms.append(
            {
                ...
                "center": 0.0,
                "scale": 1.0 / divisor,
                "group": "next175_weighted_rigidity_repair_width",
                "encoding": "asinh_sinh_exact_repair_width_score",
    ...
    if len(seen) != EXPECTED_CANDIDATE_COUNT:
        raise ValueError("NEXT175 materialized candidate count differs")
    return (
...
def _paths(
    ...
    paths.update(
        {
            "next174_manifest": roots["next174"] / n174.MANIFEST_NAME,
            "next174_audit": roots["next174"] / n174.AUDIT_NAME,
            "next174_table": roots["next174"] / n174.TABLE_NAME,
        }
    )
    return paths


def run_weighted_rigidity_repair_width_search(
    *,
    ...
    """Run the frozen NEXT175 discovery-only formula search."""
    ...
    if any(not path.is_file() for path in paths.values()):
        raise FileNotFoundError("NEXT175 input is missing")
    ...
    if require_formal_inputs and input_hashes != EXPECTED_INPUT_SHA256:
        ...
        raise ValueError(f"NEXT175 formal input identity differs: {differing}")

    manifest174 = json.loads(paths["next174_manifest"].read_text())
    audit174 = json.loads(paths["next174_audit"].read_text())
    table174 = pd.read_parquet(paths["next174_table"])
    eligible_from_table = tuple(
        sorted(
            table174.loc[
                table174["eligible_for_search"].fillna(False).astype(bool), "feature"
            ].astype(str)
        )
    )
    if (
        manifest174.get("protocol") != n174.PROTOCOL
        or manifest174.get("eligible_hypothesis_count") != len(ELIGIBLE_FEATURES)
        or manifest174.get("weighted_local_directional_rigidity_branch_terminated")
        is not False
        or manifest174.get("new_formula_searched") is not False
        or manifest174.get("opened_validation_outputs_used") is not False
        or manifest174.get("scigen_replication_endpoint_opened") is not False
        or manifest174.get("wyformer_replication_endpoint_opened") is not False
        or manifest174.get("dft_calculation_executed") is not False
        or manifest174.get("dft_values_used_by_executable_formula") is not False
        or manifest174.get("learned_energy_force_stress_proxy_used") is not False
        or manifest174.get("physical_relaxation_executed") is not False
        or manifest174.get("outputs_sha256", {}).get(n174.AUDIT_NAME)
        != input_hashes["next174_audit"]
        or manifest174.get("outputs_sha256", {}).get(n174.TABLE_NAME)
        != input_hashes["next174_table"]
        or manifest174.get("executed_source_sha256", {}).get(
            "src/next174_weighted_local_directional_rigidity_audit.py"
        )
        != _sha256_file(Path(n174.__file__).resolve())
        or tuple(sorted(ELIGIBLE_FEATURES)) != eligible_from_table
        or tuple(sorted(audit174.get("eligible_hypotheses", ())))
        != tuple(
            sorted(
                f"{feature.removeprefix('pwldr_')}__high"
                for feature in ELIGIBLE_FEATURES
            )
        )
    ):
        raise ValueError("NEXT175 eligibility provenance differs")
    ...
    if (
        ...
        raise ValueError("NEXT175 base candidate identity differs")
    ...
    if len(selected_base_specs) != 1:
        raise ValueError("NEXT175 base reconstruction differs")
    ...
    if len(base_terms) != 1 or len(base_runtime) != 1:
        raise RuntimeError("NEXT175 base materialization differs")
    ...
    if not np.isfinite(endpoint).all() or len(combined) != len(base_score):
        raise ValueError("NEXT175 endpoint row accounting differs")

    specs = build_candidate_specs(base_candidate_key=base_candidate_key)
    combined, virtual_terms, runtime = materialize_interval_width_candidates(
        features=combined,
    ...
    if int(result["candidate_count"]) != EXPECTED_CANDIDATE_COUNT:
        raise RuntimeError("NEXT175 evaluator candidate count differs")
    ...
    if len(base_records) != 1:
        raise RuntimeError("NEXT175 unmodified base count differs")
    ...
    frozen_formula = {
        "protocol": PROTOCOL,
        "kind": "weighted_rigidity_repair_width_no_dft_score",
        "base_candidate_key": base_candidate_key,
    ...
    source_paths = {
        ...
        "src/next174_weighted_local_directional_rigidity_audit.py": Path(n174.__file__).resolve(),
        "src/next175_weighted_rigidity_repair_width_search.py": Path(__file__).resolve(),
    }
    ...
    try:
        ...
        _write_json(
            ...
            {
                "protocol": PROTOCOL,
                "evaluation_mode": "fixed_weighted_rigidity_repair_width_search",
                "base_endpoint_reproduced": True,
                "rows": {
        ...
        manifest = {
            ...
            "freeze_authorized": passes,
            "requires_unopened_internal_validation_before_claim": True,
            "weighted_rigidity_repair_width_branch_terminated": not passes,
        ...
        if any(
            ...
            raise RuntimeError("NEXT175 input changed before publication")
        if any(
            _sha256_file(path) != source_hashes[name]
            for name, path in source_paths.items()
        ):
            raise RuntimeError("NEXT175 source changed before publication")
...
def main() -> int:
    ...
    manifest = run_weighted_rigidity_repair_width_search(
