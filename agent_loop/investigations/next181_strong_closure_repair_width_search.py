#!/usr/bin/env python3
"""Finite no-DFT repair-width search over strong-neighborhood closure."""
...
PROTOCOL = "2026-08-08-next181-strong-closure-repair-width-search-v1"
MANIFEST_NAME = "MANIFEST.json"
CATALOGUE_NAME = "NEXT181_STRONG_CLOSURE_REPAIR_WIDTH_CATALOGUE.json"
EVALUATION_NAME = "NEXT181_DISCOVERY_EVALUATION.json"
FORMULA_NAME = "NEXT181_FROZEN_CANDIDATE.json"
SEARCH_NAME = "next181_strong_closure_repair_width_search.parquet"
EXPECTED_DESIGN_SHA256 = "4dc1043078f13930e26d4a7df70e05406baf68f5fda7f0aac39918fe83c2896c"
EXPECTED_BASE_CANDIDATE_KEY_SHA256 = n175.EXPECTED_BASE_CANDIDATE_KEY_SHA256
BROAD_THRESHOLD = n180.BROAD_THRESHOLD
SAFE_THRESHOLD = n180.SAFE_THRESHOLD
...
SCORE_COMPOSITION = (
    ...
    "max(0,base_score-alpha*(safe-broad)*strong_neighborhood_closure)"
)
SEARCH_WORKERS = 4
EXPECTED_INPUT_SHA256 = {
    **n180.EXPECTED_INPUT_SHA256,
    "design": EXPECTED_DESIGN_SHA256,
    "next180_manifest": "68c5d0f50b9cfcb69b023d3f5e8f5e844cd226649251b6568a6f3af04a93c68b",
    "next180_audit": "a88030a29db33db5c7f72a8d54c57009f3b580209d6091bd9ee3d25888235a0d",
    "next180_table": "be12c6d024b782e497d38b6b8915bf42ac0b6970ae6be124cccab7382062b5f2",
}


def strong_closure_protect_score(
    *,
    base_score: object,
    base_support: object,
    feature: object,
    ...
    """Subtract a bounded fraction of repair width using strong closure."""
    ...
    if (
        ...
        raise ValueError("NEXT181 protection input differs")
    finite = np.isfinite(closure)
    if np.any((closure[finite] < -1.0e-12) | (closure[finite] > 1.0 + 1.0e-12)):
        raise ValueError("NEXT181 strong closure is outside [0,1]")
    active = (
        support
        & finite
...
def build_candidate_specs(*, base_candidate_key: str) -> list[dict[str, object]]:
    ...
    if len(specs) != EXPECTED_CANDIDATE_COUNT or len({str(x["candidate_key"]) for x in specs}) != EXPECTED_CANDIDATE_COUNT:
        raise RuntimeError("NEXT181 candidate universe differs")
    return specs


def materialize_strong_closure_candidates(
    *,
    ...
    if score.shape != (len(features),) or support.shape != score.shape:
        raise ValueError("NEXT181 base score shape differs")
    if set(ELIGIBLE_FEATURES) - set(features.columns):
        raise ValueError("NEXT181 eligible feature schema differs")
    ...
    for raw in specs:
        ...
        if (
            ...
            raise ValueError("NEXT181 candidate spec differs")
        seen.add(key)
        ...
        corrected, corrected_support, _ = strong_closure_protect_score(
            base_score=score,
        ...
        if not np.isfinite(encoded[corrected_support]).all():
            raise ValueError("NEXT181 virtual score encoding overflowed")
        digest = hashlib.sha256(key.encode()).hexdigest()[:24]
        virtual_id = f"next181_virtual_candidate__{digest}"
        ...
        terms.append(
            {
                ...
                "center": 0.0,
                "scale": 1.0 / divisor,
                "group": "next181_strong_closure_repair_width",
                "encoding": "asinh_sinh_exact_strong_closure_score",
    ...
    if len(seen) != EXPECTED_CANDIDATE_COUNT:
        raise ValueError("NEXT181 materialized candidate count differs")
...
def _paths(roots: Mapping[str, Path], freeze_path: Path, design_path: Path) -> dict[str, Path]:
    paths = n180._paths(roots, freeze_path, design_path)
    paths.update(
        {
            "next180_manifest": roots["next180"] / n180.MANIFEST_NAME,
            "next180_audit": roots["next180"] / n180.AUDIT_NAME,
            "next180_table": roots["next180"] / n180.TABLE_NAME,
        }
    )
    return paths


def run_strong_closure_repair_width_search(
    *,
    ...
    """Run the frozen discovery-only NEXT181 formula search."""
    ...
    roots = {
        ...
        **{f"next{s}": Path(v).resolve() for s, v in (
            (98,next98_dir),(110,next110_dir),(111,next111_dir),(113,next113_dir),(114,next114_dir),
            (116,next116_dir),(117,next117_dir),(120,next120_dir),(121,next121_dir),(122,next122_dir),
            (124,next124_dir),(125,next125_dir),(129,next129_dir),(130,next130_dir),(133,next133_dir),
            (134,next134_dir),(163,next163_dir),(164,next164_dir),(168,next168_dir),(173,next173_dir),
            (179,next179_dir),(180,next180_dir),
        )},
    }
    ...
    if any(not path.is_file() for path in paths.values()):
        raise FileNotFoundError("NEXT181 input is missing")
    ...
    if require_formal_inputs and input_hashes != EXPECTED_INPUT_SHA256:
        ...
        raise ValueError(f"NEXT181 formal input identity differs: {differing}")

    manifest180 = json.loads(paths["next180_manifest"].read_text())
    audit180 = json.loads(paths["next180_audit"].read_text())
    table180 = pd.read_parquet(paths["next180_table"])
    eligible_from_table = tuple(sorted(table180.loc[table180["eligible_for_search"].fillna(False).astype(bool), "feature"].astype(str)))
    if (
        manifest180.get("protocol") != n180.PROTOCOL
        or manifest180.get("eligible_hypothesis_count") != len(ELIGIBLE_FEATURES)
        or manifest180.get("strong_neighborhood_directional_closure_branch_terminated") is not False
        or manifest180.get("new_formula_searched") is not False
        or manifest180.get("opened_validation_outputs_used") is not False
        or manifest180.get("scigen_replication_endpoint_opened") is not False
        or manifest180.get("wyformer_replication_endpoint_opened") is not False
        or manifest180.get("dft_calculation_executed") is not False
        or manifest180.get("dft_values_used_by_executable_formula") is not False
        or manifest180.get("learned_energy_force_stress_proxy_used") is not False
        or manifest180.get("physical_relaxation_executed") is not False
        or manifest180.get("outputs_sha256", {}).get(n180.AUDIT_NAME) != input_hashes["next180_audit"]
        or manifest180.get("outputs_sha256", {}).get(n180.TABLE_NAME) != input_hashes["next180_table"]
        or manifest180.get("executed_source_sha256", {}).get("src/next180_strong_neighborhood_directional_closure_audit.py") != _sha256_file(Path(n180.__file__).resolve())
        or tuple(sorted(ELIGIBLE_FEATURES)) != eligible_from_table
        or tuple(sorted(audit180.get("eligible_hypotheses", ()))) != tuple(sorted(f"{x.removeprefix('psndc_')}__high" for x in ELIGIBLE_FEATURES))
    ):
        raise ValueError("NEXT181 eligibility provenance differs")
    ...
    if hashlib.sha256(base_candidate_key.encode()).hexdigest() != EXPECTED_BASE_CANDIDATE_KEY_SHA256:
        raise ValueError("NEXT181 base candidate identity differs")
    ...
    combined = extended.merge(pd.concat(closure_frames, ignore_index=True), on="material_id", how="inner", validate="one_to_one")
    ...
    selected_base_specs = [spec for spec in base_specs if str(spec["candidate_key"]) == base_candidate_key]
    if len(selected_base_specs) != 1:
        raise ValueError("NEXT181 base reconstruction differs")
    combined, base_terms, base_runtime = n163.materialize_candidates(features=combined, physical_terms=physical_terms, specs=selected_base_specs)
    if len(base_terms) != 1 or len(base_runtime) != 1:
        raise RuntimeError("NEXT181 base materialization differs")
    ...
    combined = combined.merge(endpoint_frame, on="material_id", how="inner", validate="one_to_one")
    ...
    if not np.isfinite(endpoint).all() or len(combined) != len(base_score):
        raise ValueError("NEXT181 endpoint row accounting differs")

    specs = build_candidate_specs(base_candidate_key=base_candidate_key)
    combined, virtual_terms, runtime = materialize_strong_closure_candidates(features=combined, base_score=base_score, base_support=base_support, specs=specs)
    ...
    if int(result["candidate_count"]) != EXPECTED_CANDIDATE_COUNT:
        raise RuntimeError("NEXT181 evaluator candidate count differs")
    ...
    def decorate(record: dict[str, object]) -> None:
        ...
        record["strong_closure_feature"] = spec["strong_closure_feature"]
    ...
    base_records = [x for x in result["candidate_records"] if x["strong_closure_feature"] is None]
    if len(base_records) != 1:
        raise RuntimeError("NEXT181 unmodified base count differs")
    n175.n170._verify_base_reproduction(record=base_records[0], published=pd.read_parquet(paths["next163_search"]), candidate_key=base_candidate_key)
    ...
    frozen_formula = {
        "protocol": PROTOCOL,
        "kind": "strong_neighborhood_closure_repair_width_no_dft_score",
        "base_candidate_key": base_candidate_key,
        "base_formula": prior_evaluation["selected_formula"],
        "strong_closure_feature": selected_spec["strong_closure_feature"],
        "attenuation": float(selected_spec["attenuation"]),
        "broad_threshold": BROAD_THRESHOLD,
        "safe_threshold": SAFE_THRESHOLD,
        "repair_width": REPAIR_WIDTH,
        "interval_policy": "BROAD_INCLUSIVE_SAFE_EXCLUSIVE_ON_ORIGINAL_BASE_SCORE",
        "feature_definition": "max_relative_incident_weight_strong_direction_closure",
        "missing_policy": "TERM_OFF_KEEP_BASE",
        "score_composition": SCORE_COMPOSITION,
    ...
    counts = {}
    for (feature, alpha), frame in records_frame.assign(strong_closure_feature=records_frame["strong_closure_feature"].fillna("BASE")).groupby(["strong_closure_feature", "attenuation"], sort=True):
    ...
    source_paths = {
        ...
        "src/next181_strong_closure_repair_width_search.py": Path(__file__).resolve(),
    }
    ...
    try:
        ...
        _write_json(evaluation_path, {
            "protocol": PROTOCOL,
            "evaluation_mode": "fixed_strong_closure_repair_width_search",
            "base_endpoint_reproduced": True,
        ...
        manifest = {
            ...
            "freeze_authorized": passes,
            "requires_unopened_internal_validation_before_claim": True,
            "strong_closure_repair_width_branch_terminated": not passes,
        ...
        if any(_sha256_file(path) != input_hashes[name] for name, path in paths.items()):
            raise RuntimeError("NEXT181 input changed before publication")
        if any(_sha256_file(path) != source_hashes[name] for name, path in source_paths.items()):
            raise RuntimeError("NEXT181 source changed before publication")
...
def main() -> int:
    ...
    stages = (98,110,111,113,114,116,117,120,121,122,124,125,129,130,133,134,163,164,168,173,179,180)
    for stage in stages:
    ...
    manifest = run_strong_closure_repair_width_search(
...
__all__ = [
    "ATTENUATIONS","BROAD_THRESHOLD","ELIGIBLE_FEATURES","EXPECTED_CANDIDATE_COUNT","REPAIR_WIDTH","SAFE_THRESHOLD",
    "build_candidate_specs","materialize_strong_closure_candidates","run_strong_closure_repair_width_search","strong_closure_protect_score",
]
