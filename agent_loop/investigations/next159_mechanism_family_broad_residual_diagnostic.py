#!/usr/bin/env python3
"""Diagnose BROAD residuals of frozen NEXT158 AUC+SAFE12 candidates."""
...
PROTOCOL = "2026-08-08-next159-mechanism-family-broad-residual-diagnostic-v1"
MANIFEST_NAME = "MANIFEST.json"
DIAGNOSTIC_NAME = "NEXT159_MECHANISM_FAMILY_BROAD_RESIDUAL_DIAGNOSTIC.json"
PER_CANDIDATE_NAME = "next159_mechanism_family_broad_residual_by_candidate.parquet"
EXPECTED_DESIGN_SHA256 = "2b81e2db1acd130c20ba04a288af9484fadb0f2e0a3503e283ec23c66ceb1a1a"
EXPECTED_DIAGNOSTIC_CANDIDATE_COUNT = 3
EXPECTED_CANDIDATE_KEY_SHA256 = "298e04881ce0b1135e0d909e4578e5ea20d771b96785c9be8a2332b4135dfcd3"
...
def select_auc_safe_candidates(records: pd.DataFrame) -> pd.DataFrame:
    ...
    if required - set(records.columns) or records["candidate_key"].astype(str).duplicated().any():
        raise ValueError("NEXT159 published candidate schema differs")
    ...
    if selected.empty or not np.isfinite(
        ...
        raise ValueError("NEXT159 published thresholds differ")
...
def _verify_reproduction(
    *, rerun: Sequence[Mapping[str, object]], published: pd.DataFrame
    ...
    if {str(record["candidate_key"]) for record in rerun} != set(prior.index.astype(str)):
        raise RuntimeError("NEXT159 rerun identities differ")
    for record in rerun:
        ...
        if any(
            ...
            raise RuntimeError("NEXT159 does not reproduce NEXT158 AUC+SAFE result")


def run_mechanism_family_broad_residual_diagnostic(
    *,
    ...
    design_path: Path,
    output_dir: Path,
    search_workers: int = n158.SEARCH_WORKERS,
    ...
    if any(not path.is_file() for path in paths.values()):
        raise FileNotFoundError("NEXT159 input is missing")
    ...
    if require_formal_inputs and input_hashes != EXPECTED_INPUT_SHA256:
        ...
        raise ValueError(f"NEXT159 formal input identity differs: {differing}")
    manifest158 = json.loads(paths["next158_manifest"].read_text())
    outputs158 = manifest158.get("outputs_sha256", {})
    if (
        ...
        or manifest158.get("executed_source_sha256", {}).get(
            "src/next158_mechanism_family_consensus_search.py"
        )
        != _sha256_file(Path(n158.__file__).resolve())
        or outputs158.get(n158.SEARCH_NAME) != input_hashes["next158_search"]
    ):
        raise ValueError("NEXT159 prior provenance differs")
    ...
    specs = n158.build_candidate_specs(bases=bases, physical_term_ids=physical_ids)
    ...
    if len(combined) != len(extended) or not np.isfinite(endpoint).all():
        raise ValueError("NEXT159 endpoint row accounting differs")
    combined, virtual_terms, runtime = n158.materialize_candidates(
    ...
    published_all = pd.read_parquet(paths["next158_search"])
    if set(published_all["candidate_key"].astype(str)) != set(virtual_by_candidate):
        raise ValueError("NEXT159 published universe differs")
    ...
    if require_formal_inputs and (
        ...
        or set(published["coordination_protection_weight"].astype(float)) != {0.0}
        or set(published["packing_protection_weight"].astype(float)) != {0.5}
    ):
        raise ValueError("NEXT159 frozen diagnostic population differs")
    ...
    for _, row in published.iterrows():
        ...
        if tables is None:
            raise RuntimeError("NEXT159 candidate has no threshold table")
        ...
        if diagnostic["passes_broad"] or bool(row["passes_broad_all_cells"]):
            raise RuntimeError("NEXT159 contradicts NEXT158 BROAD result")
    ...
    summary = {
        "protocol": PROTOCOL,
        "diagnostic_mode": "offline_discovery_label_next158_broad_constraint_residual",
    ...
    source_paths = {
        ...
        "src/next159_mechanism_family_broad_residual_diagnostic.py": Path(__file__).resolve(),
    }
    source_hashes = {
    ...
    try:
        ...
        if any(_sha256_file(path) != input_hashes[name] for name, path in paths.items()):
            raise RuntimeError("NEXT159 input changed before publication")
        if any(
            _sha256_file(path) != source_hashes[name]
            for name, path in source_paths.items()
        ):
            raise RuntimeError("NEXT159 source changed before publication")
...
def main() -> int:
    ...
    parser.add_argument("--search-workers", type=int, default=n158.SEARCH_WORKERS)
    parser.add_argument("--allow-nonformal-inputs", action="store_true")
    args = parser.parse_args()
    manifest = run_mechanism_family_broad_residual_diagnostic(
...
__all__ = [
    ...
    "run_mechanism_family_broad_residual_diagnostic",
