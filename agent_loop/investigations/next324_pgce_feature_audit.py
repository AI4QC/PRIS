#!/usr/bin/env python3
"""Audit frozen PGCE hypotheses on physically isolated discovery outcomes."""
...
import src.next323_periodic_global_contact_equilibrium as n323
from src.next102_cross_source_dobvr_features import _sha256_file, _write_json


PROTOCOL = "2026-08-09-next324-pgce-feature-audit-v1"
MANIFEST_NAME = "MANIFEST.json"
CATALOGUE_NAME = "NEXT324_PGCE_FEATURE_CATALOGUE.json"
AUDIT_NAME = "NEXT324_PGCE_FEATURE_AUDIT.json"
TABLE_NAME = "next324_pgce_feature_audit.parquet"
HYPOTHESES = tuple((name, n323.FEATURE_DIRECTIONS[name]) for name in n323.FEATURE_NAMES)
QUANTILES = n268.QUANTILES
EXPECTED_NEXT268_SOURCE_SHA256 = (
    "36af0e632c8c3aae93f78271f837f693d49eff6ae88cdcad8edde42b3e0eaf64"
)
EXPECTED_NEXT323_SOURCE_SHA256 = (
    "338cdbc942cc814279206c8ee9682719f4d7b49f49e4ea2c6eb117cec51aeabf"
)
NEXT323_SOURCE_PATH = "src/next323_periodic_global_contact_equilibrium.py"
REQUIRED_STAGES = (*n268.REQUIRED_STAGES, 323)
...
EXPECTED_INPUT_SHA256 = {
    **{
    ...
    "design": n323.EXPECTED_DESIGN_SHA256,
    "next323_manifest": "34db0a7cc9a1428d6e228003078dd644121f9aef45d524f5d56e19a19389372f",
    "next323_catalogue": "d8aa8e0b5383d90a5511eadf658b9bb887b35583ce3e2c1edcb6225c211cb1ad",
    "next323_scigen": "c07d063692b8b2347ce1094467405b87b6c88e5393f2c043783e66960c412115",
    "next323_wyformer": "3441643c1eb708de42366c510f1a313778e6a7a4081700d01510a53b566024d9",
}
...
def select_eligible_hypotheses(
    ...
    try:
        ...
        raise ValueError("NEXT324 audit ranking table differs") from exc
...
def bounded_protection(
    *, values: object, direction: str, q_lo: float, q_hi: float
    ...
    if direction != "protected_high":
        raise ValueError("NEXT324 bounded protection inputs differ")
    try:
        return n268.bounded_protection(
            values=values, direction=direction, q_lo=q_lo, q_hi=q_hi
        )
    except ValueError as exc:
        raise ValueError("NEXT324 bounded protection inputs differ") from exc


def _index_pgce_by_prefixed_material_id(
    *, table: pd.DataFrame, source: str, expected_material_ids: object
    ...
    if set(extra["material_id"]) != expected:
        raise ValueError(f"NEXT324 {source} material identity differs")
...
def _paths(
    *,
    ...
    paths.update(
        {
            "design": design_path,
            "next323_manifest": roots["next323"] / n323.MANIFEST_NAME,
            "next323_catalogue": roots["next323"] / n323.CATALOGUE_NAME,
            "next323_scigen": roots["next323"] / n323.FEATURE_FILES["scigen"],
            "next323_wyformer": roots["next323"] / n323.FEATURE_FILES["wyformer"],
        }
    )
    return paths


def _verify_next323(paths: Mapping[str, Path], input_hashes: Mapping[str, str]):
    ...
    manifest = json.loads(paths["next323_manifest"].read_text())
    catalogue = json.loads(paths["next323_catalogue"].read_text())
    tables = {
        "scigen": pd.read_parquet(paths["next323_scigen"]),
        "wyformer": pd.read_parquet(paths["next323_wyformer"]),
    }
    expected_outputs = {
        n323.CATALOGUE_NAME: input_hashes["next323_catalogue"],
        n323.FEATURE_FILES["scigen"]: input_hashes["next323_scigen"],
        n323.FEATURE_FILES["wyformer"]: input_hashes["next323_wyformer"],
    }
    if (
        _sha256_file(Path(n268.__file__).resolve()) != EXPECTED_NEXT268_SOURCE_SHA256
        or _sha256_file(Path(n323.__file__).resolve()) != EXPECTED_NEXT323_SOURCE_SHA256
        or manifest.get("protocol") != n323.PROTOCOL
        or manifest.get("next324_audit_authorized") is not True
        ...
        or manifest.get("executed_source_sha256", {}).get(NEXT323_SOURCE_PATH)
        != EXPECTED_NEXT323_SOURCE_SHA256
        or any(manifest.get(key) is not value for key, value in BOUNDARY_FLAGS.items())
        or catalogue.get("feature_names") != list(n323.FEATURE_NAMES)
        or catalogue.get("feature_directions") != n323.FEATURE_DIRECTIONS
        or catalogue.get("feature_count") != len(n323.FEATURE_NAMES)
        or catalogue.get("graph")
        != "NEXT279 reciprocal directed active-facet periodic contact incidences"
        or catalogue.get("formula")
        != "max alpha subject to site equilibrium, reciprocal equality, sum(f)=1, f_c>=alpha/M"
        or catalogue.get("coefficients")
        != "dimensionless dual contact certificates, not physical forces or stresses"
        or catalogue.get("minimum_formal_coverage") != n323.MINIMUM_FORMAL_COVERAGE
        or catalogue.get("directions_frozen_before_outcome") is not True
    ):
        raise ValueError("NEXT324 NEXT323 provenance differs")
    for source, expected_rows in n323.EXPECTED_ROWS.items():
        table = tables[source]
        supported = table["pgce_supported"].fillna(False).astype(bool)
        values = np.column_stack(
            [
                pd.to_numeric(table[name], errors="coerce").to_numpy(float)
                for name in n323.FEATURE_NAMES
            ]
        )
        finite = np.isfinite(values)
        sites = pd.to_numeric(table["pgce_site_count"], errors="coerce")
        contacts = pd.to_numeric(
            table["pgce_directed_contact_count"], errors="coerce"
        )
        reciprocal_pairs = pd.to_numeric(
            table["pgce_reciprocal_pair_count"], errors="coerce"
        )
        residual = pd.to_numeric(
            table["pgce_maximum_equilibrium_residual"], errors="coerce"
        )
        if (
            ...
            or float(supported.mean()) < n323.MINIMUM_FORMAL_COVERAGE
            ...
            or not (reciprocal_pairs[supported] > 0).all()
            or not (2 * reciprocal_pairs[supported] == contacts[supported]).all()
            or not (
                residual[supported].isna()
                | (
                    np.isfinite(residual[supported])
                    & (residual[supported] <= n323.PUBLICATION_RESIDUAL_TOLERANCE)
                )
            ).all()
            ...
            raise ValueError(f"NEXT324 NEXT323 {source} table differs")
    return (*prior[:-1], tables)


def run_pgce_feature_audit(
    *,
    ...
    """Audit exactly the one frozen PGCE hypothesis."""

    if not set(REQUIRED_STAGES).issubset(stage_dirs):
        raise ValueError("NEXT324 stage directory universe differs")
    if not set(REQUIRED_DESIGN_STAGES).issubset(design_paths):
        raise ValueError("NEXT324 design path universe differs")
    roots = {
    ...
    if any(not path.is_file() for path in paths.values()):
        raise FileNotFoundError("NEXT324 input is missing")
    ...
    if require_formal_inputs and input_hashes != EXPECTED_INPUT_SHA256:
        ...
        raise ValueError(f"NEXT324 formal input identity differs: {differing}")
    (
        eligible,
        eligible214,
        primary_key,
        ...
        formula222,
        pgce_tables,
    ) = _verify_next323(paths, input_hashes)
    n227 = n268.n227
    ...
    for source_name in ("scigen", "wyformer"):
        indexed = _index_pgce_by_prefixed_material_id(
            table=pgce_tables[source_name],
            source=source_name,
    ...
    if cohort_counts != n227.EXPECTED_COHORT_COUNTS:
        raise ValueError("NEXT324 rejected cohort counts differ")
    ...
    for feature, direction in HYPOTHESES:
        ...
        if not len(finite):
            raise ValueError(f"NEXT324 {feature} has no finite discovery values")
    ...
    catalogue = {
        ...
        "features": list(n323.FEATURE_NAMES),
        "feature_directions": n323.FEATURE_DIRECTIONS,
    ...
    audit = {
        "protocol": PROTOCOL,
        "audit_mode": "fixed_next224_rejected_extreme_pgce_audit",
        "base_candidate_key_sha256": n227.EXPECTED_BASE_CANDIDATE_KEY_SHA256,
        "base_threshold": n227.EXPECTED_BASE_THRESHOLD,
        "cohort_counts": {
        ...
        "next325_search_authorized": bool(selected is not None),
    ...
    source_paths = {
        ...
        NEXT323_SOURCE_PATH: Path(n323.__file__).resolve(),
        "src/next324_pgce_feature_audit.py": Path(__file__).resolve(),
    }
    ...
    try:
        ...
        manifest = {
            ...
            "next325_search_authorized": bool(selected is not None),
            "pgce_branch_terminated": selected is None,
            "new_formula_searched": False,
            "new_formula_selected": False,
        ...
        if any(_sha256_file(path) != input_hashes[name] for name, path in paths.items()):
            raise RuntimeError("NEXT324 input changed before publication")
        if any(_sha256_file(path) != source_hashes[name] for name, path in source_paths.items()):
            raise RuntimeError("NEXT324 source changed before publication")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    manifest = run_pgce_feature_audit(
