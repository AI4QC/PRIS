#!/usr/bin/env python3
"""Diagnose the frozen NEXT285 power-cell shape--volume BROAD residual."""
...
import src.next285_power_cell_shape_volume_margin_local_search as n285
from src.next102_cross_source_dobvr_features import _sha256_file, _write_json


PROTOCOL = "2026-08-09-next286-power-cell-shape-volume-broad-diagnostic-v1"
MANIFEST_NAME = "MANIFEST.json"
DIAGNOSTIC_NAME = "NEXT286_PSVC_BROAD_DIAGNOSTIC.json"
TABLE_NAME = "next286_power_cell_shape_volume_broad_diagnostic.parquet"
EXPECTED_DESIGN_SHA256 = n285.EXPECTED_DESIGN_SHA256
EXPECTED_AMENDMENT_SHA256 = n285.EXPECTED_AMENDMENT_SHA256
EXPECTED_NEXT262_SOURCE_SHA256 = (
    "2e074b9575985fadf1e680b367c72ab2e3c9b21bbd996ad3e1af1f00b73f7421"
)
EXPECTED_NEXT285_SOURCE_SHA256 = (
    "81a6b5b5b2dd75c6e109f27999ccb9d2c8115303e7274ded4000e7c7e352f0d1"
)
EXPECTED_CANDIDATE_COUNT = 42
EXPECTED_CANDIDATE_KEY_SHA256 = (
    "35f3b483d8e2a1159b973ab4f747c2158e92bbdcc20b6f97c13aaf5d4af13ca6"
)
EXPECTED_NEXT285_SEARCH_SHA256 = (
    "929e9f943e7344b187418643ca1602c011d2771f60be60d276e6d982309aaa5f"
)
NEXT270_REFERENCE_FAILED_COUNT = 5
NEXT270_REFERENCE_SHORTFALL = 0.0955435292756307
SEARCH_WORKERS = n285.SEARCH_WORKERS
BOUNDARY_FLAGS = n285.BOUNDARY_FLAGS
REQUIRED_STAGES = (*n285.REQUIRED_STAGES, 285)
REQUIRED_DESIGN_STAGES = (*n285.REQUIRED_DESIGN_STAGES, 285)
EXPECTED_INPUT_SHA256 = {
    **{
        key: value
        for key, value in n285.EXPECTED_INPUT_SHA256.items()
        if key not in {"design", "amendment"}
    },
    "next285_design": n285.EXPECTED_INPUT_SHA256["design"],
    "next285_amendment": n285.EXPECTED_INPUT_SHA256["amendment"],
    "design": EXPECTED_DESIGN_SHA256,
    "amendment": EXPECTED_AMENDMENT_SHA256,
    "next285_manifest": (
        "c1b68258a224c46ce27214a2e4a714f6994682fe91f5516a6292a4f42b7b991c"
    ),
    "next285_catalogue": (
        "55bcca2990ff7b398ea0c58de80d8edd7e6a7f196f4c844005a6b12a207d8e3c"
    ),
    "next285_evaluation": (
        "eb75799dd5eb21a196e080e357eb61ef5109c5a031aa0faf5cfa9189dd6a8f99"
    ),
    "next285_formula": (
        "07d914b8eba6c59f595e4962a9f2ae7c080e00d1f2ddf4e755b89c1ba995d014"
    ),
    "next285_search": EXPECTED_NEXT285_SEARCH_SHA256,
}
...
def select_diagnostic_candidates(published: pd.DataFrame) -> pd.DataFrame:
    try:
        ...
        raise ValueError("NEXT286 published candidate columns differ") from exc
...
def candidate_key_sha256(frame: pd.DataFrame) -> str:
    try:
        ...
        raise ValueError("NEXT286 candidate identity table differs") from exc
...
def select_closest_residual(frame: pd.DataFrame) -> pd.Series:
    try:
        ...
        raise ValueError("NEXT286 residual population differs") from exc
...
def _paths(
    *,
    ...
    paths = n285._paths(
        ...
        design_paths={
            stage: design_paths[stage] for stage in n285.REQUIRED_DESIGN_STAGES
        },
        design_path=design_paths[285],
        amendment_path=amendment_path,
    )
    paths["next285_design"] = paths.pop("design")
    paths["next285_amendment"] = paths.pop("amendment")
    paths.update(
        {
            "design": design_path,
            "amendment": amendment_path,
            "next285_manifest": roots["next285"] / n285.MANIFEST_NAME,
            "next285_catalogue": roots["next285"] / n285.CATALOGUE_NAME,
            "next285_evaluation": roots["next285"] / n285.EVALUATION_NAME,
            "next285_formula": roots["next285"] / n285.FORMULA_NAME,
            "next285_search": roots["next285"] / n285.SEARCH_NAME,
        }
    )
    return paths


def _verify_next285(paths: Mapping[str, Path], input_hashes: Mapping[str, str]):
    prior_paths = {key: paths[key] for key in n285.EXPECTED_INPUT_SHA256}
    prior_paths["design"] = paths["next285_design"]
    prior_paths["amendment"] = paths["next285_amendment"]
    prior_hashes = {key: input_hashes[key] for key in n285.EXPECTED_INPUT_SHA256}
    prior_hashes["design"] = input_hashes["next285_design"]
    prior_hashes["amendment"] = input_hashes["next285_amendment"]
    prior = n285._verify_next284(prior_paths, prior_hashes)
    manifest = json.loads(paths["next285_manifest"].read_text())
    catalogue = json.loads(paths["next285_catalogue"].read_text())
    evaluation = json.loads(paths["next285_evaluation"].read_text())
    formula = json.loads(paths["next285_formula"].read_text())
    table = pd.read_parquet(paths["next285_search"])
    selected = select_diagnostic_candidates(table)
    expected_outputs = {
        n285.CATALOGUE_NAME: input_hashes["next285_catalogue"],
        n285.EVALUATION_NAME: input_hashes["next285_evaluation"],
        n285.FORMULA_NAME: input_hashes["next285_formula"],
        n285.SEARCH_NAME: input_hashes["next285_search"],
    }
    if (
        manifest.get("protocol") != n285.PROTOCOL
        or manifest.get("candidate_count") != n285.EXPECTED_CANDIDATE_COUNT
        or manifest.get("eligible_new_candidate_count") != n285.EXPECTED_ELIGIBLE_COUNT
        or manifest.get("passes_all_cross_source_discovery_gates") is not False
        or manifest.get("next286_diagnostic_authorized") is not True
        or manifest.get("next286_candidate_count") != EXPECTED_CANDIDATE_COUNT
        or manifest.get("next286_candidate_key_sha256") != EXPECTED_CANDIDATE_KEY_SHA256
        or manifest.get("outputs_sha256") != expected_outputs
        or manifest.get("executed_source_sha256", {}).get(
            "src/next285_power_cell_shape_volume_margin_local_search.py"
        )
        != EXPECTED_NEXT285_SOURCE_SHA256
        or _sha256_file(Path(n285.__file__).resolve()) != EXPECTED_NEXT285_SOURCE_SHA256
        ...
        or catalogue.get("candidate_count") != n285.EXPECTED_CANDIDATE_COUNT
        ...
        or evaluation.get("next286_candidate_count") != EXPECTED_CANDIDATE_COUNT
        or evaluation.get("next286_candidate_key_sha256") != EXPECTED_CANDIDATE_KEY_SHA256
        or formula.get("kind") != "psvc_triangular_margin_local_x0_no_dft_score"
        ...
        or len(table) != n285.EXPECTED_CANDIDATE_COUNT
        or len(selected) != EXPECTED_CANDIDATE_COUNT
        or candidate_key_sha256(selected) != EXPECTED_CANDIDATE_KEY_SHA256
    ):
        raise ValueError("NEXT286 NEXT285 provenance differs")
    return (*prior, selected)


def run_power_cell_shape_volume_broad_diagnostic(
    *,
    ...
    """Reproduce NEXT285 and compute its unchanged discovery BROAD residual."""

    if not set(REQUIRED_STAGES).issubset(stage_dirs):
        raise ValueError("NEXT286 stage directory universe differs")
    if not set(REQUIRED_DESIGN_STAGES).issubset(design_paths):
        raise ValueError("NEXT286 design path universe differs")
    roots = {
    ...
    if type(search_workers) is not int or search_workers <= 0:
        raise ValueError("NEXT286 search_workers must be a positive exact integer")
    if any(not path.is_file() for path in paths.values()):
        raise FileNotFoundError("NEXT286 input is missing")
    ...
    if require_formal_inputs and input_hashes != EXPECTED_INPUT_SHA256:
        ...
        raise ValueError(f"NEXT286 formal input identity differs: {differing}")
    (
        eligible_prior,
        eligible214,
        primary_key,
        ...
        eligible284,
        published285,
    ) = _verify_next285(paths, input_hashes)
    combined, feature_tables, base_score, base_support, endpoint, _ = (
        n285.n227._reconstruct_next224_frontier(
            paths=paths,
    ...
    all_specs = n285.build_psvc_candidate_specs(
        base_candidate_key=base_key, eligible_table=eligible284
    )
    keys = set(published285["candidate_key"].astype(str))
    specs = [spec for spec in all_specs if str(spec["candidate_key"]) in keys]
    if len(specs) != EXPECTED_CANDIDATE_COUNT:
        raise ValueError("NEXT286 selected specification universe differs")
    virtual, terms, runtime, activity = n285.materialize_psvc_candidates(
        features=combined,
    ...
    evaluator = (
        n285.n223.n222.n215.n214.n212.n210.n208.n205.n203.n202.n200.n194.n130.n125
    ...
    n164 = n285.n223.n222.n215.n214.n164
    n164._verify_reproduction(rerun=rerun["candidate_records"], published=published285)
    ...
    published_by_key = published285.set_index("candidate_key", drop=False)
    ...
    for term in terms:
        ...
        if not np.array_equal(support, base_support):
            raise RuntimeError("NEXT286 candidate support differs from NEXT214")
        tables = n164._threshold_tables(
            score=score, supported=support, endpoint=endpoint, cells=cells
        )
        if tables is None:
            raise RuntimeError("NEXT286 candidate has no threshold table")
        ...
        if residual["passes_broad"]:
            raise RuntimeError("NEXT286 contradicts NEXT285 BROAD result")
    ...
    if (
        ...
        raise RuntimeError("NEXT286 residual population differs")
    closest = select_closest_residual(per_candidate)
    improves = n285.n223.n222.strictly_improves(
        closest,
        {
    ...
    summary = {
        "protocol": PROTOCOL,
        "diagnostic_mode": "offline_discovery_label_next285_broad_residual",
        "candidate_count": len(per_candidate),
        "candidate_key_sha256": EXPECTED_CANDIDATE_KEY_SHA256,
        "next285_record_population_reproduced": True,
        "next285_candidate_universe_reproduced": True,
        ...
        "power_cell_shape_volume_branch_closed": bool(not improves),
    ...
    source_paths = {
        ...
        "src/next285_power_cell_shape_volume_margin_local_search.py": Path(n285.__file__).resolve(),
        "src/next286_power_cell_shape_volume_broad_diagnostic.py": Path(__file__).resolve(),
    }
    ...
    try:
        ...
        manifest = {
            ...
            "next285_record_population_reproduced": True,
            "next285_candidate_universe_reproduced": True,
            "next285_all_gate_candidate_count": 0,
        ...
        if any(_sha256_file(path) != input_hashes[name] for name, path in paths.items()):
            raise RuntimeError("NEXT286 input changed before publication")
        if any(_sha256_file(path) != source_hashes[name] for name, path in source_paths.items()):
            raise RuntimeError("NEXT286 source changed before publication")
...
def main() -> int:
    ...
    manifest = run_power_cell_shape_volume_broad_diagnostic(
