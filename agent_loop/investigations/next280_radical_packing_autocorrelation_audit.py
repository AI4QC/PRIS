#!/usr/bin/env python3
"""Audit frozen radical-packing autocorrelation hypotheses on discovery."""
...
PROTOCOL = "2026-08-09-next280-radical-packing-autocorrelation-audit-v1"
MANIFEST_NAME = "MANIFEST.json"
CATALOGUE_NAME = "NEXT280_PRPA_FEATURE_CATALOGUE.json"
AUDIT_NAME = "NEXT280_PRPA_FEATURE_AUDIT.json"
TABLE_NAME = "next280_radical_packing_autocorrelation_audit.parquet"
EXPECTED_DESIGN_SHA256 = n279.EXPECTED_DESIGN_SHA256
HYPOTHESES = tuple(
    (name, n279.FEATURE_DIRECTIONS[name]) for name in n279.FEATURE_NAMES
)
QUANTILES = n268.QUANTILES
EXPECTED_NEXT268_SOURCE_SHA256 = (
    "36af0e632c8c3aae93f78271f837f693d49eff6ae88cdcad8edde42b3e0eaf64"
)
EXPECTED_NEXT279_SOURCE_SHA256 = (
    "b224e46424de199122b8f90061612edfcfd0318fb85178d404b6119f967df5c2"
)
NEXT279_SOURCE_PATH = "src/next279_radical_packing_autocorrelation.py"
REQUIRED_STAGES = (*n268.REQUIRED_STAGES, 279)
...
EXPECTED_INPUT_SHA256 = {
    **{
    ...
    "amendment": n279.EXPECTED_AMENDMENT_SHA256,
    "second_amendment": n279.EXPECTED_SECOND_AMENDMENT_SHA256,
    "next279_manifest": (
        "29ddf52a0203b0797c5b0e339c9815ff115dd521eb1f65b4c34c4676d8f73485"
    ),
    "next279_catalogue": (
        "7d3389bb1766cdc9e51126a153e3e2fbfc756655b8229aabcee2023d8bda7c5f"
    ),
    "next279_scigen": (
        "542d5ad5a9fc4a5b0b163fa88865a081bac1c707b0b966c86b99e58009b813dc"
    ),
    "next279_wyformer": (
        "df986f751b67e73ab42d9a81c3a6ca20e04e398adf0dc76ae3cb3b3096edcc0a"
    ),
}
...
def select_eligible_hypotheses(
    ...
    try:
        ...
        raise ValueError("NEXT280 audit ranking table differs") from exc
...
def bounded_protection(
    *, values: object, direction: str, q_lo: float, q_hi: float
    ...
    try:
        ...
        raise ValueError("NEXT280 bounded protection inputs differ") from exc


def _index_prpa_by_prefixed_material_id(
    *, table: pd.DataFrame, source: str, expected_material_ids: object
    ...
    if set(extra["material_id"]) != expected:
        raise ValueError(f"NEXT280 {source} material identity differs")
    ...
    paths.update(
        {
            ...
            "second_amendment": second_amendment_path,
            "next279_manifest": roots["next279"] / n279.MANIFEST_NAME,
            "next279_catalogue": roots["next279"] / n279.CATALOGUE_NAME,
            "next279_scigen": roots["next279"] / n279.FEATURE_FILES["scigen"],
            "next279_wyformer": roots["next279"] / n279.FEATURE_FILES["wyformer"],
        }
    )
    return paths


def _verify_next279(paths: Mapping[str, Path], input_hashes: Mapping[str, str]):
    ...
    manifest = json.loads(paths["next279_manifest"].read_text())
    catalogue = json.loads(paths["next279_catalogue"].read_text())
    tables = {
        "scigen": pd.read_parquet(paths["next279_scigen"]),
        "wyformer": pd.read_parquet(paths["next279_wyformer"]),
    }
    expected_outputs = {
        n279.CATALOGUE_NAME: input_hashes["next279_catalogue"],
        n279.FEATURE_FILES["scigen"]: input_hashes["next279_scigen"],
        n279.FEATURE_FILES["wyformer"]: input_hashes["next279_wyformer"],
    }
    if (
        _sha256_file(Path(n268.__file__).resolve()) != EXPECTED_NEXT268_SOURCE_SHA256
        or _sha256_file(Path(n279.__file__).resolve())
        != EXPECTED_NEXT279_SOURCE_SHA256
        or manifest.get("protocol") != n279.PROTOCOL
        or manifest.get("next280_audit_authorized") is not True
        ...
        or manifest.get("executed_source_sha256", {}).get(NEXT279_SOURCE_PATH)
        != EXPECTED_NEXT279_SOURCE_SHA256
        or any(manifest.get(key) is not value for key, value in BOUNDARY_FLAGS.items())
        or catalogue.get("feature_names") != list(n279.FEATURE_NAMES)
        or catalogue.get("feature_directions") != n279.FEATURE_DIRECTIONS
        or catalogue.get("feature_count") != len(n279.FEATURE_NAMES)
        or catalogue.get("contact_graph")
        != "reciprocal_active_periodic_radical_facet_incidences"
        or catalogue.get("labels_opened") is not False
    ):
        raise ValueError("NEXT280 NEXT279 provenance differs")
    for source, expected_rows in (("scigen", 13_470), ("wyformer", 5_232)):
        ...
        finite = np.column_stack(
            [
                ...
                for name in n279.FEATURE_NAMES
            ]
        )
        ...
        if (
            ...
            raise ValueError(f"NEXT280 NEXT279 {source} table differs")
    return (*prior[:-1], tables)


def run_radical_packing_autocorrelation_audit(
    *,
    ...
    """Audit the eight frozen PRPA hypotheses."""

    if not set(REQUIRED_STAGES).issubset(stage_dirs):
        raise ValueError("NEXT280 stage directory universe differs")
    if not set(REQUIRED_DESIGN_STAGES).issubset(design_paths):
        raise ValueError("NEXT280 design path universe differs")
    roots = {
    ...
    paths = _paths(
        ...
        second_amendment_path=Path(second_amendment_path).resolve(),
    )
    if os.path.lexists(target):
        raise FileExistsError(str(target))
    if any(not path.is_file() for path in paths.values()):
        raise FileNotFoundError("NEXT280 input is missing")
    ...
    if require_formal_inputs and input_hashes != EXPECTED_INPUT_SHA256:
        ...
        raise ValueError(f"NEXT280 formal input identity differs: {differing}")
    (
        eligible,
        eligible214,
        primary_key,
        ...
        formula222,
        prpa_tables,
    ) = _verify_next279(paths, input_hashes)
    n227 = n268.n227
    ...
    for source_name in ("scigen", "wyformer"):
        indexed = _index_prpa_by_prefixed_material_id(
            table=prpa_tables[source_name],
            source=source_name,
    ...
    if cohort_counts != n227.EXPECTED_COHORT_COUNTS:
        raise ValueError("NEXT280 rejected cohort counts differ")
    rows: list[dict[str, object]] = []
    for feature, direction in HYPOTHESES:
        values = pd.to_numeric(combined[feature], errors="coerce").to_numpy(float)
        finite = values[np.isfinite(values)]
        if not len(finite):
            raise ValueError(f"NEXT280 {feature} has no finite discovery values")
    ...
    catalogue = {
        ...
        "second_amendment_sha256": input_hashes["second_amendment"],
        "base_candidate_key_sha256": n227.EXPECTED_BASE_CANDIDATE_KEY_SHA256,
        "base_threshold": n227.EXPECTED_BASE_THRESHOLD,
        "features": list(n279.FEATURE_NAMES),
        "feature_directions": n279.FEATURE_DIRECTIONS,
    ...
    audit = {
        "protocol": PROTOCOL,
        "audit_mode": "fixed_next224_rejected_extreme_radical_packing_autocorrelation_audit",
        "base_candidate_key_sha256": n227.EXPECTED_BASE_CANDIDATE_KEY_SHA256,
        "base_threshold": n227.EXPECTED_BASE_THRESHOLD,
        "cohort_counts": {
        ...
        "next281_search_authorized": bool(selected is not None),
    ...
    source_paths = {
        ...
        NEXT279_SOURCE_PATH: Path(n279.__file__).resolve(),
        "src/next280_radical_packing_autocorrelation_audit.py": Path(__file__).resolve(),
    }
    ...
    try:
        ...
        manifest = {
            ...
            "next281_search_authorized": bool(selected is not None),
            "radical_packing_autocorrelation_branch_terminated": selected is None,
            "new_formula_searched": False,
            "new_formula_selected": False,
        ...
        if any(_sha256_file(path) != input_hashes[name] for name, path in paths.items()):
            raise RuntimeError("NEXT280 input changed before publication")
        if any(_sha256_file(path) != source_hashes[name] for name, path in source_paths.items()):
            raise RuntimeError("NEXT280 source changed before publication")
...
def main() -> int:
    ...
    manifest = run_radical_packing_autocorrelation_audit(
