#!/usr/bin/env python3
"""Audit frozen radius-weighted Delaunay-cage hypotheses on discovery outcomes."""
...
import src.next291_radius_weighted_delaunay_cage_isotropy as n291
from src.next102_cross_source_dobvr_features import _sha256_file, _write_json


PROTOCOL = "2026-08-09-next292-rwdci-feature-audit-v1"
MANIFEST_NAME = "MANIFEST.json"
CATALOGUE_NAME = "NEXT292_RWDCI_FEATURE_CATALOGUE.json"
AUDIT_NAME = "NEXT292_RWDCI_FEATURE_AUDIT.json"
TABLE_NAME = "next292_rwdci_feature_audit.parquet"
HYPOTHESES = tuple((name, n291.FEATURE_DIRECTIONS[name]) for name in n291.FEATURE_NAMES)
QUANTILES = n268.QUANTILES
EXPECTED_NEXT268_SOURCE_SHA256 = (
    "36af0e632c8c3aae93f78271f837f693d49eff6ae88cdcad8edde42b3e0eaf64"
)
EXPECTED_NEXT291_SOURCE_SHA256 = (
    "741afec1960b2dcf393339b38ecbedef410b84a987cae1d3664d9861e794dfdd"
)
NEXT291_SOURCE_PATH = "src/next291_radius_weighted_delaunay_cage_isotropy.py"
REQUIRED_STAGES = (*n268.REQUIRED_STAGES, 291)
...
EXPECTED_INPUT_SHA256 = {
    **{
    ...
    "design": n291.EXPECTED_DESIGN_SHA256,
    "next291_manifest": (
        "df360691a1de9b6dcb7bebdc558c273deb939f948a96c1cb978052eec4bb4406"
    ),
    "next291_catalogue": (
        "e2217ec8bcb47882ab5b1726aaaf270ef01c9efefb806e375435eb92d80429a2"
    ),
    "next291_scigen": (
        "a58493819c525f673b51e0d8d03df0a27656782fcb930c4af4b96b70c890ea3f"
    ),
    "next291_wyformer": (
        "054dcef98e5e973ae6f1c195e1de22d42929e18769da604955ad5726449b517c"
    ),
}
...
def select_eligible_hypotheses(
    ...
    try:
        ...
        raise ValueError("NEXT292 audit ranking table differs") from exc
...
def bounded_protection(
    *, values: object, direction: str, q_lo: float, q_hi: float
    ...
    if direction != "protected_high":
        raise ValueError("NEXT292 bounded protection inputs differ")
    try:
        return n268.bounded_protection(
            values=values, direction=direction, q_lo=q_lo, q_hi=q_hi
        )
    except ValueError as exc:
        raise ValueError("NEXT292 bounded protection inputs differ") from exc


def _index_rwdci_by_prefixed_material_id(
    *, table: pd.DataFrame, source: str, expected_material_ids: object
    ...
    if set(extra["material_id"]) != expected:
        raise ValueError(f"NEXT292 {source} material identity differs")
...
def _paths(
    *,
    ...
    paths.update(
        {
            "design": design_path,
            "next291_manifest": roots["next291"] / n291.MANIFEST_NAME,
            "next291_catalogue": roots["next291"] / n291.CATALOGUE_NAME,
            "next291_scigen": roots["next291"] / n291.FEATURE_FILES["scigen"],
            "next291_wyformer": roots["next291"] / n291.FEATURE_FILES["wyformer"],
        }
    )
    return paths


def _verify_next291(paths: Mapping[str, Path], input_hashes: Mapping[str, str]):
    ...
    manifest = json.loads(paths["next291_manifest"].read_text())
    catalogue = json.loads(paths["next291_catalogue"].read_text())
    tables = {
        "scigen": pd.read_parquet(paths["next291_scigen"]),
        "wyformer": pd.read_parquet(paths["next291_wyformer"]),
    }
    expected_outputs = {
        n291.CATALOGUE_NAME: input_hashes["next291_catalogue"],
        n291.FEATURE_FILES["scigen"]: input_hashes["next291_scigen"],
        n291.FEATURE_FILES["wyformer"]: input_hashes["next291_wyformer"],
    }
    if (
        _sha256_file(Path(n268.__file__).resolve()) != EXPECTED_NEXT268_SOURCE_SHA256
        or _sha256_file(Path(n291.__file__).resolve()) != EXPECTED_NEXT291_SOURCE_SHA256
        or manifest.get("protocol") != n291.PROTOCOL
        or manifest.get("next292_audit_authorized") is not True
        ...
        or manifest.get("executed_source_sha256", {}).get(NEXT291_SOURCE_PATH)
        != EXPECTED_NEXT291_SOURCE_SHA256
        or any(manifest.get(key) is not value for key, value in BOUNDARY_FLAGS.items())
        or catalogue.get("feature_names") != list(n291.FEATURE_NAMES)
        or catalogue.get("feature_directions") != n291.FEATURE_DIRECTIONS
        or catalogue.get("feature_count") != len(n291.FEATURE_NAMES)
        or catalogue.get("quantile_method") != "inverted_cdf"
        or catalogue.get("labels_opened") is not False
    ):
        raise ValueError("NEXT292 NEXT291 provenance differs")
    for source, expected_rows in (("scigen", 13_470), ("wyformer", 5_232)):
        ...
        finite = np.column_stack(
            [
                ...
                for name in n291.FEATURE_NAMES
            ]
        )
        ...
        if (
            ...
            raise ValueError(f"NEXT292 NEXT291 {source} table differs")
    return (*prior[:-1], tables)


def run_rwdci_feature_audit(
    *,
    ...
    """Audit exactly the sixteen frozen RWDCI hypotheses."""

    if not set(REQUIRED_STAGES).issubset(stage_dirs):
        raise ValueError("NEXT292 stage directory universe differs")
    if not set(REQUIRED_DESIGN_STAGES).issubset(design_paths):
        raise ValueError("NEXT292 design path universe differs")
    roots = {
    ...
    if any(not path.is_file() for path in paths.values()):
        raise FileNotFoundError("NEXT292 input is missing")
    ...
    if require_formal_inputs and input_hashes != EXPECTED_INPUT_SHA256:
        ...
        raise ValueError(f"NEXT292 formal input identity differs: {differing}")
    (
        eligible,
        eligible214,
        primary_key,
        ...
        formula222,
        rwdci_tables,
    ) = _verify_next291(paths, input_hashes)
    n227 = n268.n227
    ...
    for source_name in ("scigen", "wyformer"):
        indexed = _index_rwdci_by_prefixed_material_id(
            table=rwdci_tables[source_name],
            source=source_name,
    ...
    if cohort_counts != n227.EXPECTED_COHORT_COUNTS:
        raise ValueError("NEXT292 rejected cohort counts differ")
    rows: list[dict[str, object]] = []
    for feature, direction in HYPOTHESES:
        values = pd.to_numeric(combined[feature], errors="coerce").to_numpy(float)
        finite = values[np.isfinite(values)]
        if not len(finite):
            raise ValueError(f"NEXT292 {feature} has no finite discovery values")
    ...
    catalogue = {
        ...
        "features": list(n291.FEATURE_NAMES),
        "feature_directions": n291.FEATURE_DIRECTIONS,
    ...
    audit = {
        "protocol": PROTOCOL,
        "audit_mode": "fixed_next224_rejected_extreme_rwdci_audit",
        "base_candidate_key_sha256": n227.EXPECTED_BASE_CANDIDATE_KEY_SHA256,
        "base_threshold": n227.EXPECTED_BASE_THRESHOLD,
        "cohort_counts": {
        ...
        "next293_search_authorized": bool(selected is not None),
    ...
    source_paths = {
        ...
        NEXT291_SOURCE_PATH: Path(n291.__file__).resolve(),
        "src/next292_rwdci_feature_audit.py": Path(__file__).resolve(),
    }
    ...
    try:
        ...
        manifest = {
            ...
            "next293_search_authorized": bool(selected is not None),
            "rwdci_branch_terminated": selected is None,
            "new_formula_searched": False,
            "new_formula_selected": False,
        ...
        if any(_sha256_file(path) != input_hashes[name] for name, path in paths.items()):
            raise RuntimeError("NEXT292 input changed before publication")
        if any(_sha256_file(path) != source_hashes[name] for name, path in source_paths.items()):
            raise RuntimeError("NEXT292 source changed before publication")
...
def main() -> int:
    ...
    manifest = run_rwdci_feature_audit(
