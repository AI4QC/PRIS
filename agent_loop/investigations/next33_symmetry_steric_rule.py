"""Bounded NEXT33 development scan and frozen symmetry-steric application."""
...
PROTOCOL = "2026-08-03-next33-symmetry-steric-rule-v1"
SCAN_NAME = "next33_development_scan.parquet"
FROZEN_RULE_NAME = "NEXT33_FROZEN_SYMMETRY_STERIC_RULE.json"
PREDICTIONS_NAME = "next33_predictions.parquet"
...
TERM_CATALOGUE: Mapping[str, Mapping[str, object]] = {
    "sym_onset_high": _term("sym_recovery_onset_rel", 1.0),
    "sym_gain_high": _term("sym_recovery_gain_log2", 1.0),
    "sym_orbit_collapse_high": _term("sym_orbit_collapse", 1.0),
    "sym_residual_rms_high": _term("sym_recovery_residual_rms_rel", 1.0),
    "sym_residual_q95_high": _term("sym_recovery_residual_q95_rel", 1.0),
    "sym_residual_max_high": _term("sym_recovery_residual_max_rel", 1.0),
    "steric_rep12_pa_high": _term("steric_rep12_pa", 1.0),
    "steric_rep12_site_q95_high": _term("steric_rep12_site_q95", 1.0),
    "steric_rep12_site_max_high": _term("steric_rep12_site_max", 1.0),
    ...
    "steric_rep12_tensor_deviator_high": _term("steric_rep12_tensor_deviator", 1.0),
    "steric_overlap2_vector_rms_high": _term("steric_overlap2_vector_rms", 1.0),
    "steric_overlap2_vector_q95_high": _term("steric_overlap2_vector_q95", 1.0),
    "steric_overlap2_tensor_deviator_high": _term("steric_overlap2_tensor_deviator", 1.0),
    "cov_q01_low": _term("cov_q01", -1.0),
...
SYMMETRY_TERMS = tuple(name for name in TERM_CATALOGUE if name.startswith("sym_"))
PHYSICAL_TERMS = tuple(name for name in TERM_CATALOGUE if name not in SYMMETRY_TERMS)
DIAGNOSTIC_ONLY_TERM_SETS = tuple((name,) for name in SYMMETRY_TERMS)

_MECHANISM_PAIRS = (
    ("steric_rep12_pa_high", "steric_rep12_vector_rms_high"),
    ("steric_rep12_site_q95_high", "steric_rep12_vector_q95_high"),
    ("steric_rep12_site_max_high", "steric_rep12_vector_max_high"),
    ("steric_rep12_vector_rms_high", "steric_rep12_tensor_deviator_high"),
    ("steric_overlap2_vector_rms_high", "steric_overlap2_tensor_deviator_high"),
    ("steric_overlap2_vector_q95_high", "steric_rep12_vector_q95_high"),
    ("steric_rep12_pa_high", "cov_q05_low"),
    ("steric_rep12_vector_rms_high", "sivr_site_imbalance_high"),
    ("sym_onset_high", "steric_rep12_vector_rms_high"),
    ("sym_gain_high", "steric_rep12_pa_high"),
    ("sym_orbit_collapse_high", "steric_overlap2_vector_rms_high"),
    ("sym_residual_rms_high", "steric_rep12_vector_rms_high"),
    ("sym_residual_q95_high", "steric_overlap2_vector_q95_high"),
    ("sym_residual_max_high", "steric_rep12_vector_max_high"),
    ("sym_residual_rms_high", "sivr_site_imbalance_high"),
    ("sym_gain_high", "cov_q05_low"),
    ("cov_q05_low", "sivr_edge_mismatch_high"),
)
CANDIDATE_TERM_SETS = (
    DIAGNOSTIC_ONLY_TERM_SETS
    + tuple((name,) for name in PHYSICAL_TERMS)
    + _MECHANISM_PAIRS
)
...
def scan_development_candidates(
    ...
    """Run the frozen NEXT33 catalogue on exposed development endpoints once."""
    ...
    for terms in CANDIDATE_TERM_SETS:
        ...
        promotion_eligible = terms not in DIAGNOSTIC_ONLY_TERM_SETS
        ...
        for fraction in REJECTION_FRACTIONS:
            ...
            rows.append(row)
            if promotion_eligible and all(gates.values()):
                passing.append(
                    (row, {term: parameters[term] for term in terms})
                )
    ...
    rule: dict[str, object] = {
        ...
        "symmetry_only": all(term in SYMMETRY_TERMS for term in terms),
...
def compute_symmetry_steric_risk(
    features: pd.DataFrame, rule: Mapping[str, object]
) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """Apply a frozen NEXT33 rule without endpoint access or refitting."""

    if rule.get("protocol") != PROTOCOL or rule.get("eligible") is not True:
        raise ValueError("frozen NEXT33 rule is not eligible")
    ...
    if not isinstance(raw_terms, list) or not raw_terms or len(raw_terms) > 2:
        raise ValueError("frozen NEXT33 terms are invalid")
    if not isinstance(raw_parameters, Mapping):
        raise ValueError("frozen NEXT33 term parameters are invalid")
    terms = tuple(str(term) for term in raw_terms)
    if (
        terms not in CANDIDATE_TERM_SETS
        or terms in DIAGNOSTIC_ONLY_TERM_SETS
        or set(raw_parameters) != set(terms)
    ):
        raise ValueError("frozen NEXT33 formula is not promotion eligible")
    ...
    for term in terms:
        ...
        if any(
            ...
            raise ValueError("frozen NEXT33 term specification changed")
    ...
    if not np.isfinite(threshold):
        raise ValueError("frozen NEXT33 threshold is invalid")
    reject = support & (score >= threshold)
...
def _validate_feature_artifact(
    ...
    if feature_path.name != FEATURE_NAME or not feature_path.is_file():
        raise ValueError("NEXT33 feature path/name is invalid")
    manifest = _strict_json(manifest_path, role="NEXT33 feature manifest")
    ...
    if (
        ...
        raise ValueError("NEXT33 feature artifact crossed the label-free boundary")
    ...
    if forbidden:
        raise ValueError(f"NEXT33 feature columns crossed the label-free boundary: {forbidden}")
    if "material_id" not in frame or frame.material_id.isna().any() or frame.material_id.astype(str).duplicated().any():
        raise ValueError("NEXT33 feature material IDs are invalid")
    frame = frame.copy()
...
def freeze_development_rule(
    *,
    ...
    features, _manifest = _validate_feature_artifact(
        paths["features"], paths["feature_manifest"]
    )
    if paths["endpoints"].name != ENDPOINT_NAME or not paths["endpoints"].is_file():
        raise ValueError("NEXT33 development endpoint path/name is invalid")
    endpoint_manifest = _strict_json(
        paths["endpoints_manifest"], role="NEXT33 development endpoint manifest"
    )
    ...
    if (
        ...
        raise ValueError("NEXT33 development endpoint artifact is invalid")
    ...
    manifest: dict[str, object] = {
        "protocol": PROTOCOL,
        "mode": "exposed_rattled_relax_next33_development_scan",
        "labels_opened_for_development": True,
        "confirmation_rows_used": 0,
        "confirmation_labels_used_for_selection": False,
        "symmetry_only_promotion_forbidden": True,
        "promoted": rule is not None,
        "candidate_term_sets": [list(terms) for terms in CANDIDATE_TERM_SETS],
        "diagnostic_only_term_sets": [
            list(terms) for terms in DIAGNOSTIC_ONLY_TERM_SETS
        ],
        "rejection_fractions": list(REJECTION_FRACTIONS),
        "inputs_sha256": input_hashes,
        "executed_source_sha256": {"src/next33_symmetry_steric_rule.py": source_hash},
    ...
    try:
        ...
        if any(_sha256(path) != input_hashes[role] for role, path in paths.items()):
            raise RuntimeError("NEXT33 development input changed before publication")
        if _sha256(source_path) != source_hash:
            raise RuntimeError("NEXT33 rule source changed before publication")
...
def apply_frozen_rule(
    *,
    ...
    """Publish label-free NEXT33 predictions for one or more new cohorts."""
    ...
    if (
        ...
        raise ValueError("NEXT33 rule application inputs are invalid")
    rule_manifest = _strict_json(rule_manifest_path, role="NEXT33 frozen-rule manifest")
    outputs = rule_manifest.get("outputs_sha256")
    if (
        rule_manifest.get("protocol") != PROTOCOL
        or rule_manifest.get("promoted") is not True
        or rule_manifest.get("symmetry_only_promotion_forbidden") is not True
        or not isinstance(outputs, Mapping)
        or outputs.get(FROZEN_RULE_NAME) != _sha256(rule_path)
    ):
        raise ValueError("NEXT33 frozen rule is not promoted and hash-locked")
    try:
        rule = json.loads(rule_path.read_text("utf-8"))
    except (OSError, UnicodeError, json.JSONDecodeError) as exc:
        raise ValueError("NEXT33 frozen rule JSON is invalid") from exc
    if not isinstance(rule, dict):
        raise ValueError("NEXT33 frozen rule JSON is invalid")
    ...
    for feature, feature_manifest in zip(features, feature_manifests, strict=True):
        ...
        score, supported, reject = compute_symmetry_steric_risk(frame, rule)
        source = (
    ...
    if predictions.material_id.duplicated().any():
        raise ValueError("NEXT33 prediction material IDs are duplicated")
    source_path = Path(__file__).resolve()
    source_hash = _sha256(source_path)
    ...
    manifest: dict[str, object] = {
        "protocol": PROTOCOL,
        "mode": "prospective_label_free_symmetry_steric_prediction",
        "labels_opened": False,
    ...
    try:
        ...
        if _sha256(rule_path) != input_hashes["frozen_rule"] or _sha256(
            ...
            raise RuntimeError("NEXT33 frozen rule changed before publication")
        for item in feature_inputs:
            if _sha256(Path(item["feature_path"])) != item["feature_sha256"] or _sha256(
                Path(item["manifest_path"])
            ) != item["manifest_sha256"]:
                raise RuntimeError("NEXT33 feature changed before publication")
        if _sha256(source_path) != source_hash:
            raise RuntimeError("NEXT33 rule source changed before publication")
        _publish_directory_no_replace(staging, target)
    finally:
