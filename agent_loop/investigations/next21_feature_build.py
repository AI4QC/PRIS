#!/usr/bin/env python3
"""Build sealed NEXT21 features from sanitized unrelaxed geometry only."""
...
PROTOCOL = "2026-08-02-next21-geometry-only-feature-build-v1"
FEATURE_NAME = "next21_normalized_madelung_features.parquet"
...
def build_feature_batch(
    *,
    ...
    """Build one deterministic, no-replace normalized Madelung batch."""
    ...
    try:
        ...
        manifest = {
            ...
            "counts": {
                ...
                "supported": int(features["nm_supported"].sum()),
                "valence_policy": {
                    str(key): int(value)
            ...
            "executed_source_sha256": {
                ...
                "src/next21_normalized_madelung.py": _sha256(
                    Path(__file__).with_name("next21_normalized_madelung.py")
                ),
                "src/next21_feature_build.py": _sha256(Path(__file__)),
            },
