#!/usr/bin/env python3
"""Robust discovery search over the NEXT65 x0 physics-coupling catalogue."""
...
PROTOCOL = "2026-08-03-next66-odac23-physics-coupling-robust-search-v1"
DESIGN_SHA256 = "3c85091f1474ba1a1f2afcb5eac75868a11c60b5ea46cd9239da1f2ea8175b1d"
...
FORMULA_NAME = "NEXT66_ODAC23_PHYSICS_COUPLING_CANDIDATE.json"
SEARCH_NAME = "NEXT66_ODAC23_PHYSICS_COUPLING_SEARCH.json"
PREDICTIONS_NAME = "next66_odac23_physics_coupling_predictions.parquet"
...
def _read_json(path: Path) -> dict[str, object]:
    ...
    if not isinstance(value, dict):
        raise ValueError("NEXT66 manifest must be an object")
    return value
...
def run_physics_coupling_search(
    *,
    feature_dir: Path,
    ...
    if any(not path.is_file() for path in paths.values()):
        raise FileNotFoundError("NEXT66 input is missing")
    ...
    if any(hashes[name] != digest for name, digest in expected.items()):
        raise ValueError("NEXT66 frozen input hash differs")
    ...
    if (
        ...
        raise ValueError("NEXT66 discovery-only provenance differs")
    ...
    if set(labels["partition_role"]) != {"discovery"}:
        raise ValueError("NEXT66 received non-discovery labels")
    joined = features.merge(labels, on="material_id", how="inner", validate="one_to_one")
    if len(joined) != len(labels):
        raise ValueError("NEXT66 robust discovery identity differs")
    ...
    manifest: dict[str, object] = {
        "protocol": PROTOCOL,
        "mode": "physics_coupling_augmented_robust_discovery_search",
        ...
        "executed_source_sha256": {
            "src/next66_odac23_physics_coupling_search.py": source_hash
        },
    ...
    try:
        ...
        if _sha256(source_path) != source_hash:
            raise RuntimeError("NEXT66 source changed before publication")
        if any(_sha256(path) != hashes[name] for name, path in paths.items()):
            raise RuntimeError("NEXT66 input changed before publication")
...
def main() -> None:
    args = _parse_args()
    manifest = run_physics_coupling_search(
...
__all__ = ["PROTOCOL", "run_physics_coupling_search"]
