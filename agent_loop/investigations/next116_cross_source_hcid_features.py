#!/usr/bin/env python3
"""Discovery-only cross-source materialization of NEXT115 HCID features."""
...
PROTOCOL = "2026-08-08-next116-cross-source-discovery-hcid-v1"
GRAPH_MODE = "voronoi"
DISCOVERY_ROLE = "discovery"
MANIFEST_NAME = "MANIFEST.json"
CATALOGUE_NAME = "NEXT116_HCID_FEATURE_CATALOGUE.json"
FEATURE_FILES = {
    "scigen": "scigen_discovery_hcid_features.parquet",
    "wyformer": "wyformer_discovery_hcid_features.parquet",
}
CATALOGUE_MODES = ("core", "expanded")
NUMERIC_FEATURE_NAMES = tuple(
    name.replace("hcid_", f"hcid_{mode}_", 1)
...
STATUS_COLUMNS = tuple(
    ...
    for mode in CATALOGUE_MODES
    for name in (
        f"hcid_{mode}_supported",
        f"hcid_{mode}_failure",
        f"hcid_{mode}_catalogue_sha256",
        f"hcid_{mode}_pymatgen_version",
        f"hcid_{mode}_scipy_version",
    )
)
...
EXPECTED_INPUT_SHA256 = {
    ...
    "design": "54a0e744190132fc8aa9f799e370da8cdc9d6578e1c67a780bb8f5927989a10b",
}


def compute_hcid_feature_row(structure, *, graph_mode: str) -> dict[str, object]:
    """Compute core and expanded HCID with independent abstention."""
    ...
    for mode in CATALOGUE_MODES:
        try:
            ...
            statuses.update(
                {
                    f"hcid_{mode}_supported": False,
                    f"hcid_{mode}_failure": f"{type(exc).__name__}: {exc}",
                    f"hcid_{mode}_catalogue_sha256": None,
                    f"hcid_{mode}_pymatgen_version": None,
                    f"hcid_{mode}_scipy_version": None,
                }
            )
            continue
        for name, value in result.features.items():
            row[name.replace("hcid_", f"hcid_{mode}_", 1)] = float(value)
        statuses.update(
            {
                f"hcid_{mode}_supported": bool(result.supported),
                f"hcid_{mode}_failure": result.failure_reason,
                f"hcid_{mode}_catalogue_sha256": result.catalogue_sha256,
                f"hcid_{mode}_pymatgen_version": result.pymatgen_version,
                f"hcid_{mode}_scipy_version": result.scipy_version,
            }
        )
    row.update(statuses)
    if tuple(row) != FEATURE_COLUMNS:
        raise RuntimeError("NEXT116 feature row schema differs")
    return row
...
def _compute_scigen_payload(item: tuple[str, bytes]) -> tuple[str, dict[str, object]]:
    ...
    try:
        ...
        row = compute_hcid_feature_row(structure, graph_mode=GRAPH_MODE)
    except Exception as exc:
...
def _source_counts(table: pd.DataFrame) -> dict[str, object]:
    ...
    for mode in CATALOGUE_MODES:
        supported = f"hcid_{mode}_supported"
        failure = f"hcid_{mode}_failure"
...
def _solver_thread_environment(
    *, require_formal_inputs: bool
    ...
    if require_formal_inputs and any(value != "1" for value in values.values()):
        raise ValueError("NEXT116 formal run requires a single-thread solver environment")
    return values


def build_cross_source_discovery_hcid_features(
    *,
    ...
    """Materialize HCID only from physically split discovery geometries."""
    ...
    if any(not path.is_file() for path in paths.values()):
        raise FileNotFoundError("NEXT116 discovery input is missing")
    input_hashes = {name: _sha256_file(path) for name, path in paths.items()}
    if require_formal_inputs and input_hashes != EXPECTED_INPUT_SHA256:
        raise ValueError("NEXT116 formal input identity differs")
    ...
    source_paths = {
        ...
        "src/next116_cross_source_hcid_features.py": Path(__file__).resolve(),
    }
    ...
    try:
        for source in ("scigen", "wyformer"):
            ...
            if len(table) != len(metadata[source]) or any(
                ...
                raise RuntimeError(f"NEXT116 {source} row accounting differs")
        ...
        manifest: dict[str, object] = {
            "protocol": PROTOCOL,
            "mode": "cross_source_discovery_only_raw_x0_hcid_feature_freeze",
            "graph_mode": GRAPH_MODE,
            "workers": workers,
        ...
        if any(_sha256_file(path) != input_hashes[name] for name, path in paths.items()):
            raise RuntimeError("NEXT116 input changed before publication")
        if any(_sha256_file(path) != source_hashes[name] for name, path in source_paths.items()):
            raise RuntimeError("NEXT116 source changed before publication")
