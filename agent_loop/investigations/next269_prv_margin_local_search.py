#!/usr/bin/env python3
"""Frozen one-term PRV margin-local search on the NEXT224 frontier."""
...
n227 = n268.n227
n223 = n261.n223
PROTOCOL = "2026-08-09-next269-prv-margin-local-search-v1"
MANIFEST_NAME = "MANIFEST.json"
CATALOGUE_NAME = "NEXT269_PRV_MARGIN_LOCAL_CATALOGUE.json"
EVALUATION_NAME = "NEXT269_DISCOVERY_EVALUATION.json"
FORMULA_NAME = "NEXT269_FROZEN_CANDIDATE.json"
SEARCH_NAME = "next269_prv_margin_local_search.parquet"
SCORE_COMPOSITION = "nonnegative_next224_plus_triangular_margin_local_signed_prv_term"
...
EXPECTED_DESIGN_SHA256 = n268.EXPECTED_DESIGN_SHA256
...
REQUIRED_STAGES = (*n268.REQUIRED_STAGES, 268)
REQUIRED_DESIGN_STAGES = (*n268.REQUIRED_DESIGN_STAGES, 268)
...
EXPECTED_INPUT_SHA256 = {
    **{
    ...
    "next268_manifest": (
        "11d9185cdfa524e5b8105a68a8405259043d64218a2d4e66386805199b49c8d3"
    ),
    "next268_catalogue": (
        "b55572d48c452cd33ad11f7cb8a4dcf9471855b9316e24e43eccf132ce7613e5"
    ),
    "next268_audit": (
        "1c1ec5125d1ec9c4c7b50646c4cb2701c8e6dfa02f8e4d44898b5b9c34396912"
    ),
    "next268_table": (
        "f827bd61878a62601fece9f54b99a1ae69d018274b9da6aba9d93893c77213d5"
    ),
}


def prv_margin_local_score(
    *,
    base_score: object,
    base_support: object,
    protection: object,
    ...
    try:
        ...
        raise ValueError("NEXT269 margin-local score inputs differ") from exc


def build_prv_candidate_specs(
    *,
    ...
    if len({str(spec["candidate_key"]) for spec in result}) != len(result):
        raise RuntimeError("NEXT269 candidate keys are not unique")
    return result


def materialize_prv_candidates(
    *,
    ...
    for term in terms:
        ...
        new_id = "next269_virtual_candidate__" + hashlib.sha256(
            key.encode()
        ).hexdigest()[:24]
        ...
        term.update(
            {
                "term_id": new_id,
                "feature": new_column,
                "group": "next269_prv_margin_local",
            }
        )
...
def _paths(
    *,
    ...
    paths.update(
        {
            "design": design_path,
            "next268_manifest": roots["next268"] / n268.MANIFEST_NAME,
            "next268_catalogue": roots["next268"] / n268.CATALOGUE_NAME,
            "next268_audit": roots["next268"] / n268.AUDIT_NAME,
            "next268_table": roots["next268"] / n268.TABLE_NAME,
        }
    )
    return paths
...
def _verify_next268(
    ...
    base_paths = {key: paths[key] for key in n268.EXPECTED_INPUT_SHA256}
    base_paths["design"] = paths["next268_design"]
    base_hashes = {key: input_hashes[key] for key in n268.EXPECTED_INPUT_SHA256}
    base_hashes["design"] = input_hashes["next268_design"]
    prior = n268._verify_next267(base_paths, base_hashes)
    manifest = json.loads(paths["next268_manifest"].read_text())
    catalogue = json.loads(paths["next268_catalogue"].read_text())
    audit = json.loads(paths["next268_audit"].read_text())
    table = pd.read_parquet(paths["next268_table"])
    ...
    expected_outputs = {
        n268.CATALOGUE_NAME: input_hashes["next268_catalogue"],
        n268.AUDIT_NAME: input_hashes["next268_audit"],
        n268.TABLE_NAME: input_hashes["next268_table"],
    }
    if (
        manifest.get("protocol") != n268.PROTOCOL
        or manifest.get("eligible_hypothesis_count")
        != EXPECTED_ELIGIBLE_HYPOTHESIS_COUNT
        or manifest.get("eligible_hypothesis_sha256")
        != EXPECTED_ELIGIBLE_HYPOTHESIS_SHA256
        or manifest.get("next269_search_authorized") is not True
        ...
        or manifest.get("executed_source_sha256", {}).get(
            "src/next268_prv_feature_audit.py"
        )
        != EXPECTED_NEXT268_SOURCE_SHA256
        or _sha256_file(Path(n268.__file__).resolve())
        != EXPECTED_NEXT268_SOURCE_SHA256
        ...
        != EXPECTED_ELIGIBLE_HYPOTHESIS_SHA256
        or audit.get("next269_search_authorized") is not True
        ...
        raise ValueError("NEXT269 NEXT268 provenance differs")
...
def _formula_from_spec(spec: Mapping[str, object] | None) -> dict[str, object]:
    ...
    return {
        "protocol": PROTOCOL,
        "kind": "prv_triangular_margin_local_x0_no_dft_score",
        "selected": True,
...
def select_best_new_record(records: pd.DataFrame) -> pd.Series | None:
    try:
        ...
        raise ValueError("NEXT269 reporting selection schema differs") from exc


def _attach_prv_features(
    *,
    combined: pd.DataFrame,
    feature_tables: Mapping[str, pd.DataFrame],
    prv_tables: Mapping[str, pd.DataFrame],
) -> None:
    source = combined["source_dataset"].astype(str).to_numpy()
    for source_name in ("scigen", "wyformer"):
        indexed = n268._index_prv_by_prefixed_material_id(
            table=prv_tables[source_name],
            source=source_name,
        ...
        for name in n268.n267.FEATURE_NAMES:
            combined.loc[mask, name] = ordered_ids.map(indexed[name]).to_numpy()


def run_prv_margin_local_search(
    *,
    ...
    """Run the complete frozen discovery-only NEXT269 search."""

    if not set(REQUIRED_STAGES).issubset(stage_dirs):
        raise ValueError("NEXT269 stage directory universe differs")
    if not set(REQUIRED_DESIGN_STAGES).issubset(design_paths):
        raise ValueError("NEXT269 design path universe differs")
    roots = {
    ...
    if type(search_workers) is not int or search_workers <= 0:
        raise ValueError("NEXT269 search_workers must be a positive exact integer")
    if any(not path.is_file() for path in paths.values()):
        raise FileNotFoundError("NEXT269 input is missing")
    ...
    if require_formal_inputs and input_hashes != EXPECTED_INPUT_SHA256:
        ...
        raise ValueError(f"NEXT269 formal input identity differs: {differing}")
    (
        eligible_prior,
        eligible214,
        primary_key,
        ...
        prv_tables,
        eligible268,
    ) = _verify_next268(paths, input_hashes)
    ...
    if (
        ...
        raise ValueError("NEXT269 NEXT224 base identity differs")
    specs = build_prv_candidate_specs(
    ...
    if len(specs) != EXPECTED_CANDIDATE_COUNT or eligible_count != EXPECTED_ELIGIBLE_COUNT:
        raise RuntimeError("NEXT269 frozen candidate universe differs")
    virtual, terms, runtime, activity = materialize_prv_candidates(
        features=combined,
        base_score=base_score,
    ...
    if (
        ...
        raise RuntimeError("NEXT269 evaluator accounting differs")
    ...
    if len(no_op) != 1 or len(reference_no_op) != 1:
        raise RuntimeError("NEXT269 no-op reproduction identity differs")
    ...
    if selected_row is not None:
        ...
        if str(selected_result["selected"]["record"]["candidate_key"]) != selected_key:
            raise RuntimeError("NEXT269 selected candidate reproduction differs")
        ...
        if selected_spec.get("eligible_new_candidate") is not True:
            raise RuntimeError("NEXT269 reproduction control was selected")
    ...
    if selected is not None and passes != bool(
        ...
        raise RuntimeError("NEXT269 all-gate selection differs")
    if passes and selected is None:
        raise RuntimeError("NEXT269 all-gate candidate was not selected")
    ...
    evaluation = {
        "protocol": PROTOCOL,
        "evaluation_mode": "frozen_prv_margin_local_discovery_search",
        ...
        "next270_diagnostic_authorized": bool(not passes and diagnostic_keys),
        "next270_candidate_count": len(diagnostic_keys),
        "next270_candidate_key_sha256": diagnostic_sha,
    ...
    source_paths = {
        ...
        "src/next269_prv_margin_local_search.py": Path(__file__).resolve(),
    }
    ...
    try:
        ...
        if any(_sha256_file(path) != input_hashes[name] for name, path in paths.items()):
            raise RuntimeError("NEXT269 input changed before publication")
        if any(_sha256_file(path) != source_hashes[name] for name, path in source_paths.items()):
            raise RuntimeError("NEXT269 source changed before publication")
...
def main() -> int:
    ...
    manifest = run_prv_margin_local_search(
