#!/usr/bin/env python3
"""Diagnose the frozen NEXT237 chemistry-conditioned BROAD residual."""
...
PROTOCOL = "2026-08-09-next238-chemistry-conditioned-broad-diagnostic-v1"
MANIFEST_NAME = "MANIFEST.json"
DIAGNOSTIC_NAME = "NEXT238_CHEMISTRY_CONDITIONED_BROAD_DIAGNOSTIC.json"
TABLE_NAME = "next238_chemistry_conditioned_broad_diagnostic.parquet"
EXPECTED_DESIGN_SHA256 = n237.EXPECTED_DESIGN_SHA256
EXPECTED_NEXT237_SOURCE_SHA256 = (
    "5ac86442f4d39c34669f3e9e12f0e12f0574639668dc96874c9b473ee940364c"
)
EXPECTED_CANDIDATE_COUNT = 2140
EXPECTED_CANDIDATE_KEY_SHA256 = (
    "005ebc0a0c56cab758e75903430cde2811e196d5105064b1f9ded68156d887a9"
)
EXPECTED_NEXT237_SEARCH_SHA256 = (
    "d1658f35132bfb778ac78631f6fc2b39e74a1fc8b0ea30378d52e112cc329423"
)
EXPECTED_FRONTIER_FAILED_COUNT = n237.n236.EXPECTED_FRONTIER_FAILED_COUNT
EXPECTED_FRONTIER_SHORTFALL = n237.n236.EXPECTED_FRONTIER_SHORTFALL
SEARCH_WORKERS = n237.SEARCH_WORKERS
BOUNDARY_FLAGS = n237.BOUNDARY_FLAGS
REQUIRED_STAGES = (*n237.REQUIRED_STAGES, 237)
REQUIRED_DESIGN_STAGES = (*n237.REQUIRED_DESIGN_STAGES, 237)
EXPECTED_INPUT_SHA256 = {
    **{
        key: value
        for key, value in n237.EXPECTED_INPUT_SHA256.items()
        if key != "design"
    },
    "next237_design": n237.EXPECTED_INPUT_SHA256["design"],
    "design": EXPECTED_DESIGN_SHA256,
    "next237_manifest": (
        "d45778d83f7c7505c6264a452984f7a4e21329e5eb44e332059e84964c417b36"
    ),
    "next237_catalogue": (
        "f63df69bb191f8472032297c6c89eabc45153a018b1d2237824358f706fed9a5"
    ),
    "next237_evaluation": (
        "028ac9aae8b02d3377b273d805fb03895b84a4a6ef23aad32509ad94615fbd2c"
    ),
    "next237_formula": (
        "22c6fbfb5d5cc8622e49ee31699d0f851aa30f212f90be8178bc7711f11e27ae"
    ),
    "next237_search": EXPECTED_NEXT237_SEARCH_SHA256,
}
...
def select_diagnostic_candidates(published: pd.DataFrame) -> pd.DataFrame:
    ...
    if not isinstance(published, pd.DataFrame) or required - set(published.columns):
        raise ValueError("NEXT238 published candidate columns differ")
    mask = (
...
def candidate_key_sha256(frame: pd.DataFrame) -> str:
    if (
        ...
        raise ValueError("NEXT238 candidate identity table differs")
...
def select_closest_residual(frame: pd.DataFrame) -> pd.Series:
    ...
    if not isinstance(frame, pd.DataFrame) or frame.empty or required - set(frame.columns):
        raise ValueError("NEXT238 residual population differs")
...
def _paths(
    *,
    ...
    paths = n237._paths(
        ...
        design_paths={
            stage: design_paths[stage] for stage in n237.REQUIRED_DESIGN_STAGES
        },
        design_path=design_paths[237],
    )
    paths["next237_design"] = paths.pop("design")
    paths.update(
        {
            "design": design_path,
            "next237_manifest": roots["next237"] / n237.MANIFEST_NAME,
            "next237_catalogue": roots["next237"] / n237.CATALOGUE_NAME,
            "next237_evaluation": roots["next237"] / n237.EVALUATION_NAME,
            "next237_formula": roots["next237"] / n237.FORMULA_NAME,
            "next237_search": roots["next237"] / n237.SEARCH_NAME,
        }
    )
    return paths
...
def _verify_next237(
    ...
    prior_paths["design"] = paths["next237_design"]
    prior_hashes = dict(input_hashes)
    prior_hashes["design"] = input_hashes["next237_design"]
    prior = n237._verify_next236(prior_paths, prior_hashes)
    manifest = json.loads(paths["next237_manifest"].read_text())
    catalogue = json.loads(paths["next237_catalogue"].read_text())
    evaluation = json.loads(paths["next237_evaluation"].read_text())
    formula = json.loads(paths["next237_formula"].read_text())
    table = pd.read_parquet(paths["next237_search"])
    selected = select_diagnostic_candidates(table)
    expected_outputs = {
        n237.CATALOGUE_NAME: input_hashes["next237_catalogue"],
        n237.EVALUATION_NAME: input_hashes["next237_evaluation"],
        n237.FORMULA_NAME: input_hashes["next237_formula"],
        n237.SEARCH_NAME: input_hashes["next237_search"],
    }
    if (
        manifest.get("protocol") != n237.PROTOCOL
        or manifest.get("candidate_count") != n237.EXPECTED_CANDIDATE_COUNT
        or manifest.get("eligible_new_candidate_count")
        != n237.EXPECTED_ELIGIBLE_COUNT
        or manifest.get("passes_all_cross_source_discovery_gates") is not False
        or manifest.get("next238_diagnostic_authorized") is not True
        or manifest.get("next238_candidate_count") != EXPECTED_CANDIDATE_COUNT
        or manifest.get("next238_candidate_key_sha256")
        != EXPECTED_CANDIDATE_KEY_SHA256
        or manifest.get("outputs_sha256") != expected_outputs
        or manifest.get("executed_source_sha256", {}).get(
            "src/next237_chemistry_conditioned_margin_local_search.py"
        )
        != EXPECTED_NEXT237_SOURCE_SHA256
        or _sha256_file(Path(n237.__file__).resolve())
        != EXPECTED_NEXT237_SOURCE_SHA256
        or any(manifest.get(key) is not value for key, value in BOUNDARY_FLAGS.items())
        or catalogue.get("candidate_count") != n237.EXPECTED_CANDIDATE_COUNT
        or evaluation.get("passes_all_cross_source_discovery_gates") is not False
        or evaluation.get("next238_candidate_count") != EXPECTED_CANDIDATE_COUNT
        or evaluation.get("next238_candidate_key_sha256")
        != EXPECTED_CANDIDATE_KEY_SHA256
        or formula.get("kind")
        != "chemistry_conditioned_triangular_margin_local_x0_no_dft_score"
        or formula.get("selected") is not True
        or any(
            formula.get(key) is None
            for key in ("hypothesis", "feature", "conditioner", "direction")
        )
        or any(
        ...
        or len(table) != n237.EXPECTED_CANDIDATE_COUNT
        or len(selected) != EXPECTED_CANDIDATE_COUNT
        or candidate_key_sha256(selected) != EXPECTED_CANDIDATE_KEY_SHA256
    ):
        raise ValueError("NEXT238 NEXT237 provenance differs")
    return (*prior, selected)


def run_chemistry_conditioned_broad_diagnostic(
    *,
    ...
    """Reproduce NEXT237 and compute its unchanged discovery BROAD residual."""

    if not set(REQUIRED_STAGES).issubset(stage_dirs):
        raise ValueError("NEXT238 stage directory universe differs")
    if not set(REQUIRED_DESIGN_STAGES).issubset(design_paths):
        raise ValueError("NEXT238 design path universe differs")
    roots = {
    ...
    if type(search_workers) is not int or search_workers <= 0:
        raise ValueError("NEXT238 search_workers must be a positive exact integer")
    if any(not path.is_file() for path in paths.values()):
        raise FileNotFoundError("NEXT238 input is missing")
    ...
    if require_formal_inputs and input_hashes != EXPECTED_INPUT_SHA256:
        ...
        raise ValueError(f"NEXT238 formal input identity differs: {differing}")
    (
        eligible_prior,
        eligible214,
        primary_key,
        ...
        eligible236,
        published237,
    ) = _verify_next237(paths, input_hashes)
    combined, _, base_score, base_support, endpoint, _ = (
        n237.n227._reconstruct_next224_frontier(
            paths=paths,
    ...
    all_specs = n237.build_conditioned_candidate_specs(
        base_candidate_key=base_key,
        eligible_table=eligible236,
    )
    keys = set(published237["candidate_key"].astype(str))
    specs = [spec for spec in all_specs if str(spec["candidate_key"]) in keys]
    if len(specs) != EXPECTED_CANDIDATE_COUNT:
        raise ValueError("NEXT238 selected specification universe differs")
    virtual, terms, runtime, activity = n237.materialize_conditioned_candidates(
        features=combined,
    ...
    evaluator = (
        n237.n223.n222.n215.n214.n212.n210.n208.n205.n203.n202.n200.n194.n130.n125
    ...
    n164 = n237.n223.n222.n215.n214.n164
    n164._verify_reproduction(
        rerun=rerun["candidate_records"], published=published237
    )
    ...
    published_by_key = published237.set_index("candidate_key", drop=False)
    ...
    for term in terms:
        ...
        if not np.array_equal(support, base_support):
            raise RuntimeError("NEXT238 candidate support differs from NEXT214")
        tables = n164._threshold_tables(
            score=score, supported=support, endpoint=endpoint, cells=cells
        )
        if tables is None:
            raise RuntimeError("NEXT238 candidate has no threshold table")
        ...
        if residual["passes_broad"]:
            raise RuntimeError("NEXT238 contradicts NEXT237 BROAD result")
        ...
        records.append(
            {
                ...
                "conditioner": str(spec["conditioner"]),
    ...
    if (
        ...
        raise RuntimeError("NEXT238 residual population differs")
    closest = select_closest_residual(per_candidate)
    improves = n237.n223.n222.strictly_improves(
        closest,
        {
            "failed_constraint_count": EXPECTED_FRONTIER_FAILED_COUNT,
            "normalized_shortfall_sum": EXPECTED_FRONTIER_SHORTFALL,
        },
    )
    ...
    def residual_record(row: pd.Series) -> dict[str, object]:
        return {
            ...
            "stratum_edges": json.loads(str(row["stratum_edges_json"])),
            "q_lo_by_stratum": json.loads(str(row["q_lo_by_stratum_json"])),
            "q_hi_by_stratum": json.loads(str(row["q_hi_by_stratum_json"])),
    ...
    summary = {
        "protocol": PROTOCOL,
        "diagnostic_mode": "offline_discovery_label_next237_broad_residual",
        "candidate_count": len(per_candidate),
        "candidate_key_sha256": EXPECTED_CANDIDATE_KEY_SHA256,
        "next237_record_population_reproduced": True,
        "next237_candidate_universe_reproduced": True,
        "global_closest": residual_record(closest),
        "next235_reference_failed_constraint_count": EXPECTED_FRONTIER_FAILED_COUNT,
        "next235_reference_normalized_shortfall_sum": EXPECTED_FRONTIER_SHORTFALL,
        ...
        "chemistry_conditioned_certificate_branch_closed": bool(not improves),
    ...
    source_paths = {
        ...
        "src/next237_chemistry_conditioned_margin_local_search.py": Path(
            n237.__file__
        ).resolve(),
        "src/next238_chemistry_conditioned_broad_diagnostic.py": Path(
            __file__
        ).resolve(),
    }
    source_hashes = {
    ...
    try:
        ...
        manifest = {
            ...
            "next237_record_population_reproduced": True,
            "next237_candidate_universe_reproduced": True,
            "next237_all_gate_candidate_count": 0,
        ...
        if any(
            ...
            raise RuntimeError("NEXT238 input changed before publication")
        if any(
            _sha256_file(path) != source_hashes[name]
            for name, path in source_paths.items()
        ):
            raise RuntimeError("NEXT238 source changed before publication")
...
def main() -> int:
    ...
    manifest = run_chemistry_conditioned_broad_diagnostic(
...
__all__ = [
    ...
    "run_chemistry_conditioned_broad_diagnostic",
