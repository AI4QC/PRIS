#!/usr/bin/env python3
"""Audit frozen PRV hypotheses inside the exact NEXT224 rejected cohort."""
...
PROTOCOL = "2026-08-09-next268-prv-feature-audit-v1"
MANIFEST_NAME = "MANIFEST.json"
CATALOGUE_NAME = "NEXT268_PRV_FEATURE_CATALOGUE.json"
AUDIT_NAME = "NEXT268_PRV_FEATURE_AUDIT.json"
TABLE_NAME = "next268_prv_feature_audit.parquet"
EXPECTED_DESIGN_SHA256 = n267.EXPECTED_DESIGN_SHA256
HYPOTHESES = tuple(
    (name, direction)
    for name in n267.FEATURE_NAMES
...
NEXT267_SOURCE_PATH = "src/next267_periodic_radical_voronoi_packing.py"
REQUIRED_STAGES = (*n227.REQUIRED_STAGES, 267)
REQUIRED_DESIGN_STAGES = (*n227.REQUIRED_DESIGN_STAGES, 227, 267)
...
EXPECTED_INPUT_SHA256 = {
    **{
    ...
    "next267_design": EXPECTED_DESIGN_SHA256,
...
def select_eligible_hypotheses(
    ...
    try:
        ...
        raise ValueError("NEXT268 audit ranking table differs") from exc
...
def bounded_protection(
    *, values: object, direction: str, q_lo: float, q_hi: float
    ...
    try:
        ...
        raise ValueError("NEXT268 bounded protection inputs differ") from exc


def _index_prv_by_prefixed_material_id(
    *, table: pd.DataFrame, source: str, expected_material_ids: object
    ...
    if set(extra["material_id"]) != expected:
        raise ValueError(f"NEXT268 {source} material identity differs")
...
def _paths(
    *,
    ...
    paths.update(
        {
            "next267_design": design_paths[267],
            "design": design_path,
            "next267_manifest": roots["next267"] / n267.MANIFEST_NAME,
            "next267_catalogue": roots["next267"] / n267.CATALOGUE_NAME,
            "next267_scigen": roots["next267"] / n267.FEATURE_FILES["scigen"],
            "next267_wyformer": roots["next267"] / n267.FEATURE_FILES["wyformer"],
        }
    )
    return paths
...
def _verify_next267(
    ...
    manifest = json.loads(paths["next267_manifest"].read_text())
    catalogue = json.loads(paths["next267_catalogue"].read_text())
    tables = {
        "scigen": pd.read_parquet(paths["next267_scigen"]),
        "wyformer": pd.read_parquet(paths["next267_wyformer"]),
    }
    expected_outputs = {
        n267.CATALOGUE_NAME: input_hashes["next267_catalogue"],
        n267.FEATURE_FILES["scigen"]: input_hashes["next267_scigen"],
        n267.FEATURE_FILES["wyformer"]: input_hashes["next267_wyformer"],
    }
    if (
        _sha256_file(Path(n227.__file__).resolve()) != EXPECTED_NEXT227_SOURCE_SHA256
        or manifest.get("protocol") != n267.PROTOCOL
        ...
        or manifest.get("executed_source_sha256", {}).get(NEXT267_SOURCE_PATH)
        != EXPECTED_NEXT267_SOURCE_SHA256
        or _sha256_file(Path(n267.__file__).resolve()) != EXPECTED_NEXT267_SOURCE_SHA256
        or any(manifest.get(key) is not value for key, value in n267.BOUNDARY_FLAGS.items())
        or catalogue.get("feature_names") != list(n267.FEATURE_NAMES)
        or catalogue.get("feature_count") != len(n267.FEATURE_NAMES)
        or catalogue.get("power_distance")
        != "squared_euclidean_minus_radius_squared"
        or catalogue.get("radius_policy")
        != ["atomic_radius_calculated", "atomic_radius_fallback"]
        or catalogue.get("quantile_method") != "inverted_cdf"
        or catalogue.get("volume_tiling_relative_tolerance")
        != n267.VOLUME_TILING_RELATIVE_TOLERANCE
        or catalogue.get("labels_opened") is not False
    ):
        raise ValueError("NEXT268 NEXT267 provenance differs")
    for source, expected_rows in (("scigen", 13_470), ("wyformer", 5_232)):
        ...
        if (
            ...
            raise ValueError(f"NEXT268 NEXT267 {source} table differs")
    return (*prior, tables)


def run_prv_feature_audit(
    *,
    ...
    """Audit frozen PRV hypotheses without searching a formula."""

    if not set(REQUIRED_STAGES).issubset(stage_dirs):
        raise ValueError("NEXT268 stage directory universe differs")
    if not set(REQUIRED_DESIGN_STAGES).issubset(design_paths):
        raise ValueError("NEXT268 design path universe differs")
    roots = {
    ...
    if any(not path.is_file() for path in paths.values()):
        raise FileNotFoundError("NEXT268 input is missing")
    ...
    if require_formal_inputs and input_hashes != EXPECTED_INPUT_SHA256:
        ...
        raise ValueError(f"NEXT268 formal input identity differs: {differing}")
    (
        eligible,
        eligible214,
        primary_key,
        ...
        formula222,
        prv_tables,
    ) = _verify_next267(paths, input_hashes)
    ...
    for source_name in ("scigen", "wyformer"):
        indexed = _index_prv_by_prefixed_material_id(
    ...
    if cohort_counts != n227.EXPECTED_COHORT_COUNTS:
        raise ValueError("NEXT268 rejected cohort counts differ")
    rows: list[dict[str, object]] = []
    for feature, direction in HYPOTHESES:
        values = pd.to_numeric(combined[feature], errors="coerce").to_numpy(float)
        finite = values[np.isfinite(values)]
        if not len(finite):
            raise ValueError(f"NEXT268 {feature} has no finite discovery values")
    ...
    catalogue = {
        ...
        "features": list(n267.FEATURE_NAMES),
    ...
    audit = {
        "protocol": PROTOCOL,
        "audit_mode": "fixed_next224_rejected_extreme_prv_feature_audit",
        "base_candidate_key_sha256": n227.EXPECTED_BASE_CANDIDATE_KEY_SHA256,
        "base_threshold": n227.EXPECTED_BASE_THRESHOLD,
        "cohort_counts": {
        ...
        "next269_search_authorized": bool(selected is not None),
    ...
    source_paths = {
        ...
        NEXT267_SOURCE_PATH: Path(n267.__file__).resolve(),
        "src/next268_prv_feature_audit.py": Path(__file__).resolve(),
    }
    ...
    try:
        ...
        manifest = {
            ...
            "next269_search_authorized": bool(selected is not None),
            "prv_branch_terminated": selected is None,
            "new_formula_searched": False,
            "new_formula_selected": False,
        ...
        if any(_sha256_file(path) != input_hashes[name] for name, path in paths.items()):
            raise RuntimeError("NEXT268 input changed before publication")
        if any(_sha256_file(path) != source_hashes[name] for name, path in source_paths.items()):
            raise RuntimeError("NEXT268 source changed before publication")
...
def main() -> int:
    ...
    manifest = run_prv_feature_audit(
