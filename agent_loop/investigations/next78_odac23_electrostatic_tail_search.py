#!/usr/bin/env python3
"""Electrostatic-augmented finite anchored tail search with a safety margin."""
...
PROTOCOL = "2026-08-03-next78-odac23-electrostatic-augmented-tail-search-v1"
DESIGN_SHA256 = "eb81da8e922bdd1a6c731a680e3457cd0fa6601b4ce5404b9d2a5e2d7ab5ab0e"
...
CANDIDATE_FEATURE_NAMES = (
    ...
    + tuple(ANALYTIC_ELECTROSTATIC_FEATURE_NAMES)
)
FORMULA_NAME = "NEXT78_ODAC23_ELECTROSTATIC_TAIL_CANDIDATE.json"
SEARCH_NAME = "NEXT78_ODAC23_ELECTROSTATIC_TAIL_SEARCH.json"
PREDICTIONS_NAME = "next78_odac23_electrostatic_tail_predictions.parquet"
...
def _read_json(path: Path) -> dict[str, object]:
    ...
    if not isinstance(value, dict):
        raise ValueError("NEXT78 JSON must be an object")
    return value
...
def run_electrostatic_tail_search(
    *,
    feature_dir: Path,
    ...
    """Search robust discovery only and keep every non-discovery endpoint closed."""
    ...
    if any(not path.is_file() for path in paths.values()):
        raise FileNotFoundError("NEXT78 input is missing")
    ...
    if any(hashes[name] != digest for name, digest in expected.items()):
        raise ValueError("NEXT78 frozen input hash differs")
    ...
    if (
        ...
        raise ValueError("NEXT78 discovery-only provenance differs")
    ...
    if set(labels["partition_role"]) != {"discovery"}:
        raise ValueError("NEXT78 received non-discovery labels")
    joined = features.merge(labels, on="material_id", how="inner", validate="one_to_one")
    if len(joined) != len(labels):
        raise ValueError("NEXT78 discovery identity differs")
    ...
    formula = {
        **result["selected_formula"],
        ...
        "formula_family": "sealed NEXT67 axis plus finite electrostatic-augmented corrections",
    ...
    manifest: dict[str, object] = {
        "protocol": PROTOCOL,
        "mode": "finite_electrostatic_augmented_tail_discovery_search",
        ...
        "executed_source_sha256": {
            "src/next78_odac23_electrostatic_tail_search.py": source_hash
        },
    ...
    try:
        ...
        if _sha256(source_path) != source_hash:
            raise RuntimeError("NEXT78 source changed before publication")
        if any(_sha256(path) != hashes[name] for name, path in paths.items()):
            raise RuntimeError("NEXT78 input changed before publication")
...
def main() -> None:
    args = _parse_args()
    manifest = run_electrostatic_tail_search(
