"""Bounded NEXT35 development scan and immutable label-free rule replay."""
...
PROTOCOL = "2026-08-03-next35-coulomb-steric-balance-rule-v1"
SCAN_NAME = "next35_development_scan.parquet"
FROZEN_RULE_NAME = "NEXT35_FROZEN_COULOMB_STERIC_BALANCE_RULE.json"
PREDICTIONS_NAME = "next35_predictions.parquet"
...
TERM_CATALOGUE: Mapping[str, Mapping[str, object]] = {
    "acsb_opposition_deficit_high": _term("acsb_opposition_deficit"),
    "acsb_global_residual_high": _term("acsb_global_residual"),
    "acsb_site_residual_rms_high": _term("acsb_site_residual_rms"),
    "acsb_site_residual_q95_high": _term("acsb_site_residual_q95"),
    "acsb_site_residual_max_high": _term("acsb_site_residual_max"),
    "acsb_site_direction_deficit_q95_high": _term(
        "acsb_site_direction_deficit_q95"
    ),
    "acsb_active_disagreement_fraction_high": _term(
        "acsb_active_disagreement_fraction"
    ),
    "aefi_residual_max_high": _term("aefi_residual_max"),
...
MECHANISM_PAIRS = (
    ("acsb_global_residual_high", "steric_rep12_vector_rms_high"),
    ("acsb_site_residual_q95_high", "steric_rep12_vector_max_high"),
    ("acsb_opposition_deficit_high", "aefi_residual_max_high"),
    ("acsb_active_disagreement_fraction_high", "sivr_site_imbalance_high"),
    ("acsb_site_direction_deficit_q95_high", "steric_rep12_vector_rms_high"),
    ("acsb_global_residual_high", "sivr_site_imbalance_high"),
...
def scan_development_candidates(
    ...
    """Evaluate the exact frozen NEXT35 catalogue on exposed development rows."""
    ...
    winning = scan.candidate_id.eq(str(winner["candidate_id"])) & (
        scan.rejection_fraction.eq(float(winner["rejection_fraction"]))
    )
    ...
    rule: dict[str, object] = {
        ...
        "classical_analytic_electrostatics_used": any(
            term.startswith(("acsb_", "aefi_")) for term in terms
        ),
        "analytic_steric_field_used": any(
            term.startswith(("acsb_", "steric_", "sivr_")) for term in terms
        ),
...
def compute_coulomb_steric_balance_risk(
    features: pd.DataFrame, rule: Mapping[str, object]
) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """Apply one frozen NEXT35 rule without endpoint access or refitting."""

    if rule.get("protocol") != PROTOCOL or rule.get("eligible") is not True:
        raise ValueError("frozen NEXT35 rule is not eligible")
    ...
    if not isinstance(raw_terms, list) or not raw_terms or len(raw_terms) > 2:
        raise ValueError("frozen NEXT35 terms are invalid")
    if not isinstance(raw_parameters, Mapping):
        raise ValueError("frozen NEXT35 term parameters are invalid")
    terms = tuple(str(term) for term in raw_terms)
    if terms not in CANDIDATE_TERM_SETS or set(raw_parameters) != set(terms):
        raise ValueError("frozen NEXT35 formula is not in the catalogue")
    ...
    for term in terms:
        ...
        if any(
            ...
            raise ValueError("frozen NEXT35 term specification changed")
    ...
    if not np.isfinite(threshold):
        raise ValueError("frozen NEXT35 threshold is invalid")
    reject = support & (score >= threshold)
...
def _validate_feature_artifact(
    ...
    if feature_path.name != FEATURE_NAME or not feature_path.is_file():
        raise ValueError("NEXT35 feature path/name is invalid")
    manifest = _strict_json(manifest_path, role="NEXT35 feature manifest")
    ...
    if (
        ...
        or manifest.get("analytic_steric_field_used") is not True
        ...
        raise ValueError("NEXT35 feature artifact crossed the label-free boundary")
    ...
    if forbidden:
        raise ValueError(
            f"NEXT35 feature columns crossed the label-free boundary: {forbidden}"
        )
    if (
        ...
        raise ValueError("NEXT35 feature material IDs are invalid")
    frame = frame.copy()
...
def freeze_development_rule(
    *,
    ...
    if paths["endpoints"].name != ENDPOINT_NAME or not paths["endpoints"].is_file():
        raise ValueError("NEXT35 development endpoint path/name is invalid")
    endpoint_manifest = _strict_json(
        paths["endpoints_manifest"], role="NEXT35 development endpoint manifest"
    )
    ...
    if (
        ...
        raise ValueError("NEXT35 development endpoint artifact is invalid")
    ...
    manifest: dict[str, object] = {
        "protocol": PROTOCOL,
        "mode": "exposed_rattled_relax_next35_development_scan",
        ...
        "executed_source_sha256": {
            "src/next35_coulomb_steric_balance_rule.py": source_hash
        },
    ...
    try:
        ...
        if any(_sha256(path) != input_hashes[role] for role, path in paths.items()):
            raise RuntimeError("NEXT35 development input changed before publication")
        if _sha256(source_path) != source_hash:
            raise RuntimeError("NEXT35 rule source changed before publication")
...
def apply_frozen_rule(
    *,
    ...
    """Publish label-free NEXT35 predictions for one or more sealed cohorts."""
    ...
    if (
        ...
        raise ValueError("NEXT35 rule application inputs are invalid")
    rule_manifest = _strict_json(rule_manifest_path, role="NEXT35 frozen-rule manifest")
    ...
    if (
        ...
        raise ValueError("NEXT35 frozen rule is not promoted and hash-locked")
    try:
        rule = json.loads(rule_path.read_text("utf-8"))
    except (OSError, UnicodeError, json.JSONDecodeError) as exc:
        raise ValueError("NEXT35 frozen rule JSON is invalid") from exc
    if not isinstance(rule, dict):
        raise ValueError("NEXT35 frozen rule JSON is invalid")
    ...
    for feature, feature_manifest in zip(features, feature_manifests, strict=True):
        ...
        score, supported, reject = compute_coulomb_steric_balance_risk(frame, rule)
        source = (
    ...
    if predictions.material_id.duplicated().any():
        raise ValueError("NEXT35 prediction material IDs are duplicated")
    source_path = Path(__file__).resolve()
    source_hash = _sha256(source_path)
    ...
    manifest: dict[str, object] = {
        "protocol": PROTOCOL,
        "mode": "prospective_label_free_coulomb_steric_balance_prediction",
        "labels_opened": False,
    ...
    try:
        ...
        if _sha256(rule_path) != input_hashes["frozen_rule"] or _sha256(
            ...
            raise RuntimeError("NEXT35 frozen rule changed before publication")
        for item in feature_inputs:
            if _sha256(Path(item["feature_path"])) != item["feature_sha256"] or _sha256(
                Path(item["manifest_path"])
            ) != item["manifest_sha256"]:
                raise RuntimeError("NEXT35 feature changed before publication")
        if _sha256(source_path) != source_hash:
            raise RuntimeError("NEXT35 rule source changed before publication")
        _publish_directory_no_replace(staging, target)
    finally:
...
__all__ = [
    ...
    "TERM_CATALOGUE",
    "apply_frozen_rule",
    "compute_coulomb_steric_balance_risk",
