#!/usr/bin/env python3
"""Run the frozen joint radical-packing margin-local discovery search."""
...
import src.next272_joint_radical_feature_audit as n272
from src.next102_cross_source_dobvr_features import _sha256_file, _write_json


n227 = n272.n268.n227
n223 = n269.n223
PROTOCOL = "2026-08-09-next273-joint-radical-margin-local-search-v1"
MANIFEST_NAME = "MANIFEST.json"
CATALOGUE_NAME = "NEXT273_JOINT_RADICAL_MARGIN_LOCAL_CATALOGUE.json"
EVALUATION_NAME = "NEXT273_DISCOVERY_EVALUATION.json"
FORMULA_NAME = "NEXT273_FROZEN_CANDIDATE.json"
SEARCH_NAME = "next273_joint_radical_margin_local_search.parquet"
SCORE_COMPOSITION = (
    "nonnegative_next224_plus_triangular_margin_local_signed_joint_radical_term"
)
...
EXPECTED_ELIGIBLE_HYPOTHESIS_COUNT = 7
EXPECTED_ELIGIBLE_HYPOTHESIS_SHA256 = (
    "a6a808c96392a1e9c432a0389910bb6b080d3dd5e855607cd91ad389a3157075"
)
...
EXPECTED_DESIGN_SHA256 = n272.EXPECTED_DESIGN_SHA256
...
BOUNDARY_FLAGS = n272.BOUNDARY_FLAGS
REQUIRED_STAGES = (*n272.REQUIRED_STAGES, 272)
REQUIRED_DESIGN_STAGES = (*n272.REQUIRED_DESIGN_STAGES, 272)
EXPECTED_NEXT269_SOURCE_SHA256 = (
    "5d87161be5a54182642b1df1012abe2771c1ec8ea0165f8c724ae90e4e8e3f95"
)
EXPECTED_NEXT272_SOURCE_SHA256 = (
    "27a87deb93bf9bba9d38136f9dccfcfac40b0debc9c6a140a0f73e0488aedf8e"
)
EXPECTED_INPUT_SHA256 = {
    **{
        key: value
        for key, value in n272.EXPECTED_INPUT_SHA256.items()
        if key != "design"
    },
    "next272_design": n272.EXPECTED_INPUT_SHA256["design"],
    "design": EXPECTED_DESIGN_SHA256,
    "next272_manifest": (
        "1f56cf371ebecb6c73046a1168f84a36b9461175a93ce0e6ca6b16b6318d4308"
    ),
    "next272_catalogue": (
        "f9d0b7a136bbf304a8ec1199842aa967a3c09d3b02bede6e4b423dbc11155018"
    ),
    "next272_audit": (
        "cc394a370caf31f0abd2eae54390dd3dfa5599e8b2e4e756d0579db0cd526797"
    ),
    "next272_table": (
        "6101f1fa0420474bc3cf55fc058dffbf591f97b63e056a2a8b1541f4da0ce6cd"
    ),
}


def joint_margin_local_score(**kwargs: object):
    try:
        return n269.prv_margin_local_score(**kwargs)
    except ValueError as exc:
        raise ValueError("NEXT273 margin-local score inputs differ") from exc


def build_joint_candidate_specs(
    *,
    ...
    if len({str(spec["candidate_key"]) for spec in result}) != len(result):
        raise RuntimeError("NEXT273 candidate keys are not unique")
    return result


def materialize_joint_candidates(
    *,
    ...
    for term in terms:
        ...
        new_id = "next273_virtual_candidate__" + hashlib.sha256(
            key.encode()
        ).hexdigest()[:24]
        ...
        term.update(
            {
                "term_id": new_id,
                "feature": new_column,
                "group": "next273_joint_radical_margin_local",
            }
        )
...
def _paths(
    *,
    ...
    paths = n272._paths(
        ...
        design_paths={
            stage: design_paths[stage] for stage in n272.REQUIRED_DESIGN_STAGES
        },
        design_path=design_paths[272],
    )
    paths["next272_design"] = paths.pop("design")
    paths.update(
        {
            "design": design_path,
            "next272_manifest": roots["next272"] / n272.MANIFEST_NAME,
            "next272_catalogue": roots["next272"] / n272.CATALOGUE_NAME,
            "next272_audit": roots["next272"] / n272.AUDIT_NAME,
            "next272_table": roots["next272"] / n272.TABLE_NAME,
        }
    )
    return paths


def _verify_next272(paths: Mapping[str, Path], input_hashes: Mapping[str, str]):
    prior_paths = {key: paths[key] for key in n272.EXPECTED_INPUT_SHA256}
    prior_paths["design"] = paths["next272_design"]
    prior_hashes = {key: input_hashes[key] for key in n272.EXPECTED_INPUT_SHA256}
    prior_hashes["design"] = input_hashes["next272_design"]
    prior = n272._verify_next271(prior_paths, prior_hashes)
    manifest = json.loads(paths["next272_manifest"].read_text())
    catalogue = json.loads(paths["next272_catalogue"].read_text())
    audit = json.loads(paths["next272_audit"].read_text())
    table = pd.read_parquet(paths["next272_table"])
    ...
    expected_outputs = {
        n272.CATALOGUE_NAME: input_hashes["next272_catalogue"],
        n272.AUDIT_NAME: input_hashes["next272_audit"],
        n272.TABLE_NAME: input_hashes["next272_table"],
    }
    if (
        _sha256_file(Path(n269.__file__).resolve()) != EXPECTED_NEXT269_SOURCE_SHA256
        or _sha256_file(Path(n272.__file__).resolve()) != EXPECTED_NEXT272_SOURCE_SHA256
        or manifest.get("protocol") != n272.PROTOCOL
        or manifest.get("eligible_hypothesis_count")
        != EXPECTED_ELIGIBLE_HYPOTHESIS_COUNT
        or manifest.get("eligible_hypothesis_sha256")
        != EXPECTED_ELIGIBLE_HYPOTHESIS_SHA256
        or manifest.get("next273_search_authorized") is not True
        ...
        or manifest.get("executed_source_sha256", {}).get(
            "src/next272_joint_radical_feature_audit.py"
        )
        != EXPECTED_NEXT272_SOURCE_SHA256
        ...
        != EXPECTED_ELIGIBLE_HYPOTHESIS_SHA256
        or audit.get("next273_search_authorized") is not True
        ...
        raise ValueError("NEXT273 NEXT272 provenance differs")
...
def _formula_from_spec(spec: Mapping[str, object] | None) -> dict[str, object]:
    ...
    return {
        "protocol": PROTOCOL,
        "kind": "joint_radical_triangular_margin_local_x0_no_dft_score",
        "selected": True,
...
def select_best_new_record(records: pd.DataFrame) -> pd.Series | None:
    try:
        ...
        raise ValueError("NEXT273 reporting selection schema differs") from exc


def _attach_joint_features(
    *,
    combined: pd.DataFrame,
    feature_tables: Mapping[str, pd.DataFrame],
    joint_tables: Mapping[str, pd.DataFrame],
) -> None:
    source = combined["source_dataset"].astype(str).to_numpy()
    for source_name in ("scigen", "wyformer"):
        indexed = n272._index_joint_by_prefixed_material_id(
            table=joint_tables[source_name],
            source=source_name,
        ...
        for name in n272.n271.FEATURE_NAMES:
            combined.loc[mask, name] = ordered_ids.map(indexed[name]).to_numpy()


def run_joint_radical_margin_local_search(
    *,
    ...
    """Run the complete frozen discovery-only NEXT273 search."""

    if not set(REQUIRED_STAGES).issubset(stage_dirs):
        raise ValueError("NEXT273 stage directory universe differs")
    if not set(REQUIRED_DESIGN_STAGES).issubset(design_paths):
        raise ValueError("NEXT273 design path universe differs")
    roots = {
    ...
    if type(search_workers) is not int or search_workers <= 0:
        raise ValueError("NEXT273 search_workers must be a positive exact integer")
    if any(not path.is_file() for path in paths.values()):
        raise FileNotFoundError("NEXT273 input is missing")
    ...
    if require_formal_inputs and input_hashes != EXPECTED_INPUT_SHA256:
        ...
        raise ValueError(f"NEXT273 formal input identity differs: {differing}")
    (
        eligible_prior,
        eligible214,
        primary_key,
        ...
        joint_tables,
        eligible272,
    ) = _verify_next272(paths, input_hashes)
    ...
    if (
        ...
        raise ValueError("NEXT273 NEXT224 base identity differs")
    specs = build_joint_candidate_specs(
    ...
    if len(specs) != EXPECTED_CANDIDATE_COUNT or eligible_count != EXPECTED_ELIGIBLE_COUNT:
        raise RuntimeError("NEXT273 frozen candidate universe differs")
    virtual, terms, runtime, activity = materialize_joint_candidates(
        features=combined,
        base_score=base_score,
    ...
    if (
        ...
        raise RuntimeError("NEXT273 evaluator accounting differs")
    ...
    if len(no_op) != 1 or len(reference_no_op) != 1:
        raise RuntimeError("NEXT273 no-op reproduction identity differs")
    ...
    if selected_row is not None:
        ...
        if str(selected_result["selected"]["record"]["candidate_key"]) != selected_key:
            raise RuntimeError("NEXT273 selected candidate reproduction differs")
        ...
        if selected_spec.get("eligible_new_candidate") is not True:
            raise RuntimeError("NEXT273 reproduction control was selected")
    ...
    if selected is not None and passes != bool(selected["record"]["passes_all_discovery_gates"]):
        raise RuntimeError("NEXT273 all-gate selection differs")
    if passes and selected is None:
        raise RuntimeError("NEXT273 all-gate candidate was not selected")
    ...
    catalogue = {
        ...
        "base_threshold": EXPECTED_BASE_THRESHOLD,
        "repair_width": REPAIR_WIDTH,
        "eligible_hypotheses": list(eligible272["hypothesis"].astype(str)),
        "eligible_hypothesis_count": len(eligible272),
    ...
    evaluation = {
        "protocol": PROTOCOL,
        "evaluation_mode": "frozen_joint_radical_margin_local_discovery_search",
        ...
        "next274_diagnostic_authorized": bool(not passes and diagnostic_keys),
        "next274_candidate_count": len(diagnostic_keys),
        "next274_candidate_key_sha256": diagnostic_sha,
    ...
    source_paths = {
        ...
        "src/next272_joint_radical_feature_audit.py": Path(n272.__file__).resolve(),
        "src/next273_joint_radical_margin_local_search.py": Path(__file__).resolve(),
    }
    ...
    try:
        ...
        if any(_sha256_file(path) != input_hashes[name] for name, path in paths.items()):
            raise RuntimeError("NEXT273 input changed before publication")
        if any(_sha256_file(path) != source_hashes[name] for name, path in source_paths.items()):
            raise RuntimeError("NEXT273 source changed before publication")
...
def main() -> int:
    ...
    manifest = run_joint_radical_margin_local_search(
