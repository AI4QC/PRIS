#!/usr/bin/env python3
"""Audit frozen PCFC hypotheses on physically isolated discovery outcomes."""
...
PROTOCOL = "2026-08-09-next296-pcfc-feature-audit-v1"
MANIFEST_NAME = "MANIFEST.json"
CATALOGUE_NAME = "NEXT296_PCFC_FEATURE_CATALOGUE.json"
AUDIT_NAME = "NEXT296_PCFC_FEATURE_AUDIT.json"
TABLE_NAME = "next296_pcfc_feature_audit.parquet"
HYPOTHESES = tuple((name, n295.FEATURE_DIRECTIONS[name]) for name in n295.FEATURE_NAMES)
QUANTILES = n268.QUANTILES
EXPECTED_NEXT268_SOURCE_SHA256 = (
    "36af0e632c8c3aae93f78271f837f693d49eff6ae88cdcad8edde42b3e0eaf64"
)
EXPECTED_NEXT295_SOURCE_SHA256 = (
    "4b92811e7f3c7ac60c1506104a18d2bd9d0fe06c6202e7f34cb996b32cd649a3"
)
NEXT295_SOURCE_PATH = "src/next295_positive_contact_force_closure.py"
REQUIRED_STAGES = (*n268.REQUIRED_STAGES, 295)
...
EXPECTED_INPUT_SHA256 = {
    **{
    ...
    "design": n295.EXPECTED_DESIGN_SHA256,
    "next295_manifest": (
        "a8a9d3dcd5c81d9f2e81f58f936de3dbcf1cd028e4dc28d8d3eb0ffd06e83635"
    ),
    "next295_catalogue": (
        "19eb14662887ddcb40723ace4a4e7d1143166227604dd9be323ee09742156f92"
    ),
    "next295_scigen": (
        "a9f98481cf2959e29470f693e5b44108b60ded42174b2c14b5c805d69efe7d2c"
    ),
    "next295_wyformer": (
        "fce2ebbf89a1089b39f789d14e3aa20d67f48c43249d912717f6030e9aa6d34f"
    ),
}
...
def select_eligible_hypotheses(
    ...
    try:
        ...
        raise ValueError("NEXT296 audit ranking table differs") from exc
...
def bounded_protection(
    *, values: object, direction: str, q_lo: float, q_hi: float
    ...
    if direction != "protected_high":
        raise ValueError("NEXT296 bounded protection inputs differ")
    try:
        return n268.bounded_protection(
            values=values, direction=direction, q_lo=q_lo, q_hi=q_hi
        )
    except ValueError as exc:
        raise ValueError("NEXT296 bounded protection inputs differ") from exc


def _index_pcfc_by_prefixed_material_id(
    *, table: pd.DataFrame, source: str, expected_material_ids: object
    ...
    if set(extra["material_id"]) != expected:
        raise ValueError(f"NEXT296 {source} material identity differs")
...
def _paths(
    *,
    ...
    paths.update(
        {
            "design": design_path,
            "next295_manifest": roots["next295"] / n295.MANIFEST_NAME,
            "next295_catalogue": roots["next295"] / n295.CATALOGUE_NAME,
            "next295_scigen": roots["next295"] / n295.FEATURE_FILES["scigen"],
            "next295_wyformer": roots["next295"] / n295.FEATURE_FILES["wyformer"],
        }
    )
    return paths


def _verify_next295(paths: Mapping[str, Path], input_hashes: Mapping[str, str]):
    ...
    manifest = json.loads(paths["next295_manifest"].read_text())
    catalogue = json.loads(paths["next295_catalogue"].read_text())
    tables = {
        "scigen": pd.read_parquet(paths["next295_scigen"]),
        "wyformer": pd.read_parquet(paths["next295_wyformer"]),
    }
    expected_outputs = {
        n295.CATALOGUE_NAME: input_hashes["next295_catalogue"],
        n295.FEATURE_FILES["scigen"]: input_hashes["next295_scigen"],
        n295.FEATURE_FILES["wyformer"]: input_hashes["next295_wyformer"],
    }
    if (
        _sha256_file(Path(n268.__file__).resolve()) != EXPECTED_NEXT268_SOURCE_SHA256
        or _sha256_file(Path(n295.__file__).resolve()) != EXPECTED_NEXT295_SOURCE_SHA256
        or manifest.get("protocol") != n295.PROTOCOL
        or manifest.get("next296_audit_authorized") is not True
        ...
        or manifest.get("executed_source_sha256", {}).get(NEXT295_SOURCE_PATH)
        != EXPECTED_NEXT295_SOURCE_SHA256
        or any(manifest.get(key) is not value for key, value in BOUNDARY_FLAGS.items())
        or catalogue.get("feature_names") != list(n295.FEATURE_NAMES)
        or catalogue.get("feature_directions") != n295.FEATURE_DIRECTIONS
        or catalogue.get("feature_count") != len(n295.FEATURE_NAMES)
        or catalogue.get("graph_mode") != "crystalnn"
        or catalogue.get("quantile_method") != "inverted_cdf"
        or catalogue.get("labels_opened") is not False
    ):
        raise ValueError("NEXT296 NEXT295 provenance differs")
    for source, expected_rows in n295.EXPECTED_ROWS.items():
        table = tables[source]
        supported = table["pcfc_supported"].fillna(False).astype(bool)
        values = np.column_stack(
            [
                pd.to_numeric(table[name], errors="coerce").to_numpy(float)
                for name in n295.FEATURE_NAMES
            ]
        )
        ...
        if (
            ...
            or float(supported.mean()) < n295.MINIMUM_FORMAL_COVERAGE
            ...
            raise ValueError(f"NEXT296 NEXT295 {source} table differs")
    return (*prior[:-1], tables)


def run_pcfc_feature_audit(
    *,
    ...
    """Audit exactly the thirteen frozen PCFC hypotheses."""

    if not set(REQUIRED_STAGES).issubset(stage_dirs):
        raise ValueError("NEXT296 stage directory universe differs")
    if not set(REQUIRED_DESIGN_STAGES).issubset(design_paths):
        raise ValueError("NEXT296 design path universe differs")
    roots = {
    ...
    if any(not path.is_file() for path in paths.values()):
        raise FileNotFoundError("NEXT296 input is missing")
    ...
    if require_formal_inputs and input_hashes != EXPECTED_INPUT_SHA256:
        ...
        raise ValueError(f"NEXT296 formal input identity differs: {differing}")
    (
        eligible,
        eligible214,
        primary_key,
        ...
        formula222,
        pcfc_tables,
    ) = _verify_next295(paths, input_hashes)
    n227 = n268.n227
    ...
    for source_name in ("scigen", "wyformer"):
        indexed = _index_pcfc_by_prefixed_material_id(
            table=pcfc_tables[source_name],
            source=source_name,
    ...
    if cohort_counts != n227.EXPECTED_COHORT_COUNTS:
        raise ValueError("NEXT296 rejected cohort counts differ")
    rows: list[dict[str, object]] = []
    for feature, direction in HYPOTHESES:
        values = pd.to_numeric(combined[feature], errors="coerce").to_numpy(float)
        finite = values[np.isfinite(values)]
        if not len(finite):
            raise ValueError(f"NEXT296 {feature} has no finite discovery values")
    ...
    catalogue = {
        ...
        "features": list(n295.FEATURE_NAMES),
        "feature_directions": n295.FEATURE_DIRECTIONS,
    ...
    audit = {
        "protocol": PROTOCOL,
        "audit_mode": "fixed_next224_rejected_extreme_pcfc_audit",
        "base_candidate_key_sha256": n227.EXPECTED_BASE_CANDIDATE_KEY_SHA256,
        "base_threshold": n227.EXPECTED_BASE_THRESHOLD,
        "cohort_counts": {
        ...
        "next297_search_authorized": bool(selected is not None),
    ...
    source_paths = {
        ...
        NEXT295_SOURCE_PATH: Path(n295.__file__).resolve(),
        "src/next296_pcfc_feature_audit.py": Path(__file__).resolve(),
    }
    ...
    try:
        ...
        manifest = {
            ...
            "next297_search_authorized": bool(selected is not None),
            "pcfc_branch_terminated": selected is None,
            "new_formula_searched": False,
            "new_formula_selected": False,
        ...
        if any(_sha256_file(path) != input_hashes[name] for name, path in paths.items()):
            raise RuntimeError("NEXT296 input changed before publication")
        if any(_sha256_file(path) != source_hashes[name] for name, path in source_paths.items()):
            raise RuntimeError("NEXT296 source changed before publication")
...
def main() -> int:
    ...
    manifest = run_pcfc_feature_audit(
