#!/usr/bin/env python3
"""Audit frozen PBVEU on physically isolated discovery outcomes only."""
...
import src.next367_periodic_bond_valence_equal_uniformity as n367
import src.next367_pbveu_formal_build as n367b
from src.next102_cross_source_dobvr_features import _sha256_file, _write_json


PROTOCOL = "2026-08-13-next368-pbveu-feature-audit-v1"
MANIFEST_NAME = "MANIFEST.json"
CATALOGUE_NAME = "NEXT368_PBVEU_FEATURE_CATALOGUE.json"
AUDIT_NAME = "NEXT368_PBVEU_FEATURE_AUDIT.json"
TABLE_NAME = "next368_pbveu_feature_audit.parquet"
HYPOTHESES = tuple(
    (name, n367.FEATURE_DIRECTIONS[name]) for name in n367.FEATURE_NAMES
)
QUANTILES = n268.QUANTILES
REQUIRED_STAGES = (*n268.REQUIRED_STAGES, 367)
...
EXPECTED_NEXT367_SOURCE_SHA256 = "fe3247b4c2043261d7a4945d7f8415f8cbe1927014b8c58dc1a2fbd55c59dd5d"
NEXT367_SOURCE_PATH = "src/next367_periodic_bond_valence_equal_uniformity.py"
EXPECTED_INPUT_SHA256 = {
    **{
    ...
    "design": n367.DESIGN_SHA256,
    "next367_manifest": "79ec74501bcd3f375f9b33c5364827ed5f6f5939d1f8ff782788234f637707ed",
    "next367_catalogue": "4f333022ee399187d0b66100b584cdc874901ae550ca3347c32618f2a7ac62f6",
    "next367_scigen": "df009a3c36c262ac6cbfe869b76edc2865df08462eb3719fb364d677a301a9c3",
    "next367_wyformer": "3bef926807091f5f345138d8ae41f974a2166d388deed17aa0c7c9b14985f845",
}
...
def select_eligible_hypotheses(frame: pd.DataFrame):
    try:
        ...
        raise ValueError("NEXT368 audit ranking table differs") from exc
...
def bounded_protection(
    *, values: object, direction: str, q_lo: float, q_hi: float
    ...
    if direction != "protected_high":
        raise ValueError("NEXT368 bounded protection inputs differ")
    try:
        return n268.bounded_protection(
            values=values, direction=direction, q_lo=q_lo, q_hi=q_hi
        )
    except ValueError as exc:
        raise ValueError("NEXT368 bounded protection inputs differ") from exc


def _index_pbveu_by_prefixed_material_id(
    *, table: pd.DataFrame, source: str, expected_material_ids: object
    ...
    if set(extra["material_id"]) != set(
        ...
        raise ValueError(f"NEXT368 {source} material identity differs")
...
def _paths(
    *, roots: Mapping[str, Path], next135_freeze_path: Path,
    ...
    paths.update(
        {
            "design": design_path,
            "next367_manifest": roots["next367"] / n367b.MANIFEST_NAME,
            "next367_catalogue": roots["next367"] / n367b.CATALOGUE_NAME,
            "next367_scigen": roots["next367"] / n367b.FEATURE_FILES["scigen"],
            "next367_wyformer": roots["next367"] / n367b.FEATURE_FILES["wyformer"],
        }
    )
    return paths


def _verify_next367(paths: Mapping[str, Path], input_hashes: Mapping[str, str]):
    ...
    manifest = json.loads(paths["next367_manifest"].read_text())
    catalogue = json.loads(paths["next367_catalogue"].read_text())
    tables = {
        "scigen": pd.read_parquet(paths["next367_scigen"]),
        "wyformer": pd.read_parquet(paths["next367_wyformer"]),
    }
    expected_outputs = {
        n367b.CATALOGUE_NAME: input_hashes["next367_catalogue"],
        n367b.FEATURE_FILES["scigen"]: input_hashes["next367_scigen"],
        n367b.FEATURE_FILES["wyformer"]: input_hashes["next367_wyformer"],
    }
    if (
        _sha256_file(Path(n268.__file__).resolve()) != EXPECTED_NEXT268_SOURCE_SHA256
        or _sha256_file(Path(n367.__file__).resolve()) != EXPECTED_NEXT367_SOURCE_SHA256
        or manifest.get("protocol") != n367b.PROTOCOL
        or manifest.get("next368_audit_authorized") is not True
        ...
        or manifest.get("executed_source_sha256", {}).get(NEXT367_SOURCE_PATH)
        != EXPECTED_NEXT367_SOURCE_SHA256
        or any(manifest.get(key) is not value for key, value in BOUNDARY_FLAGS.items())
        or catalogue.get("feature_names") != list(n367.FEATURE_NAMES)
        or catalogue.get("feature_directions") != n367.FEATURE_DIRECTIONS
        or catalogue.get("feature_count") != 1
        or catalogue.get("graph")
        != "NEXT19 opposite-sign periodic Voronoi multigraph"
        or catalogue.get("formula")
        != "q10_i exp(-KL(bond-valence shares at i || uniform degree at i))"
        or catalogue.get("minimum_formal_coverage")
        != n367b.MINIMUM_FORMAL_COVERAGE
        or catalogue.get("directions_frozen_before_outcome") is not True
    ):
        raise ValueError("NEXT368 NEXT367 provenance differs")
    for source, expected_rows in n367b.EXPECTED_ROWS.items():
        table = tables[source]
        supported = table["pbveu_supported"].fillna(False).astype(bool)
        values = pd.to_numeric(
            table[n367.FEATURE_NAMES[0]], errors="coerce"
        ).to_numpy(float)
        sites = pd.to_numeric(table["pbveu_site_count"], errors="coerce")
        edges = pd.to_numeric(table["pbveu_edge_count"], errors="coerce")
        minimum_degree = pd.to_numeric(table["pbveu_minimum_degree"], errors="coerce")
        maximum_degree = pd.to_numeric(table["pbveu_maximum_degree"], errors="coerce")
        exact_fraction = pd.to_numeric(
            table["pbveu_parameter_exact_fraction"], errors="coerce"
        )
        generic_fraction = pd.to_numeric(
            table["pbveu_parameter_generic_fraction"], errors="coerce"
        )
        if (
            ...
            or float(supported.mean()) < n367b.MINIMUM_FORMAL_COVERAGE
            ...
            or not (minimum_degree[supported] >= 1).all()
            or not (maximum_degree[supported] >= minimum_degree[supported]).all()
            or not (
                np.isfinite(exact_fraction[supported])
                & (exact_fraction[supported] >= 0.0)
                & (exact_fraction[supported] <= 1.0)
            ).all()
            or not (
                np.isfinite(generic_fraction[supported])
                & (generic_fraction[supported] >= 0.0)
                & (generic_fraction[supported] <= 1.0)
            ).all()
            or not (sites[~supported] == 0).all()
            or not (edges[~supported] == 0).all()
        ):
            raise ValueError(f"NEXT368 NEXT367 {source} table differs")
    return (*prior[:-1], tables)


def run_pbveu_feature_audit(
    *,
    ...
    """Audit exactly the one frozen PBVEU hypothesis."""

    if not set(REQUIRED_STAGES).issubset(stage_dirs):
        raise ValueError("NEXT368 stage directory universe differs")
    if not set(REQUIRED_DESIGN_STAGES).issubset(design_paths):
        raise ValueError("NEXT368 design path universe differs")
    roots = {
    ...
    if any(not path.is_file() for path in paths.values()):
        raise FileNotFoundError("NEXT368 input is missing")
    ...
    if require_formal_inputs and input_hashes != EXPECTED_INPUT_SHA256:
        ...
        raise ValueError(f"NEXT368 formal input identity differs: {differing}")
    (
        eligible, eligible214, primary_key, base_start_key, formula214,
        current_key, formula222, pbveu_tables,
    ) = _verify_next367(paths, input_hashes)
    n227 = n268.n227
    ...
    for source_name in ("scigen", "wyformer"):
        indexed = _index_pbveu_by_prefixed_material_id(
            table=pbveu_tables[source_name],
            source=source_name,
        ...
        combined.loc[mask, n367.FEATURE_NAMES[0]] = ordered_ids.map(
            indexed[n367.FEATURE_NAMES[0]]
        ).to_numpy()
    ...
    if cohort_counts != n227.EXPECTED_COHORT_COUNTS:
        raise ValueError("NEXT368 rejected cohort counts differ")
    ...
    for feature, direction in HYPOTHESES:
        ...
        if not len(finite):
            raise ValueError(f"NEXT368 {feature} has no finite discovery values")
    ...
    catalogue = {
        ...
        "features": list(n367.FEATURE_NAMES),
        "feature_directions": n367.FEATURE_DIRECTIONS,
        "hypotheses": [
    ...
    audit = {
        "protocol": PROTOCOL,
        "audit_mode": "fixed_next224_rejected_extreme_pbveu_audit",
        "base_candidate_key_sha256": n227.EXPECTED_BASE_CANDIDATE_KEY_SHA256,
        "base_threshold": n227.EXPECTED_BASE_THRESHOLD,
        "cohort_counts": {
            key: {
        ...
        "next369_search_authorized": bool(selected is not None),
    ...
    source_paths = {
        ...
        NEXT367_SOURCE_PATH: Path(n367.__file__).resolve(),
        "src/next367_pbveu_formal_build.py": Path(n367b.__file__).resolve(),
        "src/next368_pbveu_feature_audit.py": Path(__file__).resolve(),
    }
    source_hashes = {
    ...
    try:
        ...
        manifest = {
            ...
            "next369_search_authorized": bool(selected is not None),
            "pbveu_branch_terminated": selected is None,
            "new_formula_searched": False,
            "new_formula_selected": False,
        ...
        if any(
            ...
            raise RuntimeError("NEXT368 input changed before publication")
        if any(
            _sha256_file(path) != source_hashes[name]
            for name, path in source_paths.items()
        ):
            raise RuntimeError("NEXT368 source changed before publication")
...
__all__ = [
    ...
    "_index_pbveu_by_prefixed_material_id", "bounded_protection",
    "run_pbveu_feature_audit", "select_eligible_hypotheses",
]
