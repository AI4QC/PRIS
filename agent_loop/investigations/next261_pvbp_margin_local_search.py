#!/usr/bin/env python3
"""Frozen one-term PVBP margin-local search on the NEXT224 frontier."""
...
PROTOCOL = "2026-08-09-next261-pvbp-margin-local-search-v1"
MANIFEST_NAME = "MANIFEST.json"
CATALOGUE_NAME = "NEXT261_PVBP_MARGIN_LOCAL_CATALOGUE.json"
EVALUATION_NAME = "NEXT261_DISCOVERY_EVALUATION.json"
FORMULA_NAME = "NEXT261_FROZEN_CANDIDATE.json"
SEARCH_NAME = "next261_pvbp_margin_local_search.parquet"
SCORE_COMPOSITION = "nonnegative_next224_plus_triangular_margin_local_signed_pvbp_term"
...
EXPECTED_ELIGIBLE_HYPOTHESIS_SHA256 = (
    "ff120843deb9b005768979ea1c2e9cd1b2e909119157e905cb3563e43e3e8255"
)
...
EXPECTED_DESIGN_SHA256 = n260.EXPECTED_DESIGN_SHA256
...
BOUNDARY_FLAGS = n260.BOUNDARY_FLAGS
REQUIRED_STAGES = (*n260.REQUIRED_STAGES, 260)
REQUIRED_DESIGN_STAGES = (*n260.REQUIRED_DESIGN_STAGES, 260)
EXPECTED_NEXT257_SOURCE_SHA256 = (
    "10c13d4e82af11d46d9f69d5a0ce372fafdcdd4d7398d1639f2dc070c1d12086"
)
EXPECTED_NEXT260_SOURCE_SHA256 = (
    "3df87ed971e7a3e5e2f21af84b3609837cdd8542e9e3ed2708ef25b4b7037a84"
)
EXPECTED_INPUT_SHA256 = {
    **{
        key: value
        for key, value in n260.EXPECTED_INPUT_SHA256.items()
        if key != "design"
    },
    "next260_design": n260.EXPECTED_INPUT_SHA256["design"],
    "design": EXPECTED_DESIGN_SHA256,
    "next260_manifest": (
        "23505225d7006652e384fb84b3f7517fcb5e2cc18de6c977e289c2550bd48e70"
    ),
    "next260_catalogue": (
        "6fb9bdf91188391b62af75a29560e06080e9719140b088f6046eee2a09d88de2"
    ),
    "next260_audit": (
        "25d354d6b834a8aa6913165a75333db6fc6a35f66a27bfed85bc2282a6db04e1"
    ),
    "next260_table": (
        "e87af035d4f0b28e4c48868098e9a05fa0c1fb81eeb72a2f5d29f142fff0569c"
    ),
}


def pvbp_margin_local_score(
    *,
    base_score: object,
    base_support: object,
    protection: object,
    ...
    try:
        return n257.dvci_margin_local_score(
        ...
        raise ValueError("NEXT261 margin-local score inputs differ") from exc


def build_pvbp_candidate_specs(
    *,
    ...
    """Build one no-op and the complete frozen PVBP search grammar."""

    specs = n257.build_dvci_candidate_specs(
    ...
    if len({str(spec["candidate_key"]) for spec in result}) != len(result):
        raise RuntimeError("NEXT261 candidate keys are not unique")
    return result


def materialize_pvbp_candidates(
    *,
    ...
    """Encode exact PVBP-augmented scores as virtual evaluator terms."""
    ...
    for term in terms:
        ...
        new_id = "next261_virtual_candidate__" + hashlib.sha256(
            key.encode()
        ).hexdigest()[:24]
        ...
        term.update(
            {
                "term_id": new_id,
                "feature": new_column,
                "group": "next261_pvbp_margin_local",
            }
        )
...
def _paths(
    *,
    ...
    paths = n260._paths(
        ...
        design_paths={
            stage: design_paths[stage] for stage in n260.REQUIRED_DESIGN_STAGES
        },
        design_path=design_paths[260],
    )
    paths["next260_design"] = paths.pop("design")
    paths.update(
        {
            "design": design_path,
            "next260_manifest": roots["next260"] / n260.MANIFEST_NAME,
            "next260_catalogue": roots["next260"] / n260.CATALOGUE_NAME,
            "next260_audit": roots["next260"] / n260.AUDIT_NAME,
            "next260_table": roots["next260"] / n260.TABLE_NAME,
        }
    )
    return paths
...
def _verify_next260(
    ...
    base_paths = {key: paths[key] for key in n260.EXPECTED_INPUT_SHA256}
    base_paths["design"] = paths["next260_design"]
    base_hashes = {key: input_hashes[key] for key in n260.EXPECTED_INPUT_SHA256}
    base_hashes["design"] = input_hashes["next260_design"]
    prior = n260._verify_next259(base_paths, base_hashes)
    manifest = json.loads(paths["next260_manifest"].read_text())
    catalogue = json.loads(paths["next260_catalogue"].read_text())
    audit = json.loads(paths["next260_audit"].read_text())
    table = pd.read_parquet(paths["next260_table"])
    ...
    expected_outputs = {
        n260.CATALOGUE_NAME: input_hashes["next260_catalogue"],
        n260.AUDIT_NAME: input_hashes["next260_audit"],
        n260.TABLE_NAME: input_hashes["next260_table"],
    }
    if (
        manifest.get("protocol") != n260.PROTOCOL
        or manifest.get("eligible_hypothesis_count")
        != EXPECTED_ELIGIBLE_HYPOTHESIS_COUNT
        or manifest.get("eligible_hypothesis_sha256")
        != EXPECTED_ELIGIBLE_HYPOTHESIS_SHA256
        or manifest.get("next261_search_authorized") is not True
        ...
        or manifest.get("executed_source_sha256", {}).get(
            "src/next260_pvbp_feature_audit.py"
        )
        != EXPECTED_NEXT260_SOURCE_SHA256
        or _sha256_file(Path(n260.__file__).resolve())
        != EXPECTED_NEXT260_SOURCE_SHA256
        ...
        != EXPECTED_ELIGIBLE_HYPOTHESIS_SHA256
        or audit.get("next261_search_authorized") is not True
        ...
        raise ValueError("NEXT261 NEXT260 provenance differs")
...
def _formula_from_spec(spec: Mapping[str, object] | None) -> dict[str, object]:
    ...
    return {
        "protocol": PROTOCOL,
        "kind": "pvbp_triangular_margin_local_x0_no_dft_score",
        "selected": True,
...
def select_best_new_record(records: pd.DataFrame) -> pd.Series | None:
    ...
    try:
        return n257.select_best_new_record(records)
    except ValueError as exc:
        raise ValueError("NEXT261 reporting selection schema differs") from exc


def _attach_pvbp_features(
    *,
    combined: pd.DataFrame,
    feature_tables: Mapping[str, pd.DataFrame],
    pvbp_tables: Mapping[str, pd.DataFrame],
) -> None:
    source = combined["source_dataset"].astype(str).to_numpy()
    for source_name in ("scigen", "wyformer"):
        indexed = n260._index_pvbp_by_prefixed_material_id(
        ...
        for name in n260.n259.FEATURE_NAMES:
            combined.loc[mask, name] = ordered_ids.map(indexed[name]).to_numpy()


def run_pvbp_margin_local_search(
    *,
    ...
    """Run the complete frozen discovery-only NEXT261 search."""

    if not set(REQUIRED_STAGES).issubset(stage_dirs):
        raise ValueError("NEXT261 stage directory universe differs")
    if not set(REQUIRED_DESIGN_STAGES).issubset(design_paths):
        raise ValueError("NEXT261 design path universe differs")
    roots = {
    ...
    if type(search_workers) is not int or search_workers <= 0:
        raise ValueError("NEXT261 search_workers must be a positive exact integer")
    if any(not path.is_file() for path in paths.values()):
        raise FileNotFoundError("NEXT261 input is missing")
    ...
    if require_formal_inputs and input_hashes != EXPECTED_INPUT_SHA256:
        ...
        raise ValueError(f"NEXT261 formal input identity differs: {differing}")
    (
        eligible_prior,
        eligible214,
        primary_key,
        ...
        pvbp_tables,
        eligible260,
    ) = _verify_next260(paths, input_hashes)
    ...
    _attach_pvbp_features(
        combined=combined,
        feature_tables=feature_tables,
        pvbp_tables=pvbp_tables,
    )
    ...
    if (
        ...
        raise ValueError("NEXT261 NEXT224 base identity differs")
    specs = build_pvbp_candidate_specs(
        base_candidate_key=base_key,
        eligible_table=eligible260,
    )
    ...
    if len(specs) != EXPECTED_CANDIDATE_COUNT or eligible_count != EXPECTED_ELIGIBLE_COUNT:
        raise RuntimeError("NEXT261 frozen candidate universe differs")
    virtual, terms, runtime, activity = materialize_pvbp_candidates(
        features=combined,
        base_score=base_score,
    ...
    if (
        ...
        raise RuntimeError("NEXT261 evaluator accounting differs")
    ...
    if len(no_op) != 1 or len(reference_no_op) != 1:
        raise RuntimeError("NEXT261 no-op reproduction identity differs")
    ...
    if selected_row is not None:
        ...
        if str(selected_result["selected"]["record"]["candidate_key"]) != selected_key:
            raise RuntimeError("NEXT261 selected candidate reproduction differs")
        ...
        if selected_spec.get("eligible_new_candidate") is not True:
            raise RuntimeError("NEXT261 reproduction control was selected")
    ...
    if selected is not None and passes != bool(
        ...
        raise RuntimeError("NEXT261 all-gate selection differs")
    if passes and selected is None:
        raise RuntimeError("NEXT261 all-gate candidate was not selected")
    ...
    catalogue = {
        ...
        "base_threshold": EXPECTED_BASE_THRESHOLD,
        "repair_width": REPAIR_WIDTH,
        "eligible_hypotheses": list(eligible260["hypothesis"].astype(str)),
        "eligible_hypothesis_count": len(eligible260),
    ...
    evaluation = {
        "protocol": PROTOCOL,
        "evaluation_mode": "frozen_pvbp_margin_local_discovery_search",
        ...
        "next262_diagnostic_authorized": bool(not passes and diagnostic_keys),
        "next262_candidate_count": len(diagnostic_keys),
        "next262_candidate_key_sha256": diagnostic_sha,
    ...
    source_paths = {
        ...
        "src/next260_pvbp_feature_audit.py": Path(n260.__file__).resolve(),
        "src/next261_pvbp_margin_local_search.py": Path(__file__).resolve(),
    }
    ...
    try:
        ...
        if any(_sha256_file(path) != input_hashes[name] for name, path in paths.items()):
            raise RuntimeError("NEXT261 input changed before publication")
        if any(_sha256_file(path) != source_hashes[name] for name, path in source_paths.items()):
            raise RuntimeError("NEXT261 source changed before publication")
...
def main() -> int:
    ...
    manifest = run_pvbp_margin_local_search(
...
__all__ = [
    ...
    "build_pvbp_candidate_specs",
    "materialize_pvbp_candidates",
    "pvbp_margin_local_score",
    "run_pvbp_margin_local_search",
