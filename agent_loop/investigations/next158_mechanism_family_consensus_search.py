#!/usr/bin/env python3
"""Frozen search over capped contribution means across physical families."""
...
PROTOCOL = "2026-08-08-next158-mechanism-family-consensus-search-v1"
MANIFEST_NAME = "MANIFEST.json"
CATALOGUE_NAME = "NEXT158_MECHANISM_FAMILY_CONSENSUS_CATALOGUE.json"
EVALUATION_NAME = "NEXT158_DISCOVERY_EVALUATION.json"
SEARCH_NAME = "next158_mechanism_family_consensus_candidate_search.parquet"
EXPECTED_DESIGN_SHA256 = "3901d4b1282f53150083c267534002365470c5d14a5536c3a7734dafbe8a544a"
...
EXPECTED_CANDIDATE_KEY_SHA256 = "5f7697c38cd895267717c8d4779ed9c395de4c9cecdb814d70eb7a31ccd94483"
AGGREGATION = "sum_family_means_clip_each_0p5"
...
def family_capped_mean_sum(
    ...
    if (
        ...
        raise ValueError("NEXT158 contribution matrix differs")
    ...
    for index, term_id in enumerate(term_ids):
        ...
        if len(matches) != 1:
            raise ValueError("NEXT158 term-to-family assignment differs")
        members[matches[0]].append(index)
    if any(not indices for indices in members.values()):
        raise ValueError("NEXT158 family coverage differs")
...
def compose_family_consensus_score(
    *,
    ...
    if (
        ...
        raise ValueError("NEXT158 score arrays differ")
    ...
    if (
        ...
        raise ValueError("NEXT158 supported score values differ")
    score = np.full(len(values), np.nan, dtype=float)
    if supported.any():
        score[supported] = family_capped_mean_sum(values[supported], term_ids)
...
def build_candidate_specs(
    *, bases: pd.DataFrame, physical_term_ids: set[str]
    ...
    if {"term_ids_json", "weights_json"} - set(bases.columns):
        raise ValueError("NEXT158 base schema differs")
    ...
    for _, row in bases.iterrows():
        ...
        if (
            ...
            raise ValueError("NEXT158 physical base formula differs")
        family_members = {
            family: [
                term_id
        ...
        if (
            ...
            raise ValueError("NEXT158 frozen family membership differs")
...
def materialize_candidates(
    *,
    ...
    if len(physical_by_id) != len(physical_terms):
        raise ValueError("NEXT158 physical term identities are duplicated")
    if {
        ...
        raise ValueError("NEXT158 protection feature schema differs")
    ...
    for raw_spec in specs:
        ...
        if (
            ...
            raise ValueError("NEXT158 candidate materialization differs")
        seen.add(key)
        ...
        score, supported = compose_family_consensus_score(
        ...
        if not np.isfinite(encoded[supported]).all():
            raise ValueError("NEXT158 virtual score encoding overflowed")
        digest = hashlib.sha256(key.encode()).hexdigest()[:24]
        virtual_id = f"next158_virtual_candidate__{digest}"
        ...
        virtual_terms.append(
            {
                ...
                "center": 0.0,
                "scale": 1.0 / divisor,
                "group": "next158_mechanism_family_consensus_candidate",
                "encoding": "asinh_sinh_exact_mechanism_family_consensus_score",
...
def run_mechanism_family_consensus_search(
    *,
    ...
    if any(not path.is_file() for path in paths.values()):
        raise FileNotFoundError("NEXT158 input is missing")
    ...
    if require_formal_inputs and input_hashes != EXPECTED_INPUT_SHA256:
        ...
        raise ValueError(f"NEXT158 formal input identity differs: {differing}")
    ...
    if (
        ...
        raise ValueError("NEXT158 family-audit provenance differs")
    ...
    if require_formal_inputs and (
        ...
        raise ValueError("NEXT158 frozen candidate universe differs")
    ...
    def decorate(record: dict[str, object]) -> None:
        ...
        record["score_composition"] = (
            "max(0,sum_family(mean_family(min(weighted_base_contribution,0.5)))"
    ...
    formula["kind"] = (
        "mechanism_family_consensus_physical_base_with_bounded_protections"
    )
    ...
    catalogue = {
        ...
        "next157_selected_statistic": "family_capped_mean_sum",
        "contribution_cap": CONTRIBUTION_CAP,
        "family_prefixes": {
    ...
    source_paths = {
        ...
        "src/next158_mechanism_family_consensus_search.py": Path(__file__).resolve(),
    }
    source_hashes = {
    ...
    try:
        ...
        _write_json(
            ...
            {
                "protocol": PROTOCOL,
                "evaluation_mode": "fixed_mechanism_family_consensus_search",
                "rows": {
        ...
        manifest = {
            ...
            "freeze_authorized": passes,
            "requires_unopened_internal_validation_before_claim": True,
            "mechanism_family_consensus_branch_terminated": not passes,
        ...
        if any(_sha256_file(path) != input_hashes[name] for name, path in paths.items()):
            raise RuntimeError("NEXT158 input changed before publication")
        if any(
            _sha256_file(path) != source_hashes[name]
            for name, path in source_paths.items()
        ):
            raise RuntimeError("NEXT158 source changed before publication")
...
def main() -> int:
    ...
    manifest = run_mechanism_family_consensus_search(
...
__all__ = [
    ...
    "compose_family_consensus_score",
    "family_capped_mean_sum",
    "run_mechanism_family_consensus_search",
]
