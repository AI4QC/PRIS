#!/usr/bin/env python3
"""Audit frozen graph-weighted local rigidity in the NEXT164 repair shell."""
...
PROTOCOL = "2026-08-08-next174-weighted-local-directional-rigidity-audit-v1"
MANIFEST_NAME = "MANIFEST.json"
AUDIT_NAME = "NEXT174_WEIGHTED_LOCAL_DIRECTIONAL_RIGIDITY_AUDIT.json"
TABLE_NAME = "next174_weighted_local_directional_rigidity_audit.parquet"
MIN_FULL_SUPPORT = n169.MIN_FULL_SUPPORT
EXPECTED_DESIGN_SHA256 = n173.EXPECTED_DESIGN_SHA256
EXPECTED_CANDIDATE_KEY_SHA256 = n169.EXPECTED_CANDIDATE_KEY_SHA256
SAFE_THRESHOLD = n169.SAFE_THRESHOLD
BROAD_THRESHOLD = n169.BROAD_THRESHOLD
EXPECTED_INPUT_SHA256 = {
    **n169.EXPECTED_INPUT_SHA256,
    "design": EXPECTED_DESIGN_SHA256,
    "next173_manifest": "a32c923a956658081f58d9048665f2ecae422e7aa4fb3a4ccce4db74f783d755",
    "next173_catalogue": "4231c4556ae9ff24be9873aa03c5ede321f17534d492bae90db624eb6b2e0102",
    "next173_scigen_features": "2132dae5e357d849040fe2f7b54cedd222df30d09d485b5f9832e4619f705224",
    "next173_wyformer_features": "4bbf2b0867c04e63da22434ef15de1af591831241d63ef18bde52d552fa278a2",
}

HYPOTHESES = {
    f"{mode}_{suffix}__high": (f"pwldr_{mode}_{suffix}", 1)
...
def select_weighted_rigidity_hypothesis(
    ...
    if (
        ...
        raise ValueError("NEXT174 audit record schema differs")
...
def _paths(
    ...
    paths.update(
        {
            "next173_manifest": roots["next173"] / n173.MANIFEST_NAME,
            "next173_catalogue": roots["next173"] / n173.CATALOGUE_NAME,
            "next173_scigen_features": roots["next173"] / n173.SCIGEN_NAME,
            "next173_wyformer_features": roots["next173"] / n173.WYFORMER_NAME,
        }
    )
    return paths


def run_weighted_local_directional_rigidity_audit(
    *,
    ...
    """Run the frozen discovery-only NEXT174 audit and publish atomically."""
    ...
    if any(not path.is_file() for path in paths.values()):
        raise FileNotFoundError("NEXT174 input is missing")
    ...
    if require_formal_inputs and input_hashes != EXPECTED_INPUT_SHA256:
        ...
        raise ValueError(f"NEXT174 formal input identity differs: {differing}")
    ...
    if (
        ...
        raise ValueError("NEXT174 closest-candidate provenance differs")
    manifest173 = json.loads(paths["next173_manifest"].read_text())
    if (
        manifest173.get("protocol") != n173.PROTOCOL
        or manifest173.get("labels_or_endpoints_opened") is not False
        or manifest173.get("validation_geometry_opened") is not False
        or manifest173.get("replication_geometry_opened") is not False
        or manifest173.get("dft_calculation_executed") is not False
        or manifest173.get("dft_values_used_by_features") is not False
        or manifest173.get("learned_energy_force_stress_proxy_used") is not False
        or manifest173.get("physical_relaxation_executed") is not False
        or manifest173.get("outputs_sha256", {}).get(n173.CATALOGUE_NAME)
        != input_hashes["next173_catalogue"]
        or manifest173.get("outputs_sha256", {}).get(n173.SCIGEN_NAME)
        != input_hashes["next173_scigen_features"]
        or manifest173.get("outputs_sha256", {}).get(n173.WYFORMER_NAME)
        != input_hashes["next173_wyformer_features"]
        or manifest173.get("executed_source_sha256", {}).get(
            "src/next173_weighted_local_directional_rigidity.py"
        )
        != _sha256_file(Path(n173.__file__).resolve())
    ):
        raise ValueError("NEXT174 weighted-rigidity provenance differs")
    ...
    if len(selected_specs) != 1:
        raise ValueError("NEXT174 candidate reconstruction differs")
    ...
    if len(combined) != len(extended) or not np.isfinite(endpoint).all():
        raise ValueError("NEXT174 endpoint row accounting differs")
    ...
    if len(virtual_terms) != 1 or len(runtime) != 1:
        raise RuntimeError("NEXT174 score materialization differs")
    ...
    for hypothesis in sorted(HYPOTHESES):
        ...
        for population, mask in populations.items():
            ...
            if result is None:
                raise ValueError("NEXT174 AUC population differs")
    ...
    audit_table, selected = select_weighted_rigidity_hypothesis(
    ...
    source_paths = {
        ...
        "src/next174_weighted_local_directional_rigidity_audit.py": Path(__file__).resolve(),
    }
    ...
    try:
        ...
        manifest = {
            ...
            "weighted_local_directional_rigidity_branch_terminated": terminated,
            "new_formula_searched": False,
        ...
        if any(
            ...
            raise RuntimeError("NEXT174 input changed before publication")
        if any(
            _sha256_file(path) != source_hashes[name]
            for name, path in source_paths.items()
        ):
            raise RuntimeError("NEXT174 source changed before publication")
...
def main() -> int:
    ...
    manifest = run_weighted_local_directional_rigidity_audit(
