#!/usr/bin/env python3
"""Audit frozen MOSPC hypotheses on physically isolated discovery outcomes."""
...
PROTOCOL = "2026-08-09-next300-mospc-feature-audit-v1"
MANIFEST_NAME = "MANIFEST.json"
CATALOGUE_NAME = "NEXT300_MOSPC_FEATURE_CATALOGUE.json"
AUDIT_NAME = "NEXT300_MOSPC_FEATURE_AUDIT.json"
TABLE_NAME = "next300_mospc_feature_audit.parquet"
HYPOTHESES = tuple((name, n299.FEATURE_DIRECTIONS[name]) for name in n299.FEATURE_NAMES)
QUANTILES = n268.QUANTILES
EXPECTED_NEXT268_SOURCE_SHA256 = (
    "36af0e632c8c3aae93f78271f837f693d49eff6ae88cdcad8edde42b3e0eaf64"
)
EXPECTED_NEXT299_SOURCE_SHA256 = (
    "dca4a9683af6c4141d792f39a895d9c5f9e4e88399113d28defd7eb2788c064e"
)
NEXT299_SOURCE_PATH = "src/next299_minimal_opposite_sign_periodic_cage.py"
REQUIRED_STAGES = (*n268.REQUIRED_STAGES, 299)
...
EXPECTED_INPUT_SHA256 = {
    **{
    ...
    "design": n299.EXPECTED_DESIGN_SHA256,
    "next299_manifest": (
        "c34c188a49d1e90a058520c094e83068c585f6ef7db98bfbd4626c4374a9b6fd"
    ),
    "next299_catalogue": (
        "b3e751e4ec989b469973da53a0b3583ea49cc178dba162f7db732dd0a48414c7"
    ),
    "next299_scigen": (
        "46dc1577ef47145e487a5711c91f4374fc90671117f78711f750ec76ee4cfbc1"
    ),
    "next299_wyformer": (
        "b522d5ee2b7d9b1f9d9b26f318cffca597211963cd6bd60ad33f5e959ba1f0c9"
    ),
}
...
def select_eligible_hypotheses(
    ...
    try:
        ...
        raise ValueError("NEXT300 audit ranking table differs") from exc
...
def bounded_protection(
    *, values: object, direction: str, q_lo: float, q_hi: float
    ...
    if direction != "protected_high":
        raise ValueError("NEXT300 bounded protection inputs differ")
    try:
        return n268.bounded_protection(
            values=values, direction=direction, q_lo=q_lo, q_hi=q_hi
        )
    except ValueError as exc:
        raise ValueError("NEXT300 bounded protection inputs differ") from exc


def _index_mospc_by_prefixed_material_id(
    *, table: pd.DataFrame, source: str, expected_material_ids: object
    ...
    if set(extra["material_id"]) != expected:
        raise ValueError(f"NEXT300 {source} material identity differs")
...
def _paths(
    *,
    ...
    paths.update(
        {
            "design": design_path,
            "next299_manifest": roots["next299"] / n299.MANIFEST_NAME,
            "next299_catalogue": roots["next299"] / n299.CATALOGUE_NAME,
            "next299_scigen": roots["next299"] / n299.FEATURE_FILES["scigen"],
            "next299_wyformer": roots["next299"] / n299.FEATURE_FILES["wyformer"],
        }
    )
    return paths


def _verify_next299(paths: Mapping[str, Path], input_hashes: Mapping[str, str]):
    ...
    manifest = json.loads(paths["next299_manifest"].read_text())
    catalogue = json.loads(paths["next299_catalogue"].read_text())
    tables = {
        "scigen": pd.read_parquet(paths["next299_scigen"]),
        "wyformer": pd.read_parquet(paths["next299_wyformer"]),
    }
    expected_outputs = {
        n299.CATALOGUE_NAME: input_hashes["next299_catalogue"],
        n299.FEATURE_FILES["scigen"]: input_hashes["next299_scigen"],
        n299.FEATURE_FILES["wyformer"]: input_hashes["next299_wyformer"],
    }
    if (
        _sha256_file(Path(n268.__file__).resolve()) != EXPECTED_NEXT268_SOURCE_SHA256
        or _sha256_file(Path(n299.__file__).resolve()) != EXPECTED_NEXT299_SOURCE_SHA256
        or manifest.get("protocol") != n299.PROTOCOL
        or manifest.get("next300_audit_authorized") is not True
        ...
        or manifest.get("executed_source_sha256", {}).get(NEXT299_SOURCE_PATH)
        != EXPECTED_NEXT299_SOURCE_SHA256
        or any(manifest.get(key) is not value for key, value in BOUNDARY_FLAGS.items())
        or catalogue.get("feature_names") != list(n299.FEATURE_NAMES)
        or catalogue.get("feature_directions") != n299.FEATURE_DIRECTIONS
        or catalogue.get("feature_count") != len(n299.FEATURE_NAMES)
        or catalogue.get("cage_order") != 4
        or catalogue.get("fourth_distance_ties_included") is not True
        or catalogue.get("outside_certificate")
        != "sigma_min_times_max_0_R_plus_1_minus_D_infinity"
        or catalogue.get("quantile_method") != "inverted_cdf"
        or catalogue.get("labels_opened") is not False
    ):
        raise ValueError("NEXT300 NEXT299 provenance differs")
    for source, expected_rows in n299.EXPECTED_ROWS.items():
        table = tables[source]
        supported = table["mospc_supported"].fillna(False).astype(bool)
        values = np.column_stack(
            [
                pd.to_numeric(table[name], errors="coerce").to_numpy(float)
                for name in n299.FEATURE_NAMES
            ]
        )
        ...
        if (
            ...
            or float(supported.mean()) < n299.MINIMUM_FORMAL_COVERAGE
            ...
            or not (maximum[supported] <= n299.MAX_RETAINED_CAGE_SIZE).all()
            ...
            or not (minimum[~supported] == 0).all()
            or not (maximum[~supported] == 0).all()
            or not (translation[~supported] == 0).all()
        ):
            raise ValueError(f"NEXT300 NEXT299 {source} table differs")
    return (*prior[:-1], tables)


def run_mospc_feature_audit(
    *,
    ...
    """Audit exactly the thirteen frozen MOSPC hypotheses."""

    if not set(REQUIRED_STAGES).issubset(stage_dirs):
        raise ValueError("NEXT300 stage directory universe differs")
    if not set(REQUIRED_DESIGN_STAGES).issubset(design_paths):
        raise ValueError("NEXT300 design path universe differs")
    roots = {
    ...
    if any(not path.is_file() for path in paths.values()):
        raise FileNotFoundError("NEXT300 input is missing")
    ...
    if require_formal_inputs and input_hashes != EXPECTED_INPUT_SHA256:
        ...
        raise ValueError(f"NEXT300 formal input identity differs: {differing}")
    (
        eligible,
        eligible214,
        primary_key,
        ...
        formula222,
        mospc_tables,
    ) = _verify_next299(paths, input_hashes)
    n227 = n268.n227
    ...
    for source_name in ("scigen", "wyformer"):
        indexed = _index_mospc_by_prefixed_material_id(
    ...
    if cohort_counts != n227.EXPECTED_COHORT_COUNTS:
        raise ValueError("NEXT300 rejected cohort counts differ")
    rows: list[dict[str, object]] = []
    for feature, direction in HYPOTHESES:
        values = pd.to_numeric(combined[feature], errors="coerce").to_numpy(float)
        finite = values[np.isfinite(values)]
        if not len(finite):
            raise ValueError(f"NEXT300 {feature} has no finite discovery values")
    ...
    catalogue = {
        ...
        "features": list(n299.FEATURE_NAMES),
        "feature_directions": n299.FEATURE_DIRECTIONS,
    ...
    audit = {
        "protocol": PROTOCOL,
        "audit_mode": "fixed_next224_rejected_extreme_mospc_audit",
        "base_candidate_key_sha256": n227.EXPECTED_BASE_CANDIDATE_KEY_SHA256,
        "base_threshold": n227.EXPECTED_BASE_THRESHOLD,
        "cohort_counts": {
        ...
        "next301_search_authorized": bool(selected is not None),
    ...
    source_paths = {
        ...
        NEXT299_SOURCE_PATH: Path(n299.__file__).resolve(),
        "src/next300_mospc_feature_audit.py": Path(__file__).resolve(),
    }
    ...
    try:
        ...
        manifest = {
            ...
            "next301_search_authorized": bool(selected is not None),
            "mospc_branch_terminated": selected is None,
            "new_formula_searched": False,
            "new_formula_selected": False,
        ...
        if any(_sha256_file(path) != input_hashes[name] for name, path in paths.items()):
            raise RuntimeError("NEXT300 input changed before publication")
        if any(_sha256_file(path) != source_hashes[name] for name, path in source_paths.items()):
            raise RuntimeError("NEXT300 source changed before publication")
...
def main() -> int:
    ...
    manifest = run_mospc_feature_audit(
