#!/usr/bin/env python3
"""Discovery-only finite search against the robust NEXT60 scaffold endpoint."""
...
PROTOCOL = "2026-08-03-next61-odac23-robust-endpoint-discovery-search-v1"
DESIGN_SHA256 = "7c54d644d10d09211b8c552eb9725ee560ab8d063fcf936f34ac5663ca8d2b07"
...
FORMULA_NAME = "NEXT61_ODAC23_ROBUST_DISCOVERY_CANDIDATE.json"
SEARCH_NAME = "NEXT61_ODAC23_ROBUST_DISCOVERY_SEARCH.json"
PREDICTIONS_NAME = "next61_odac23_robust_discovery_predictions.parquet"
...
def run_robust_discovery_search(
    *,
    feature_dir: Path,
    ...
    if any(not path.is_file() for path in paths.values()):
        raise FileNotFoundError("NEXT61 input is missing")
    ...
    if any(hashes[name] != digest for name, digest in expected.items()):
        raise ValueError("NEXT61 frozen input hash differs")
    ...
    if (
        ...
        raise ValueError("NEXT61 robust discovery provenance differs")
    ...
    if set(labels["partition_role"]) != {"discovery"}:
        raise ValueError("NEXT61 received non-discovery labels")
    joined = features.merge(labels, on="material_id", how="inner", validate="one_to_one")
    if len(joined) != len(labels) or not set(labels["material_id"]).issubset(
        set(features["material_id"])
    ):
        raise ValueError("NEXT61 robust discovery identity differs")
    ...
    formula = {
        **result["selected_formula"],
        "protocol": PROTOCOL,
        "training_partition": "ODAC23 official train / frozen robust discovery role only",
        "endpoint_definition": {
    ...
    manifest: dict[str, object] = {
        "protocol": PROTOCOL,
        "mode": "robust_scaffold_discovery_only_finite_search",
        ...
        "executed_source_sha256": {
            "src/next61_odac23_robust_endpoint_search.py": source_hash,
    ...
    try:
        ...
        if _sha256(source_path) != source_hash:
            raise RuntimeError("NEXT61 source changed before publication")
        if any(_sha256(path) != hashes[name] for name, path in paths.items()):
            raise RuntimeError("NEXT61 input changed before publication")
...
def main() -> None:
    args = _parse_args()
    manifest = run_robust_discovery_search(
...
__all__ = ["PROTOCOL", "run_robust_discovery_search"]
