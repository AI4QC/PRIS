#!/usr/bin/env python3
"""LOKO stability for the np-next-20260801c six-family law loop."""
...
def _reference_by_kind(path: Path | None, *, floor: float) -> dict[str, dict[str, float]]:
    if path is None:
        return {}
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = run_leave_one_kind_out(
        ...
        reference_by_kind=_reference_by_kind(args.reference_report, floor=args.floor),
        candidate_builder=build_next3_candidate_sets,
    )
    _rename_variant(report, LEGACY_ADDITIVE_NAME, ADDITIVE_GUARDED_VARIANT)
    report["protocol"]["experiment"] = "np-next-20260801c"
    report["provenance"] = {
