#!/usr/bin/env python3
"""Discovery-only materialization of NEXT104 CMVF certificates."""
...
PROTOCOL = "2026-08-04-next105-cross-source-discovery-cmvf-v1"
GRAPH_MODE = "voronoi"
DISCOVERY_ROLE = "discovery"
MANIFEST_NAME = "MANIFEST.json"
CATALOGUE_NAME = "NEXT105_CMVF_FEATURE_CATALOGUE.json"
FEATURE_FILES = {
    "scigen": "scigen_discovery_cmvf_features.parquet",
    "wyformer": "wyformer_discovery_cmvf_features.parquet",
}
CATALOGUE_MODES = ("core", "expanded")
NUMERIC_FEATURE_NAMES = tuple(
    name.replace("cmvf_", f"cmvf_{mode}_", 1)
...
STATUS_COLUMNS = tuple(
    ...
    for mode in CATALOGUE_MODES
    for name in (
        f"cmvf_{mode}_supported",
        f"cmvf_{mode}_failure",
        f"cmvf_{mode}_catalogue_sha256",
        f"cmvf_{mode}_pymatgen_version",
        f"cmvf_{mode}_scipy_version",
    )
)
...
EXPECTED_INPUT_SHA256 = {
    ...
    "design": "855ef8c65dfe7423c405ad88ad49a9fa455f17863fac3b4ad70262905343c38b",
}


def compute_cmvf_feature_row(structure, *, graph_mode: str) -> dict[str, object]:
    """Compute core and expanded certificates with independent abstention."""
    ...
    for mode in CATALOGUE_MODES:
        try:
            ...
            statuses.update(
                {
                    f"cmvf_{mode}_supported": False,
                    f"cmvf_{mode}_failure": f"{type(exc).__name__}: {exc}",
                    f"cmvf_{mode}_catalogue_sha256": None,
                    f"cmvf_{mode}_pymatgen_version": None,
                    f"cmvf_{mode}_scipy_version": None,
                }
            )
            continue
        for name, value in result.features.items():
            row[name.replace("cmvf_", f"cmvf_{mode}_", 1)] = float(value)
        statuses.update(
            {
                f"cmvf_{mode}_supported": bool(result.supported),
                f"cmvf_{mode}_failure": result.failure_reason,
                f"cmvf_{mode}_catalogue_sha256": result.catalogue_sha256,
                f"cmvf_{mode}_pymatgen_version": result.pymatgen_version,
                f"cmvf_{mode}_scipy_version": result.scipy_version,
            }
        )
    row.update(statuses)
    if tuple(row) != FEATURE_COLUMNS:
        raise RuntimeError("NEXT105 feature row schema differs")
    return row
...
def _compute_scigen_payload(item: tuple[str, bytes]) -> tuple[str, dict[str, object]]:
    ...
    try:
        ...
        row = compute_cmvf_feature_row(structure, graph_mode=GRAPH_MODE)
    except Exception as exc:
...
def _source_counts(table: pd.DataFrame) -> dict[str, object]:
    ...
    for mode in CATALOGUE_MODES:
        supported = f"cmvf_{mode}_supported"
        failure = f"cmvf_{mode}_failure"
...
def _solver_thread_environment(
    *, require_formal_inputs: bool
    ...
    if require_formal_inputs and any(value != "1" for value in values.values()):
        raise ValueError(
            "NEXT105 formal run requires a single-thread solver environment"
        )
    return values


def build_cross_source_discovery_cmvf_features(
    *,
    ...
    """Materialize CMVF only from physically split discovery geometries."""
    ...
    if any(not path.is_file() for path in paths.values()):
        raise FileNotFoundError("NEXT105 discovery input is missing")
    input_hashes = {name: _sha256_file(path) for name, path in paths.items()}
    if require_formal_inputs and input_hashes != EXPECTED_INPUT_SHA256:
        raise ValueError("NEXT105 formal input identity differs")
    ...
    source_paths = {
        ...
        "src/next105_cross_source_cmvf_features.py": Path(__file__).resolve(),
    }
    ...
    try:
        for source in ("scigen", "wyformer"):
            ...
            if len(table) != len(metadata[source]) or any(
                ...
                raise RuntimeError(f"NEXT105 {source} row accounting differs")
        ...
        manifest: dict[str, object] = {
            "protocol": PROTOCOL,
            "mode": "cross_source_discovery_only_raw_x0_cmvf_feature_freeze",
            "graph_mode": GRAPH_MODE,
            "workers": workers,
        ...
        if any(_sha256_file(path) != input_hashes[name] for name, path in paths.items()):
            raise RuntimeError("NEXT105 input changed before publication")
        if any(_sha256_file(path) != source_hashes[name] for name, path in source_paths.items()):
            raise RuntimeError("NEXT105 source changed before publication")
...
def main() -> int:
    ...
    manifest = build_cross_source_discovery_cmvf_features(
