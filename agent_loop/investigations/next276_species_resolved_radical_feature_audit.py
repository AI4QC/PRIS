#!/usr/bin/env python3
"""Audit frozen species-resolved radical-packing hypotheses on discovery."""
...
PROTOCOL = "2026-08-09-next276-species-resolved-radical-feature-audit-v1"
MANIFEST_NAME = "MANIFEST.json"
CATALOGUE_NAME = "NEXT276_PRVS_FEATURE_CATALOGUE.json"
AUDIT_NAME = "NEXT276_PRVS_FEATURE_AUDIT.json"
TABLE_NAME = "next276_species_resolved_radical_feature_audit.parquet"
EXPECTED_DESIGN_SHA256 = n275.EXPECTED_DESIGN_SHA256
HYPOTHESES = tuple(
    (name, direction)
    for name in n275.FEATURE_NAMES
...
EXPECTED_NEXT275_SOURCE_SHA256 = (
    "369b6b933b039a6eb28ef48900e58c2af7056b75ec6f751cfd851b4074c5622e"
)
NEXT275_SOURCE_PATH = "src/next275_species_resolved_radical_packing.py"
REQUIRED_STAGES = (*n268.REQUIRED_STAGES, 275)
...
EXPECTED_INPUT_SHA256 = {
    **{
    ...
    "next275_manifest": (
        "682956a0fef541f4358686793976a2692a1e4d8844bdeb280d0a0cd898148734"
    ),
    "next275_catalogue": (
        "e041f07a7e321c73bf3d2576998fc2367b9f060101967fb1451f8c44f34416cb"
    ),
    "next275_scigen": (
        "95cf4f9a87c68cc0dfcad8d4e58608064c1093ef48df4656b6c0a9302616cd9f"
    ),
    "next275_wyformer": (
        "8848492c3c3aa40eb6d4e9ba73c8cc7cfc8565d0472176b61f407897ba463215"
    ),
}
...
def select_eligible_hypotheses(
    ...
    try:
        ...
        raise ValueError("NEXT276 audit ranking table differs") from exc
...
def bounded_protection(
    *, values: object, direction: str, q_lo: float, q_hi: float
    ...
    try:
        ...
        raise ValueError("NEXT276 bounded protection inputs differ") from exc


def _index_prvs_by_prefixed_material_id(
    *, table: pd.DataFrame, source: str, expected_material_ids: object
    ...
    if set(extra["material_id"]) != expected:
        raise ValueError(f"NEXT276 {source} material identity differs")
...
def _paths(
    *,
    ...
    paths.update(
        {
            "design": design_path,
            "next275_manifest": roots["next275"] / n275.MANIFEST_NAME,
            "next275_catalogue": roots["next275"] / n275.CATALOGUE_NAME,
            "next275_scigen": roots["next275"] / n275.FEATURE_FILES["scigen"],
            "next275_wyformer": roots["next275"] / n275.FEATURE_FILES["wyformer"],
        }
    )
    return paths


def _verify_next275(paths: Mapping[str, Path], input_hashes: Mapping[str, str]):
    ...
    manifest = json.loads(paths["next275_manifest"].read_text())
    catalogue = json.loads(paths["next275_catalogue"].read_text())
    tables = {
        "scigen": pd.read_parquet(paths["next275_scigen"]),
        "wyformer": pd.read_parquet(paths["next275_wyformer"]),
    }
    expected_outputs = {
        n275.CATALOGUE_NAME: input_hashes["next275_catalogue"],
        n275.FEATURE_FILES["scigen"]: input_hashes["next275_scigen"],
        n275.FEATURE_FILES["wyformer"]: input_hashes["next275_wyformer"],
    }
    if (
        _sha256_file(Path(n268.__file__).resolve()) != EXPECTED_NEXT268_SOURCE_SHA256
        or _sha256_file(Path(n275.__file__).resolve()) != EXPECTED_NEXT275_SOURCE_SHA256
        or manifest.get("protocol") != n275.PROTOCOL
        or manifest.get("next276_audit_authorized") is not True
        ...
        or manifest.get("executed_source_sha256", {}).get(NEXT275_SOURCE_PATH)
        != EXPECTED_NEXT275_SOURCE_SHA256
        or any(manifest.get(key) is not value for key, value in BOUNDARY_FLAGS.items())
        or catalogue.get("feature_names") != list(n275.FEATURE_NAMES)
        or catalogue.get("feature_count") != len(n275.FEATURE_NAMES)
        or catalogue.get("decomposition")
        != "exact_population_within_between_species_variance"
        or catalogue.get("singleton_species_cv") != 0.0
        or catalogue.get("labels_opened") is not False
    ):
        raise ValueError("NEXT276 NEXT275 provenance differs")
    for source, expected_rows in (("scigen", 13_470), ("wyformer", 5_232)):
        ...
        tiling = pd.to_numeric(
            table["prvs_volume_tiling_relative_error"], errors="coerce"
        )
        if (
            ...
            raise ValueError(f"NEXT276 NEXT275 {source} table differs")
    return (*prior[:-1], tables)


def run_species_resolved_radical_feature_audit(
    *,
    ...
    """Audit the frozen twenty species-resolved PRV hypotheses."""

    if not set(REQUIRED_STAGES).issubset(stage_dirs):
        raise ValueError("NEXT276 stage directory universe differs")
    if not set(REQUIRED_DESIGN_STAGES).issubset(design_paths):
        raise ValueError("NEXT276 design path universe differs")
    roots = {
    ...
    if any(not path.is_file() for path in paths.values()):
        raise FileNotFoundError("NEXT276 input is missing")
    ...
    if require_formal_inputs and input_hashes != EXPECTED_INPUT_SHA256:
        ...
        raise ValueError(f"NEXT276 formal input identity differs: {differing}")
    (
        eligible,
        eligible214,
        primary_key,
        ...
        formula222,
        prvs_tables,
    ) = _verify_next275(paths, input_hashes)
    n227 = n268.n227
    ...
    for source_name in ("scigen", "wyformer"):
        indexed = _index_prvs_by_prefixed_material_id(
    ...
    if cohort_counts != n227.EXPECTED_COHORT_COUNTS:
        raise ValueError("NEXT276 rejected cohort counts differ")
    rows: list[dict[str, object]] = []
    for feature, direction in HYPOTHESES:
        values = pd.to_numeric(combined[feature], errors="coerce").to_numpy(float)
        finite = values[np.isfinite(values)]
        if not len(finite):
            raise ValueError(f"NEXT276 {feature} has no finite discovery values")
    ...
    catalogue = {
        ...
        "features": list(n275.FEATURE_NAMES),
    ...
    audit = {
        "protocol": PROTOCOL,
        "audit_mode": "fixed_next224_rejected_extreme_species_resolved_radical_audit",
        "base_candidate_key_sha256": n227.EXPECTED_BASE_CANDIDATE_KEY_SHA256,
        "base_threshold": n227.EXPECTED_BASE_THRESHOLD,
        "cohort_counts": {
        ...
        "next277_search_authorized": bool(selected is not None),
    ...
    source_paths = {
        ...
        NEXT275_SOURCE_PATH: Path(n275.__file__).resolve(),
        "src/next276_species_resolved_radical_feature_audit.py": Path(__file__).resolve(),
    }
    ...
    try:
        ...
        manifest = {
            ...
            "next277_search_authorized": bool(selected is not None),
            "species_resolved_radical_branch_terminated": selected is None,
            "new_formula_searched": False,
            "new_formula_selected": False,
        ...
        if any(_sha256_file(path) != input_hashes[name] for name, path in paths.items()):
            raise RuntimeError("NEXT276 input changed before publication")
        if any(_sha256_file(path) != source_hashes[name] for name, path in source_paths.items()):
            raise RuntimeError("NEXT276 source changed before publication")
...
def main() -> int:
    ...
    manifest = run_species_resolved_radical_feature_audit(
