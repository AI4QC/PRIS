...
import experiments.next445_pcabsm_label_blind_probe as p
...
def test_hashes_cover_pcabp_and_pcabsm_surface() -> None:
    hashes = p.probe_source_hashes()
    assert hashes["design"] == p.DESIGN_SHA256
    assert "src/next440_path_constrained_apriori_bond_positivity.py" in hashes
    assert "src/next445_path_constrained_bond_strength_matching.py" in hashes
    assert tuple(hashes)[-2:] == (
        "tests/test_next445_path_constrained_bond_strength_matching.py",
        "tests/test_next445_pcabsm_label_blind_probe.py",
    )
