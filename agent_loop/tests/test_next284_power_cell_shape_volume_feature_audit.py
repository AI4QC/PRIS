...
from src.next284_power_cell_shape_volume_feature_audit import (
    HYPOTHESES,
    NEXT283_SOURCE_PATH,
    REQUIRED_DESIGN_STAGES,
    REQUIRED_STAGES,
    _index_psvc_by_prefixed_material_id,
    bounded_protection,
    run_power_cell_shape_volume_feature_audit,
    select_eligible_hypotheses,
)


def test_hypothesis_universe_freezes_exactly_six_one_way_directions() -> None:
    assert HYPOTHESES == (
...
def test_next283_provenance_uses_exact_executed_source_path() -> None:
    assert NEXT283_SOURCE_PATH == "src/next283_power_cell_shape_volume_coupling.py"


def test_bounded_mapping_supports_frozen_directions_and_abstention() -> None:
...
def test_psvc_identity_alignment_adds_reconstruction_source_prefix() -> None:
    table = pd.DataFrame(
        {"material_id": ["a", "b"], "psvc_sphericity_mean": [0.7, 0.8]}
    )
...
def test_audit_interface_excludes_validation_and_replication() -> None:
    parameters = tuple(inspect.signature(run_power_cell_shape_volume_feature_audit).parameters)
...
def test_audit_fails_closed_on_missing_input(tmp_path) -> None:
    with pytest.raises(FileNotFoundError, match="NEXT284 input is missing"):
        run_power_cell_shape_volume_feature_audit(
