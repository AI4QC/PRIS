...
import src.next376_pcrl_formal_build as b


def _probe() -> dict[str, object]:
    return {
        "protocol": "2026-08-13-next375-pcrl-label-blind-probe-v1",
        "design_sha256": b.n375.DESIGN_SHA256,
        "minimum_novelty_joint_finite": 40,
        "probe_passed": True,
        "next376_formal_build_authorized": True,
        "gates": {
            "support": True,
            "closed_domain": True,
            "nondegenerate": True,
...
def test_frozen_formal_schema_and_coverage_are_exact() -> None:
    assert b.PROTOCOL == "2026-08-13-next376-pcrl-formal-build-v1"
...
def test_label_free_statistics_require_finite_nondegenerate_feature() -> None:
    table = pd.DataFrame({b.n375.FEATURE_NAMES[0]: np.linspace(0.0, 1.0, 40)})
    result = b.label_free_statistics(table)
    assert result[b.n375.FEATURE_NAMES[0]]["minimum"] == 0.0
    assert result[b.n375.FEATURE_NAMES[0]]["maximum"] == 1.0
    assert result[b.n375.FEATURE_NAMES[0]]["unique_rounded_10"] == 40
    with pytest.raises(RuntimeError, match="empty"):
        b.label_free_statistics(pd.DataFrame({b.n375.FEATURE_NAMES[0]: [np.nan]}))


def test_builder_interface_contains_no_endpoint_or_label_input() -> None:
    parameters = tuple(inspect.signature(b.build_cross_source_pcrl_features).parameters)
    assert parameters == (
        "scigen_cohort_dir",
        "wyformer_cohort_dir",
        "design_path",
...
def test_no_dft_boundary_flags_are_false() -> None:
    assert all(value is False for value in b.n375.BOUNDARY_FLAGS.values())
