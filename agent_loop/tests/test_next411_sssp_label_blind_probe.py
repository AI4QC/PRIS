...
import experiments.next411_sssp_label_blind_probe as p
...
def test_hashes_cover_the_frozen_execution_surface() -> None:
    ...
    assert "src/next407_beck_ecn_simplicity.py" in hashes
    assert tuple(hashes)[-2:] == (
        "tests/test_next411_same_sign_shell_purity.py",
        "tests/test_next411_sssp_label_blind_probe.py",
    )


def test_formal_prior_root_resolves_stage_directories_by_required_files(
    tmp_path: Path,
) -> None:
    expected = {
        "next166": {
