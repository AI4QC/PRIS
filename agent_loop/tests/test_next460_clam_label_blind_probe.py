...
import experiments.next460_clam_label_blind_probe as p
...
def test_hashes_cover_asset_pcabsm_pfpu_and_clam_surface() -> None:
    ...
    assert "src/next455_path_field_participation_uniformity.py" in hashes
    assert "src/next460_characteristic_lewis_anion_matching.py" in hashes
    assert tuple(hashes)[-2:] == (
        "tests/test_next460_characteristic_lewis_anion_matching.py",
        "tests/test_next460_clam_label_blind_probe.py",
    )
