...
import src.next511_mcpa_formal_build as b


def _probe() -> dict[str, object]:
    return {
        "protocol": "2026-08-13-next510-mcpa-label-blind-novelty-probe-v1",
        "design_sha256": b.n510.DESIGN_SHA256,
        "minimum_novelty_joint_finite": 40,
        "probe_passed": True,
        "next511_formal_build_authorized": True,
        "gates": {
            key: True
...
def test_schema_rows_and_coverage_gate_are_frozen() -> None:
    assert b.PROTOCOL == "2026-08-13-next511-mcpa-formal-build-v1"
    assert b.EXPECTED_ROWS == {"scigen": 13_470, "wyformer": 5_232}
    assert b.MINIMUM_FORMAL_COVERAGE == 0.95
    assert b.EXPECTED_PROBE_SHA256 == "e145807b295dd30a687c1a97dae62249748ec7409743d8707afc51c3dfb1173d"
...
def test_label_free_statistics_require_finite_feature() -> None:
    table = pd.DataFrame({b.n510.FEATURE_NAMES[0]: np.linspace(0, 0.5, 40)})
    result = b.label_free_statistics(table)[b.n510.FEATURE_NAMES[0]]
    assert result["minimum"] == 0 and result["maximum"] == 0.5
    assert result["unique_rounded_10"] == 40
    with pytest.raises(RuntimeError, match="empty"):
        b.label_free_statistics(pd.DataFrame({b.n510.FEATURE_NAMES[0]: [np.nan]}))


def test_builder_has_no_endpoint_label_or_later_geometry_input() -> None:
    parameters = tuple(
        inspect.signature(b.build_cross_source_mcpa_features).parameters
    )
    assert parameters == (
        "scigen_cohort_dir",
        "wyformer_cohort_dir",
