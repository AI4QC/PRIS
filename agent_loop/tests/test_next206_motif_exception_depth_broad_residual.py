...
from src.next206_motif_exception_depth_broad_residual import (
...
def test_formal_interface_has_only_discovery_endpoints() -> None:
    parameters = tuple(
        inspect.signature(run_motif_exception_depth_broad_residual).parameters
    )
    assert "next205_dir" in parameters and "next204_dir" in parameters
    assert not any(
...
def test_formal_diagnostic_fails_closed_on_missing_input(tmp_path) -> None:
    signature = inspect.signature(run_motif_exception_depth_broad_residual)
    kwargs = {
        name: tmp_path / name
    ...
    with pytest.raises(FileNotFoundError, match="NEXT206 input is missing"):
        run_motif_exception_depth_broad_residual(**kwargs)
