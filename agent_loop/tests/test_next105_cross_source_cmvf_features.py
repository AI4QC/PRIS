...
from src.next105_cross_source_cmvf_features import (
    ...
    PROTOCOL,
    build_cross_source_discovery_cmvf_features,
    compute_cmvf_feature_row,
)
import src.next105_cross_source_cmvf_features as next105


def test_next105_schema_is_additive_and_has_no_endpoint_quantities() -> None:
    assert PROTOCOL == "2026-08-04-next105-cross-source-discovery-cmvf-v1"
...
def test_row_computes_core_and_expanded_certificates_independently() -> None:
    ...
    row = compute_cmvf_feature_row(structure, graph_mode="voronoi")

    assert tuple(row) == FEATURE_COLUMNS
    assert row["cmvf_core_supported"] is True
    assert row["cmvf_expanded_supported"] is True
    assert row["cmvf_core_failure"] is None
    assert row["cmvf_expanded_failure"] is None
    assert len(str(row["cmvf_core_catalogue_sha256"])) == 64
    assert len(str(row["cmvf_expanded_catalogue_sha256"])) == 64
...
def _synthetic_cohorts(root: Path) -> tuple[Path, Path]:
    ...
    for role in ("internal_validation", "internal_replication"):
        (scigen / SCIGEN_GEOMETRY_NAMES[role]).write_bytes(
            b"NEXT105 must never parse this non-discovery payload"
        )
    pd.DataFrame(
        [{
...
def test_builder_reads_only_discovery_and_has_no_endpoint_argument(
    ...
    design.write_text("frozen NEXT104 design\n", encoding="utf-8")
...
def test_formal_solver_thread_environment_is_single_thread_and_audited(
    ...
    assert next105._solver_thread_environment(require_formal_inputs=True) == expected

    monkeypatch.setenv("OMP_NUM_THREADS", "2")
    with pytest.raises(ValueError, match="single-thread solver environment"):
        next105._solver_thread_environment(require_formal_inputs=True)
