...
from src.next316_pcgr_feature_audit import (
    HYPOTHESES,
    NEXT315_SOURCE_PATH,
    REQUIRED_DESIGN_STAGES,
    REQUIRED_STAGES,
    _index_pcgr_by_prefixed_material_id,
    bounded_protection,
...
def test_next315_provenance_uses_exact_executed_source_path() -> None:
    assert NEXT315_SOURCE_PATH == "src/next315_periodic_charge_green_resistance.py"
...
def test_pcgr_identity_alignment_adds_reconstruction_source_prefix() -> None:
    table = pd.DataFrame(
        {"material_id": ["a", "b"], "pcgr_charge_resistance": [0.1, 0.2]}
    )
...
def test_audit_interface_excludes_validation_and_replication() -> None:
    parameters = tuple(inspect.signature(run_pcgr_feature_audit).parameters)
...
def test_audit_fails_closed_on_missing_input(tmp_path) -> None:
    with pytest.raises(FileNotFoundError, match="NEXT316 input is missing"):
        run_pcgr_feature_audit(
            ...
            design_path=tmp_path / "design315",
