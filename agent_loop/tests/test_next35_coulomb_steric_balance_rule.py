...
from src.next35_coulomb_steric_balance_rule import (
    CANDIDATE_TERM_SETS,
    FROZEN_RULE_NAME,
    MANIFEST_NAME,
    MECHANISM_PAIRS,
...
def test_catalogue_is_exact_bounded_and_all_balance_terms_are_high_risk() -> None:
    balance = {name: spec for name, spec in TERM_CATALOGUE.items() if name.startswith("acsb_")}
    assert len(balance) == 7
    assert all(spec["direction"] == 1.0 for spec in balance.values())
    assert len(TERM_CATALOGUE) == 11
    assert len(MECHANISM_PAIRS) == 7
    assert len(CANDIDATE_TERM_SETS) == 18
    assert len(set(CANDIDATE_TERM_SETS)) == 18
    ...
    assert not any(
        ...
        for token in ("energy", "force", "stress", "dft", "relax", "label", "target", "mlip")
    )
...
def test_scan_has_90_rows_and_promotes_perfect_balance_term() -> None:
    endpoints = _endpoints()
    features = pd.DataFrame(
        {
            "material_id": endpoints.material_id,
            "acsb_global_residual": np.linspace(2.0, 0.0, len(endpoints)),
        }
    )
    ...
    assert len(scan) == 90
    assert scan.candidate_id.nunique() == 18
    assert rule is not None
    assert rule["terms"] == ["acsb_global_residual_high"]
    ...
    features = pd.DataFrame(
        {
            "material_id": endpoints.material_id,
            "acsb_global_residual": np.linspace(2.0, 0.0, len(endpoints)),
            "acsb_opposition_deficit": 1.0,
        }
    )
    scan, rule = scan_development_candidates(features, endpoints)
    assert not scan.loc[scan.candidate_id.eq("acsb_opposition_deficit_high"), "enabled"].any()
    assert scan.loc[scan.candidate_id.eq("acsb_global_residual_high"), "enabled"].all()
    assert rule is not None
...
def test_replay_does_not_refit_frozen_constants() -> None:
    endpoints = _endpoints()
    features = pd.DataFrame(
        {"material_id": endpoints.material_id, "acsb_global_residual": np.linspace(2.0, 0.0, len(endpoints))}
    )
    ...
    replay = pd.DataFrame(
        {
            ...
            "acsb_global_residual": [1e12, -1e12],
        }
    )
    score, supported, rejected = compute_coulomb_steric_balance_risk(replay, rule)
...
def _sealed_inputs(tmp_path: Path) -> tuple[Path, Path, Path, Path]:
    ...
    endpoint_manifest_path.write_text(
        json.dumps(
            {"protocol": ENDPOINT_PROTOCOL, "labels_opened": True, "outputs_sha256": {ENDPOINT_NAME: _sha256(endpoint_path)}}
        )
        + "\n"
    )
...
def test_freeze_and_label_free_replay_are_hash_locked_and_no_replace(tmp_path: Path) -> None:
    feature_path, feature_manifest_path, endpoint_path, endpoint_manifest_path = _sealed_inputs(tmp_path)
    ...
    assert (freeze_dir / SCAN_NAME).is_file() and (freeze_dir / FROZEN_RULE_NAME).is_file()
