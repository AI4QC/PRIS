...
from src.next113_cross_source_cmvom_features import (
    ...
    PROTOCOL,
    build_cross_source_discovery_cmvom_features,
    compute_cmvom_feature_row,
)
import src.next113_cross_source_cmvom_features as next113
...
def test_formal_identity_and_schema_are_frozen_without_endpoint_quantities() -> None:
    assert PROTOCOL == "2026-08-08-next113-cross-source-discovery-cmvom-v1"
    ...
    assert EXPECTED_INPUT_SHA256["design"] == (
        "fe35baf70b6b88a9dfb92f3afe396445ed91ea75462d67bd6d8ded94330cf08d"
    )
    forbidden = (
        "energy",
        "force",
...
def test_row_computes_core_and_expanded_morphology_independently() -> None:
    structure = Structure(
        Lattice.cubic(4.2),
        ["Na", "O"],
    ...
    row = compute_cmvom_feature_row(structure, graph_mode="voronoi")

    assert tuple(row) == FEATURE_COLUMNS
    assert row["cmvom_core_supported"] is True
    assert row["cmvom_expanded_supported"] is True
    assert row["cmvom_core_failure"] is None
    assert row["cmvom_expanded_failure"] is None
    assert len(str(row["cmvom_core_catalogue_sha256"])) == 64
    assert len(str(row["cmvom_expanded_catalogue_sha256"])) == 64
...
def _synthetic_cohorts(root: Path) -> tuple[Path, Path]:
    ...
    for role in ("internal_validation", "internal_replication"):
        (scigen / SCIGEN_GEOMETRY_NAMES[role]).write_bytes(
            b"NEXT113 must never parse this non-discovery payload"
        )
    pd.DataFrame(
        [{
...
def test_formal_solver_thread_environment_is_single_thread_and_audited(
    ...
    assert next113._solver_thread_environment(require_formal_inputs=True) == expected

    monkeypatch.setenv("OMP_NUM_THREADS", "2")
    with pytest.raises(ValueError, match="single-thread solver environment"):
        next113._solver_thread_environment(require_formal_inputs=True)
