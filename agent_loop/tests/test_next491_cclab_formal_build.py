...
import src.next491_cclab_formal_build as b


def _probe() -> dict[str, object]:
    return {
        "protocol": "2026-08-13-next490-cclab-label-blind-novelty-probe-v1",
        "design_sha256": b.n490.DESIGN_SHA256,
        "asset_sha256": b.n490.ASSET_SHA256,
        "minimum_novelty_joint_finite": 40,
        "probe_passed": True,
        "next491_formal_build_authorized": True,
        "gates": {
            key: True
...
def test_schema_rows_and_coverage_gate_are_frozen() -> None:
    assert b.PROTOCOL == "2026-08-13-next491-cclab-formal-build-v1"
...
def test_label_free_statistics_require_finite_feature() -> None:
    table = pd.DataFrame({b.n490.FEATURE_NAMES[0]: np.linspace(0, 0.5, 40)})
    result = b.label_free_statistics(table)[b.n490.FEATURE_NAMES[0]]
    assert result["minimum"] == 0 and result["maximum"] == 0.5
    assert result["unique_rounded_10"] == 40
    with pytest.raises(RuntimeError, match="empty"):
        b.label_free_statistics(pd.DataFrame({b.n490.FEATURE_NAMES[0]: [np.nan]}))


def test_builder_has_no_endpoint_or_label_input() -> None:
    parameters = tuple(inspect.signature(b.build_cross_source_cclab_features).parameters)
    assert parameters == (
        "scigen_cohort_dir",
        "wyformer_cohort_dir",
        "design_path",
