...
from src.next37_self_stress_compatibility_rule import (
    CANDIDATE_TERM_SETS,
    FROZEN_RULE_NAME,
    MANIFEST_NAME,
    MECHANISM_PAIRS,
...
def test_catalogue_is_exact_bounded_and_projection_terms_are_high_risk() -> None:
    projection = {
        name: specification
        for name, specification in TERM_CATALOGUE.items()
        if name.startswith("sscp_")
    }
    assert len(projection) == 6
    assert all(specification["direction"] == 1.0 for specification in projection.values())
...
def test_scan_has_85_rows_and_promotes_perfect_projection_term() -> None:
    endpoints = _endpoints()
    features = pd.DataFrame(
        {
            "material_id": endpoints.material_id,
            "sscp_load_rms": np.linspace(2.0, 0.0, len(endpoints)),
        }
    )
    ...
    assert rule["terms"] == ["sscp_load_rms_high"]
    ...
    features = pd.DataFrame(
        {
            "material_id": endpoints.material_id,
            "sscp_load_rms": np.linspace(2.0, 0.0, len(endpoints)),
            "sscp_load_fraction": 1.0,
        }
    )
    scan, rule = scan_development_candidates(features, endpoints)
    assert not scan.loc[
        scan.candidate_id.eq("sscp_load_fraction_high"), "enabled"
    ].any()
    assert scan.loc[scan.candidate_id.eq("sscp_load_rms_high"), "enabled"].all()
    assert rule is not None
    ...
    replay = pd.DataFrame(
        {
            ...
            "sscp_load_rms": [1.0e12, -1.0e12],
        }
    )

    score, supported, rejected = compute_self_stress_compatibility_risk(replay, rule)
