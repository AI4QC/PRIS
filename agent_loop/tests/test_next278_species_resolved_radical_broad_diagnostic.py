...
import src.next278_species_resolved_radical_broad_diagnostic as n
...
def test_diagnostic_selection_is_exact_auc_safe_non_broad_population() -> None:
    ...
    selected = n.select_diagnostic_candidates(frame)
...
def test_candidate_digest_is_sorted_and_order_independent() -> None:
    ...
    assert n.candidate_key_sha256(first) == expected
    assert n.candidate_key_sha256(second) == expected
...
def test_closest_residual_uses_failure_count_then_shortfall_then_key() -> None:
    ...
    assert n.select_closest_residual(frame)["candidate_key"] == "a"


def test_diagnostic_interface_excludes_validation_and_replication() -> None:
    parameters = tuple(
        inspect.signature(n.run_species_resolved_radical_broad_diagnostic).parameters
    )
...
def test_diagnostic_fails_closed_on_missing_input(tmp_path) -> None:
    with pytest.raises(FileNotFoundError, match="NEXT278 input is missing"):
        n.run_species_resolved_radical_broad_diagnostic(
            ...
            design_path=tmp_path / "design277",
