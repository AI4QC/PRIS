...
from src.next414_sssp_margin_local_search import (
    AMPLITUDE_FRACTIONS,
    EXPECTED_CANDIDATE_COUNT,
    EXPECTED_ELIGIBLE_COUNT,
    LOCAL_WIDTH_FRACTIONS,
    build_sssp_candidate_specs,
    run_sssp_margin_local_search,
    select_best_new_record,
    sssp_margin_local_score,
)
...
def _eligible_table() -> pd.DataFrame:
    return pd.DataFrame(
        {
            "hypothesis": ["sssp_same_sign_shell_purity_q10__protected_high"],
            "feature": ["sssp_same_sign_shell_purity_q10"],
...
def test_margin_local_score_is_signed_local_and_support_preserving() -> None:
    ...
    score, got_support, active, weight = sssp_margin_local_score(
        base_score=base,
...
def test_search_interface_excludes_validation_and_replication() -> None:
    parameters = tuple(inspect.signature(run_sssp_margin_local_search).parameters)
    assert "next412_dir" in parameters and "next413_dir" in parameters
...
def test_verified_audit_contract_has_seven_frontier_fields_then_tables_and_eligible() -> None:
    assert len(inspect.signature(run_sssp_margin_local_search).parameters) > 0
    # The audited frontier contract is seven NEXT224 reconstruction values,
    # followed by the SSSP source tables and the eligible audit table.
    source = inspect.getsource(run_sssp_margin_local_search)
    unpack = source[source.index("eligible_prior,") : source.index(") = _verify_next413")]
    assert "_prior_feature_formula" not in unpack
    assert unpack.index("formula222,") < unpack.index("sssp_tables,")
    assert unpack.index("sssp_tables,") < unpack.index("eligible413,")
...
def test_search_fails_closed_on_missing_input(tmp_path) -> None:
    ...
    with pytest.raises(FileNotFoundError, match="NEXT414 input is missing"):
        run_sssp_margin_local_search(
