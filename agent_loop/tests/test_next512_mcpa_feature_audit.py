...
import src.next512_mcpa_feature_audit as n


def test_hypothesis_universe_freezes_one_high_direction() -> None:
    assert n.HYPOTHESES == (
        ("mcpa_madelung_charge_potential_antitonicity", "protected_high"),
    )
...
def test_bounded_mapping_supports_only_frozen_high_direction() -> None:
    ...
    with pytest.raises(ValueError, match="NEXT512 bounded"):
...
def test_formal_rows_require_bounded_pair_semantics() -> None:
    assert n._mcpa_rows_are_consistent(
        values=np.asarray([0.0, 0.5, 1.0]),
        sites=np.asarray([2, 3, 4]),
        pairs=np.asarray([1, 2, 6]),
        minimum=np.asarray([-1.0, -0.5, 0.0]),
        mean=np.asarray([-1.0, 0.0, 1.0]),
        maximum=np.asarray([-1.0, 0.5, 1.0]),
        identity_error=np.asarray([0.0, 1e-15, 1e-14]),
    ).all()
    changed = n._mcpa_rows_are_consistent(
        values=np.asarray([-0.1, 0.5, 1.1]),
        sites=np.asarray([1, 3, 4]),
        pairs=np.asarray([0, 2, 6]),
        minimum=np.asarray([-1.1, 0.2, 0.0]),
        mean=np.asarray([-1.0, 0.0, 1.1]),
        maximum=np.asarray([-1.0, 0.1, 1.0]),
...
def test_audit_interface_excludes_validation_and_replication() -> None:
    parameters = tuple(inspect.signature(n.run_mcpa_feature_audit).parameters)
    assert "next511_dir" in parameters and "stage_dirs" in parameters
...
def test_audit_fails_closed_on_missing_input(tmp_path) -> None:
    with pytest.raises(FileNotFoundError, match="NEXT512 input is missing"):
        n.run_mcpa_feature_audit(
