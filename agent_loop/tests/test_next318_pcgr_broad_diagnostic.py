...
from src.next318_pcgr_broad_diagnostic import (
...
def test_diagnostic_selection_is_exact_eligible_auc_safe_non_broad_population() -> None:
    frame = pd.DataFrame(
        {
...
def test_diagnostic_interface_excludes_validation_and_replication() -> None:
    parameters = tuple(inspect.signature(run_pcgr_broad_diagnostic).parameters)
...
def test_diagnostic_fails_closed_on_missing_input(tmp_path) -> None:
    with pytest.raises(FileNotFoundError, match="NEXT318 input is missing"):
        run_pcgr_broad_diagnostic(
