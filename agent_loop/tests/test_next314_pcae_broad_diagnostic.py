...
from src.next314_pcae_broad_diagnostic import (
...
def test_diagnostic_interface_excludes_validation_and_replication() -> None:
    parameters = tuple(inspect.signature(run_pcae_broad_diagnostic).parameters)
...
def test_diagnostic_fails_closed_on_missing_input(tmp_path) -> None:
    with pytest.raises(FileNotFoundError, match="NEXT314 input is missing"):
        run_pcae_broad_diagnostic(
