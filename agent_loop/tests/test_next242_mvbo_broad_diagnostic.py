...
from src.next242_mvbo_broad_diagnostic import (
...
def test_diagnostic_interface_excludes_validation_and_replication() -> None:
    parameters = tuple(inspect.signature(run_mvbo_broad_diagnostic).parameters)
...
def test_diagnostic_fails_closed_on_missing_input(tmp_path) -> None:
    with pytest.raises(FileNotFoundError, match="NEXT242 input is missing"):
        run_mvbo_broad_diagnostic(
            ...
            stage_dirs={stage: tmp_path / f"next{stage}" for stage in range(98, 242)},
            next135_freeze_path=tmp_path / "next135",
            design_paths={
                stage: tmp_path / f"design{stage}" for stage in range(202, 242)
            },
