...
from src.next308_pbvhl_feature_audit import (
    HYPOTHESES,
    NEXT307_SOURCE_PATH,
    REQUIRED_DESIGN_STAGES,
    REQUIRED_STAGES,
    _index_pbvhl_by_prefixed_material_id,
    bounded_protection,
...
def test_hypothesis_universe_freezes_exactly_four_low_directions() -> None:
...
def test_next307_provenance_uses_exact_executed_source_path() -> None:
    assert NEXT307_SOURCE_PATH == "src/next307_periodic_bond_valence_hodge_loop.py"
...
def test_pbvhl_identity_alignment_adds_reconstruction_source_prefix() -> None:
    table = pd.DataFrame(
        {"material_id": ["a", "b"], "pbvhl_cycle_fraction": [0.1, 0.2]}
    )
...
def test_audit_interface_excludes_validation_and_replication() -> None:
    parameters = tuple(inspect.signature(run_pbvhl_feature_audit).parameters)
...
def test_audit_fails_closed_on_missing_input(tmp_path) -> None:
    with pytest.raises(FileNotFoundError, match="NEXT308 input is missing"):
        run_pbvhl_feature_audit(
            ...
            design_path=tmp_path / "design307",
