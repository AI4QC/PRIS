...
from src.next172_repair_interval_broad_residual_diagnostic import (
...
def test_selector_rejects_duplicate_or_incomplete_candidate_records() -> None:
    ...
    with pytest.raises(ValueError, match="NEXT172 published candidate schema differs"):
...
def test_formal_interface_has_no_validation_or_replication_endpoint() -> None:
    parameters = tuple(
        inspect.signature(run_repair_interval_broad_residual_diagnostic).parameters
    )
    assert "next171_dir" in parameters
...
def test_formal_diagnostic_fails_closed_on_missing_input(tmp_path) -> None:
    signature = inspect.signature(run_repair_interval_broad_residual_diagnostic)
    kwargs = {
        name: tmp_path / name
    ...
    with pytest.raises(FileNotFoundError, match="NEXT172 input is missing"):
        run_repair_interval_broad_residual_diagnostic(**kwargs)
