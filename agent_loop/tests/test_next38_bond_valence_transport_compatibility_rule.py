...
from src.next38_bond_valence_transport_compatibility_rule import (
    CANDIDATE_TERM_SETS,
    FROZEN_RULE_NAME,
    MANIFEST_NAME,
    MECHANISM_PAIRS,
...
def test_catalogue_is_exact_bounded_and_bvtc_terms_are_high_risk() -> None:
    bvtc = {
        name: specification
        for name, specification in TERM_CATALOGUE.items()
        if name.startswith("bvtc_")
    }
    assert len(bvtc) == 6
    assert all(specification["direction"] == 1.0 for specification in bvtc.values())
...
def test_scan_has_85_rows_and_promotes_perfect_bvtc_term() -> None:
    endpoints = _endpoints()
    features = pd.DataFrame(
        {
            "material_id": endpoints.material_id,
            "bvtc_compatible_rms": np.linspace(2.0, 0.0, len(endpoints)),
        }
    )
    scan, rule = scan_development_candidates(features, endpoints)
    assert len(scan) == 85 and scan.candidate_id.nunique() == 17
    assert rule is not None
    assert rule["terms"] == ["bvtc_compatible_rms_high"]
    ...
    features = pd.DataFrame(
        {
            "material_id": endpoints.material_id,
            "bvtc_compatible_rms": np.linspace(2.0, 0.0, len(endpoints)),
            "bvtc_incompatible_fraction": 1.0,
        }
    )
    scan, rule = scan_development_candidates(features, endpoints)
    assert not scan.loc[
        scan.candidate_id.eq("bvtc_incompatible_fraction_high"), "enabled"
    ].any()
    assert scan.loc[scan.candidate_id.eq("bvtc_compatible_rms_high"), "enabled"].all()
    assert rule is not None
    ...
    replay = pd.DataFrame(
        {
            ...
            "bvtc_compatible_rms": [1.0e12, -1.0e12],
        }
    )
    score, supported, rejected = compute_bond_valence_transport_compatibility_risk(
        replay, rule
    )
