...
import src.next421_p4bss_formal_build as b


def _probe() -> dict[str, object]:
    return {
        "protocol": "2026-08-13-next420-p4bss-label-blind-probe-v1",
        "design_sha256": b.n420.DESIGN_SHA256,
        "minimum_novelty_joint_finite": 40,
        "probe_passed": True,
        "next421_formal_build_authorized": True,
        "gates": {
            "support": True,
            "closed_domain": True,
            "nondegenerate": True,
...
def test_frozen_formal_schema_and_coverage_are_exact() -> None:
    assert b.PROTOCOL == "2026-08-13-next421-p4bss-formal-build-v1"
...
def test_label_free_statistics_require_finite_feature() -> None:
    table = pd.DataFrame({b.n420.FEATURE_NAMES[0]: np.linspace(0.01, 1, 40)})
    result = b.label_free_statistics(table)[b.n420.FEATURE_NAMES[0]]
    assert result["minimum"] == 0.01 and result["maximum"] == 1.0
    assert result["unique_rounded_10"] == 40
    with pytest.raises(RuntimeError, match="empty"):
        b.label_free_statistics(pd.DataFrame({b.n420.FEATURE_NAMES[0]: [np.nan]}))


def test_builder_interface_contains_no_endpoint_or_label_input() -> None:
    parameters = tuple(inspect.signature(b.build_cross_source_p4bss_features).parameters)
    assert parameters == (
...
def test_no_dft_boundary_flags_are_false() -> None:
    assert all(value is False for value in b.n420.BOUNDARY_FLAGS.values())
