...
import src.next384_pspc_formal_build as b


def _probe() -> dict[str, object]:
    return {
        "protocol": "2026-08-13-next383-pspc-label-blind-probe-v1",
        "design_sha256": b.n383.DESIGN_SHA256,
        "minimum_novelty_joint_finite": 40,
        "walk_depth": b.n383.WALK_DEPTH,
        "probe_passed": True,
        "next384_formal_build_authorized": True,
        "gates": {
            "support": True, "closed_domain": True, "nondegenerate": True,
            "invariant": True, "novel": True,
        },
...
def test_frozen_formal_schema_and_coverage_are_exact() -> None:
    assert b.PROTOCOL == "2026-08-13-next384-pspc-formal-build-v1"
...
def test_label_free_statistics_require_finite_feature() -> None:
    table = pd.DataFrame({b.n383.FEATURE_NAMES[0]: np.linspace(0, 1, 40)})
    result = b.label_free_statistics(table)
    assert result[b.n383.FEATURE_NAMES[0]]["minimum"] == 0.0
    assert result[b.n383.FEATURE_NAMES[0]]["maximum"] == 1.0
    assert result[b.n383.FEATURE_NAMES[0]]["unique_rounded_10"] == 40
    with pytest.raises(RuntimeError, match="empty"):
        b.label_free_statistics(pd.DataFrame({b.n383.FEATURE_NAMES[0]: [np.nan]}))


def test_builder_interface_contains_no_endpoint_or_label_input() -> None:
    parameters = tuple(inspect.signature(b.build_cross_source_pspc_features).parameters)
    assert parameters == (
...
def test_no_dft_boundary_flags_are_false() -> None:
    assert all(value is False for value in b.n383.BOUNDARY_FLAGS.values())
