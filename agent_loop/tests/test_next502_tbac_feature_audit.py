...
import src.next502_tbac_feature_audit as n


def test_hypothesis_universe_freezes_one_high_direction() -> None:
    assert n.HYPOTHESES == (
        ("tbac_topological_bond_angular_correspondence", "protected_high"),
    )
...
def test_bounded_mapping_supports_only_frozen_high_direction() -> None:
    ...
    with pytest.raises(ValueError, match="NEXT502 bounded"):
...
def test_formal_rows_require_bounded_finite_supported_semantics() -> None:
    assert n._tbac_rows_are_consistent(
        values=np.asarray([0.0, 0.5, 1.0]),
        negative_edges=np.asarray([1, 0, 2]),
        mismatches=np.asarray([1.0, 0.5, 0.0]),
    ).all()
    changed = n._tbac_rows_are_consistent(
        values=np.asarray([-0.1, 0.5, 1.1]),
        negative_edges=np.asarray([0, 0, 0]),
        mismatches=np.asarray([0.0, 0.5, 1.0]),
    )
    assert changed.tolist() == [False, True, False]


def test_audit_interface_excludes_validation_and_replication() -> None:
    parameters = tuple(inspect.signature(n.run_tbac_feature_audit).parameters)
    assert "next501_dir" in parameters and "stage_dirs" in parameters
...
def test_audit_fails_closed_on_missing_input(tmp_path) -> None:
    with pytest.raises(FileNotFoundError, match="NEXT502 input is missing"):
        n.run_tbac_feature_audit(
