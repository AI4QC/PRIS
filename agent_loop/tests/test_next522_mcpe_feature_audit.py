...
import src.next522_mcpe_feature_audit as n


def test_hypothesis_universe_freezes_one_high_direction() -> None:
    assert n.HYPOTHESES == (
        ("mcpe_madelung_chemical_potential_equalization", "protected_high"),
    )
...
def test_bounded_mapping_supports_only_frozen_high_direction() -> None:
    ...
    with pytest.raises(ValueError, match="NEXT522 bounded"):
...
def test_formal_rows_require_bounded_all_pair_semantics() -> None:
    assert n._mcpe_rows_are_consistent(
        values=np.asarray([0.0, 0.5, 1.0]),
        sites=np.asarray([2, 3, 4]),
        pairs=np.asarray([4, 9, 16]),
        mean_discrepancy=np.asarray([1.0, 0.5, 0.0]),
        maximum_discrepancy=np.asarray([1.0, 0.8, 0.0]),
        chemical_spread=np.asarray([3.0, 2.0, 0.0]),
        electronegativity_spread=np.asarray([1.0, 1.0, 1.0]),
        hardness_spread=np.asarray([2.0, 2.0, 2.0]),
        identity_error=np.asarray([0.0, 1e-15, 1e-14]),
    ).all()
    changed = n._mcpe_rows_are_consistent(
        values=np.asarray([-0.1, 0.5, 1.1]),
        sites=np.asarray([1, 3, 4]),
        pairs=np.asarray([1, 8, 15]),
        mean_discrepancy=np.asarray([1.1, 0.2, -0.1]),
        maximum_discrepancy=np.asarray([1.0, 0.1, 0.0]),
        chemical_spread=np.asarray([-1.0, 2.0, 0.0]),
        electronegativity_spread=np.asarray([1.0, -1.0, 1.0]),
        hardness_spread=np.asarray([2.0, 2.0, -1.0]),
        identity_error=np.asarray([0.0, 1e-9, 1e-14]),
    )
    assert changed.tolist() == [False, False, False]


def test_audit_interface_excludes_validation_and_replication() -> None:
    parameters = tuple(inspect.signature(n.run_mcpe_feature_audit).parameters)
    assert "next521_dir" in parameters and "stage_dirs" in parameters
...
def test_audit_fails_closed_on_missing_input(tmp_path) -> None:
    with pytest.raises(FileNotFoundError, match="NEXT522 input is missing"):
        n.run_mcpe_feature_audit(
            ...
            stage_dirs={stage: tmp_path / f"n{stage}" for stage in n.REQUIRED_STAGES},
            next135_freeze_path=tmp_path / "freeze",
            design_paths={
                stage: tmp_path / f"d{stage}" for stage in n.REQUIRED_DESIGN_STAGES
            },
            design_path=tmp_path / "design",
            correction_certificate_path=tmp_path / "correction",
            next412_dir=tmp_path / "n412",
            next521_dir=tmp_path / "n521",
