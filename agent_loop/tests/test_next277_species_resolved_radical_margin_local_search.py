...
import src.next277_species_resolved_radical_margin_local_search as n


def _eligible_table() -> pd.DataFrame:
    return pd.DataFrame(
        {
            "hypothesis": ["prvs_chebyshev_between_cv__protected_low"],
            "feature": ["prvs_chebyshev_between_cv"],
...
def test_frozen_grid_builds_one_control_and_twenty_one_new_candidates() -> None:
    specs = n.build_prvs_candidate_specs(
    ...
    assert len(specs) == n.EXPECTED_CANDIDATE_COUNT == 22
    assert n.EXPECTED_ELIGIBLE_COUNT == 21
...
def test_margin_local_score_is_signed_local_and_support_preserving() -> None:
    ...
    score, got_support, active, weight = n.prvs_margin_local_score(
        base_score=base,
...
def test_search_interface_excludes_validation_and_replication() -> None:
    parameters = tuple(inspect.signature(n.run_prvs_margin_local_search).parameters)
...
def test_search_fails_closed_on_missing_input(tmp_path) -> None:
    with pytest.raises(FileNotFoundError, match="NEXT277 input is missing"):
        n.run_prvs_margin_local_search(
            ...
            design_path=tmp_path / "design276",
