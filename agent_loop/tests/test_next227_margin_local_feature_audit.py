...
from src.next227_margin_local_feature_audit import (
    audit_one_source,
...
def test_formal_interface_has_discovery_but_no_validation_or_replication() -> None:
    parameters = tuple(inspect.signature(run_margin_local_feature_audit).parameters)
...
def test_formal_audit_fails_closed_on_missing_input(tmp_path) -> None:
    with pytest.raises(FileNotFoundError, match="NEXT227 input is missing"):
        run_margin_local_feature_audit(
            ...
            stage_dirs={stage: tmp_path / f"next{stage}" for stage in range(98, 227)},
            next135_freeze_path=tmp_path / "next135",
            design_paths={
                stage: tmp_path / f"design{stage}" for stage in range(202, 228)
            },
            design_path=tmp_path / "design227",
