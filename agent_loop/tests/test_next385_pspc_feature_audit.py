...
import src.next385_pspc_feature_audit as n


def test_hypothesis_universe_freezes_one_high_direction() -> None:
    assert n.HYPOTHESES == (("pspc_skeletal_nb3_collision_q10", "protected_high"),)
...
def test_bounded_mapping_supports_only_frozen_high_direction() -> None:
    ...
    with pytest.raises(ValueError, match="NEXT385 bounded"):
...
def test_selection_uses_frozen_gates_and_reporting_rank() -> None:
    frame = pd.DataFrame({
        "hypothesis": ["z", "a", "b"], "passes_raw_gates": [True, True, False],
...
def test_identity_alignment_adds_source_prefix() -> None:
    table = pd.DataFrame({"material_id": ["a", "b"], "pspc_skeletal_nb3_collision_q10": [0.1, 0.2]})
...
def test_audit_interface_excludes_validation_and_replication() -> None:
    parameters = tuple(inspect.signature(n.run_pspc_feature_audit).parameters)
    assert "next384_dir" in parameters and "stage_dirs" in parameters
...
def test_audit_fails_closed_on_missing_input(tmp_path) -> None:
    with pytest.raises(FileNotFoundError, match="NEXT385 input is missing"):
        n.run_pspc_feature_audit(
            ...
            design_path=tmp_path/"design", next384_dir=tmp_path/"n384",
