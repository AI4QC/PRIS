...
import experiments.next371_pchardi_label_blind_probe as p


def test_novelty_population_includes_all_features_through_next367() -> None:
    assert p.BASE_FEATURE_FILES == {
        "scigen": "features_discovery.parquet",
        "wyformer": "wyformer_x0_features_discovery.parquet",
    }
    assert tuple(p.PRIOR_MODULES)[-5:] == (
        "next323", "next347", "next355", "next359", "next367"
    )
    assert len(p.PRIOR_MODULES) == 29
    assert set(p.PRIOR_FILE_NAMES) == set(p.PRIOR_MODULES)
    assert p.MINIMUM_JOINT_FINITE == 40


def test_probe_gates_allow_zero_and_reject_novelty_boundary() -> None:
    sources = {
        source: {
            "supported": 80,
...
def test_json_safe_converts_nonfinite_diagnostic_without_changing_gate() -> None:
...
def test_probe_source_hashes_cover_complete_frozen_execution() -> None:
    ...
    assert tuple(hashes) == (
        ...
        "src/next371_periodic_chardi_return_consistency.py",
        "experiments/next371_pchardi_label_blind_probe.py",
        "tests/test_next371_periodic_chardi_return_consistency.py",
        "tests/test_next371_pchardi_label_blind_probe.py",
    )
