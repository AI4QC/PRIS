...
import src.next367_pbveu_formal_build as n


def test_formal_schema_rows_and_gates_are_frozen() -> None:
    assert n.PROTOCOL == "2026-08-13-next367-pbveu-formal-build-v1"
    assert n.MANIFEST_NAME == "MANIFEST.json"
    assert n.CATALOGUE_NAME == "NEXT367_PBVEU_FEATURE_CATALOGUE.json"
    ...
    assert n.EXPECTED_PROBE_SHA256 == (
        "d20ee0f170592cae423b0f3e8aa5d830ab48c280b1d61dd6a9ad35496f6d6b1b"
    )


def test_payload_worker_produces_real_row_and_fails_closed() -> None:
    ...
    material_id, row = n._compute_wyformer_payload(
        ("wy-real", json.dumps(structure.as_dict()))
    )
    assert material_id == "wy-real"
    assert row["pbveu_supported"] is True
    assert np.isfinite(row[n.n367.FEATURE_NAMES[0]])
    bad_id, bad = n._compute_wyformer_payload(("wy-bad", "not-json"))
    assert bad_id == "wy-bad"
    assert bad["pbveu_supported"] is False
    assert np.isnan(bad[n.n367.FEATURE_NAMES[0]])


def test_label_free_statistics_are_exact_and_ignore_abstentions() -> None:
    table = pd.DataFrame({n.n367.FEATURE_NAMES[0]: [0.1, 0.2, np.nan, 0.8, 1.0]})
    statistics = n._label_free_statistics(table)[n.n367.FEATURE_NAMES[0]]
    assert statistics == {
        "minimum": 0.1,
        "q10": 0.1,
...
def test_builder_interface_exposes_no_outcome_path_and_fails_closed(tmp_path) -> None:
    parameters = tuple(inspect.signature(n.build_cross_source_pbveu_features).parameters)
    assert parameters == (
        "scigen_cohort_dir",
        "wyformer_cohort_dir",
        "design_path",
    ...
    with pytest.raises(FileNotFoundError, match="NEXT367 input is missing"):
        n.build_cross_source_pbveu_features(
