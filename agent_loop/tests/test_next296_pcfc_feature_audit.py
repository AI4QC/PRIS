...
from src.next296_pcfc_feature_audit import (
    HYPOTHESES,
    NEXT295_SOURCE_PATH,
    REQUIRED_DESIGN_STAGES,
    REQUIRED_STAGES,
    _index_pcfc_by_prefixed_material_id,
    bounded_protection,
...
def test_hypothesis_universe_freezes_exactly_thirteen_high_directions() -> None:
    assert HYPOTHESES == (
        ("pcfc_uniform_closure_min", "protected_high"),
        ("pcfc_uniform_closure_q10", "protected_high"),
        ("pcfc_uniform_closure_mean", "protected_high"),
        ("pcfc_weighted_closure_min", "protected_high"),
        ("pcfc_weighted_closure_q10", "protected_high"),
        ("pcfc_weighted_closure_mean", "protected_high"),
        ("pcfc_uniform_equilibrium_min", "protected_high"),
        ("pcfc_uniform_equilibrium_q10", "protected_high"),
        ("pcfc_uniform_equilibrium_mean", "protected_high"),
        ("pcfc_weighted_equilibrium_min", "protected_high"),
        ("pcfc_weighted_equilibrium_q10", "protected_high"),
        ("pcfc_weighted_equilibrium_mean", "protected_high"),
        ("pcfc_locally_enclosed_fraction", "protected_high"),
    )


def test_next295_provenance_uses_exact_executed_source_path() -> None:
    assert NEXT295_SOURCE_PATH == "src/next295_positive_contact_force_closure.py"
...
def test_pcfc_identity_alignment_adds_reconstruction_source_prefix() -> None:
    table = pd.DataFrame(
        {"material_id": ["a", "b"], "pcfc_uniform_closure_min": [0.7, 0.8]}
    )
...
def test_audit_interface_excludes_validation_and_replication() -> None:
    parameters = tuple(inspect.signature(run_pcfc_feature_audit).parameters)
...
def test_audit_fails_closed_on_missing_input(tmp_path) -> None:
    with pytest.raises(FileNotFoundError, match="NEXT296 input is missing"):
        run_pcfc_feature_audit(
            ...
            design_path=tmp_path / "design295",
