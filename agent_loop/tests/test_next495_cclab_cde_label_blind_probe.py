...
import experiments.next495_cclab_cde_label_blind_probe as p
...
def test_gates_include_shared_domain_identity_and_conservatism() -> None:
    ...
    passing["scigen"]["conservatism_violations"] = 1
    assert not p.evaluate_gates(passing, 80)["conservative"]
...
def test_hashes_cover_design_base_certificate_core_probe_and_tests() -> None:
    ...
    assert "next491_coverage_certificate" in hashes
    ...
    assert tuple(hashes)[-1] == "tests/test_next495_cclab_cde_label_blind_probe.py"
