...
from src.next185_conditional_closure_broad_residual_diagnostic import (
...
def test_formal_interface_has_only_discovery_endpoints() -> None:
    ...
    assert "next184_dir" in parameters and "next183_dir" in parameters
    assert not any(
...
def test_formal_diagnostic_fails_closed_on_missing_input(tmp_path) -> None:
    ...
    with pytest.raises(FileNotFoundError, match="NEXT185 input is missing"):
        run_conditional_closure_broad_residual_diagnostic(**kwargs)
