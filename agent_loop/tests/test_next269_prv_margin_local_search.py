...
from src.next269_prv_margin_local_search import (
    AMPLITUDE_FRACTIONS,
    EXPECTED_CANDIDATE_COUNT,
    EXPECTED_ELIGIBLE_COUNT,
    LOCAL_WIDTH_FRACTIONS,
    build_prv_candidate_specs,
    prv_margin_local_score,
    run_prv_margin_local_search,
...
def test_margin_local_score_is_signed_local_and_support_preserving() -> None:
    ...
    score, got_support, active, weight = prv_margin_local_score(
        base_score=base,
...
def test_search_interface_excludes_validation_and_replication() -> None:
    parameters = tuple(inspect.signature(run_prv_margin_local_search).parameters)
...
def test_search_fails_closed_on_missing_input(tmp_path) -> None:
    from src.next269_prv_margin_local_search import REQUIRED_DESIGN_STAGES, REQUIRED_STAGES

    with pytest.raises(FileNotFoundError, match="NEXT269 input is missing"):
        run_prv_margin_local_search(
