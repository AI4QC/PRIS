...
from src.next196_signed_local_closure_broad_residual import (
...
def test_formal_interface_has_only_discovery_endpoints() -> None:
    parameters = tuple(
        inspect.signature(run_signed_local_closure_broad_residual).parameters
    )
    assert "next195_dir" in parameters and "next194_dir" in parameters
    assert not any(
...
def test_formal_diagnostic_fails_closed_on_missing_input(tmp_path) -> None:
    signature = inspect.signature(run_signed_local_closure_broad_residual)
    kwargs = {
        name: tmp_path / name
    ...
    with pytest.raises(FileNotFoundError, match="NEXT196 input is missing"):
        run_signed_local_closure_broad_residual(**kwargs)
