...
from src.next324_pgce_feature_audit import (
    HYPOTHESES,
    NEXT323_SOURCE_PATH,
    REQUIRED_DESIGN_STAGES,
    REQUIRED_STAGES,
    _index_pgce_by_prefixed_material_id,
    bounded_protection,
...
def test_next323_provenance_uses_exact_executed_source_path() -> None:
    assert NEXT323_SOURCE_PATH == "src/next323_periodic_global_contact_equilibrium.py"
...
def test_pgce_identity_alignment_adds_reconstruction_source_prefix() -> None:
    table = pd.DataFrame(
        {"material_id": ["a", "b"], "pgce_all_facet_participation_floor": [0.1, 0.2]}
    )
...
def test_audit_interface_excludes_validation_and_replication() -> None:
    parameters = tuple(inspect.signature(run_pgce_feature_audit).parameters)
...
def test_audit_fails_closed_on_missing_input(tmp_path) -> None:
    with pytest.raises(FileNotFoundError, match="NEXT324 input is missing"):
        run_pgce_feature_audit(
            ...
            design_path=tmp_path / "design323",
