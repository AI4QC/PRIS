...
import experiments.next430_ecslo_label_blind_probe as p
...
def test_prior_universe_and_gates_are_frozen() -> None:
    ...
    sources = {
        source: {
            "supported": 80, "minimum": 0.0, "maximum": 1.0,
...
def test_hashes_cover_aprbs_and_ecslo_execution_surface() -> None:
    ...
    assert tuple(hashes)[-2:] == (
        "tests/test_next430_endpoint_strength_length_order.py",
        "tests/test_next430_ecslo_label_blind_probe.py",
    )
