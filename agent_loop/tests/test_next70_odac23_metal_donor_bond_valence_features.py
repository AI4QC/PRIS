...
def test_metal_donor_bond_valence_is_finite_and_supercell_invariant() -> None:
    atoms = bulk("NaCl", "rocksalt", a=5.64)
    primitive = compute_metal_donor_bond_valence_features(atoms)
    repeated = compute_metal_donor_bond_valence_features(atoms.repeat((2, 1, 1)))

    assert primitive.supported
    assert repeated.supported
    assert tuple(primitive.features) == METAL_DONOR_BV_FEATURE_NAMES
...
def test_metal_donor_bond_valence_fails_open_without_metal() -> None:
