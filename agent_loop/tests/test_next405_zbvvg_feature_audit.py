...
import src.next405_zbvvg_feature_audit as n


def test_hypothesis_universe_freezes_one_high_direction() -> None:
    assert n.HYPOTHESES == (("zbvvg_zachara_over_linear_gain_q10", "protected_high"),)
...
def test_bounded_mapping_supports_only_frozen_high_direction() -> None:
    ...
    with pytest.raises(ValueError, match="NEXT405 bounded"):
...
def test_selection_uses_frozen_gates_and_reporting_rank() -> None:
    frame = pd.DataFrame({"hypothesis": ["z", "a", "b"], "passes_raw_gates": [True, True, False], "ranking_min_worst_fold_auc": [0.6, 0.6, 0.9], "ranking_min_aggregate_auc": [0.7, 0.7, 0.9], "ranking_mean_aggregate_auc": [0.8, 0.8, 0.9]})
...
def test_identity_alignment_adds_source_prefix() -> None:
    table = pd.DataFrame({"material_id": ["a", "b"], "zbvvg_zachara_over_linear_gain_q10": [0.1, 0.2]})
...
def test_audit_interface_excludes_validation_and_replication() -> None:
    parameters = tuple(inspect.signature(n.run_zbvvg_feature_audit).parameters)
    assert "next404_dir" in parameters and "stage_dirs" in parameters
...
def test_audit_fails_closed_on_missing_input(tmp_path) -> None:
    with pytest.raises(FileNotFoundError, match="NEXT405 input is missing"):
        n.run_zbvvg_feature_audit(
            scigen_feature_dir=tmp_path/"sf", scigen_discovery_endpoint_dir=tmp_path/"se",
            wyformer_feature_dir=tmp_path/"wf", wyformer_discovery_endpoint_dir=tmp_path/"we",
            stage_dirs={stage: tmp_path/f"n{stage}" for stage in n.REQUIRED_STAGES},
            next135_freeze_path=tmp_path/"freeze",
            design_paths={stage: tmp_path/f"d{stage}" for stage in n.REQUIRED_DESIGN_STAGES},
            design_path=tmp_path/"design", next404_dir=tmp_path/"n404",
            output_dir=tmp_path/"out", require_formal_inputs=False,
        )
