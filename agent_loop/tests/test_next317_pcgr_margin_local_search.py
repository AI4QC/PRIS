...
from src.next317_pcgr_margin_local_search import (
    AMPLITUDE_FRACTIONS,
...
def _eligible_table() -> pd.DataFrame:
    return pd.DataFrame(
        {
            "hypothesis": ["pcgr_voltage_drop_max__protected_low"],
            "feature": ["pcgr_voltage_drop_max"],
...
def test_margin_local_score_is_signed_local_and_support_preserving() -> None:
    ...
    score, got_support, active, weight = pcgr_margin_local_score(
        base_score=base,
...
def test_search_interface_excludes_validation_and_replication() -> None:
    parameters = tuple(inspect.signature(run_pcgr_margin_local_search).parameters)
...
def test_search_fails_closed_on_missing_input(tmp_path) -> None:
    with pytest.raises(FileNotFoundError, match="NEXT317 input is missing"):
        run_pcgr_margin_local_search(
