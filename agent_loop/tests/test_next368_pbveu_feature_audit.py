...
import src.next368_pbveu_feature_audit as n


def test_hypothesis_universe_freezes_one_high_direction() -> None:
    assert n.HYPOTHESES == (
        ("pbveu_equal_valence_uniformity_q10", "protected_high"),
    )
...
def test_bounded_mapping_supports_only_frozen_high_direction() -> None:
    ...
    with pytest.raises(ValueError, match="NEXT368 bounded protection inputs differ"):
...
def test_pbveu_identity_alignment_adds_reconstruction_source_prefix() -> None:
    table = pd.DataFrame(
        {
            "material_id": ["a", "b"],
            "pbveu_equal_valence_uniformity_q10": [0.1, 0.2],
        }
    )
    indexed = n._index_pbveu_by_prefixed_material_id(
        table=table,
        source="scigen",
    ...
    with pytest.raises(ValueError, match="material identity differs"):
        n._index_pbveu_by_prefixed_material_id(
            table=table,
            source="scigen",
...
def test_audit_interface_excludes_validation_and_replication() -> None:
    parameters = tuple(inspect.signature(n.run_pbveu_feature_audit).parameters)
...
def test_audit_fails_closed_on_missing_input(tmp_path) -> None:
    with pytest.raises(FileNotFoundError, match="NEXT368 input is missing"):
        n.run_pbveu_feature_audit(
            ...
            design_path=tmp_path / "design367",
