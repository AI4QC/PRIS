...
from src.next270_prv_broad_diagnostic import (
    candidate_key_sha256,
    run_prv_broad_diagnostic,
...
def test_diagnostic_interface_excludes_validation_and_replication() -> None:
    parameters = tuple(inspect.signature(run_prv_broad_diagnostic).parameters)
...
def test_diagnostic_fails_closed_on_missing_input(tmp_path) -> None:
    from src.next270_prv_broad_diagnostic import REQUIRED_DESIGN_STAGES, REQUIRED_STAGES

    with pytest.raises(FileNotFoundError, match="NEXT270 input is missing"):
        run_prv_broad_diagnostic(
