...
from src.next301_mospc_margin_local_search import (
    AMPLITUDE_FRACTIONS,
    EXPECTED_CANDIDATE_COUNT,
    EXPECTED_ELIGIBLE_COUNT,
    LOCAL_WIDTH_FRACTIONS,
    build_mospc_candidate_specs,
    mospc_margin_local_score,
    run_mospc_margin_local_search,
...
def _eligible_table() -> pd.DataFrame:
    ...
    return pd.DataFrame(
        {
            "hypothesis": [f"{name}__protected_high" for name in features],
            "feature": features,
            "direction": ["protected_high"] * 5,
            "q_lo": [0.1915417335, 0.2079712504, 0.0, 0.2089621846, 0.2294959275],
            "q_hi": [0.9958545736, 0.9959447674, 1.0, 0.9966062077, 0.9966329012],
        }
    )


def test_frozen_grid_builds_one_control_and_one_hundred_five_new_candidates() -> None:
    ...
    assert len(specs) == EXPECTED_CANDIDATE_COUNT == 106
    assert EXPECTED_ELIGIBLE_COUNT == 105
    assert sum(bool(spec["eligible_new_candidate"]) for spec in specs) == 105
...
def test_margin_local_score_is_signed_local_and_support_preserving() -> None:
    ...
    score, got_support, active, weight = mospc_margin_local_score(
        base_score=base,
...
def test_search_interface_excludes_validation_and_replication() -> None:
    parameters = tuple(inspect.signature(run_mospc_margin_local_search).parameters)
...
def test_search_fails_closed_on_missing_input(tmp_path) -> None:
    from src.next301_mospc_margin_local_search import REQUIRED_DESIGN_STAGES, REQUIRED_STAGES

    with pytest.raises(FileNotFoundError, match="NEXT301 input is missing"):
        run_mospc_margin_local_search(
