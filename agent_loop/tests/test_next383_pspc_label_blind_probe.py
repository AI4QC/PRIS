...
import experiments.next383_pspc_label_blind_probe as p


def test_novelty_population_includes_all_formal_features_through_next379() -> None:
    assert tuple(p.PRIOR_MODULES)[-5:] == (
        "next355", "next359", "next367", "next375", "next379"
    )
    assert len(p.PRIOR_MODULES) == 31
...
def test_probe_gates_accept_zero_and_reject_novelty_boundary() -> None:
    ...
    sources["scigen"]["maximum"] = 1.0 + 1e-9
...
def test_novelty_requires_forty_joint_finite_observations() -> None:
    ...
    sparse[:4] = np.arange(4)
    enough = np.full(80, np.nan)
    enough[:40] = np.arange(40)[::-1]
...
def test_probe_source_hashes_cover_complete_frozen_execution() -> None:
    ...
    assert tuple(hashes) == (
        ...
        "src/next383_periodic_skeletal_path_collision.py",
        "experiments/next383_pspc_label_blind_probe.py",
        "tests/test_next383_periodic_skeletal_path_collision.py",
        "tests/test_next383_pspc_label_blind_probe.py",
    )
