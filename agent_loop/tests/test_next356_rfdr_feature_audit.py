...
import src.next356_rfdr_feature_audit as n


def test_hypothesis_universe_freezes_one_high_direction() -> None:
    assert n.HYPOTHESES == (("rfdr_deviatoric_retention_floor", "protected_high"),)
...
def test_bounded_mapping_supports_only_frozen_high_direction() -> None:
    ...
    with pytest.raises(ValueError, match="NEXT356 bounded protection inputs differ"):
...
def test_rfdr_identity_alignment_adds_reconstruction_source_prefix() -> None:
    table = pd.DataFrame(
        {"material_id": ["a", "b"], "rfdr_deviatoric_retention_floor": [0.1, 0.2]}
    )
    indexed = n._index_rfdr_by_prefixed_material_id(
        table=table, source="scigen", expected_material_ids=pd.Series(["scigen:a", "scigen:b"])
    )
    assert indexed.index.tolist() == ["scigen:a", "scigen:b"]
    with pytest.raises(ValueError, match="material identity differs"):
        n._index_rfdr_by_prefixed_material_id(
            table=table, source="scigen", expected_material_ids=pd.Series(["scigen:a", "scigen:c"])
        )


def test_audit_interface_excludes_validation_and_replication() -> None:
    parameters = tuple(inspect.signature(n.run_rfdr_feature_audit).parameters)
...
def test_audit_fails_closed_on_missing_input(tmp_path) -> None:
    with pytest.raises(FileNotFoundError, match="NEXT356 input is missing"):
        n.run_rfdr_feature_audit(
            ...
            design_paths={stage: tmp_path / f"design{stage}" for stage in n.REQUIRED_DESIGN_STAGES},
            design_path=tmp_path / "design355",
