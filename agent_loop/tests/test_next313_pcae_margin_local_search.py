...
from src.next313_pcae_margin_local_search import (
    AMPLITUDE_FRACTIONS,
    ...
    return pd.DataFrame(
        {
            ...
            "q_lo": [0.8528395147, 0.6877644168],
            "q_hi": [1.9028127604, 1.0745614571],
        }
    )
...
def test_margin_local_score_is_signed_local_and_support_preserving() -> None:
    ...
    score, got_support, active, weight = pcae_margin_local_score(
        base_score=base,
...
def test_search_interface_excludes_validation_and_replication() -> None:
    parameters = tuple(inspect.signature(run_pcae_margin_local_search).parameters)
...
def test_search_fails_closed_on_missing_input(tmp_path) -> None:
    with pytest.raises(FileNotFoundError, match="NEXT313 input is missing"):
        run_pcae_margin_local_search(
