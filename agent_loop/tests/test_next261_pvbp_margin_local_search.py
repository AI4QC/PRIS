...
from src.next261_pvbp_margin_local_search import (
    AMPLITUDE_FRACTIONS,
...
def _eligible_table() -> pd.DataFrame:
    features = [
        ...
        "pvbp_radius_q75",
        "pvbp_radius_q90",
        "pvbp_radius_upper_quartile_mean",
    ]
    return pd.DataFrame(
        {
            "hypothesis": [f"{name}__protected_low" for name in features],
            "feature": features,
            "direction": ["protected_low"] * 4,
            "q_lo": [0.704852, 0.706627, 0.708025, 0.707321],
            "q_hi": [0.831394, 0.922724, 0.986171, 0.971202],
        }
    )
...
def test_margin_local_score_is_signed_local_and_support_preserving() -> None:
    ...
    score, got_support, active, weight = pvbp_margin_local_score(
        base_score=base,
...
def test_search_interface_excludes_validation_and_replication() -> None:
    parameters = tuple(inspect.signature(run_pvbp_margin_local_search).parameters)
...
def test_search_fails_closed_on_missing_input(tmp_path) -> None:
    with pytest.raises(FileNotFoundError, match="NEXT261 input is missing"):
        run_pvbp_margin_local_search(
            ...
            stage_dirs={stage: tmp_path / f"next{stage}" for stage in range(98, 261)},
            next135_freeze_path=tmp_path / "next135",
            design_paths={
                stage: tmp_path / f"design{stage}" for stage in range(202, 261)
            },
