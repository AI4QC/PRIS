...
from src.next348_parc_feature_audit import (
    HYPOTHESES,
    NEXT347_SOURCE_PATH,
    QUANTILES,
    REQUIRED_DESIGN_STAGES,
    REQUIRED_STAGES,
    _index_parc_by_prefixed_material_id,
    bounded_protection,
...
def test_hypothesis_universe_freezes_one_high_direction() -> None:
    ...
    assert QUANTILES == (1.0 / 16.0, 15.0 / 16.0)


def test_next347_provenance_uses_exact_executed_source_path() -> None:
    assert NEXT347_SOURCE_PATH == (
...
def test_parc_identity_alignment_adds_reconstruction_source_prefix() -> None:
    table = pd.DataFrame(
        {
            "material_id": ["a", "b"],
            "parc_allocation_redistribution_protection": [0.1, 0.2],
        }
    )
...
def test_audit_interface_excludes_validation_and_replication() -> None:
    parameters = tuple(inspect.signature(run_parc_feature_audit).parameters)
...
def test_audit_fails_closed_on_missing_input(tmp_path) -> None:
    with pytest.raises(FileNotFoundError, match="NEXT348 input is missing"):
        run_parc_feature_audit(
            ...
            design_path=tmp_path / "design347",
