...
from src.next217_repair_band_broad_diagnostic import (
...
def test_formal_interface_has_discovery_but_no_validation_or_replication() -> None:
    parameters = tuple(inspect.signature(run_repair_band_broad_diagnostic).parameters)
    assert "next216_dir" in parameters and "next215_dir" in parameters
...
def test_formal_diagnostic_fails_closed_on_missing_input(tmp_path) -> None:
    signature = inspect.signature(run_repair_band_broad_diagnostic)
    kwargs = {
        name: tmp_path / name
    ...
    with pytest.raises(FileNotFoundError, match="NEXT217 input is missing"):
        run_repair_band_broad_diagnostic(**kwargs)
