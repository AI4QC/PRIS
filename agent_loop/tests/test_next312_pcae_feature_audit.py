...
from src.next312_pcae_feature_audit import (
    HYPOTHESES,
    NEXT311_SOURCE_PATH,
    REQUIRED_DESIGN_STAGES,
    REQUIRED_STAGES,
    _index_pcae_by_prefixed_material_id,
    bounded_protection,
...
def test_hypothesis_universe_freezes_exactly_three_low_directions() -> None:
...
def test_next311_provenance_uses_exact_executed_source_path() -> None:
    assert NEXT311_SOURCE_PATH == "src/next311_periodic_charge_alternation_eigenmode.py"
...
def test_pcae_identity_alignment_adds_reconstruction_source_prefix() -> None:
    table = pd.DataFrame(
        {"material_id": ["a", "b"], "pcae_unit_eigen_defect": [0.1, 0.2]}
    )
...
def test_audit_interface_excludes_validation_and_replication() -> None:
    parameters = tuple(inspect.signature(run_pcae_feature_audit).parameters)
...
def test_audit_fails_closed_on_missing_input(tmp_path) -> None:
    with pytest.raises(FileNotFoundError, match="NEXT312 input is missing"):
        run_pcae_feature_audit(
            ...
            design_path=tmp_path / "design311",
