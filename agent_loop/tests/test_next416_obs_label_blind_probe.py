...
import experiments.next416_obs_label_blind_probe as p
...
def test_hashes_cover_frozen_execution_surface_and_sssp_control() -> None:
    hashes = p.probe_source_hashes()
    assert hashes["design"] == p.DESIGN_SHA256
    assert "src/next411_same_sign_shell_purity.py" in hashes
    assert "src/next416_opposite_bridge_separation.py" in hashes
    assert tuple(hashes)[-2:] == (
        "tests/test_next416_opposite_bridge_separation.py",
        "tests/test_next416_obs_label_blind_probe.py",
    )
