...
from src.next257_dvci_margin_local_search import (
    AMPLITUDE_FRACTIONS,
...
def _eligible_table() -> pd.DataFrame:
    return pd.DataFrame(
        {
            "hypothesis": [
                "dvci_closure_mean__protected_high",
                "dvci_closure_q25__protected_high",
            ],
            "feature": ["dvci_closure_mean", "dvci_closure_q25"],
            "direction": ["protected_high", "protected_high"],
            "q_lo": [0.639663, 0.521107],
            "q_hi": [0.975568, 0.969117],
        }
    )
...
def test_margin_local_score_is_signed_local_and_support_preserving() -> None:
    ...
    score, got_support, active, weight = dvci_margin_local_score(
        base_score=base,
...
def test_search_interface_excludes_validation_and_replication() -> None:
    parameters = tuple(inspect.signature(run_dvci_margin_local_search).parameters)
...
def test_search_fails_closed_on_missing_input(tmp_path) -> None:
    with pytest.raises(FileNotFoundError, match="NEXT257 input is missing"):
        run_dvci_margin_local_search(
            ...
            stage_dirs={stage: tmp_path / f"next{stage}" for stage in range(98, 257)},
            next135_freeze_path=tmp_path / "next135",
            design_paths={
                stage: tmp_path / f"design{stage}" for stage in range(202, 257)
            },
