...
from src.next238_chemistry_conditioned_broad_diagnostic import (
...
def test_diagnostic_population_is_exact_eligible_auc_safe_nonbroad_subset() -> None:
    ...
    assert select_diagnostic_candidates(published)["candidate_key"].tolist() == [
        "d"
    ]


def test_closest_order_is_frozen_lexicographic_order() -> None:
    frame = pd.DataFrame(
        {
            "candidate_key": ["z", "a", "b"],
...
def test_diagnostic_interface_excludes_validation_and_replication() -> None:
    parameters = tuple(
        inspect.signature(run_chemistry_conditioned_broad_diagnostic).parameters
    )
...
def test_diagnostic_fails_closed_on_missing_input(tmp_path) -> None:
    with pytest.raises(FileNotFoundError, match="NEXT238 input is missing"):
        run_chemistry_conditioned_broad_diagnostic(
            ...
            stage_dirs={stage: tmp_path / f"next{stage}" for stage in range(98, 238)},
            next135_freeze_path=tmp_path / "next135",
            design_paths={
                stage: tmp_path / f"design{stage}" for stage in range(202, 238)
            },
            design_path=tmp_path / "design238",
