...
from src.next262_pvbp_broad_diagnostic import (
...
def test_diagnostic_interface_excludes_validation_and_replication() -> None:
    parameters = tuple(inspect.signature(run_pvbp_broad_diagnostic).parameters)
...
def test_diagnostic_fails_closed_on_missing_input(tmp_path) -> None:
    with pytest.raises(FileNotFoundError, match="NEXT262 input is missing"):
        run_pvbp_broad_diagnostic(
            ...
            stage_dirs={stage: tmp_path / f"next{stage}" for stage in range(98, 262)},
            next135_freeze_path=tmp_path / "next135",
            design_paths={
                stage: tmp_path / f"design{stage}" for stage in range(202, 262)
            },
