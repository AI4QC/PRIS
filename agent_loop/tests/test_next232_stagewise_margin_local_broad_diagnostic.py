...
from src.next232_stagewise_margin_local_broad_diagnostic import (
...
def test_digest_is_order_independent_and_closest_order_is_lexicographic() -> None:
    frame = pd.DataFrame(
        {
            "candidate_key": ["z", "a", "b"],
...
def test_formal_interface_has_discovery_but_no_validation_or_replication() -> None:
    parameters = tuple(
        inspect.signature(run_stagewise_margin_local_broad_diagnostic).parameters
    )
...
def test_formal_diagnostic_fails_closed_on_missing_input(tmp_path) -> None:
    with pytest.raises(FileNotFoundError, match="NEXT232 input is missing"):
        run_stagewise_margin_local_broad_diagnostic(
            ...
            stage_dirs={stage: tmp_path / f"next{stage}" for stage in range(98, 232)},
            next135_freeze_path=tmp_path / "next135",
            design_paths={stage: tmp_path / f"design{stage}" for stage in range(202, 232)},
            design_path=tmp_path / "design232",
