...
from src.next302_mospc_broad_diagnostic import (
    candidate_key_sha256,
    run_mospc_broad_diagnostic,
...
def test_diagnostic_interface_excludes_validation_and_replication() -> None:
    parameters = tuple(inspect.signature(run_mospc_broad_diagnostic).parameters)
...
def test_diagnostic_fails_closed_on_missing_input(tmp_path) -> None:
    from src.next302_mospc_broad_diagnostic import REQUIRED_DESIGN_STAGES, REQUIRED_STAGES

    with pytest.raises(FileNotFoundError, match="NEXT302 input is missing"):
        run_mospc_broad_diagnostic(
