...
import src.next273_joint_radical_margin_local_search as n
    ...
    return pd.DataFrame(
        {
            ...
            "feature": features,
            "direction": ["protected_low"] * 7,
            "q_lo": np.arange(7, dtype=float),
            "q_hi": np.arange(7, dtype=float) + 1.0,
        }
    )


def test_frozen_grid_builds_one_control_and_147_new_candidates() -> None:
    specs = n.build_joint_candidate_specs(
        base_candidate_key="base", eligible_table=_eligible_table()
    )
    assert n.LOCAL_WIDTH_FRACTIONS == (1 / 64, 1 / 32, 1 / 16, 1 / 8, 1 / 4, 1 / 2, 1.0)
    assert n.AMPLITUDE_FRACTIONS == (1 / 4, 1 / 2, 1.0)
    assert len(specs) == n.EXPECTED_CANDIDATE_COUNT == 148
    assert n.EXPECTED_ELIGIBLE_COUNT == 147
    assert sum(bool(spec["eligible_new_candidate"]) for spec in specs) == 147
...
def test_margin_local_score_is_signed_local_and_support_preserving() -> None:
    ...
    score, got_support, active, weight = n.joint_margin_local_score(
        base_score=base,
...
def test_reporting_selection_excludes_the_reproduction_control() -> None:
    ...
    selected = n.select_best_new_record(records)
...
def test_search_interface_excludes_validation_and_replication() -> None:
    parameters = tuple(inspect.signature(n.run_joint_radical_margin_local_search).parameters)
...
def test_search_fails_closed_on_missing_input(tmp_path) -> None:
    with pytest.raises(FileNotFoundError, match="NEXT273 input is missing"):
        n.run_joint_radical_margin_local_search(
            ...
            design_path=tmp_path / "design272",
