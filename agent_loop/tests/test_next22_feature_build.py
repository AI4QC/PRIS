...
from src.next22_feature_build import FEATURE_NAME, MANIFEST_NAME, build_feature_batch
...
def test_builder_writes_sealed_no_replace_bond_valence_features(tmp_path: Path) -> None:
        ...
        json.dumps(
            {
                ...
                "outputs_sha256": {"geometry_only_frames.zip": _sha(archive_path)},
            }
        )
        + "\n"
    )
    ...
    assert frame["scbv_supported"].tolist() == [True]
    assert frame.loc[0, "scbv_mismatch_rms"] >= 0.0
    assert not any(
    ...
    assert sealed["outputs_sha256"][FEATURE_NAME] == _sha(feature_path)
