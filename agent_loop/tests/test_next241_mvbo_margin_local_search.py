...
from src.next241_mvbo_margin_local_search import (
    AMPLITUDE_FRACTIONS,
...
def _eligible_table() -> pd.DataFrame:
    return pd.DataFrame(
        {
            "hypothesis": ["mvbo_facet_evenness_std__protected_low"],
            "feature": ["mvbo_facet_evenness_std"],
...
def test_margin_local_score_is_signed_local_and_support_preserving() -> None:
    ...
    score, got_support, active, weight = mvbo_margin_local_score(
        base_score=base,
...
def test_search_interface_excludes_validation_and_replication() -> None:
    parameters = tuple(inspect.signature(run_mvbo_margin_local_search).parameters)
...
def test_search_fails_closed_on_missing_input(tmp_path) -> None:
    with pytest.raises(FileNotFoundError, match="NEXT241 input is missing"):
        run_mvbo_margin_local_search(
            ...
            stage_dirs={stage: tmp_path / f"next{stage}" for stage in range(98, 241)},
            next135_freeze_path=tmp_path / "next135",
            design_paths={
                stage: tmp_path / f"design{stage}" for stage in range(202, 241)
            },
