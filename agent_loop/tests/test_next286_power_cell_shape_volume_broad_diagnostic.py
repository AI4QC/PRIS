...
from src.next286_power_cell_shape_volume_broad_diagnostic import (
    candidate_key_sha256,
    run_power_cell_shape_volume_broad_diagnostic,
...
def test_diagnostic_interface_excludes_validation_and_replication() -> None:
    parameters = tuple(
        inspect.signature(run_power_cell_shape_volume_broad_diagnostic).parameters
    )
...
def test_diagnostic_fails_closed_on_missing_input(tmp_path) -> None:
    ...
    with pytest.raises(FileNotFoundError, match="NEXT286 input is missing"):
        run_power_cell_shape_volume_broad_diagnostic(
