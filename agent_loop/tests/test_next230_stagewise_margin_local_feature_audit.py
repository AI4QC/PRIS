...
from src.next230_stagewise_margin_local_feature_audit import (
    audit_one_source,
...
def test_rejected_extreme_cohort_uses_inclusive_threshold_and_support() -> None:
...
def test_schema_selection_reuses_exact_raw_numeric_policy() -> None:
    frame = pd.DataFrame(
        {
            "z_feature": [1.0, 2.0],
            "a_feature": [2, 3],
...
def test_source_audit_and_opposite_direction_veto_are_reused() -> None:
...
def test_formal_interface_has_discovery_but_no_validation_or_replication() -> None:
    parameters = tuple(
        inspect.signature(run_stagewise_margin_local_feature_audit).parameters
    )
...
def test_formal_audit_fails_closed_on_missing_input(tmp_path) -> None:
    with pytest.raises(FileNotFoundError, match="NEXT230 input is missing"):
        run_stagewise_margin_local_feature_audit(
            ...
            stage_dirs={stage: tmp_path / f"next{stage}" for stage in range(98, 230)},
            next135_freeze_path=tmp_path / "next135",
            design_paths={stage: tmp_path / f"design{stage}" for stage in range(202, 230)},
            design_path=tmp_path / "design230",
