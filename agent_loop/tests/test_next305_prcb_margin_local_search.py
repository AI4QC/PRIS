...
from src.next305_prcb_margin_local_search import (
    AMPLITUDE_FRACTIONS,
    ...
    return pd.DataFrame(
        {
            ...
            "feature": features,
            "direction": ["protected_high"] * 4,
            "q_lo": [0.1972296421, 0.2227545761, 0.2105036996, 0.2456531869],
            "q_hi": [0.9961467364, 0.9962340262, 0.9969941268, 0.9970029194],
        }
    )
...
def test_margin_local_score_is_signed_local_and_support_preserving() -> None:
    ...
    score, got_support, active, weight = prcb_margin_local_score(
        base_score=base,
...
def test_search_interface_excludes_validation_and_replication() -> None:
    parameters = tuple(inspect.signature(run_prcb_margin_local_search).parameters)
...
def test_search_fails_closed_on_missing_input(tmp_path) -> None:
    with pytest.raises(FileNotFoundError, match="NEXT305 input is missing"):
        run_prcb_margin_local_search(
