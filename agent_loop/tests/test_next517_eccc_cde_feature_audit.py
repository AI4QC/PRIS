...
import src.next517_eccc_cde_feature_audit as n


def test_hypothesis_universe_freezes_one_high_direction() -> None:
    assert n.HYPOTHESES == (("eccc_cde_conservative_domain_extension", "protected_high"),)
...
def test_bounded_mapping_supports_only_frozen_high_direction() -> None:
    ...
    with pytest.raises(ValueError, match="NEXT517 bounded"):
...
def test_formal_rows_require_bounded_finite_supported_semantics() -> None:
    assert n._cde_rows_are_consistent(
        values=np.asarray([0.0, 0.5, 1.0]),
        unknown_cations=np.asarray([1, 0, 2]),
        isolated_sites=np.asarray([1, 0, 3]),
    ).all()
    changed = n._cde_rows_are_consistent(
        values=np.asarray([-0.1, 0.5, 1.1]),
        unknown_cations=np.asarray([0, 0, 0]),
        isolated_sites=np.asarray([0, 0, 0]),
    )
    assert changed.tolist() == [False, True, False]


def test_audit_interface_excludes_validation_and_replication() -> None:
    parameters = tuple(inspect.signature(n.run_eccc_cde_feature_audit).parameters)
    assert "next516_dir" in parameters and "stage_dirs" in parameters
...
def test_audit_fails_closed_on_missing_input(tmp_path) -> None:
    with pytest.raises(FileNotFoundError, match="NEXT517 input is missing"):
        n.run_eccc_cde_feature_audit(
