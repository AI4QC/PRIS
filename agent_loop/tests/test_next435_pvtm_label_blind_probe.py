...
import experiments.next435_pvtm_label_blind_probe as p
...
def test_prior_universe_and_gates_are_frozen() -> None:
    ...
    sources = {
        source: {
            "supported": 80, "minimum": 0.0, "maximum": 0.9,
            "unique_rounded_10": 20, "maximum_invariance_error": 0.0,
...
def test_hashes_cover_ecslo_and_pvtm_surface() -> None:
    hashes = p.probe_source_hashes()
    assert hashes["design"] == p.DESIGN_SHA256
    assert "src/next430_endpoint_strength_length_order.py" in hashes
    assert "src/next435_positive_valence_transport_margin.py" in hashes
    assert tuple(hashes)[-2:] == (
        "tests/test_next435_positive_valence_transport_margin.py",
        "tests/test_next435_pvtm_label_blind_probe.py",
    )
