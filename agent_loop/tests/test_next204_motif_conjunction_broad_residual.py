...
from src.next204_motif_conjunction_broad_residual import (
...
def test_formal_interface_has_only_discovery_endpoints() -> None:
    parameters = tuple(
        inspect.signature(run_motif_conjunction_broad_residual).parameters
    )
...
def test_formal_diagnostic_fails_closed_on_missing_input(tmp_path) -> None:
    signature = inspect.signature(run_motif_conjunction_broad_residual)
    kwargs = {
        name: tmp_path / name
    ...
    with pytest.raises(FileNotFoundError, match="NEXT204 input is missing"):
        run_motif_conjunction_broad_residual(**kwargs)
