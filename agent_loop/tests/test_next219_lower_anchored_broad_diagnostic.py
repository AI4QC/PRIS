...
from src.next219_lower_anchored_broad_diagnostic import (
...
def test_formal_interface_has_discovery_but_no_validation_or_replication() -> None:
    parameters = tuple(
        inspect.signature(run_lower_anchored_broad_diagnostic).parameters
    )
...
def test_formal_diagnostic_fails_closed_on_missing_input(tmp_path) -> None:
    with pytest.raises(FileNotFoundError, match="NEXT219 input is missing"):
        run_lower_anchored_broad_diagnostic(
            ...
            stage_dirs={stage: tmp_path / f"next{stage}" for stage in range(98, 219)},
            next135_freeze_path=tmp_path / "next135",
            design_paths={stage: tmp_path / f"design{stage}" for stage in range(202, 219)},
            design_path=tmp_path / "design219",
