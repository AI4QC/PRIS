...
import src.next355_rfdr_formal_build as n


def test_formal_schema_rows_and_gates_are_frozen() -> None:
    assert n.PROTOCOL == "2026-08-13-next355-rfdr-formal-build-v1"
    assert n.MANIFEST_NAME == "MANIFEST.json"
    assert n.CATALOGUE_NAME == "NEXT355_RFDR_FEATURE_CATALOGUE.json"
...
def test_payload_workers_produce_real_rows_and_fail_closed() -> None:
    atoms = __import__("ase.build", fromlist=["bulk"]).bulk("NaCl", "rocksalt", a=5.64, cubic=True)
    ...
    assert np.isfinite(row[n.n355.FEATURE_NAMES[0]])
    bad_id, bad = n._compute_wyformer_payload(("wy-bad", "not-json"))
    assert bad_id == "wy-bad"
    assert bad["rfdr_supported"] is False
    assert np.isnan(bad[n.n355.FEATURE_NAMES[0]])


def test_label_free_statistics_are_exact_and_ignore_abstentions() -> None:
    table = pd.DataFrame({n.n355.FEATURE_NAMES[0]: [0.0, 0.2, np.nan, 0.8, 1.0]})
    statistics = n._label_free_statistics(table)[n.n355.FEATURE_NAMES[0]]
    assert statistics == {
        "minimum": 0.0,
        "q10": 0.0,
...
def test_builder_interface_exposes_no_outcome_path_and_fails_closed(tmp_path) -> None:
    parameters = tuple(inspect.signature(n.build_cross_source_rfdr_features).parameters)
    assert parameters == (
    ...
    with pytest.raises(FileNotFoundError, match="NEXT355 input is missing"):
        n.build_cross_source_rfdr_features(
