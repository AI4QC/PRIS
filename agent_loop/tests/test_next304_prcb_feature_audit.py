...
from src.next304_prcb_feature_audit import (
    HYPOTHESES,
    NEXT303_SOURCE_PATH,
    REQUIRED_DESIGN_STAGES,
    REQUIRED_STAGES,
    _index_prcb_by_prefixed_material_id,
    bounded_protection,
...
def test_next303_provenance_uses_exact_executed_source_path() -> None:
    assert NEXT303_SOURCE_PATH == "src/next303_periodic_reciprocal_cage_balance.py"
...
def test_prcb_identity_alignment_adds_reconstruction_source_prefix() -> None:
    table = pd.DataFrame(
        {"material_id": ["a", "b"], "prcb_uniform_closure_min": [0.7, 0.8]}
    )
...
def test_audit_interface_excludes_validation_and_replication() -> None:
    parameters = tuple(inspect.signature(run_prcb_feature_audit).parameters)
...
def test_audit_fails_closed_on_missing_input(tmp_path) -> None:
    with pytest.raises(FileNotFoundError, match="NEXT304 input is missing"):
        run_prcb_feature_audit(
            ...
            design_path=tmp_path / "design303",
