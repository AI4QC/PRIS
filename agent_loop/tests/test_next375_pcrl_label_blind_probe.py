...
import experiments.next375_pcrl_label_blind_probe as p


def test_novelty_population_includes_all_formal_features_through_next367() -> None:
...
def test_probe_source_hashes_cover_the_complete_frozen_execution() -> None:
    hashes = p.probe_source_hashes()
    assert tuple(hashes) == (
        "design",
        "src/next267_periodic_radical_voronoi_packing.py",
        "src/next375_periodic_coordination_reciprocity_likelihood.py",
        "experiments/next375_pcrl_label_blind_probe.py",
        "tests/test_next375_periodic_coordination_reciprocity_likelihood.py",
        "tests/test_next375_pcrl_label_blind_probe.py",
    )
