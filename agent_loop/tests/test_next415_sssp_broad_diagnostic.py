...
from src.next415_sssp_broad_diagnostic import (
    candidate_key_sha256,
    run_sssp_broad_diagnostic,
...
def test_interface_excludes_validation_and_replication() -> None:
    parameters = tuple(inspect.signature(run_sssp_broad_diagnostic).parameters)
    assert {"next412_dir", "next413_dir", "next414_dir"} <= set(parameters)
...
def test_diagnostic_fails_closed_on_missing_input(tmp_path) -> None:
    ...
    with pytest.raises(FileNotFoundError, match="NEXT415 input is missing"):
        run_sssp_broad_diagnostic(
            ...
            next413_dir=tmp_path / "n413",
            next414_dir=tmp_path / "n414",
