...
from src.next300_mospc_feature_audit import (
    HYPOTHESES,
    NEXT299_SOURCE_PATH,
    REQUIRED_DESIGN_STAGES,
    REQUIRED_STAGES,
    _index_mospc_by_prefixed_material_id,
    bounded_protection,
...
def test_hypothesis_universe_freezes_exactly_thirteen_high_directions() -> None:
    assert HYPOTHESES == (
        ("mospc_uniform_closure_min", "protected_high"),
        ("mospc_uniform_closure_q10", "protected_high"),
        ("mospc_uniform_closure_mean", "protected_high"),
        ("mospc_inverse_square_closure_min", "protected_high"),
        ("mospc_inverse_square_closure_q10", "protected_high"),
        ("mospc_inverse_square_closure_mean", "protected_high"),
        ("mospc_uniform_equilibrium_min", "protected_high"),
        ("mospc_uniform_equilibrium_q10", "protected_high"),
        ("mospc_uniform_equilibrium_mean", "protected_high"),
        ("mospc_inverse_square_equilibrium_min", "protected_high"),
        ("mospc_inverse_square_equilibrium_q10", "protected_high"),
        ("mospc_inverse_square_equilibrium_mean", "protected_high"),
        ("mospc_locally_enclosed_fraction", "protected_high"),
    )


def test_next299_provenance_uses_exact_executed_source_path() -> None:
    assert NEXT299_SOURCE_PATH == "src/next299_minimal_opposite_sign_periodic_cage.py"
...
def test_mospc_identity_alignment_adds_reconstruction_source_prefix() -> None:
    table = pd.DataFrame(
        {"material_id": ["a", "b"], "mospc_uniform_closure_min": [0.7, 0.8]}
    )
...
def test_audit_interface_excludes_validation_and_replication() -> None:
    parameters = tuple(inspect.signature(run_mospc_feature_audit).parameters)
...
def test_audit_fails_closed_on_missing_input(tmp_path) -> None:
    with pytest.raises(FileNotFoundError, match="NEXT300 input is missing"):
        run_mospc_feature_audit(
            ...
            design_path=tmp_path / "design299",
