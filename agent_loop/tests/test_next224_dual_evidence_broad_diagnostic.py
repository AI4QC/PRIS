...
from src.next224_dual_evidence_broad_diagnostic import (
...
def test_selects_exact_eligible_auc_safe_non_broad_population_and_digest() -> None:
    frame = pd.DataFrame(
        {
            "candidate_key": ["z", "a", "m", "b", "control"],
            "eligible_new_candidate": [True, True, True, True, False],
            "passes_source_auc_gates": [True, True, False, True, True],
            "passes_safe_all_cells": [True, True, True, False, True],
...
def test_formal_interface_has_discovery_but_no_validation_or_replication() -> None:
    parameters = tuple(
        inspect.signature(run_dual_evidence_broad_diagnostic).parameters
    )
...
def test_formal_diagnostic_fails_closed_on_missing_input(tmp_path) -> None:
    with pytest.raises(FileNotFoundError, match="NEXT224 input is missing"):
        run_dual_evidence_broad_diagnostic(
            ...
            stage_dirs={stage: tmp_path / f"next{stage}" for stage in range(98, 224)},
            next135_freeze_path=tmp_path / "next135",
            design_paths={stage: tmp_path / f"design{stage}" for stage in range(202, 224)},
            design_path=tmp_path / "design224",
