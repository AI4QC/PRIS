...
from src.next268_prv_feature_audit import (
    HYPOTHESES,
    NEXT267_SOURCE_PATH,
    REQUIRED_DESIGN_STAGES,
    REQUIRED_STAGES,
    _index_prv_by_prefixed_material_id,
    bounded_protection,
    run_prv_feature_audit,
...
def test_next267_provenance_uses_exact_executed_source_path() -> None:
    assert NEXT267_SOURCE_PATH == "src/next267_periodic_radical_voronoi_packing.py"
...
def test_audit_interface_excludes_validation_and_replication() -> None:
    parameters = tuple(inspect.signature(run_prv_feature_audit).parameters)
...
def test_prv_identity_alignment_adds_reconstruction_source_prefix() -> None:
    table = pd.DataFrame(
        {"material_id": ["a", "b"], "prv_allocation_total_variation": [0.2, 0.3]}
    )
...
def test_audit_fails_closed_on_missing_input(tmp_path) -> None:
    with pytest.raises(FileNotFoundError, match="NEXT268 input is missing"):
        run_prv_feature_audit(
            ...
            design_path=tmp_path / "design267",
