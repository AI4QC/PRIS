...
success. Our
agents proposed and implemented hypotheses, and selected thresholds on a discovery split.
They then designed counterexamples and attempted refutation with held-out data and physical
checks (Fig.~\ref{fig:loop}a). Refuted claims remained in the record as diagnostics for
the next cycle, making that search a sequence of falsifiable experiments (Supplementary
...
\begin{figure}[p]
  ...
  detection (green). The inset enlarges the earliest investigations. The strip beneath
...
Fig.~\ref{si-sifig:ranking-data}). Each damaged structure
came from an experimental parent by a known displacement, strain or element exchange.
...
gave a passing claim another way to fail. Eleven initially successful conclusions later
failed these checks (Supplementary Section~\ref{si-note:s6}). The failures arose from misleading metrics
...
million evaluations and form PRIS.

The eight laws bundle into nested sets, each demanding a fuller model of an ionic crystal
...
(Fig.~\ref{fig:rules}a). Charge neutrality is a composition-only baseline and detected no
damage because every perturbation preserves composition. Fixed minimum-distance cutoffs are
element-blind and detected little damage.
...
A single contact floor encodes only the repulsive wall. Expansion lengthens rather than
shortens contacts. A cation--anion exchange can leave every coordinate unchanged. Distinct
failures can share the same closest distance. Set~2 therefore judges the rigid-ion lattice,
...
Supplementary Figs.~\ref{si-sifig:perclass} and~\ref{si-sifig:amplitude}). Set~4 detected more by combining laws against different failures, not by tightening one
cutoff.
...
\begin{figure}[!t]
  ...
  shown as the ratio to the frozen values. Unity means no change. The right-hand
  ...
  a common percentage scale. The dashed line marks equality. Marker shape identifies the
  law in both the main panel and inset.}
  \label{fig:rules}
\end{figure}
\FloatBarrier
...
electrostatic thresholds moved further (details in Methods). These larger shifts changed 5.0--7.8\% of chemically damaged verdicts but only
0.25--0.33\% of experimental-structure verdicts (inset, common 0--20\% scale).
...
chemical and symmetry constraints without imposing a hierarchy. Figure~\ref{fig:anatomy}a
shows their responses to five chemically damaged variants of MgAl$_2$O$_4$. The Mg--Al exchange
approximates natural inverse-spinel order. Uniaxial compression lowers \rhoc{} from 0.99 to
0.76, whereas isotropic expansion raises it to 1.28, beyond the 1.05 ceiling of Law~2. A
cation--anion exchange leaves \rhoc{} at 0.99 but creates eight like-charge bonds. No single
distance quantity represents all five mechanisms. Each unsatisfied law
...
(Fig.~\ref{fig:anatomy}c). A floor, however, cannot detect expansion. In a
...
reduced cation--anion contact at 1.081 when mean anion coordination is at most 3.333. Low coordination with long contacts marks an open environment, so Law~3
detects expansion without Law~2's ionic-character requirement.

Packing laws cannot detect a wrong-site exchange that changes no coordinate. Law~4--Law~6
...
unchanged.
An Ewald sum does not test whether local bonds supply each ion's expected valence. Law~8
does. Bond valences decay exponentially with distance: expansion
...
(Fig.~\ref{fig:anatomy}b). Together they turn screening into diagnosis by indicating
whether the next check concerns ionic size, oxidation state, site occupancy, local
bonding or symmetry. Because these laws are physical rather than fitted, they should decide plausibility
...
already carries. In a validation queue, where generators outpace the calculations that
relax and rank their output, we benchmarked PRIS against
...
87.9\% of the damage (Fig.~\ref{fig:validation-synthesis}a). Tightened to the
same satisfaction, a distance cutoff still detected 2.6-fold less damage than Set~4 (details in Methods).
...
A plausible crystal must still cross the synthesizability barrier. Ranking synthesizability requires a
...
of isolated cation polyhedra. The last two terms describe connectivity. Atomic volume has
...
higher PSS marks a structure as easier to synthesize. Testing this
...
graph convolutional network with PU heads, and learns its
...
treated as hard-to-synthesize structures.

Set~4 and PSS answer different questions, and on the hard-to-synthesize cohort they
proved complementary. Set~4 screened
51.9\% of this cohort at 80.7\% experimental satisfaction. At the same satisfaction, PSS
screened 83.7\%
...
fell from 50.3\% to 30.4\%. Mean PSS rose from $-11.37$ to $-0.18$. Linear fits of mean
...
\begin{figure}[p]
  ...
  \textbf{d},~Set~4 violation and mean PSS across within-model CLscore percentiles for
  ...
  model's straight-line fit in the decile index.
  \textbf{e},~Held-out same-composition pair accuracy for PSS and DFT
  \(E_{\mathrm{hull}}\) as higher-confidence fractions are retained (shading, confidence
  intervals).
...
anti-predictive. Our agents then proposed a symmetry explanation. Auditing the same pairs within composition
refuted it. Treating ties as no decision removed the artefact.
...
\cite{wood2025uma}, predicted each candidate's bulk modulus. UMA placed 140 candidates at or
...
experimental structures using those two terms (details in Methods). That threshold removed 61
candidates from the queue. Because the score is continuous, that threshold is an operating point rather than a fixed rule. Across the threshold's range,
...
of the 260 candidates satisfied Law~7. After DFT relaxation, 113 satisfied it. No candidate
...
points span two extremes. Set~1--Set~3 retained all 140 high-property candidates but reduced
the queue by only 0--0.4\%. Set~4 reduced it by 67.3\% but lost most of those candidates
(details in Methods). Within the Set~4-violating subset, PSS screened structures that combined maximal
...
\begin{figure}[!tp]
  ...
  imposed symmetry, open squares symmetry-constrained generation. Nearly
  ...
  literature-scale DFT-relaxation estimate (dashed). The upper axis converts the same
...
(details in Methods). Overall, 40.64\% failed Law~7, including all \(P1\) structures. \(Pm\) and
\(Cm\) structures were nearly as affected (Fig.~\ref{fig:deploy}b, upper; details in Methods; charge-law
...
medians of 0.22 and 6.05\,eV per atom, respectively (Fig.~\ref{fig:deploy}c). To test whether
this separation belongs to the structures rather than to the MatterSim potential, we
recomputed 200 cells with DFT. MatterSim and DFT agreed at a Spearman rank
correlation of 0.953. Under DFT, twenty unmodified GNoME parents released a median of
...
electrostatic, bond-valence or site-complexity failures. One chemical explanation remained
to test: if artificial ordering creates the extra sites, merging similar species
should restore equivalence.
...
sampled pattern and answers the controversy for these entries: changing labels restores
equivalence without moving an atom.

An artificial ordering should cost nothing to permute. DFT relaxed every
...
site.
Wrong-element assignment has an experimental precedent. Harrison and colleagues reported at least 70
...
and anomalous metal--ligand distances first exposed the problem. Structure-factor
comparisons then confirmed it \cite{harrison2010falsified}. An element exchange at fixed coordinates redistributes formal charges, Madelung energies
...
with four reimplemented coordinate-computable check families \cite{spek2009structure}
...
\cite{iucr2012retraction}. Each entry gave \rhoc{} = 0.53 and a mean bond-valence
deviation of 0.77--0.78, violating Law~1 and Law~8.
...
synthesis. These three do not track one another. On published phonon data
...
tie polymorphs within the plausible region. PSS or energy then ranks them. At the database
level, Law~7 and label merging both trace part of GNoME's low-symmetry excess to site
splitting by artificial ordering. This is the mechanism at issue in the ordering
...
reconciling score.
The structural-plausibility check returns a falsifiable diagnosis, not a score.
...
structures it removes combine complete site splitting with open
...
Beyond queue screening, the same verdicts can label database entries and training data. A
missing charge or radius returns no verdict rather than a pass, keeping uncertain entries
separate from implausible ones.
...
record, crystal screening does not stop at a pass-or-fail verdict. It explains why a
