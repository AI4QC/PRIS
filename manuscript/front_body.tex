...
now supply candidates faster than density functional theory (DFT) calculations of
thermodynamic energies and phonon spectra, or experiments, can assess them
...
and avoiding gross overlap does not establish plausible coordination, electrostatics,
bond valence or chemical ordering. A recent Comment argues that data alone will struggle to deliver materials discovery, and that AI
...
electrostatic valence and a preference for simpler structures. Requiring only a radius
...
\cite{george2020limited}. In our evaluations, only 6.5\% of charge-balanced ionic
...
chemically damaged structures. The classical
rules, applied jointly, are too idealised, and the distance cutoff is
inexpensive but element-blind. Experimental-structure satisfaction alone cannot show what
a criterion rejects. A good law must keep experimental
...
and phase assignments \cite{leeman2024challenges}. An author correction later found
...
structural screens. Closing either requires laws that name the failing
physical variable.
...
candidate laws. Because failed claims remained on record, refutation rather than proposal
...
fails. Sets of these laws reached 82--99\% experimental-structure satisfaction. The
...
Every removal was traceable to a named mechanism. In an inverse-design task targeting a
high bulk modulus, explainable screening by PRIS and PSS reduced the DFT validation queue for
MatterGen outputs by up to 67.3\%. The same screen kept 99.2\% of the
